static const struct TrainerMon sParty_Sawyer1[] = {
    {
    .lvl = 26,
    .species = SPECIES_CARKOL,
    },
    {
    .lvl = 26,
    .species = SPECIES_GRAVELER,
    },
    {
    .lvl = 27,
    .species = SPECIES_GOLEM_ALOLAN,
    }

};

static const struct TrainerMon sParty_GruntAquaHideout1[] = {
    {
    .lvl = 54,
    .species = SPECIES_PELIPPER,
    .ability = ABILITY_DRIZZLE,
    },
    {
    .lvl = 56,
    .species = SPECIES_SEISMITOAD,
    .ability = ABILITY_SWIFT_SWIM,
    },
    {
    .lvl = 54,
    .species = SPECIES_AMPHAROS,
    },
    {
    .lvl = 55,
    .species = SPECIES_RAICHU,
    }

};

static const struct TrainerMon sParty_GruntAquaHideout2[] = {
    {
    .lvl = 54,
    .species = SPECIES_VENOMOTH,
    .ability = ABILITY_TINTED_LENS,
    },
    {
    .lvl = 54,
    .species = SPECIES_WAILORD,
    },
    {
    .lvl = 54,
    .species = SPECIES_CARRACOSTA,
    },
    {
    .lvl = 54,
    .species = SPECIES_WALREIN,
    .ability = ABILITY_FUR_COAT,
    },
    {
    .lvl = 54,
    .species = SPECIES_WEEZING_GALARIAN,
    }
};

static const struct TrainerMon sParty_GruntAquaHideout3[] = {
    {
    .lvl = 54,
    .species = SPECIES_TOXICROAK,
    },
    {
    .lvl = 54,
    .species = SPECIES_EXPLOUD,
    .ability = ABILITY_PUNK_ROCK,
    .moves = {MOVE_BOOMBURST, MOVE_SURF, MOVE_THUNDER, MOVE_FLAMETHROWER},
    },
    {
    .lvl = 54,
    .species = SPECIES_TAUROS,
    .ability = ABILITY_SHEER_FORCE,
    .heldItem = ITEM_LIFE_ORB,
    .moves = {MOVE_EARTHQUAKE, MOVE_ROCK_SLIDE, MOVE_BODY_SLAM, MOVE_FIRE_BLAST},
    }
};

static const struct TrainerMon sParty_GruntAquaHideout4[] = {
    {
    .lvl = 55,
    .species = SPECIES_SHARPEDO,
    },
    {
    .lvl = 54,
    .species = SPECIES_GOLISOPOD,
    },
    {
    .lvl = 54,
    .species = SPECIES_CACTURNE,
    .ability = ABILITY_WATER_ABSORB,
    }
};

static const struct TrainerMon sParty_GruntSeafloorCavern1[] = {
    {
     .lvl = 60,
    .species = SPECIES_HOUNDOOM,
    .ability = ABILITY_INTIMIDATE,
    .moves = {MOVE_FLAMETHROWER, MOVE_DARK_PULSE, MOVE_SOLAR_BEAM, MOVE_SUNNY_DAY}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 60,
    .species = SPECIES_SKUNTANK,
    .ability = ABILITY_AFTERMATH,
    .heldItem = ITEM_BLACK_SLUDGE,
    .moves = {MOVE_POISON_JAB, MOVE_CRUNCH, MOVE_EXPLOSION, MOVE_TOXIC}
    }
};

static const struct TrainerMon sParty_GruntSeafloorCavern2[] = {
    {
    .lvl = 60,
    .species = SPECIES_LUMINEON,
    .ability = ABILITY_STORM_DRAIN,
    .heldItem = ITEM_CHOICE_SCARF,
    .moves = {MOVE_SURF, MOVE_ICE_BEAM, MOVE_AIR_SLASH, MOVE_U_TURN}
    }
};

static const struct TrainerMon sParty_GruntSeafloorCavern3[] = {
    {
    .lvl = 60,
    .species = SPECIES_CROBAT,
    .ability = ABILITY_INFILTRATOR,
    .moves = {MOVE_HURRICANE, MOVE_CROSS_POISON, MOVE_U_TURN, MOVE_TAILWIND}
    },
    {
    .lvl = 61,
    .species = SPECIES_STARMIE,
    .ability = ABILITY_NATURAL_CURE,
    .moves = {MOVE_HYDRO_PUMP, MOVE_THUNDER, MOVE_ICE_BEAM, MOVE_RECOVER}
    }
};

static const struct TrainerMon sParty_Gabrielle1[] = {
    
    {
    .lvl = 50,
    .species = SPECIES_GOURGEIST,
    },
    {
    .lvl = 50,
    .species = SPECIES_TREVENANT,
    },
    {
    .lvl = 50,
    .species = SPECIES_SWELLOW,
    .heldItem = ITEM_FLAME_ORB
    }
};

static const struct TrainerMon sParty_GruntPetalburgWoods[] = {
    {
    .lvl = 8,
    .species = SPECIES_CLEFAIRY,
    .heldItem = ITEM_TOXIC_ORB,
    .ability = ABILITY_MAGIC_GUARD,
    .moves = {MOVE_PROTECT, MOVE_FACADE, MOVE_CHARM, MOVE_DISARMING_VOICE}
    },
    {
    .lvl = 9,
    .species = SPECIES_AXEW,
    }
};

static const struct TrainerMon sParty_Marcel[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 46,
    .species = SPECIES_MANECTRIC,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 47,
    .species = SPECIES_SHIFTRY,
    }
};

static const struct TrainerMon sParty_Alberto[] = {
    {
    .lvl = 72,
    .species = SPECIES_FLYGON,
    .moves = {MOVE_EARTH_POWER, MOVE_DRAGON_PULSE, MOVE_FLAMETHROWER, MOVE_U_TURN}
    },
    {
    .lvl = 72,
    .species = SPECIES_XATU,
    .ability = ABILITY_MAGIC_BOUNCE,
    .moves = {MOVE_DRILL_PECK, MOVE_PSYCHIC, MOVE_ROOST, MOVE_TAILWIND}
    },
    {
    .lvl = 70,
    .species = SPECIES_FEAROW,
    .ability = ABILITY_SNIPER,
    .heldItem = ITEM_SCOPE_LENS,
    .moves = {MOVE_HURRICANE, MOVE_DRILL_RUN, MOVE_U_TURN, MOVE_ROOST}
    }
};

static const struct TrainerMon sParty_Ed[] = {
    {
    .lvl = 71,
    .species = SPECIES_ZANGOOSE,
    .ability = ABILITY_TOXIC_BOOST,
    .heldItem = ITEM_TOXIC_ORB,
    .moves = {MOVE_CLOSE_COMBAT, MOVE_KNOCK_OFF, MOVE_SWORDS_DANCE, MOVE_FACADE}
    },
    {
    .lvl = 72,
    .species = SPECIES_SEVIPER,
    .ability = ABILITY_SHED_SKIN,
    .heldItem = ITEM_BLACK_SLUDGE,
    .moves = {MOVE_POISON_JAB, MOVE_IRON_TAIL, MOVE_GIGA_DRAIN, MOVE_COIL}
    }
};

static const struct TrainerMon sParty_GruntSeafloorCavern4[] = {
    {
    .lvl = 60,
    .species = SPECIES_GOREBYSS,
    .ability = ABILITY_SWIFT_SWIM,
    .moves = {MOVE_SURF, MOVE_ICE_BEAM, MOVE_SHELL_SMASH, MOVE_BATON_PASS}
    },
    {
    .lvl = 60,
    .species = SPECIES_HUNTAIL,
    .ability = ABILITY_SWIFT_SWIM,
    .moves = {MOVE_WATERFALL, MOVE_CRUNCH, MOVE_ICE_FANG, MOVE_AQUA_JET}
    }
};

static const struct TrainerMon sParty_Declan[] = {
    {
    .lvl = 41,
    .species = SPECIES_GYARADOS,
    }
};

static const struct TrainerMon sParty_GruntRusturfTunnel[] = {
    {
    .lvl = 15,
    .species = SPECIES_CLEFABLE,
    .heldItem = ITEM_IRON_BALL,
    .ability = ABILITY_MAGIC_GUARD,
    .moves = {MOVE_PROTECT, MOVE_HEADBUTT, MOVE_FLING, MOVE_DRAINING_KISS}
    },
    {
    .lvl = 15,
    .species = SPECIES_AXEW,
    },
    {
    .lvl = 15,
    .species = SPECIES_EKANS,
    .ability = ABILITY_INTIMIDATE
    }
};

static const struct TrainerMon sParty_GruntWeatherInst1[] = {
    {
    .lvl = 36,
    .species = SPECIES_JUMPLUFF,
    },
    {
    .lvl = 36,
    .species = SPECIES_BRELOOM,
    }
};

static const struct TrainerMon sParty_GruntWeatherInst2[] = {
    {
    .lvl = 38,
    .species = SPECIES_ARBOK,
    },
    {
    .lvl = 37,
    .species = SPECIES_ARCANINE,
    .ability = ABILITY_INTIMIDATE
    }
};

static const struct TrainerMon sParty_GruntWeatherInst3[] = {
    {
    .lvl = 36,
    .species = SPECIES_HONCHKROW,
    },
    {
    .lvl = 37,
    .species = SPECIES_SWOOBAT,
    },
    {
    .lvl = 39,
    .species = SPECIES_SEVIPER,
    }
};

static const struct TrainerMon sParty_GruntMuseum1[] = {
    {
    .lvl = 21,
    .species = SPECIES_ONIX,
    },
    {
    .lvl = 22,
    .species = SPECIES_GOLBAT,
    },
    {
    .lvl = 21,
    .species = SPECIES_GASTRODON,
    }
};

static const struct TrainerMon sParty_GruntMuseum2[] = {
    {
    .lvl = 19,
    .species = SPECIES_MEOWTH_ALOLAN,
    },
    {
    .lvl = 19,
    .species = SPECIES_MEOWTH_GALARIAN,
    }
};

static const struct TrainerMon sParty_GruntSpaceCenter1[] = {
    {
    .lvl = 58,
    .species = SPECIES_TORKOAL,
    .ability = ABILITY_DROUGHT,
    },
    {
    .lvl = 59,
    .species = SPECIES_EXEGGUTOR,
    .ability = ABILITY_CHLOROPHYLL,
    .moves = {MOVE_PSYSHOCK, MOVE_GIGA_DRAIN, MOVE_SYNTHESIS, MOVE_SLEEP_POWDER}
    },
    {
    .lvl = 60,
    .species = SPECIES_CROBAT,
    .ability = ABILITY_INFILTRATOR,
    .heldItem = ITEM_BLACK_SLUDGE,
    .moves = {MOVE_BRAVE_BIRD, MOVE_CROSS_POISON, MOVE_TAUNT, MOVE_ROOST}
    }
};

static const struct TrainerMon sParty_GruntMtPyre1[] = {
    {
    .lvl = 52,
    .species = SPECIES_CRABOMINABLE,
    .ability = ABILITY_IRON_FIST,
    .moves = {MOVE_MACH_PUNCH, MOVE_ICE_PUNCH, MOVE_POWER_UP_PUNCH, MOVE_DRAIN_PUNCH}
    },
    {
    .lvl = 52,
    .species = SPECIES_COPPERAJAH,
    .ability = ABILITY_SHEER_FORCE,
    .moves = {MOVE_IRON_HEAD, MOVE_BULLDOZE, MOVE_ROCK_SLIDE, MOVE_POWER_WHIP}
    }
};

static const struct TrainerMon sParty_GruntMtPyre2[] = {
    {
    .lvl = 52,
    .species = SPECIES_MILOTIC,
    .moves = {MOVE_SURF, MOVE_ICE_BEAM, MOVE_RECOVER, MOVE_TOXIC}
    },
    {
    .lvl = 52,
    .species = SPECIES_DRIFBLIM,
    .heldItem = ITEM_SITRUS_BERRY,
    .ability = ABILITY_UNBURDEN,
    .moves = {MOVE_CALM_MIND, MOVE_SHADOW_BALL, MOVE_AIR_SLASH, MOVE_STRENGTH_SAP}
    },
    {
    .lvl = 52,
    .species = SPECIES_DRAGONAIR,
    .heldItem = ITEM_EVIOLITE,
    .moves = {MOVE_COIL, MOVE_REST, MOVE_AQUA_TAIL, MOVE_DRAGON_RUSH}
    }
};

static const struct TrainerMon sParty_GruntMtPyre3[] = {
    {
    .lvl = 53,
    .species = SPECIES_MIGHTYENA,
    .ability = ABILITY_INTIMIDATE,
    .moves = {MOVE_CRUNCH, MOVE_POISON_FANG, MOVE_HOWL, MOVE_FIRE_FANG}
    },
    {
    .lvl = 53,
    .species = SPECIES_DHELMISE,
    }
};

static const struct TrainerMon sParty_GruntWeatherInst4[] = {
    {
    .lvl = 36,
    .species = SPECIES_WEAVILE,
    }
};

static const struct TrainerMon sParty_GruntAquaHideout5[] = {
    {
    .lvl = 55,
    .species = SPECIES_POLIWRATH,
    },
    {
    .lvl = 55,
    .species = SPECIES_ARBOK,
    },
    {
    .lvl = 56,
    .species = SPECIES_TOXAPEX,
    }
};

static const struct TrainerMon sParty_GruntAquaHideout6[] = {
    {
    .lvl = 56,
    .species = SPECIES_PINCURCHIN,
    .ability = ABILITY_ELECTRIC_SURGE,
    },
    {
    .lvl = 56,
    .species = SPECIES_GYARADOS,
    },
    {
    .lvl = 56,
    .species = SPECIES_RAICHU_ALOLAN,
    .ability = ABILITY_SURGE_SURFER,
    }
};

static const struct TrainerMon sParty_Fredrick[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 71,
    .species = SPECIES_GRAPPLOCT,
    .ability = ABILITY_TECHNICIAN,
    .heldItem = ITEM_BLACK_BELT,
    .moves = {MOVE_CLOSE_COMBAT, MOVE_THROAT_CHOP, MOVE_POISON_JAB, MOVE_BULK_UP}
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 73,
    .species = SPECIES_MACHAMP,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 72,
    .species = SPECIES_CONKELDURR,
    .ability = ABILITY_GUTS,
    .heldItem = ITEM_FLAME_ORB,
    .moves = {MOVE_DRAIN_PUNCH, MOVE_MACH_PUNCH, MOVE_BULK_UP, MOVE_KNOCK_OFF}
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 72,
    .species = SPECIES_BRELOOM,
    .ability = ABILITY_TECHNICIAN,
    .moves = {MOVE_MACH_PUNCH, MOVE_BULLET_SEED, MOVE_SPORE, MOVE_SWORDS_DANCE}
    }
};

static const struct TrainerMon sParty_Matt[] = {
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 55,
    .species = SPECIES_ARAQUANID,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_STICKY_WEB, MOVE_LIQUIDATION, MOVE_LEECH_LIFE, MOVE_AQUA_JET}
    },
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 56,
    .species = SPECIES_DURALUDON,
    .ability = ABILITY_HEAVY_METAL,
    .heldItem = ITEM_LIFE_ORB,
    .moves = {MOVE_EARTHQUAKE, MOVE_HEAVY_SLAM, MOVE_THUNDERBOLT, MOVE_DRACO_METEOR}
    },
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 56,
    .species = SPECIES_MACHAMP,
    .ability = ABILITY_GUTS,
    .heldItem = ITEM_FLAME_ORB,
    .moves = {MOVE_EARTHQUAKE, MOVE_PROTECT, MOVE_BULLET_PUNCH, MOVE_FACADE}
    },
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 56,
    .species = SPECIES_WAILORD,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_WATER_SPOUT, MOVE_ICE_BEAM, MOVE_CURSE, MOVE_EXPLOSION}
    },
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 56,
    .species = SPECIES_HAXORUS,
    .heldItem = ITEM_SITRUS_BERRY,
    .ability = ABILITY_MOLD_BREAKER,
    .moves = {MOVE_EARTHQUAKE, MOVE_DRAGON_DANCE, MOVE_DRAGON_CLAW, MOVE_IRON_TAIL}
    },
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 56,
    .species = SPECIES_BLASTOISE,
    .heldItem = ITEM_BLASTOISINITE,
    .ability = ABILITY_MEGA_LAUNCHER,
    .moves = {MOVE_FLASH_CANNON, MOVE_DRAGON_PULSE, MOVE_WATER_PULSE, MOVE_SHELL_SMASH}
    }

};

static const struct TrainerMon sParty_Zander[] = {
    {
    .lvl = 52,
    .species = SPECIES_HARIYAMA,
    .ability = ABILITY_GUTS,
    .heldItem = ITEM_FLAME_ORB,
    .moves = {MOVE_FAKE_OUT, MOVE_CLOSE_COMBAT, MOVE_KNOCK_OFF, MOVE_PROTECT}
    }
};

static const struct TrainerMon sParty_ShellyWeatherInstitute[] = {
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 41,
    .species = SPECIES_SHARPEDO,
    .ability = ABILITY_SPEED_BOOST,
    .moves = {MOVE_PROTECT, MOVE_CRUNCH, MOVE_WATERFALL, MOVE_AQUA_JET}
    },
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 41,
    .species = SPECIES_BISHARP,
    .heldItem = ITEM_BLACK_GLASSES,
    .moves = {MOVE_SUCKER_PUNCH, MOVE_IRON_HEAD, MOVE_NONE, MOVE_NONE}
    },
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 43,
    .species = SPECIES_MILOTIC,
    .ability = ABILITY_MARVEL_SCALE,
    .heldItem = ITEM_FLAME_ORB,
    .moves = {MOVE_SCALD, MOVE_TOXIC, MOVE_RECOVER, MOVE_ICE_BEAM}
    },
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 43,
    .species = SPECIES_MEDICHAM,
    .moves = {MOVE_FAKE_OUT, MOVE_DYNAMIC_PUNCH, MOVE_PSYCHO_CUT, MOVE_MIND_READER}
    },
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 42,
    .species = SPECIES_KINGDRA,
    .moves = {MOVE_DRAGON_RAGE, MOVE_THUNDER_WAVE, MOVE_WATERFALL, MOVE_DOUBLE_TEAM}
    }
};

static const struct TrainerMon sParty_ShellySeafloorCavern[] = {
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 61,
    .species = SPECIES_SHARPEDO,
    .ability = ABILITY_SPEED_BOOST,
    .moves = {MOVE_PROTECT, MOVE_CRUNCH, MOVE_WATERFALL, MOVE_AQUA_JET}
    },
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 61,
    .species = SPECIES_KINGAMBIT,
    .heldItem = ITEM_BLACK_GLASSES,
    .moves = {MOVE_SUCKER_PUNCH, MOVE_IRON_HEAD, MOVE_SWORDS_DANCE, MOVE_KOWTOW_CLEAVE}
    },
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 62,
    .species = SPECIES_MILOTIC,
    .ability = ABILITY_MARVEL_SCALE,
    .heldItem = ITEM_FLAME_ORB,
    .moves = {MOVE_SCALD, MOVE_TOXIC, MOVE_RECOVER, MOVE_ICE_BEAM}
    },
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 61,
    .species = SPECIES_DRAGONITE,
    .moves = {MOVE_WING_ATTACK, MOVE_EXTREMESPEED, MOVE_DRAGON_DANCE, MOVE_ROOST}
    },
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 62,
    .species = SPECIES_KINGDRA,
    .moves = {MOVE_SURF, MOVE_ICE_BEAM, MOVE_RAIN_DANCE, MOVE_DOUBLE_TEAM}
    }
};

static const struct TrainerMon sParty_Archie[] = {
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 60,
    .species = SPECIES_CROBAT,
    .ability = ABILITY_INFILTRATOR,
    .heldItem = ITEM_BLACK_SLUDGE,
    .moves = {MOVE_HURRICANE, MOVE_CROSS_POISON, MOVE_U_TURN, MOVE_TAILWIND}
    },
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 62,
    .species = SPECIES_RAIKOU,
    .heldItem = ITEM_MAGNET,
    .moves = {MOVE_THUNDER, MOVE_WEATHER_BALL, MOVE_CALM_MIND, MOVE_RAIN_DANCE}
    },
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 62,
    .species = SPECIES_FERROTHORN,
    .ability = ABILITY_IRON_BARBS,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_POWER_WHIP, MOVE_GYRO_BALL, MOVE_SPIKES, MOVE_PROTECT}
    },
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 62,
    .species = SPECIES_SUICUNE,
    .heldItem = ITEM_MYSTIC_WATER,
    .moves = {MOVE_SCALD, MOVE_ICE_BEAM, MOVE_CALM_MIND, MOVE_REST}
    },
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 62,
    .species = SPECIES_SHARPEDO,
    .ability = ABILITY_SPEED_BOOST,
    .heldItem = ITEM_SHARPEDONITE,
    .moves = {MOVE_PROTECT, MOVE_WATERFALL, MOVE_CRUNCH, MOVE_PSYCHIC_FANGS}
    },
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 62,
    .species = SPECIES_METAGROSS,
    .heldItem = ITEM_WEAKNESS_POLICY,
    .moves = {MOVE_AGILITY, MOVE_METEOR_MASH, MOVE_ZEN_HEADBUTT, MOVE_EARTHQUAKE}
    }
};

static const struct TrainerMon sParty_Leah[] = {
    {
    .lvl = 48,
    .species = SPECIES_GRUMPIG,
    },
    {
    .lvl = 48,
    .species = SPECIES_MUSHARNA,
    }
};

static const struct TrainerMon sParty_Daisy[] = {
    {
    .lvl = 17,
    .species = SPECIES_SUNFLORA,
    },
    {
    .lvl = 18,
    .species = SPECIES_ROSELIA,
    }
};

static const struct TrainerMon sParty_Rose1[] = {
    {
    .lvl = 20,
    .species = SPECIES_GLOOM,
    },
    {
    .lvl = 20,
    .species = SPECIES_SHROOMISH,
    },
    {
    .lvl = 20,
    .species = SPECIES_ROSELIA,
    }
};

static const struct TrainerMon sParty_Felix[] = {
    {
    .lvl = 71,
    .species = SPECIES_MEDICHAM,
    .moves = {MOVE_ZEN_HEADBUTT, MOVE_FAKE_OUT, MOVE_HIGH_JUMP_KICK, MOVE_FIRE_PUNCH}
    },
    {
    .lvl = 71,
    .species = SPECIES_CLAYDOL,
    .moves = {MOVE_EXPLOSION, MOVE_EARTHQUAKE, MOVE_PSYCHIC, MOVE_NONE}
    }
};

static const struct TrainerMon sParty_Violet[] = {
    {
    .lvl = 70,
    .species = SPECIES_ROSERADE,
    .ability = ABILITY_NATURAL_CURE,
    .heldItem = ITEM_BLACK_SLUDGE,
    .moves = {MOVE_SLUDGE_BOMB, MOVE_GIGA_DRAIN, MOVE_DAZZLING_GLEAM, MOVE_TOXIC_SPIKES}
    },
    {
    .lvl = 70,
    .species = SPECIES_TREVENANT,
    .ability = ABILITY_HARVEST,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_WOOD_HAMMER, MOVE_SHADOW_CLAW, MOVE_LEECH_SEED, MOVE_WILL_O_WISP}
    },
    {
    .lvl = 70,
    .species = SPECIES_BELLOSSOM,
    .ability = ABILITY_CHLOROPHYLL,
    .heldItem = ITEM_BIG_ROOT,
    .moves = {MOVE_GIGA_DRAIN, MOVE_QUIVER_DANCE, MOVE_FIERY_DANCE, MOVE_SLUDGE_BOMB}
    },
    {
    .lvl = 70,
    .species = SPECIES_TOGEKISS,
    .ability = ABILITY_SERENE_GRACE,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_AIR_SLASH, MOVE_DAZZLING_GLEAM, MOVE_THUNDER_WAVE, MOVE_ROOST}
    },
    {
    .lvl = 70,
    .species = SPECIES_GARDEVOIR,
    .ability = ABILITY_TRACE,
    .heldItem = ITEM_PIXIE_PLATE,
    .moves = {MOVE_MOONBLAST, MOVE_PSYCHIC, MOVE_CALM_MIND, MOVE_SHADOW_BALL}
    },
    {
    .lvl = 71,
    .species = SPECIES_SYLVEON,
    .ability = ABILITY_PIXILATE,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_HYPER_VOICE, MOVE_MOONBLAST, MOVE_WISH, MOVE_PROTECT}
    },
};

static const struct TrainerMon sParty_Rose2[] = {
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 32,
    .species = SPECIES_BRELOOM,
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 32,
    .species = SPECIES_BELLOSSOM,
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 32,
    .species = SPECIES_ROSERADE,
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 32,
    .species = SPECIES_COMFEY,
    }
};

static const struct TrainerMon sParty_Rose3[] = {
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 37,
    .species = SPECIES_BRELOOM,
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 37,
    .species = SPECIES_BELLOSSOM,
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 37,
    .species = SPECIES_ROSERADE,
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 37,
    .species = SPECIES_COMFEY,
    }
};

static const struct TrainerMon sParty_Rose4[] = {
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 43,
    .species = SPECIES_BRELOOM,
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 44,
    .species = SPECIES_BELLOSSOM,
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 44,
    .species = SPECIES_ROSERADE,
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 44,
    .species = SPECIES_COMFEY,
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 44,
    .species = SPECIES_LURANTIS,
    .ability = ABILITY_CONTRARY,
    }
};

static const struct TrainerMon sParty_Rose5[] = {
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 52,
    .species = SPECIES_BRELOOM,
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 52,
    .species = SPECIES_BELLOSSOM,
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 52,
    .species = SPECIES_ROSERADE,
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 52,
    .species = SPECIES_COMFEY,
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 52,
    .species = SPECIES_LURANTIS,
    .ability = ABILITY_CONTRARY,
    }
};

static const struct TrainerMon sParty_Dusty1[] = {
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 29,
    .species = SPECIES_SANDSLASH,
    .moves = {MOVE_DIG, MOVE_CRUSH_CLAW, MOVE_ROCK_SLIDE, MOVE_SWORDS_DANCE}
    }
};

static const struct TrainerMon sParty_Chip[] = {
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 44,
    .species = SPECIES_CLAYDOL,
    .moves = {MOVE_PSYBEAM, MOVE_SELF_DESTRUCT, MOVE_SANDSTORM, MOVE_ANCIENT_POWER}
    },
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 44,
    .species = SPECIES_HIPPOWDON,
    },
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 45,
    .species = SPECIES_EXCADRILL,
    }
};

static const struct TrainerMon sParty_Foster[] = {
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 32,
    .species = SPECIES_DONPHAN,
    },
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 34,
    .species = SPECIES_MAMOSWINE,
    }
};

static const struct TrainerMon sParty_Dusty2[] = {
    {
    .iv = TRAINER_PARTY_IVS(8, 8, 8, 8, 8, 8),
    .lvl = 35,
    .species = SPECIES_SANDSLASH,
    .moves = {MOVE_EARTHQUAKE, MOVE_CRUSH_CLAW, MOVE_ROCK_SLIDE, MOVE_SWORDS_DANCE}
    }
};

static const struct TrainerMon sParty_Dusty3[] = {
    {
    .iv = TRAINER_PARTY_IVS(8, 8, 8, 8, 8, 8),
    .lvl = 39,
    .species = SPECIES_SANDSLASH,
    .moves = {MOVE_EARTHQUAKE, MOVE_CRUSH_CLAW, MOVE_ROCK_SLIDE, MOVE_SWORDS_DANCE}
    }
};

static const struct TrainerMon sParty_Dusty4[] = {
    {
    .iv = TRAINER_PARTY_IVS(9, 9, 9, 9, 9, 9),
    .lvl = 42,
    .species = SPECIES_SANDSLASH,
    .moves = {MOVE_EARTHQUAKE, MOVE_CRUSH_CLAW, MOVE_ROCK_SLIDE, MOVE_SWORDS_DANCE}
    }
};

static const struct TrainerMon sParty_Dusty5[] = {
    {
    .iv = TRAINER_PARTY_IVS(10, 10, 10, 10, 10, 10),
    .lvl = 46,
    .species = SPECIES_SANDSLASH,
    .moves = {MOVE_EARTHQUAKE, MOVE_CRUSH_CLAW, MOVE_ROCK_SLIDE, MOVE_SWORDS_DANCE}
    }
};

static const struct TrainerMon sParty_GabbyAndTy1[] = {
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 23,
    .species = SPECIES_MAGNEMITE,
    },
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 23,
    .species = SPECIES_WHISMUR,
    },
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 23,
    .species = SPECIES_FLAAFFY,
    }
};

static const struct TrainerMon sParty_GabbyAndTy2[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 37,
    .species = SPECIES_MAGNETON,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 37,
    .species = SPECIES_LOUDRED,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 37,
    .species = SPECIES_AMPHAROS,
    }
};

static const struct TrainerMon sParty_GabbyAndTy3[] = {
    {
    .iv = TRAINER_PARTY_IVS(18, 18, 18, 18, 18, 18),
    .lvl = 40,
    .species = SPECIES_MAGNEZONE,
    .moves = {MOVE_TRI_ATTACK, MOVE_THUNDER_WAVE, MOVE_FLASH_CANNON, MOVE_THUNDERBOLT}
    },
    {
    .iv = TRAINER_PARTY_IVS(18, 18, 18, 18, 18, 18),
    .lvl = 39,
    .species = SPECIES_LOUDRED,
    .moves = {MOVE_HYPER_VOICE, MOVE_EXTRASENSORY, MOVE_PROTECT, MOVE_BRICK_BREAK}
    },
    {
    .iv = TRAINER_PARTY_IVS(18, 18, 18, 18, 18, 18),
    .lvl = 40,
    .species = SPECIES_AMPHAROS,
    .moves = {MOVE_THUNDERBOLT, MOVE_FIRE_PUNCH, MOVE_PROTECT, MOVE_DRAGON_TAIL}
    }
};

static const struct TrainerMon sParty_GabbyAndTy4[] = {
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 46,
    .species = SPECIES_MAGNEZONE,
    .moves = {MOVE_TRI_ATTACK, MOVE_THUNDER_WAVE, MOVE_FLASH_CANNON, MOVE_THUNDERBOLT}
    },
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 46,
    .species = SPECIES_EXPLOUD,
    .moves = {MOVE_HYPER_VOICE, MOVE_FLAMETHROWER, MOVE_PROTECT, MOVE_SHADOW_BALL}
    },
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 46,
    .species = SPECIES_SCYTHER,
    .ability = ABILITY_TECHNICIAN,
    .moves = {MOVE_DUAL_WINGBEAT, MOVE_SWORDS_DANCE, MOVE_BUG_BITE, MOVE_ROOST}
    },
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 46,
    .species = SPECIES_AMPHAROS,
    .moves = {MOVE_THUNDERBOLT, MOVE_FIRE_PUNCH, MOVE_PROTECT, MOVE_DRAGON_PULSE}
    }
};

static const struct TrainerMon sParty_GabbyAndTy5[] = {
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 53,
    .species = SPECIES_MAGNEZONE,
    .moves = {MOVE_TRI_ATTACK, MOVE_THUNDER_WAVE, MOVE_FLASH_CANNON, MOVE_THUNDERBOLT}
    },
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 52,
    .species = SPECIES_EXPLOUD,
    .moves = {MOVE_HYPER_VOICE, MOVE_FLAMETHROWER, MOVE_PROTECT, MOVE_SHADOW_BALL}
    },
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 53,
    .species = SPECIES_SCIZOR,
    .moves = {MOVE_BULLET_PUNCH, MOVE_SWORDS_DANCE, MOVE_BUG_BITE, MOVE_ROOST}
    },
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 53,
    .species = SPECIES_AMPHAROS,
    .moves = {MOVE_THUNDERBOLT, MOVE_FIRE_PUNCH, MOVE_PROTECT, MOVE_DRAGON_PULSE}
    },
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 53,
    .species = SPECIES_NOIVERN,
    .moves = {MOVE_DRACO_METEOR, MOVE_HEAT_WAVE, MOVE_SUPER_FANG, MOVE_HURRICANE}
    }
};

static const struct TrainerMon sParty_GabbyAndTy6[] = {
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 58,
    .species = SPECIES_MAGNEZONE,
    .heldItem = ITEM_AIR_BALLOON,
    .moves = {MOVE_TRI_ATTACK, MOVE_THUNDER_WAVE, MOVE_FLASH_CANNON, MOVE_THUNDERBOLT}
    },
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 58,
    .species = SPECIES_EXPLOUD,
    .moves = {MOVE_HYPER_VOICE, MOVE_FLAMETHROWER, MOVE_PROTECT, MOVE_SHADOW_BALL}
    },
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 58,
    .species = SPECIES_SCIZOR,
    .moves = {MOVE_BULLET_PUNCH, MOVE_SWORDS_DANCE, MOVE_BUG_BITE, MOVE_ROOST}
    },
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 58,
    .species = SPECIES_AMPHAROS,
    .moves = {MOVE_THUNDERBOLT, MOVE_FIRE_PUNCH, MOVE_PROTECT, MOVE_DRAGON_PULSE}
    },
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 59,
    .species = SPECIES_NOIVERN,
    .moves = {MOVE_DRACO_METEOR, MOVE_HEAT_WAVE, MOVE_SUPER_FANG, MOVE_HURRICANE}
    }
};

static const struct TrainerMon sParty_Lola1[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 18,
    .species = SPECIES_AZUMARILL,
    .ability = ABILITY_HUGE_POWER
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 18,
    .species = SPECIES_SANDYGAST,
    }
};

static const struct TrainerMon sParty_Austina[] = {
    {
    .lvl = 26,
    .species = SPECIES_BRUXISH,
    }
};

static const struct TrainerMon sParty_Gwen[] = {
    {
    .lvl = 26,
    .species = SPECIES_LUMINEON,
    }
};

static const struct TrainerMon sParty_Lola2[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 34,
    .species = SPECIES_AZUMARILL,
    .ability = ABILITY_HUGE_POWER
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 34,
    .species = SPECIES_SANDYGAST,
    }
};

static const struct TrainerMon sParty_Lola3[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 38,
    .species = SPECIES_AZUMARILL,
    .ability = ABILITY_HUGE_POWER,
    .moves = {MOVE_AQUA_JET, MOVE_ICE_PUNCH, MOVE_AQUA_TAIL, MOVE_SUPERPOWER}
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 39,
    .species = SPECIES_PALOSSAND,
    .moves = {MOVE_GIGA_DRAIN, MOVE_EARTHQUAKE, MOVE_SHORE_UP, MOVE_SHADOW_BALL}
    }
};

static const struct TrainerMon sParty_Lola4[] = {
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 41,
    .species = SPECIES_AZUMARILL,
    .ability = ABILITY_HUGE_POWER,
    .moves = {MOVE_AQUA_JET, MOVE_ICE_PUNCH, MOVE_AQUA_TAIL, MOVE_SUPERPOWER}
    },
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 41,
    .species = SPECIES_PALOSSAND,
    .moves = {MOVE_GIGA_DRAIN, MOVE_EARTHQUAKE, MOVE_SHORE_UP, MOVE_SHADOW_BALL}
    },
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 41,
    .species = SPECIES_FLOATZEL,
    .moves = {MOVE_AQUA_JET, MOVE_ICE_FANG, MOVE_FLIP_TURN, MOVE_CRUNCH}
    }
};

static const struct TrainerMon sParty_Lola5[] = {
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 46,
    .species = SPECIES_AZUMARILL,
    .ability = ABILITY_HUGE_POWER,
    .moves = {MOVE_AQUA_JET, MOVE_ICE_PUNCH, MOVE_AQUA_TAIL, MOVE_SUPERPOWER}
    },
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 46,
    .species = SPECIES_PALOSSAND,
    .moves = {MOVE_GIGA_DRAIN, MOVE_EARTHQUAKE, MOVE_SHORE_UP, MOVE_SHADOW_BALL}
    },
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 46,
    .species = SPECIES_FLOATZEL,
    .moves = {MOVE_AQUA_JET, MOVE_ICE_FANG, MOVE_FLIP_TURN, MOVE_CRUNCH}
    },
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 46,
    .species = SPECIES_WALREIN,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_ICE_BEAM, MOVE_SURF, MOVE_FLIP_TURN, MOVE_PROTECT}
    }
};

static const struct TrainerMon sParty_Ricky1[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 17,
    .species = SPECIES_ZIGZAGOON,
    .moves = {MOVE_THUNDERBOLT, MOVE_HEADBUTT, MOVE_TAIL_WHIP, MOVE_SURF}
    }
};

static const struct TrainerMon sParty_Simon[] = {
    {
    .lvl = 16,
    .species = SPECIES_DUCKLETT,
    },
    {
    .lvl = 17,
    .species = SPECIES_QUAXWELL,
    }
};

static const struct TrainerMon sParty_Charlie[] = {
    {
    .lvl = 36,
    .species = SPECIES_RELICANTH,
    .ability = ABILITY_ROCK_HEAD,
    .moves = {MOVE_HEAD_SMASH, MOVE_DOUBLE_EDGE, MOVE_WAVE_CRASH, MOVE_ROCK_POLISH}
    }
};

static const struct TrainerMon sParty_Ricky2[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 31,
    .species = SPECIES_LINOONE,
    .heldItem = ITEM_EXPERT_BELT,
    .moves = {MOVE_DIG, MOVE_PIN_MISSILE, MOVE_THUNDERBOLT, MOVE_SURF}
    }
};

static const struct TrainerMon sParty_Ricky3[] = {
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 36,
    .species = SPECIES_LINOONE,
    .heldItem = ITEM_EXPERT_BELT,
    .moves = {MOVE_DIG, MOVE_PIN_MISSILE, MOVE_THUNDERBOLT, MOVE_SURF}
    }
};

static const struct TrainerMon sParty_Ricky4[] = {
    {
    .iv = TRAINER_PARTY_IVS(13, 13, 13, 13, 13, 13),
    .lvl = 40,
    .species = SPECIES_LINOONE,
    .heldItem = ITEM_EXPERT_BELT,
    .moves = {MOVE_DIG, MOVE_PIN_MISSILE, MOVE_THUNDERBOLT, MOVE_SURF}
    },
    {
    .iv = TRAINER_PARTY_IVS(13, 13, 13, 13, 13, 13),
    .lvl = 40,
    .species = SPECIES_FURRET,
    .heldItem = ITEM_EXPERT_BELT,
    .moves = {MOVE_ICE_BEAM, MOVE_DOUBLE_EDGE, MOVE_FLAMETHROWER, MOVE_SURF}
    },
    {
    .iv = TRAINER_PARTY_IVS(13, 13, 13, 13, 13, 13),
    .lvl = 40,
    .species = SPECIES_LICKILICKY,
    .heldItem = ITEM_EXPERT_BELT,
    .moves = {MOVE_BLIZZARD, MOVE_FIRE_BLAST, MOVE_FOCUS_BLAST, MOVE_SURF}
    }
};

static const struct TrainerMon sParty_Ricky5[] = {
    {
    .iv = TRAINER_PARTY_IVS(13, 13, 13, 13, 13, 13),
    .lvl = 45,
    .species = SPECIES_LINOONE,
    .heldItem = ITEM_EXPERT_BELT,
    .moves = {MOVE_DIG, MOVE_PIN_MISSILE, MOVE_THUNDERBOLT, MOVE_SURF}
    },
    {
    .iv = TRAINER_PARTY_IVS(13, 13, 13, 13, 13, 13),
    .lvl = 45,
    .species = SPECIES_FURRET,
    .heldItem = ITEM_EXPERT_BELT,
    .moves = {MOVE_ICE_BEAM, MOVE_DOUBLE_EDGE, MOVE_FLAMETHROWER, MOVE_SURF}
    },
    {
    .iv = TRAINER_PARTY_IVS(13, 13, 13, 13, 13, 13),
    .lvl = 46,
    .species = SPECIES_LICKILICKY,
    .heldItem = ITEM_EXPERT_BELT,
    .moves = {MOVE_BLIZZARD, MOVE_FIRE_BLAST, MOVE_FOCUS_BLAST, MOVE_SURF}
    }
};

static const struct TrainerMon sParty_Randall[] = {
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 36,
    .species = SPECIES_DEOXYS_SPEED,
    .heldItem = ITEM_ASSAULT_VEST,
    .moves = {MOVE_PSYCHO_BOOST, MOVE_SUPERPOWER, MOVE_SHADOW_BALL, MOVE_EXTREMESPEED}
    }
};

static const struct TrainerMon sParty_Parker[] = {
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 37,
    .species = SPECIES_SPINDA,
    .heldItem = ITEM_WIDE_LENS,
    .ability = ABILITY_CONTRARY,
    .moves = {MOVE_HYPNOSIS, MOVE_DIZZY_PUNCH, MOVE_SUPERPOWER, MOVE_FEINT_ATTACK}
    }
};

static const struct TrainerMon sParty_George[] = {
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 37,
    .species = SPECIES_SHAYMIN_SKY,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_AIR_SLASH, MOVE_LEECH_SEED, MOVE_GIGA_DRAIN, MOVE_SUBSTITUTE}
    }
};

static const struct TrainerMon sParty_Berke[] = {
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 37,
    .species = SPECIES_KYUREM_WHITE,
    .heldItem = ITEM_SCOPE_LENS,
    .moves = {MOVE_LASER_FOCUS, MOVE_DRACO_METEOR, MOVE_INFERNO, MOVE_BLIZZARD}
    }
};

static const struct TrainerMon sParty_Braxton[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 69,
    .species = SPECIES_SWELLOW,
    .ability = ABILITY_SCRAPPY,
    .moves = {MOVE_BOOMBURST, MOVE_AIR_SLASH, MOVE_HEAT_WAVE, MOVE_QUICK_ATTACK}
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 69,
    .species = SPECIES_FLYGON,
    .moves = {MOVE_ROOST, MOVE_DRACO_METEOR, MOVE_EARTHQUAKE, MOVE_U_TURN}
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 69,
    .species = SPECIES_WAILORD,
    .moves = {MOVE_WATER_SPOUT, MOVE_SURF, MOVE_ICE_BEAM, MOVE_HYPER_VOICE}
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 70,
    .species = SPECIES_MAGNEZONE,
    .ability = ABILITY_MAGNET_PULL,
    .moves = {MOVE_THUNDERBOLT, MOVE_FLASH_CANNON, MOVE_THUNDER_WAVE, MOVE_VOLT_SWITCH}
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 70,
    .species = SPECIES_SHIFTRY,
    .moves = {MOVE_GIGA_DRAIN, MOVE_DARK_PULSE, MOVE_DOUBLE_TEAM, MOVE_SUCKER_PUNCH}
    }
};

static const struct TrainerMon sParty_Vincent[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 55,
    .species = SPECIES_SABLEYE,
    .ability = ABILITY_PRANKSTER,
    .moves = {MOVE_WILL_O_WISP, MOVE_RECOVER, MOVE_NIGHT_SHADE, MOVE_KNOCK_OFF}

    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 55,
    .species = SPECIES_MEDICHAM,
    .moves = {MOVE_BULK_UP, MOVE_CLOSE_COMBAT, MOVE_PSYCHO_CUT, MOVE_ICE_PUNCH}
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 57,
    .species = SPECIES_SHARPEDO,
    .ability = ABILITY_SPEED_BOOST,
    .moves = {MOVE_ICE_BEAM, MOVE_DARK_PULSE, MOVE_SURF, MOVE_PROTECT}
    }
};

static const struct TrainerMon sParty_Leroy[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 56,
    .species = SPECIES_KINGAMBIT,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 57,
    .species = SPECIES_FLORGES,
    }
};

static const struct TrainerMon sParty_Wilton1[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 27,
    .species = SPECIES_EMOLGA,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 29,
    .species = SPECIES_PIGNITE,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 29,
    .species = SPECIES_VANILLISH,
    }
};

static const struct TrainerMon sParty_Edgar[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 69,
    .species = SPECIES_URSHIFU_SINGLE_STRIKE_STYLE,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 69,
    .species = SPECIES_URSHIFU_RAPID_STRIKE_STYLE,
    }
};

static const struct TrainerMon sParty_Albert[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 72,
    .species = SPECIES_MAGNEZONE,
    .ability = ABILITY_MAGNET_PULL,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 72,
    .species = SPECIES_GOTHITELLE,
    .ability = ABILITY_SHADOW_TAG,
    }
};

static const struct TrainerMon sParty_Samuel[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 72,
    .species = SPECIES_STARAPTOR,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 72,
    .species = SPECIES_HAXORUS,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 74,
    .species = SPECIES_ALAKAZAM,
    .heldItem = ITEM_ALAKAZITE,
    }
};

static const struct TrainerMon sParty_Vito[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 74,
    .species = SPECIES_DODRIO,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 75,
    .species = SPECIES_CONKELDURR,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 75,
    .species = SPECIES_HELIOLISK,
    },
    
};

static const struct TrainerMon sParty_Owen[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 74,
    .species = SPECIES_KECLEON,
    .ability = ABILITY_PROTEAN,
    .moves = {MOVE_SUCKER_PUNCH, MOVE_SHADOW_SNEAK, MOVE_BRICK_BREAK, MOVE_DOUBLE_EDGE}
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 72,
    .species = SPECIES_GOLURK,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 74,
    .species = SPECIES_SPIRITOMB,
    }
};

static const struct TrainerMon sParty_Wilton2[] = {
    {
    .iv = TRAINER_PARTY_IVS(13, 13, 13, 13, 13, 13),
    .lvl = 36,
    .species = SPECIES_EMOLGA,
    },
    {
    .iv = TRAINER_PARTY_IVS(13, 13, 13, 13, 13, 13),
    .lvl = 35,
    .species = SPECIES_PIGNITE,
    },
    {
    .iv = TRAINER_PARTY_IVS(13, 13, 13, 13, 13, 13),
    .lvl = 36,
    .species = SPECIES_VANILLISH,
    }
};

static const struct TrainerMon sParty_Wilton3[] = {
    {
    .iv = TRAINER_PARTY_IVS(14, 14, 14, 14, 14, 14),
    .lvl = 40,
    .species = SPECIES_EMOLGA,
    },
    {
    .iv = TRAINER_PARTY_IVS(14, 14, 14, 14, 14, 14),
    .lvl = 40,
    .species = SPECIES_PIGNITE,
    },
    {
    .iv = TRAINER_PARTY_IVS(14, 14, 14, 14, 14, 14),
    .lvl = 40,
    .species = SPECIES_VANILLUXE,
    }
};

static const struct TrainerMon sParty_Wilton4[] = {
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 47,
    .species = SPECIES_EMOLGA,
    },
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 47,
    .species = SPECIES_EMBOAR,
    },
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 47,
    .species = SPECIES_VANILLUXE,
    }
};

static const struct TrainerMon sParty_Wilton5[] = {
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 55,
    .species = SPECIES_EMOLGA,
    .ability = ABILITY_LIGHTNING_ROD,
    },
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 55,
    .species = SPECIES_EMBOAR,
    .ability = ABILITY_RECKLESS,
    .moves = {MOVE_WILD_CHARGE, MOVE_HEAD_SMASH, MOVE_FLARE_BLITZ, MOVE_EARTHQUAKE}
    },
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 57,
    .species = SPECIES_VANILLUXE,
    .ability = ABILITY_SNOW_WARNING
    }
};

static const struct TrainerMon sParty_Warren[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 46,
    .species = SPECIES_GIGALITH,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 46,
    .species = SPECIES_LUDICOLO,
    }
};

static const struct TrainerMon sParty_Mary[] = {
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 36,
    .species = SPECIES_PALKIA,
    .heldItem = ITEM_LAX_INCENSE,
    .moves = {MOVE_BLIZZARD, MOVE_DRACO_METEOR, MOVE_HYDRO_PUMP, MOVE_FOCUS_BLAST}
    }
};

static const struct TrainerMon sParty_Alexia[] = {
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 36,
    .species = SPECIES_ZAMAZENTA_CROWNED_SHIELD,
    .heldItem = ITEM_RUSTED_SHIELD,
    .moves = {MOVE_BEHEMOTH_BASH, MOVE_IRON_DEFENSE, MOVE_BODY_PRESS, MOVE_REST}
    }
};

static const struct TrainerMon sParty_Jody[] = {
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 36,
    .species = SPECIES_RAYQUAZA,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_DRAGON_DANCE, MOVE_FLY, MOVE_AQUA_TAIL, MOVE_DRAGON_CLAW}
    }
};

static const struct TrainerMon sParty_Wendy[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 71,
    .species = SPECIES_MAWILE,
    .moves = {MOVE_BATON_PASS, MOVE_PLAY_ROUGH, MOVE_SWORDS_DANCE, MOVE_IRON_HEAD}
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 72,
    .species = SPECIES_ROSERADE,
    .moves = {MOVE_MEGA_DRAIN, MOVE_SLUDGE_BOMB, MOVE_GRASS_WHISTLE, MOVE_LEECH_SEED}
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 72,
    .species = SPECIES_PELIPPER,
    .ability =ABILITY_DRIZZLE,
    .moves = {MOVE_FLY, MOVE_WATERFALL, MOVE_U_TURN, MOVE_PROTECT}
    }
};

static const struct TrainerMon sParty_Keira[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 58,
    .species = SPECIES_AGGRON,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 59,
    .species = SPECIES_NOIVERN,
    }
};

static const struct TrainerMon sParty_Brooke1[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 17,
    .species = SPECIES_SKORUPI,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 17,
    .species = SPECIES_PINCURCHIN,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 17,
    .species = SPECIES_CHATOT,
    }
};

static const struct TrainerMon sParty_Jennifer[] = {
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 42,
    .species = SPECIES_MISMAGIUS,
    }
};

static const struct TrainerMon sParty_Hope[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 69,
    .species = SPECIES_HIPPOWDON,
    .moves = {MOVE_EARTHQUAKE, MOVE_CRUNCH, MOVE_STONE_EDGE, MOVE_SLACK_OFF}
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 69,
    .species = SPECIES_MELMETAL_GIGANTAMAX,
    .moves = {MOVE_EARTHQUAKE, MOVE_DOUBLE_IRON_BASH, MOVE_THUNDER_PUNCH, MOVE_IRON_DEFENSE}
    }
};

static const struct TrainerMon sParty_Shannon[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 75,
    .species = SPECIES_CORVIKNIGHT,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 74,
    .species = SPECIES_TALONFLAME,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 75,
    .species = SPECIES_PIDGEOT,
    .heldItem = ITEM_PIDGEOTITE,
    }
};

static const struct TrainerMon sParty_Michelle[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 69,
    .species = SPECIES_TORKOAL,
    .ability = ABILITY_DROUGHT,
    .moves = {MOVE_STEALTH_ROCK, MOVE_LAVA_PLUME, MOVE_EARTHQUAKE, MOVE_EXPLOSION}
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 70,
    .species = SPECIES_SCOVILLAIN,
    .ability = ABILITY_CHLOROPHYLL,
    .moves = {MOVE_FLAMETHROWER, MOVE_SOLAR_BEAM, MOVE_GROWTH, MOVE_STOMPING_TANTRUM}
    },
    { 
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 70,
    .species = SPECIES_LILLIGANT_HISUIAN,
    .ability = ABILITY_CHLOROPHYLL,
    .moves = {MOVE_SOLAR_BLADE, MOVE_VICTORY_DANCE, MOVE_CLOSE_COMBAT, MOVE_SYNTHESIS}
    }
};

static const struct TrainerMon sParty_Caroline[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 69,
    .species = SPECIES_SKARMORY,
    .moves = {MOVE_BODY_PRESS, MOVE_ROOST, MOVE_IRON_DEFENSE, MOVE_BRAVE_BIRD}
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 70,
    .species = SPECIES_DUSKNOIR,
    .moves = {MOVE_DYNAMIC_PUNCH, MOVE_SHADOW_PUNCH, MOVE_TRICK_ROOM, MOVE_FIRE_PUNCH}
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 70,
    .species = SPECIES_SLOWBRO,
    .heldItem = ITEM_SLOWBRONITE,
    .moves = {MOVE_SCALD, MOVE_PSYCHIC, MOVE_TRICK_ROOM, MOVE_ICE_BEAM}
    }
};

static const struct TrainerMon sParty_Julie[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 72,
    .species = SPECIES_NINETALES,
    .ability = ABILITY_DROUGHT,
    .moves = {MOVE_FLAMETHROWER, MOVE_PSYCHIC, MOVE_ENCORE, MOVE_PROTECT}
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 72,
    .species = SPECIES_EXEGGUTOR_ALOLAN,
    .moves = {MOVE_FLAMETHROWER, MOVE_DRAGON_HAMMER, MOVE_SLEEP_POWDER, MOVE_LEAF_STORM}
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 72,
    .species = SPECIES_CAMERUPT,
    .heldItem = ITEM_CAMERUPTITE,
    .moves = {MOVE_ERUPTION, MOVE_EARTH_POWER, MOVE_ROCK_SLIDE, MOVE_STEAM_ERUPTION}
    }
};

static const struct TrainerMon sParty_Brooke2[] = {
    {
    .iv = TRAINER_PARTY_IVS(13, 13, 13, 13, 13, 13),
    .lvl = 28,
    .species = SPECIES_SKORUPI,
    },
    {
    .iv = TRAINER_PARTY_IVS(13, 13, 13, 13, 13, 13),
    .lvl = 28,
    .species = SPECIES_ELECTABUZZ,
    },
    {
    .iv = TRAINER_PARTY_IVS(13, 13, 13, 13, 13, 13),
    .lvl = 28,
    .species = SPECIES_CHATOT,
    }
};

static const struct TrainerMon sParty_Brooke3[] = {
    {
    .iv = TRAINER_PARTY_IVS(14, 14, 14, 14, 14, 14),
    .lvl = 35,
    .species = SPECIES_DRAPION,
    },
    {
    .iv = TRAINER_PARTY_IVS(14, 14, 14, 14, 14, 14),
    .lvl = 35,
    .species = SPECIES_ELECTIVIRE,
    },
    {
    .iv = TRAINER_PARTY_IVS(14, 14, 14, 14, 14, 14),
    .lvl = 35,
    .species = SPECIES_CHATOT,
    }
};

static const struct TrainerMon sParty_Brooke4[] = {
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 41,
    .species = SPECIES_DRAPION,
    },
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 41,
    .species = SPECIES_ELECTIVIRE,
    },
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 41,
    .species = SPECIES_CHATOT,
    },
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 41,
    .species = SPECIES_MAGMORTAR,
    }

};

static const struct TrainerMon sParty_Brooke5[] = {
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 46,
    .species = SPECIES_DRAPION,
    .ability = ABILITY_SNIPER,
    .moves = {MOVE_WICKED_BLOW, MOVE_CROSS_POISON, MOVE_FOCUS_ENERGY, MOVE_EARTHQUAKE}
    },
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 48,
    .species = SPECIES_ELECTIVIRE,
    },
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 46,
    .species = SPECIES_CHATOT,
    .moves = {MOVE_BOOMBURST, MOVE_AIR_SLASH, MOVE_ROOST, MOVE_U_TURN}
    },
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 48,
    .species = SPECIES_MAGMORTAR,
    .ability = ABILITY_MEGA_LAUNCHER,
    .moves = {MOVE_FLAMETHROWER, MOVE_AURA_SPHERE, MOVE_DARK_PULSE, MOVE_THUNDERBOLT}
    }
};

static const struct TrainerMon sParty_Patricia[] = {
    {
    .lvl = 57,
    .species = SPECIES_TOGEKISS,
    },
    {
    .lvl = 57,
    .species = SPECIES_URSALUNA,
    }
};

static const struct TrainerMon sParty_Kindra[] = {
    {
    .lvl = 72,
    .species = SPECIES_ZOROARK_HISUIAN,
    .moves = {MOVE_NIGHT_SLASH, MOVE_FLAMETHROWER, MOVE_SHADOW_BALL, MOVE_FOCUS_BLAST}
    },
    {
    .lvl = 72,
    .species = SPECIES_GENGAR,
    .ability = ABILITY_LEVITATE,
    .heldItem = ITEM_BLACK_SLUDGE,
    .moves = {MOVE_SHADOW_BALL, MOVE_SLUDGE_BOMB, MOVE_FOCUS_BLAST, MOVE_THUNDERBOLT}
    }
};

static const struct TrainerMon sParty_Tammy[] = {
    {
    .lvl = 42,
    .species = SPECIES_DUSKNOIR,
    },
    {
    .lvl = 43,
    .species = SPECIES_LIEPARD,
    }
};

static const struct TrainerMon sParty_Valerie1[] = {
    {
    .lvl = 52,
    .species = SPECIES_COFAGRIGUS,
    },
    {
    .lvl = 52,
    .species = SPECIES_RUNERIGUS,
    }
};

static const struct TrainerMon sParty_Tasha[] = {
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 55,
    .species = SPECIES_BANETTE,
    },
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 55,
    .species = SPECIES_SABLEYE,
    }
};

static const struct TrainerMon sParty_Valerie2[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 37,
    .species = SPECIES_GRANBULL,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 38,
    .species = SPECIES_GRAPPLOCT,
    }
};

static const struct TrainerMon sParty_Valerie3[] = {
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 41,
    .species = SPECIES_GRANBULL,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 43,
    .species = SPECIES_GRAPPLOCT,
    }
};

static const struct TrainerMon sParty_Valerie4[] = {
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 47,
    .species = SPECIES_GRANBULL,
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 47,
    .species = SPECIES_GRAPPLOCT,
    }
};

static const struct TrainerMon sParty_Valerie5[] = {
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 52,
    .species = SPECIES_GRANBULL,
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 52,
    .species = SPECIES_GRAPPLOCT,
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 52,
    .species = SPECIES_GRUMPIG,
    }
};

static const struct TrainerMon sParty_Cindy1[] = {
    {
    .lvl = 9,
    .species = SPECIES_EEVEE,
    .heldItem = ITEM_NUGGET
    }
};

static const struct TrainerMon sParty_Daphne[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 66,
    .species = SPECIES_LUVDISC,
    .heldItem = ITEM_NUGGET,
    .moves = {MOVE_ATTRACT, MOVE_DRAINING_KISS, MOVE_ICE_BEAM, MOVE_WATER_PULSE}
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 64,
    .species = SPECIES_CRAWDAUNT,
    .heldItem = ITEM_NUGGET,
    .ability = ABILITY_ADAPTABILITY,
    .moves = {MOVE_AQUA_JET, MOVE_FEINT_ATTACK, MOVE_SWORDS_DANCE, MOVE_CRABHAMMER}
    }
};

static const struct TrainerMon sParty_GruntSpaceCenter2[] = {
    {
    .lvl = 58,
    .species = SPECIES_MALAMAR,
    .ability = ABILITY_CONTRARY,
    .moves = {MOVE_SUPERPOWER, MOVE_NIGHT_SLASH, MOVE_PSYCHO_CUT, MOVE_SWITCHEROO}
    },
    {
    .lvl = 60,
    .species = SPECIES_SCRAFTY,
    .ability = ABILITY_MOXIE,
    .moves = {MOVE_KNOCK_OFF, MOVE_HIGH_JUMP_KICK, MOVE_IRON_HEAD, MOVE_DRAGON_DANCE}
    },
    {
    .lvl = 60,
    .species = SPECIES_TURTONATOR,
    .ability = ABILITY_SHELL_ARMOR,
    .moves = {MOVE_DRAGON_PULSE, MOVE_FIRE_BLAST, MOVE_SHELL_TRAP, MOVE_DRAGON_DANCE}
    }
};

static const struct TrainerMon sParty_Cindy2[] = {
    {
    .lvl = 11,
    .species = SPECIES_EEVEE,
    .heldItem = ITEM_NUGGET,
    .moves = {MOVE_QUICK_ATTACK, MOVE_LAST_RESORT, MOVE_NONE, MOVE_NONE}
    }
};

static const struct TrainerMon sParty_Brianna[] = {
    {
    .iv = TRAINER_PARTY_IVS(18, 18, 18, 18, 18, 18),
    .lvl = 66,
    .species = SPECIES_GYARADOS,
    .heldItem = ITEM_NUGGET
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 66,
    .species = SPECIES_NOCTOWL,
    .ability = ABILITY_INSOMNIA,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_HURRICANE, MOVE_AIR_SLASH, MOVE_ROOST, MOVE_HYPNOSIS}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 66,
    .species = SPECIES_LILLIGANT,
    .ability = ABILITY_CHLOROPHYLL,
    .heldItem = ITEM_BIG_ROOT,
    .moves = {MOVE_WEATHER_BALL, MOVE_GIGA_DRAIN, MOVE_SLEEP_POWDER, MOVE_QUIVER_DANCE}
    }

};

static const struct TrainerMon sParty_Naomi[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 56,
    .species = SPECIES_TANGROWTH,
    .heldItem = ITEM_NUGGET
    }
};

static const struct TrainerMon sParty_Cindy3[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 34,
    .species = SPECIES_FLAREON,
    .heldItem = ITEM_TOXIC_ORB,
    .ability = ABILITY_GUTS,
    .moves = {MOVE_FLAME_CHARGE, MOVE_QUICK_ATTACK, MOVE_DOUBLE_KICK, MOVE_PROTECT}
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 34,
    .species = SPECIES_VAPOREON,
    .heldItem = ITEM_NUGGET,
    .moves = {MOVE_SCALD, MOVE_ACID_ARMOR, MOVE_WISH, MOVE_ICE_BEAM}
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 34,
    .species = SPECIES_JOLTEON,
    .heldItem = ITEM_NUGGET,
    .moves = {MOVE_THUNDERBOLT, MOVE_BITE, MOVE_PIN_MISSILE, MOVE_VOLT_SWITCH}
    }
};

static const struct TrainerMon sParty_Cindy4[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 39,
    .species = SPECIES_FLAREON,
    .heldItem = ITEM_TOXIC_ORB,
    .ability = ABILITY_GUTS,
    .moves = {MOVE_FLAME_CHARGE, MOVE_QUICK_ATTACK, MOVE_DOUBLE_KICK, MOVE_PROTECT}
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 39,
    .species = SPECIES_VAPOREON,
    .heldItem = ITEM_NUGGET,
    .moves = {MOVE_SCALD, MOVE_ACID_ARMOR, MOVE_WISH, MOVE_ICE_BEAM}
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 39,
    .species = SPECIES_JOLTEON,
    .heldItem = ITEM_NUGGET,
    .moves = {MOVE_THUNDERBOLT, MOVE_BITE, MOVE_PIN_MISSILE, MOVE_VOLT_SWITCH}
    }
};

static const struct TrainerMon sParty_Cindy5[] = {
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 44,
    .species = SPECIES_FLAREON,
    .heldItem = ITEM_TOXIC_ORB,
    .ability = ABILITY_GUTS,
    .moves = {MOVE_FLARE_BLITZ, MOVE_QUICK_ATTACK, MOVE_DOUBLE_KICK, MOVE_PROTECT}
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 44,
    .species = SPECIES_VAPOREON,
    .heldItem = ITEM_NUGGET,
    .moves = {MOVE_SCALD, MOVE_ACID_ARMOR, MOVE_WISH, MOVE_ICE_BEAM}
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 44,
    .species = SPECIES_JOLTEON,
    .heldItem = ITEM_NUGGET,
    .moves = {MOVE_THUNDERBOLT, MOVE_BITE, MOVE_PIN_MISSILE, MOVE_VOLT_SWITCH}
    }
};

static const struct TrainerMon sParty_Cindy6[] = {
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 44,
    .species = SPECIES_FLAREON,
    .heldItem = ITEM_TOXIC_ORB,
    .ability = ABILITY_GUTS,
    .moves = {MOVE_FLARE_BLITZ, MOVE_SUPERPOWER, MOVE_DOUBLE_KICK, MOVE_PROTECT}
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 44,
    .species = SPECIES_VAPOREON,
    .heldItem = ITEM_NUGGET,
    .moves = {MOVE_SCALD, MOVE_ACID_ARMOR, MOVE_WISH, MOVE_ICE_BEAM}
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 44,
    .species = SPECIES_JOLTEON,
    .heldItem = ITEM_NUGGET,
    .moves = {MOVE_THUNDERBOLT, MOVE_BITE, MOVE_PIN_MISSILE, MOVE_VOLT_SWITCH}
    }
};

static const struct TrainerMon sParty_Melissa[] = {
    {
    .lvl = 21,
    .species = SPECIES_AZUMARILL,
    }
};

static const struct TrainerMon sParty_Sheila[] = {
    {
    .lvl = 21,
    .species = SPECIES_VIVILLON,
    }
};

static const struct TrainerMon sParty_Shirley[] = {
    {
    .lvl = 21,
    .species = SPECIES_TOGEDEMARU,
    }
};

static const struct TrainerMon sParty_Jessica1[] = {
    {
    .lvl = 46,
    .species = SPECIES_KECLEON,
    .moves = {MOVE_SHADOW_PUNCH, MOVE_SHADOW_SNEAK, MOVE_FURY_SWIPES, MOVE_FEINT_ATTACK}
    },
    {
    .lvl = 46,
    .species = SPECIES_SEVIPER,
    .moves = {MOVE_GUNK_SHOT, MOVE_POWER_WHIP, MOVE_GLARE, MOVE_CRUNCH}
    }
};

static const struct TrainerMon sParty_Connie[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 54,
    .species = SPECIES_BASCULEGION,
    }
};

static const struct TrainerMon sParty_Bridget[] = {
    {
    .iv = TRAINER_PARTY_IVS(18, 18, 18, 18, 18, 18),
    .lvl = 68,
    .species = SPECIES_NIDOQUEEN,
    .heldItem = ITEM_LIFE_ORB,
    .ability = ABILITY_SHEER_FORCE,
    .moves = {MOVE_SLUDGE_BOMB, MOVE_EARTH_POWER, MOVE_SURF, MOVE_THUNDERBOLT}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 67,
    .species = SPECIES_CHATOT,
    .ability = ABILITY_BIG_PECKS,
    .moves = {MOVE_HURRICANE, MOVE_CHATTER, MOVE_U_TURN, MOVE_NASTY_PLOT}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 67,
    .species = SPECIES_TROPIUS,
    .ability = ABILITY_CHLOROPHYLL,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_HURRICANE, MOVE_LEAF_BLADE, MOVE_EARTHQUAKE, MOVE_ROOST}
    }


};

static const struct TrainerMon sParty_Olivia[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 64,
    .species = SPECIES_LUMINEON,
    .ability = ABILITY_SWIFT_SWIM,
    .moves = {MOVE_HURRICANE, MOVE_SURF, MOVE_ICE_BEAM, MOVE_AQUA_RING}
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 65,
    .species = SPECIES_HUNTAIL,
    .moves = {MOVE_TAUNT, MOVE_CRUNCH, MOVE_LIQUIDATION, MOVE_FLIP_TURN}
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 66,
    .species = SPECIES_SKUNTANK,
    .moves = {MOVE_DARK_PULSE, MOVE_FLAMETHROWER, MOVE_SHADOW_BALL, MOVE_SLUDGE_BOMB}
    }
};

static const struct TrainerMon sParty_Tiffany[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 63,
    .species = SPECIES_SEISMITOAD,
    .ability = ABILITY_SWIFT_SWIM,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_EARTH_POWER, MOVE_SCALD, MOVE_STEALTH_ROCK, MOVE_TOXIC}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 65,
    .species = SPECIES_FERROTHORN,
    .ability = ABILITY_IRON_BARBS,
    .moves = {MOVE_BODY_PRESS, MOVE_SPIKES, MOVE_POWER_WHIP, MOVE_LEECH_SEED}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 64,
    .species = SPECIES_BEAUTIFLY,
    .ability = ABILITY_SWARM,
    .moves = {MOVE_HURRICANE, MOVE_QUIVER_DANCE, MOVE_BUG_BUZZ, MOVE_GIGA_DRAIN}
    }
};

static const struct TrainerMon sParty_Jessica2[] = {
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 44,
    .species = SPECIES_KECLEON,
    .moves = {MOVE_SHADOW_PUNCH, MOVE_SHADOW_SNEAK, MOVE_DOUBLE_EDGE, MOVE_FEINT_ATTACK}
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 44,
    .species = SPECIES_SEVIPER,
    .moves = {MOVE_GUNK_SHOT, MOVE_POWER_WHIP, MOVE_GLARE, MOVE_CRUNCH}
    }
};

static const struct TrainerMon sParty_Jessica3[] = {
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 48,
    .species = SPECIES_KECLEON,
    .moves = {MOVE_SHADOW_CLAW, MOVE_SHADOW_SNEAK, MOVE_DOUBLE_EDGE, MOVE_SUCKER_PUNCH}
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 48,
    .species = SPECIES_SEVIPER,
    .moves = {MOVE_GUNK_SHOT, MOVE_POWER_WHIP, MOVE_GLARE, MOVE_FIRE_BLAST}
    }
};

static const struct TrainerMon sParty_Jessica4[] = {
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 51,
    .species = SPECIES_KECLEON,
    .ability = ABILITY_PROTEAN,
    .moves = {MOVE_SHADOW_CLAW, MOVE_ICE_PUNCH, MOVE_FIRE_PUNCH, MOVE_SUCKER_PUNCH}
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 51,
    .species = SPECIES_SEVIPER,
    .moves = {MOVE_GUNK_SHOT, MOVE_POWER_WHIP, MOVE_GLARE, MOVE_FIRE_BLAST}
    }
};

static const struct TrainerMon sParty_Jessica5[] = {
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 56,
    .species = SPECIES_KECLEON,
    .ability = ABILITY_PROTEAN,
    .moves = {MOVE_SHADOW_CLAW, MOVE_ICE_PUNCH, MOVE_FIRE_PUNCH, MOVE_SUCKER_PUNCH}
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 56,
    .species = SPECIES_SEVIPER,
    .moves = {MOVE_GUNK_SHOT, MOVE_POWER_WHIP, MOVE_GLARE, MOVE_FIRE_BLAST}
    }
};

static const struct TrainerMon sParty_Winston1[] = {
    {
    .lvl = 9,
    .species = SPECIES_ZIGZAGOON,
    .heldItem = ITEM_NUGGET
    }
};

static const struct TrainerMon sParty_Mollie[] = {
    {
    .lvl = 33,
    .species = SPECIES_WHISCASH,
    },
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 33,
    .species = SPECIES_MEDITITE,
    }
};

static const struct TrainerMon sParty_Garret[] = {
    {
    .iv = TRAINER_PARTY_IVS(8, 8, 8, 8, 8, 8),
    .lvl = 72,
    .species = SPECIES_NINJASK,
    .ability = ABILITY_SPEED_BOOST,
    .heldItem = ITEM_FOCUS_SASH,
    .moves = {MOVE_SWORDS_DANCE, MOVE_PROTECT, MOVE_LEECH_LIFE, MOVE_BATON_PASS}
    },
    {
    .iv = TRAINER_PARTY_IVS(8, 8, 8, 8, 8, 8),
    .lvl = 72,
    .species = SPECIES_MAWILE,
    .ability = ABILITY_INTIMIDATE,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_SWORDS_DANCE, MOVE_IRON_HEAD, MOVE_PLAY_ROUGH, MOVE_BATON_PASS}
    },
    {
    .iv = TRAINER_PARTY_IVS(8, 8, 8, 8, 8, 8),
    .lvl = 72,
    .species = SPECIES_DRIFBLIM,
    .heldItem = ITEM_WEAKNESS_POLICY,
    .moves = {MOVE_HURRICANE, MOVE_SHADOW_BALL, MOVE_MINIMIZE, MOVE_BATON_PASS}
    },
    {
    .iv = TRAINER_PARTY_IVS(8, 8, 8, 8, 8, 8),
    .lvl = 72,
    .species = SPECIES_FARIGIRAF,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_HYPER_VOICE, MOVE_AGILITY, MOVE_STORED_POWER, MOVE_BATON_PASS}
    },
    {
    .iv = TRAINER_PARTY_IVS(8, 8, 8, 8, 8, 8),
    .lvl = 72,
    .species = SPECIES_ESPEON,
    .ability = ABILITY_MAGIC_BOUNCE,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_WISH, MOVE_PSYCHIC, MOVE_CALM_MIND, MOVE_BATON_PASS}
    },
    {
    .iv = TRAINER_PARTY_IVS(8, 8, 8, 8, 8, 8),
    .lvl = 72,
    .species = SPECIES_UMBREON,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_WISH, MOVE_CURSE, MOVE_PAYBACK, MOVE_BATON_PASS}
    }
};

static const struct TrainerMon sParty_Winston2[] = {
    {
    .lvl = 27,
    .species = SPECIES_LINOONE,
    .heldItem = ITEM_NUGGET
    }
};

static const struct TrainerMon sParty_Winston3[] = {
    {
    .lvl = 33,
    .species = SPECIES_LINOONE,
    .heldItem = ITEM_NUGGET
    }
};

static const struct TrainerMon sParty_Winston4[] = {
    {
    .lvl = 36,
    .species = SPECIES_LINOONE,
    .heldItem = ITEM_NUGGET
    }
};

static const struct TrainerMon sParty_Winston5[] = {
    {
    .lvl = 40,
    .species = SPECIES_LINOONE,
    .heldItem = ITEM_NUGGET,
    .moves = {MOVE_EXTREMESPEED, MOVE_BELLY_DRUM, MOVE_SHADOW_CLAW, MOVE_DIG}
    }
};

static const struct TrainerMon sParty_Steve1[] = {
    {
    .lvl = 27,
    .species = SPECIES_CUFANT,
    }
};

static const struct TrainerMon sParty_Thalia1[] = {
    {
    .lvl = 35,
    .species = SPECIES_STANTLER,
    },
    {
    .lvl = 35,
    .species = SPECIES_LANTURN,
    }
};

static const struct TrainerMon sParty_Mark[] = {
    {
    .lvl = 49,
    .species = SPECIES_GABITE,
    },
    {
    .lvl = 49,
    .species = SPECIES_AGGRON,
    }
};

static const struct TrainerMon sParty_GruntMtChimney1[] = {
    {
    .lvl = 28,
    .species = SPECIES_CAMERUPT,
    }
};

static const struct TrainerMon sParty_Steve2[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 34,
    .species = SPECIES_CUFANT,
    }
};

static const struct TrainerMon sParty_Steve3[] = {
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 37,
    .species = SPECIES_CUFANT,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 37,
    .species = SPECIES_RHYHORN,
    }
};

static const struct TrainerMon sParty_Steve4[] = {
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 44,
    .species = SPECIES_COPPERAJAH,
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 45,
    .species = SPECIES_RHYPERIOR,
    }
};

static const struct TrainerMon sParty_Steve5[] = {
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 48,
    .species = SPECIES_COPPERAJAH,
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 48,
    .species = SPECIES_RHYPERIOR,
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 48,
    .species = SPECIES_AGGRON,
    }
};

static const struct TrainerMon sParty_Luis[] = {
    {
    .lvl = 36,
    .species = SPECIES_SEAKING,
    },
    {
    .lvl = 34,
    .species = SPECIES_DRAGONAIR,
    }
};

static const struct TrainerMon sParty_Dominik[] = {
    {
    .lvl = 36,
    .species = SPECIES_CLOYSTER,
    },
    {
    .lvl = 36,
    .species = SPECIES_DEWGONG,
    },
};

static const struct TrainerMon sParty_Douglas[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 33,
    .species = SPECIES_GYARADOS,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 33,
    .species = SPECIES_MILOTIC,
    }
};

static const struct TrainerMon sParty_Darrin[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 34,
    .species = SPECIES_COFAGRIGUS,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 34,
    .species = SPECIES_EISCUE,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 34,
    .species = SPECIES_BEARTIC,
    }
};

static const struct TrainerMon sParty_Tony1[] = {
    {
    .lvl = 36,
    .species = SPECIES_LICKILICKY,
    }
};

static const struct TrainerMon sParty_Jerome[] = {
    {
    .lvl = 36,
    .species = SPECIES_TENTACRUEL,
    }
};

static const struct TrainerMon sParty_Matthew[] = {
    {
    .lvl = 36,
    .species = SPECIES_SHARPEDO,
    }
};

static const struct TrainerMon sParty_David[] = {
    {
    .lvl = 36,
    .species = SPECIES_SWAMPERT,
    },
    {
    .lvl = 35,
    .species = SPECIES_SWANNA,
    }
};

static const struct TrainerMon sParty_Spencer[] = {
    {
    .lvl = 43,
    .species = SPECIES_QWILFISH,
    },
    {
    .lvl = 43,
    .species = SPECIES_OCTILLERY,
    }
};

static const struct TrainerMon sParty_Roland[] = {
    {
    .lvl = 44,
    .species = SPECIES_RELICANTH,
    }
};

static const struct TrainerMon sParty_Nolen[] = {
    {
    .lvl = 34,
    .species = SPECIES_TENTACRUEL,
    }
};

static const struct TrainerMon sParty_Stan[] = {
    {
    .lvl = 37,
    .species = SPECIES_KINGDRA,
    }
};

static const struct TrainerMon sParty_Barry[] = {
    {
    .lvl = 37,
    .species = SPECIES_GYARADOS,
    }
};

static const struct TrainerMon sParty_Dean[] = {
    {
    .lvl = 37,
    .species = SPECIES_ALOMOMOLA,
    },
    {
    .lvl = 35,
    .species = SPECIES_JELLICENT,
    },
    {
    .lvl = 34,
    .species = SPECIES_DUOSION,
    }
};

static const struct TrainerMon sParty_Rodney[] = {
    {
    .lvl = 49,
    .species = SPECIES_TOXICROAK,
    .ability = ABILITY_DRY_SKIN,
    .heldItem = ITEM_BLACK_SLUDGE,
    .moves = {MOVE_DRAIN_PUNCH, MOVE_POISON_JAB, MOVE_SUCKER_PUNCH, MOVE_BULK_UP}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 49,
    .species = SPECIES_RAICHU_ALOLAN,
    .ability = ABILITY_SURGE_SURFER,
    .heldItem = ITEM_MAGNET,
    .moves = {MOVE_THUNDER, MOVE_FOCUS_BLAST, MOVE_NASTY_PLOT, MOVE_FAKE_OUT}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 49,
    .species = SPECIES_GOLEM_ALOLAN,
    .ability = ABILITY_MAGNET_PULL,
    .heldItem = ITEM_AIR_BALLOON,
    .moves = {MOVE_THUNDER, MOVE_EARTHQUAKE, MOVE_STEALTH_ROCK, MOVE_PROTECT}
    }
};

static const struct TrainerMon sParty_Richard[] = {
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 56,
    .species = SPECIES_PELIPPER,
    .ability = ABILITY_DRIZZLE,
    .heldItem = ITEM_DAMP_ROCK,
    .moves = {MOVE_HURRICANE, MOVE_SURF, MOVE_ROOST, MOVE_ICE_BEAM}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 55,
    .species = SPECIES_LUDICOLO,
    .ability = ABILITY_SWIFT_SWIM,
    .heldItem = ITEM_BIG_ROOT,
    .moves = {MOVE_SURF, MOVE_HYDRO_PUMP, MOVE_GIGA_DRAIN, MOVE_ICE_BEAM}
    }
};

static const struct TrainerMon sParty_Herman[] = {
    {
    .lvl = 36,
    .species = SPECIES_GLIMMORA,
    },
    {
    .lvl = 33,
    .species = SPECIES_TENTACRUEL,
    }
};

static const struct TrainerMon sParty_Santiago[] = {
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 54,
    .species = SPECIES_TENTACRUEL,
    .ability = ABILITY_CLEAR_BODY,
    .heldItem = ITEM_BLACK_SLUDGE,
    .moves = {MOVE_SCALD, MOVE_SLUDGE_BOMB, MOVE_TOXIC_SPIKES, MOVE_RAIN_DANCE}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 53,
    .species = SPECIES_WAILORD,
    .ability = ABILITY_MULTISCALE,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_WATER_SPOUT, MOVE_SURF, MOVE_ICE_BEAM, MOVE_REST}
    }
};

static const struct TrainerMon sParty_Gilbert[] = {
    {
    .lvl = 34,
    .species = SPECIES_SHARPEDO,
    }
};

static const struct TrainerMon sParty_Franklin[] = {
    {
    .lvl = 39,
    .species = SPECIES_WEAVILE,
    }
};

static const struct TrainerMon sParty_Kevin[] = {
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 54,
    .species = SPECIES_WALREIN,
    .ability = ABILITY_THICK_FAT,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_SURF, MOVE_ICE_BEAM, MOVE_BLIZZARD, MOVE_PROTECT}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 54,
    .species = SPECIES_SWAMPERT,
    .ability = ABILITY_TORRENT,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_WATERFALL, MOVE_EARTHQUAKE, MOVE_ICE_PUNCH, MOVE_STONE_EDGE}
}
};

static const struct TrainerMon sParty_Jack[] = {
    {
    .lvl = 34,
    .species = SPECIES_MILOTIC,
    }
};

static const struct TrainerMon sParty_Dudley[] = {
    {
    .lvl = 38,
    .species = SPECIES_SLOWBRO_GALARIAN,
    },
    {
    .lvl = 39,
    .species = SPECIES_SLOWKING_GALARIAN,
    },
    {
    .lvl = 38,
    .species = SPECIES_SLOWKING
    }
};

static const struct TrainerMon sParty_Chad[] = {
    {
    .lvl = 43,
    .species = SPECIES_DREDNAW,
    },
    {
    .lvl = 43,
    .species = SPECIES_WAILORD,
    }
};

static const struct TrainerMon sParty_Tony2[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 43,
    .species = SPECIES_LICKILICKY,
    .moves = {MOVE_SURF, MOVE_THUNDERBOLT, MOVE_FIRE_BLAST, MOVE_POWER_WHIP}
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 43,
    .species = SPECIES_SHARPEDO,
    .ability = ABILITY_SPEED_BOOST,
    .moves = {MOVE_CRUNCH, MOVE_LIQUIDATION, MOVE_PSYCHIC_FANGS, MOVE_ICE_FANG}
    }
};

static const struct TrainerMon sParty_Tony3[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 46,
    .species = SPECIES_LICKILICKY,
    .moves = {MOVE_SURF, MOVE_THUNDERBOLT, MOVE_FIRE_BLAST, MOVE_POWER_WHIP}
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 46,
    .species = SPECIES_SHARPEDO,
    .ability = ABILITY_SPEED_BOOST,
    .moves = {MOVE_CRUNCH, MOVE_LIQUIDATION, MOVE_PSYCHIC_FANGS, MOVE_ICE_FANG}
    }
};

static const struct TrainerMon sParty_Tony4[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 48,
    .species = SPECIES_LICKILICKY,
    .moves = {MOVE_SURF, MOVE_THUNDERBOLT, MOVE_FIRE_BLAST, MOVE_POWER_WHIP}
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 48,
    .species = SPECIES_SHARPEDO,
    .ability = ABILITY_SPEED_BOOST,
    .moves = {MOVE_CRUNCH, MOVE_LIQUIDATION, MOVE_PSYCHIC_FANGS, MOVE_ICE_FANG}
    }
};

static const struct TrainerMon sParty_Tony5[] = {
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 45,
    .species = SPECIES_STARMIE,
    .heldItem = ITEM_LIFE_ORB
    },
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 48,
    .species = SPECIES_LICKILICKY,
    .moves = {MOVE_SURF, MOVE_THUNDERBOLT, MOVE_FIRE_BLAST, MOVE_POWER_WHIP}
    },
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 48,
    .species = SPECIES_SHARPEDO,
    .ability = ABILITY_SPEED_BOOST,
    .moves = {MOVE_CRUNCH, MOVE_LIQUIDATION, MOVE_PSYCHIC_FANGS, MOVE_ICE_FANG}
    }
};

static const struct TrainerMon sParty_Takao[] = {
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 16,
    .species = SPECIES_CLOBBOPUS,
    }
};

static const struct TrainerMon sParty_Hitoshi[] = {
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 31,
    .species = SPECIES_GURDURR,
    },
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 31,
    .species = SPECIES_CRABRAWLER,
    }
};

static const struct TrainerMon sParty_Kiyo[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 30,
    .species = SPECIES_HARIYAMA,
    }
};

static const struct TrainerMon sParty_Koichi[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 26,
    .species = SPECIES_MANKEY,
    },
    {
    .iv = TRAINER_PARTY_IVS(18, 18, 18, 18, 18, 18),
    .lvl = 28,
    .species = SPECIES_MACHOKE,
    }
};

static const struct TrainerMon sParty_Nob1[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 28,
    .species = SPECIES_MACHAMP,
    }
};

static const struct TrainerMon sParty_Nob2[] = {
    {
    .iv = TRAINER_PARTY_IVS(13, 13, 13, 13, 13, 13),
    .lvl = 33,
    .species = SPECIES_MACHAMP,
    }
};

static const struct TrainerMon sParty_Nob3[] = {
    {
    .iv = TRAINER_PARTY_IVS(14, 14, 14, 14, 14, 14),
    .lvl = 35,
    .species = SPECIES_PAWMO,
    },
    {
    .iv = TRAINER_PARTY_IVS(14, 14, 14, 14, 14, 14),
    .lvl = 35,
    .species = SPECIES_MACHAMP,
    }
};

static const struct TrainerMon sParty_Nob4[] = {
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 37,
    .species = SPECIES_PAWMO,
    },
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 38,
    .species = SPECIES_MACHAMP,
    },
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 38,
    .species = SPECIES_MALAMAR,
    }
};

static const struct TrainerMon sParty_Nob5[] = {
    {
    .iv = TRAINER_PARTY_IVS(17, 17, 17, 17, 17, 17),
    .lvl = 42,
    .species = SPECIES_PAWMOT,
    .heldItem = ITEM_MAGNET
    },
    {
    .iv = TRAINER_PARTY_IVS(17, 17, 17, 17, 17, 17),
    .lvl = 42,
    .species = SPECIES_MACHAMP,
    .heldItem = ITEM_BLACK_BELT
    },
    {
    .iv = TRAINER_PARTY_IVS(17, 17, 17, 17, 17, 17),
    .lvl = 42,
    .species = SPECIES_MALAMAR,
    .heldItem = ITEM_SITRUS_BERRY
    },
    {
    .iv = TRAINER_PARTY_IVS(17, 17, 17, 17, 17, 17),
    .lvl = 42,
    .species = SPECIES_BEWEAR,
    .heldItem = ITEM_BLACK_BELT
    }
};

static const struct TrainerMon sParty_Yuji[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 28,
    .species = SPECIES_HARIYAMA,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 28,
    .species = SPECIES_MACHOKE,
    }
};

static const struct TrainerMon sParty_Daisuke[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 19,
    .species = SPECIES_MACHOP,
    }
};

static const struct TrainerMon sParty_Atsushi[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 53,
    .species = SPECIES_PANGORO,
    }
};

static const struct TrainerMon sParty_Kirk[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 17,
    .species = SPECIES_ELECTRIKE,
    .moves = {MOVE_QUICK_ATTACK, MOVE_THUNDER_WAVE, MOVE_SPARK, MOVE_EMBER}
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 17,
    .species = SPECIES_VOLTORB,
    .moves = {MOVE_CHARGE, MOVE_SHOCK_WAVE, MOVE_SCREECH, MOVE_SONIC_BOOM}
    }
};

static const struct TrainerMon sParty_GruntAquaHideout7[] = {
    {
    .lvl = 53,
    .species = SPECIES_FALINKS,
    },
    {
    .lvl = 54,
    .species = SPECIES_LUCARIO,
    },
    {
    .lvl = 54,
    .species = SPECIES_MEDICHAM,
    },
    {
    .lvl = 53,
    .species = SPECIES_MEOWSCARADA,
    },
    {
    .lvl = 53,
    .species = SPECIES_TOGEDEMARU,
    }
};

static const struct TrainerMon sParty_GruntAquaHideout8[] = {
    {
    .lvl = 55,
    .species = SPECIES_QUAQUAVAL,
    .ability = ABILITY_MOXIE,
    },
    {
    .lvl = 55,
    .species = SPECIES_GRENINJA,
    .ability = ABILITY_PROTEAN,
    },
    {
    .lvl = 55,
    .species = SPECIES_PRIMARINA,
    .ability = ABILITY_LIQUID_VOICE,
    .moves = {MOVE_HYPER_VOICE, MOVE_MOONBLAST, MOVE_REST, MOVE_CALM_MIND}
    }
};

static const struct TrainerMon sParty_Shawn[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 17,
    .species = SPECIES_LUXIO,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 17,
    .species = SPECIES_STUNFISK,
    }
};

static const struct TrainerMon sParty_Fernando1[] = {
    {
    .lvl = 72,
    .species = SPECIES_MANECTRIC,
    .ability = ABILITY_LIGHTNING_ROD,
    .moves = {MOVE_DISCHARGE, MOVE_FLAMETHROWER, MOVE_CRUNCH, MOVE_ELECTRIC_TERRAIN}
    },
    {
    .lvl = 73,
    .species = SPECIES_MORPEKO,
    .ability = ABILITY_LIGHTNING_ROD,
    .heldItem = ITEM_AIR_BALLOON,
    .moves = {MOVE_DISCHARGE, MOVE_AURA_WHEEL, MOVE_HELPING_HAND, MOVE_PROTECT}
    }
};

static const struct TrainerMon sParty_Dalton1[] = {
    {
    .lvl = 19,
    .species = SPECIES_MAGNEMITE,
    },
    {
    .lvl = 18,
    .species = SPECIES_WHISMUR,
    }
};

static const struct TrainerMon sParty_Dalton2[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 25,
    .species = SPECIES_MAGNEMITE,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 25,
    .species = SPECIES_WHISMUR,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 25,
    .species = SPECIES_MAGNEMITE,
    }
};

static const struct TrainerMon sParty_Dalton3[] = {
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 31,
    .species = SPECIES_MAGNEMITE,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 31,
    .species = SPECIES_LOUDRED,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 31,
    .species = SPECIES_MAGNEMITE,
    }
};

static const struct TrainerMon sParty_Dalton4[] = {
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 35,
    .species = SPECIES_MAGNETON,
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 35,
    .species = SPECIES_LOUDRED,
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 36,
    .species = SPECIES_MAGNEZONE,
    }
};

static const struct TrainerMon sParty_Dalton5[] = {
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 42,
    .species = SPECIES_MAGNEZONE,
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 42,
    .species = SPECIES_EXPLOUD,
    .moves = {MOVE_BOOMBURST, MOVE_FIRE_BLAST, MOVE_SHADOW_BALL, MOVE_SURF}
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 44,
    .species = SPECIES_MAGNEZONE,
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 44,
    .species = SPECIES_ELECTIVIRE,
    }
};

static const struct TrainerMon sParty_Cole[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 29,
    .species = SPECIES_FLETCHINDER,
    }
};

static const struct TrainerMon sParty_Jeff[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 28,
    .species = SPECIES_PIGNITE,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 28,
    .species = SPECIES_QUILAVA,
    }
};

static const struct TrainerMon sParty_Axle[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 29,
    .species = SPECIES_FRAXURE,
    }
};

static const struct TrainerMon sParty_Jace[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 27,
    .species = SPECIES_CUBONE,
    .heldItem = ITEM_THICK_CLUB,
    }
};

static const struct TrainerMon sParty_Keegan[] = {
    {
    .iv = TRAINER_PARTY_IVS(14, 14, 14, 14, 14, 14),
    .lvl = 27,
    .species = SPECIES_HOUNDOOM,
    }
};

static const struct TrainerMon sParty_Bernie1[] = {
    {
    .lvl = 28,
    .species = SPECIES_MAGCARGO,
    },
    {
    .lvl = 28,
    .species = SPECIES_HEATMOR,
    }
};

static const struct TrainerMon sParty_Bernie2[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 31,
    .species = SPECIES_MAGCARGO,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 31,
    .species = SPECIES_HEATMOR,
    }
};

static const struct TrainerMon sParty_Bernie3[] = {
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 35,
    .species = SPECIES_MAGCARGO,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 35,
    .species = SPECIES_HEATMOR,
    }
};

static const struct TrainerMon sParty_Bernie4[] = {
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 38,
    .species = SPECIES_MAGCARGO,
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 38,
    .species = SPECIES_HEATMOR,
    }
};

static const struct TrainerMon sParty_Bernie5[] = {
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 44,
    .species = SPECIES_MAGCARGO,
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 44,
    .species = SPECIES_HEATMOR,
    }
};

static const struct TrainerMon sParty_Drew[] = {
    {
    .lvl = 25,
    .species = SPECIES_WUGTRIO,
    .ability = ABILITY_SAND_VEIL,
    .moves = {MOVE_DIG, MOVE_TRIPLE_DIVE, MOVE_AQUA_JET, MOVE_SUCKER_PUNCH}
    }
};

static const struct TrainerMon sParty_Beau[] = {
    {
    .lvl = 23,
    .species = SPECIES_BALTOY,
    .moves = {MOVE_RAPID_SPIN, MOVE_BULLDOZE, MOVE_PSYBEAM, MOVE_ROCK_TOMB}
    },
    {
    .lvl = 22,
    .species = SPECIES_TOEDSCOOL,
    .moves = {MOVE_MEGA_DRAIN, MOVE_SAND_ATTACK, MOVE_RAPID_SPIN, MOVE_EARTH_POWER}
    },
    {
    .lvl = 22,
    .species = SPECIES_SILICOBRA,
    .moves = {MOVE_COIL, MOVE_MUD_SLAP, MOVE_ROCK_TOMB, MOVE_REST}
    }
};

static const struct TrainerMon sParty_Larry[] = {
    {
    .lvl = 28,
    .species = SPECIES_SHIFTRY,
    }
};

static const struct TrainerMon sParty_Shane[] = {
    {
    .lvl = 26,
    .species = SPECIES_MUDBRAY,
    },
    {
    .lvl = 26,
    .species = SPECIES_DEERLING_SUMMER,
    }
};

static const struct TrainerMon sParty_Justin[] = {
    {
    .lvl = 24,
    .species = SPECIES_KECLEON,
    }
};

static const struct TrainerMon sParty_Ethan1[] = {
    {
    .lvl = 31,
    .species = SPECIES_MASCHIFF,
    },
    {
    .lvl = 31,
    .species = SPECIES_VESPIQUEN,
    },
    {
    .lvl = 30,
    .species = SPECIES_FARFETCHD_GALARIAN,
    .heldItem = ITEM_LEEK,
    },
    {
    .lvl = 30,
    .species = SPECIES_MOTHIM,
    }
};

static const struct TrainerMon sParty_Autumn[] = {
    {
    .lvl = 27,
    .species = SPECIES_XATU,
    }
};

static const struct TrainerMon sParty_Travis[] = {
    {
    .lvl = 26,
    .species = SPECIES_ANORITH,
    }
};

static const struct TrainerMon sParty_Ethan2[] = {
    {
    .lvl = 31,
    .species = SPECIES_MABOSSTIFF,
    .ability = ABILITY_INTIMIDATE,
    },
    {
    .lvl = 33,
    .species = SPECIES_VESPIQUEN,
    },
    {
    .lvl = 33,
    .species = SPECIES_FARFETCHD_GALARIAN,
    .heldItem = ITEM_LEEK,
    },
    {
    .lvl = 33,
    .species = SPECIES_MOTHIM,
    }
};

static const struct TrainerMon sParty_Ethan3[] = {
    {
    .lvl = 38,
    .species = SPECIES_MABOSSTIFF,
    .ability = ABILITY_INTIMIDATE,
    },
    {
    .lvl = 38,
    .species = SPECIES_VESPIQUEN,
    },
    {
    .lvl = 38,
    .species = SPECIES_SIRFETCHD,
    .heldItem = ITEM_LEEK,
    },
    {
    .lvl = 38,
    .species = SPECIES_MOTHIM,
    }
};

static const struct TrainerMon sParty_Ethan4[] = {
    {
    .lvl = 41,
    .species = SPECIES_MABOSSTIFF,
    .ability = ABILITY_INTIMIDATE,
    },
    {
    .lvl = 41,
    .species = SPECIES_VESPIQUEN,
    .heldItem = ITEM_CHARTI_BERRY,
    },
    {
    .lvl = 41,
    .species = SPECIES_SIRFETCHD,
    .heldItem = ITEM_LEEK,
    },
    {
    .lvl = 41,
    .species = SPECIES_MOTHIM,
    }
};

static const struct TrainerMon sParty_Ethan5[] = {
    {
    .lvl = 47,
    .species = SPECIES_MASCHIFF,
    .ability = ABILITY_INTIMIDATE,
    .moves = {MOVE_PLAY_ROUGH, MOVE_CRUNCH, MOVE_WILD_CHARGE, MOVE_DESTINY_BOND}
    },
    {
    .lvl = 47,
    .species = SPECIES_VESPIQUEN,
    .heldItem = ITEM_CHARTI_BERRY,
    .moves = {MOVE_HEAL_ORDER, MOVE_ATTACK_ORDER, MOVE_DEFEND_ORDER, MOVE_DUAL_WINGBEAT}
    },
    {
    .lvl = 47,
    .species = SPECIES_SIRFETCHD,
    .heldItem = ITEM_LEEK,
    .moves = {MOVE_FIRST_IMPRESSION, MOVE_KNOCK_OFF, MOVE_SACRED_SWORD, MOVE_LEAF_BLADE}
    },
    {
    .lvl = 49,
    .species = SPECIES_MOTHIM,
    .heldItem = ITEM_CHARTI_BERRY,
    .moves = {MOVE_QUIVER_DANCE, MOVE_BUG_BUZZ, MOVE_GIGA_DRAIN, MOVE_ROOST}
    }
};

static const struct TrainerMon sParty_Brent[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 55,
    .species = SPECIES_SURSKIT,
    .moves = {MOVE_BUBBLE, MOVE_BUG_BITE, MOVE_WATER_SPORT, MOVE_NONE}
    }
};

static const struct TrainerMon sParty_Donald[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 39,
    .species = SPECIES_DUSTOX,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 39,
    .species = SPECIES_BEAUTIFLY,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 40,
    .species = SPECIES_RIBOMBEE,
    }
};

static const struct TrainerMon sParty_Taylor[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 37,
    .species = SPECIES_BUTTERFREE,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 37,
    .species = SPECIES_BEEDRILL,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 37,
    .species = SPECIES_PINSIR,
    }
};

static const struct TrainerMon sParty_Jeffrey1[] = {
    {
    .lvl = 42,
    .species = SPECIES_MASQUERAIN,
    },
    {
    .lvl = 42,
    .species = SPECIES_MASQUERAIN,
    },
    {
    .lvl = 44,
    .species = SPECIES_YANMEGA,
    }
};

static const struct TrainerMon sParty_Derek[] = {
    {
    .iv = TRAINER_PARTY_IVS(18, 18, 18, 18, 18, 18),
    .lvl = 16,
    .species = SPECIES_DUSTOX,
    },
    {
    .iv = TRAINER_PARTY_IVS(18, 18, 18, 18, 18, 18),
    .lvl = 16,
    .species = SPECIES_BEAUTIFLY,
    }
};

static const struct TrainerMon sParty_Jeffrey2[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 47,
    .species = SPECIES_MASQUERAIN,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 47,
    .species = SPECIES_MASQUERAIN,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 47,
    .species = SPECIES_YANMEGA,
    }
};

static const struct TrainerMon sParty_Jeffrey3[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 54,
    .species = SPECIES_MASQUERAIN,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 54,
    .species = SPECIES_MASQUERAIN,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 55,
    .species = SPECIES_YANMEGA,
    }
};

static const struct TrainerMon sParty_Jeffrey4[] = {
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 58,
    .species = SPECIES_MASQUERAIN,
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 58,
    .species = SPECIES_MASQUERAIN,
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 59,
    .species = SPECIES_YANMEGA,
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 61,
    .species = SPECIES_SCOLIPEDE,
    .ability = ABILITY_SPEED_BOOST,
    }
};

static const struct TrainerMon sParty_Jeffrey5[] = {
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 68,
    .species = SPECIES_MASQUERAIN,
    .ability = ABILITY_INTIMIDATE,
    .heldItem = ITEM_FOCUS_SASH,
    .moves = {MOVE_STICKY_WEB, MOVE_HYDRO_PUMP, MOVE_U_TURN, MOVE_HURRICANE}
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 68,
    .species = SPECIES_MASQUERAIN,
    .ability = ABILITY_INTIMIDATE,
    .moves = {MOVE_QUIVER_DANCE, MOVE_HYDRO_PUMP, MOVE_BUG_BUZZ, MOVE_AIR_SLASH}
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 69,
    .species = SPECIES_YANMEGA,
    .ability = ABILITY_SPEED_BOOST,
    .moves = {MOVE_GIGA_DRAIN, MOVE_HURRICANE, MOVE_PSYCHIC, MOVE_U_TURN}
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 71,
    .species = SPECIES_SCOLIPEDE,
    .ability = ABILITY_SPEED_BOOST,
    .moves = {MOVE_PROTECT, MOVE_MEGAHORN, MOVE_GUNK_SHOT, MOVE_EARTHQUAKE}
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 69,
    .species = SPECIES_CENTISKORCH,
    .heldItem = ITEM_CHARTI_BERRY,
    .moves = {MOVE_COIL, MOVE_FIRE_LASH, MOVE_KNOCK_OFF, MOVE_POWER_WHIP}
    }
};

static const struct TrainerMon sParty_Edward[] = {
    {
    .lvl = 17,
    .species = SPECIES_KADABRA,
    .moves = {MOVE_HIDDEN_POWER, MOVE_CONFUSION, MOVE_NONE, MOVE_NONE}
    }
};

static const struct TrainerMon sParty_Preston[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 56,
    .species = SPECIES_SNORLAX,
    .ability = ABILITY_THICK_FAT,
    .moves = {MOVE_BODY_SLAM, MOVE_CRUNCH, MOVE_EXPLOSION, MOVE_PROTECT}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 50,
    .species = SPECIES_ELECTRODE,
    .ability = ABILITY_AFTERMATH,
    .moves = {MOVE_THUNDERBOLT, MOVE_VOLT_SWITCH, MOVE_EXPLOSION, MOVE_PROTECT}
    }
};

static const struct TrainerMon sParty_Virgil[] = {
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 56,
    .species = SPECIES_GALLADE,
    .ability = ABILITY_JUSTIFIED,
    .moves = {MOVE_CLOSE_COMBAT, MOVE_PSYCHIC, MOVE_LEAF_BLADE, MOVE_PROTECT}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 57,
    .species = SPECIES_MOTHIM,
    .ability = ABILITY_SWARM,
    .heldItem = ITEM_SILK_SCARF,
    .moves = {MOVE_AIR_SLASH, MOVE_BUG_BUZZ, MOVE_QUIVER_DANCE, MOVE_PROTECT}
    }
};

static const struct TrainerMon sParty_Blake[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 54,
    .species = SPECIES_FARIGIRAF,
    .ability = ABILITY_ARMOR_TAIL,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 52,
    .species = SPECIES_FERROTHORN,
    .ability = ABILITY_IRON_BARBS,
    .moves = {MOVE_POWER_WHIP, MOVE_GYRO_BALL, MOVE_KNOCK_OFF, MOVE_PROTECT}
    }
};

static const struct TrainerMon sParty_William[] = {
    {
    .lvl = 49,
    .species = SPECIES_WYRDEER,
    },
    {
    .lvl = 49,
    .species = SPECIES_STANTLER,
    },
    {
    .lvl = 49,
    .species = SPECIES_ORBEETLE,
    }
};

static const struct TrainerMon sParty_Joshua[] = {
    {
    .lvl = 41,
    .species = SPECIES_MUSHARNA,
    },
    {
    .lvl = 43,
    .species = SPECIES_GOTHITELLE,
    .ability = ABILITY_SHADOW_TAG,
    }
};

static const struct TrainerMon sParty_Cameron1[] = {
    {
    .lvl = 47,
    .species = SPECIES_ALAKAZAM,
    },
    {
    .lvl = 47,
    .species = SPECIES_RUNERIGUS,
    },
};

static const struct TrainerMon sParty_Cameron2[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 33,
    .species = SPECIES_KADABRA,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 33,
    .species = SPECIES_SOLROCK,
    }
};

static const struct TrainerMon sParty_Cameron3[] = {
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 38,
    .species = SPECIES_KADABRA,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 38,
    .species = SPECIES_SOLROCK,
    }
};

static const struct TrainerMon sParty_Cameron4[] = {
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 41,
    .species = SPECIES_KADABRA,
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 41,
    .species = SPECIES_SOLROCK,
    }
};

static const struct TrainerMon sParty_Cameron5[] = {
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 45,
    .species = SPECIES_SOLROCK,
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 45,
    .species = SPECIES_ALAKAZAM,
    }
};

static const struct TrainerMon sParty_Jaclyn[] = {
    {
    .lvl = 23,
    .species = SPECIES_UNOWN_QMARK,
    .moves = {MOVE_HIDDEN_POWER, MOVE_NONE, MOVE_NONE, MOVE_NONE}
    }
};

static const struct TrainerMon sParty_Hannah[] = {
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 54,
    .species = SPECIES_ESPEON,
    .ability = ABILITY_MAGIC_BOUNCE,
    .moves = {MOVE_PSYCHIC, MOVE_DAZZLING_GLEAM, MOVE_SKILL_SWAP, MOVE_PROTECT}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 54,
    .species = SPECIES_MALAMAR,
    .ability = ABILITY_CONTRARY,
    .moves = {MOVE_SUPERPOWER, MOVE_NIGHT_SLASH, MOVE_TRICK_ROOM, MOVE_SKILL_SWAP}
    }
};

static const struct TrainerMon sParty_Samantha[] = {
    {
    .lvl = 53,
    .species = SPECIES_HATTERENE,
    .ability = ABILITY_MAGIC_BOUNCE,
    .moves = {MOVE_PSYCHIC, MOVE_DAZZLING_GLEAM, MOVE_TRICK_ROOM, MOVE_PROTECT}
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 54,
    .species = SPECIES_DUSKNOIR,
    .ability = ABILITY_PRESSURE,
    .heldItem = ITEM_ASSAULT_VEST,
    .moves = {MOVE_SHADOW_PUNCH, MOVE_EARTHQUAKE, MOVE_DYNAMIC_PUNCH, MOVE_TRICK_ROOM}
    }
};

static const struct TrainerMon sParty_Maura[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 53,
    .species = SPECIES_DRAGONITE,
    .ability = ABILITY_MULTISCALE,
    .moves = {MOVE_DRAGON_DANCE, MOVE_DRAGON_CLAW, MOVE_PROTECT, MOVE_ROOST}
    },
    {.lvl = 54,
    .species = SPECIES_GENGAR,
    .ability = ABILITY_LEVITATE,
    .moves = {MOVE_SHADOW_BALL, MOVE_SLUDGE_BOMB, MOVE_WILL_O_WISP, MOVE_DESTINY_BOND}
    }
};

static const struct TrainerMon sParty_Kayla[] = {
    {
    .lvl = 46,
    .species = SPECIES_WOBBUFFET,
    },
    {
    .lvl = 46,
    .species = SPECIES_GARDEVOIR,
    },
    {
    .lvl = 46,
    .species = SPECIES_ALAKAZAM,
    }
};

static const struct TrainerMon sParty_Alexis[] = {
    {
    .lvl = 45,
    .species = SPECIES_DELPHOX,
    },
    {
    .lvl = 45,
    .species = SPECIES_EXEGGUTOR,
    }
};

static const struct TrainerMon sParty_Jacki1[] = {
    {
    .lvl = 73,
    .species = SPECIES_ORANGURU,
    .ability = ABILITY_TELEPATHY,
    .moves = {MOVE_PSYCHIC, MOVE_INSTRUCT, MOVE_SHADOW_BALL, MOVE_HELPING_HAND}
    },
    {
    .lvl = 73,
    .species = SPECIES_GARCHOMP,
    .ability = ABILITY_ROUGH_SKIN,
    .moves = {MOVE_STOMPING_TANTRUM, MOVE_DRAGON_CLAW, MOVE_ROCK_SLIDE, MOVE_PROTECT}
    }
};

static const struct TrainerMon sParty_Jacki2[] = {
    {
    .lvl = 60,
    .species = SPECIES_ORANGURU,
    .ability = ABILITY_TELEPATHY,
    .moves = {MOVE_PSYCHIC, MOVE_INSTRUCT, MOVE_SHADOW_BALL, MOVE_HELPING_HAND}
    },
    {
    .lvl = 62,
    .species = SPECIES_GARCHOMP,
    .ability = ABILITY_ROUGH_SKIN,
    .moves = {MOVE_STOMPING_TANTRUM, MOVE_DRAGON_CLAW, MOVE_ROCK_SLIDE, MOVE_PROTECT}
    }
};

static const struct TrainerMon sParty_Jacki3[] = {
    {
    .lvl = 66,
    .species = SPECIES_ORANGURU,
    .ability = ABILITY_TELEPATHY,
    .moves = {MOVE_PSYCHIC, MOVE_INSTRUCT, MOVE_SHADOW_BALL, MOVE_HELPING_HAND}
    },
    {
    .lvl = 66,
    .species = SPECIES_GARCHOMP,
    .ability = ABILITY_ROUGH_SKIN,
    .moves = {MOVE_STOMPING_TANTRUM, MOVE_DRAGON_CLAW, MOVE_ROCK_SLIDE, MOVE_PROTECT}
    }
};

static const struct TrainerMon sParty_Jacki4[] = {
    {
    .lvl = 70,
    .species = SPECIES_ORANGURU,
    .ability = ABILITY_TELEPATHY,
    .moves = {MOVE_PSYCHIC, MOVE_INSTRUCT, MOVE_SHADOW_BALL, MOVE_HELPING_HAND}
    },
    {
    .lvl = 70,
    .species = SPECIES_GARCHOMP,
    .ability = ABILITY_ROUGH_SKIN,
    .moves = {MOVE_STOMPING_TANTRUM, MOVE_DRAGON_CLAW, MOVE_ROCK_SLIDE, MOVE_PROTECT}
    }
};

static const struct TrainerMon sParty_Jacki5[] = {
    {
    .lvl = 72,
    .species = SPECIES_ORANGURU,
    .ability = ABILITY_TELEPATHY,
    .moves = {MOVE_PSYCHIC, MOVE_INSTRUCT, MOVE_SHADOW_BALL, MOVE_HELPING_HAND}
    },
    {
    .lvl = 72,
    .species = SPECIES_GARCHOMP,
    .ability = ABILITY_ROUGH_SKIN,
    .moves = {MOVE_STOMPING_TANTRUM, MOVE_DRAGON_CLAW, MOVE_ROCK_SLIDE, MOVE_PROTECT}
    }
};

static const struct TrainerMon sParty_Walter1[] = {
    {
    .lvl = 48,
    .species = SPECIES_MANECTRIC,
    }
};

static const struct TrainerMon sParty_Micah[] = {
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 73,
    .species = SPECIES_LANDORUS_THERIAN,
    .ability = ABILITY_INTIMIDATE,
    .heldItem = ITEM_FOCUS_SASH,
    .moves = {MOVE_STEALTH_ROCK, MOVE_EARTHQUAKE, MOVE_U_TURN, MOVE_ROCK_SLIDE}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 73,
    .species = SPECIES_LUXRAY,
    .ability = ABILITY_INTIMIDATE,
    .moves = {MOVE_WILD_CHARGE, MOVE_CRUNCH, MOVE_IRON_TAIL, MOVE_SUCKER_PUNCH}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 73,
    .species = SPECIES_INCINEROAR,
    .ability = ABILITY_INTIMIDATE,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_DARKEST_LARIAT, MOVE_FLARE_BLITZ, MOVE_PARTING_SHOT, MOVE_WILL_O_WISP}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 73,
    .species = SPECIES_TAUROS_PALDEAN_AQUA_BREED,
    .ability = ABILITY_INTIMIDATE,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_RAGING_BULL, MOVE_CLOSE_COMBAT, MOVE_EARTHQUAKE, MOVE_BULK_UP}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 74,
    .species = SPECIES_OVERQWIL,
    .ability = ABILITY_INTIMIDATE,
    .heldItem = ITEM_BLACK_SLUDGE,
    .moves = {MOVE_GUNK_SHOT, MOVE_BARB_BARRAGE, MOVE_AQUA_TAIL, MOVE_THROAT_CHOP}
    }
};

static const struct TrainerMon sParty_Thomas[] = {
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 73,
    .species = SPECIES_GASTRODON,
    .ability = ABILITY_STORM_DRAIN,
    .heldItem = ITEM_ROCKY_HELMET,
    .moves = {MOVE_STEALTH_ROCK, MOVE_EARTHQUAKE, MOVE_ICE_BEAM, MOVE_SCALD}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 73,
    .species = SPECIES_SKELEDIRGE,
    .ability = ABILITY_UNAWARE,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_TORCH_SONG, MOVE_SHADOW_BALL, MOVE_SLACK_OFF, MOVE_WILL_O_WISP}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 73,
    .species = SPECIES_CORVIKNIGHT,
    .ability = ABILITY_PRESSURE,
    .heldItem = ITEM_SHED_SHELL,
    .moves = {MOVE_IRON_DEFENSE, MOVE_BRAVE_BIRD, MOVE_BODY_PRESS, MOVE_ROOST}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 73,
    .species = SPECIES_DUSKNOIR,
    .ability = ABILITY_IRON_FIST,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_POLTERGEIST, MOVE_TRICK_ROOM, MOVE_EARTHQUAKE, MOVE_DYNAMIC_PUNCH}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 74,
    .species = SPECIES_DONDOZO,
    .ability = ABILITY_UNAWARE,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_WAVE_CRASH, MOVE_CURSE, MOVE_BODY_PRESS, MOVE_REST}
    }
};

static const struct TrainerMon sParty_Walter2[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 40,
    .species = SPECIES_MANECTRIC,
    }
};

static const struct TrainerMon sParty_Walter3[] = {
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 42,
    .species = SPECIES_EELEKTROSS,
    .moves = {MOVE_DRAGON_CLAW, MOVE_THUNDERBOLT, MOVE_FLAMETHROWER, MOVE_GIGA_DRAIN}
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 42,
    .species = SPECIES_MANECTRIC,
    .moves = {MOVE_QUICK_ATTACK, MOVE_THUNDERBOLT, MOVE_OVERHEAT, MOVE_THUNDER_WAVE}
    }
};

static const struct TrainerMon sParty_Walter4[] = {
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 47,
    .species = SPECIES_EELEKTROSS,
    .moves = {MOVE_DRAGON_CLAW, MOVE_THUNDERBOLT, MOVE_FLAMETHROWER, MOVE_GIGA_DRAIN}
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 47,
    .species = SPECIES_MANECTRIC,
    .moves = {MOVE_QUICK_ATTACK, MOVE_THUNDERBOLT, MOVE_OVERHEAT, MOVE_THUNDER_WAVE}
    }
};

static const struct TrainerMon sParty_Walter5[] = {
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 47,
    .species = SPECIES_EELEKTROSS,
    .moves = {MOVE_DRAGON_CLAW, MOVE_THUNDERBOLT, MOVE_FLAMETHROWER, MOVE_GIGA_DRAIN}
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 41,
    .species = SPECIES_GOLDUCK, 
    .moves = {MOVE_SURF, MOVE_PSYCHIC, MOVE_CALM_MIND, MOVE_DISABLE}
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 47,
    .species = SPECIES_MANECTRIC,
    .moves = {MOVE_QUICK_ATTACK, MOVE_THUNDERBOLT, MOVE_OVERHEAT, MOVE_THUNDER_WAVE}
    }
};

static const struct TrainerMon sParty_Sidney[] = {
    {
    .iv = TRAINER_PARTY_IVS(30, 30, 30, 30, 30, 30),
    .lvl = 76,
    .species = SPECIES_SAMUROTT_HISUIAN,
    .heldItem = ITEM_LIFE_ORB,
    .ability = ABILITY_SHARPNESS,
    .moves = {MOVE_CEASELESS_EDGE, MOVE_SUCKER_PUNCH, MOVE_RAZOR_SHELL, MOVE_SACRED_SWORD}
    },
    {
    .iv = TRAINER_PARTY_IVS(30, 30, 30, 30, 30, 30),
    .lvl = 76,
    .species = SPECIES_TOXTRICITY,
    .heldItem = ITEM_CHOICE_SCARF,
    .ability = ABILITY_PUNK_ROCK,
    .moves = {MOVE_OVERDRIVE, MOVE_SLUDGE_WAVE, MOVE_BOOMBURST, MOVE_VOLT_SWITCH}
    },
    {
    .iv = TRAINER_PARTY_IVS(30, 30, 30, 30, 30, 30),
    .lvl = 76,
    .species = SPECIES_KINGAMBIT,
    .heldItem = ITEM_ASSAULT_VEST,
    .ability = ABILITY_SUPREME_OVERLORD,
    .moves = {MOVE_SUCKER_PUNCH, MOVE_KOWTOW_CLEAVE, MOVE_IRON_HEAD, MOVE_LOW_KICK}
    },
    {
    .iv = TRAINER_PARTY_IVS(30, 30, 30, 30, 30, 30),
    .lvl = 76,
    .species = SPECIES_OBSTAGOON,
    .heldItem = ITEM_FLAME_ORB,
    .ability = ABILITY_GUTS,
    .moves = {MOVE_OBSTRUCT, MOVE_CROSS_CHOP, MOVE_KNOCK_OFF, MOVE_FACADE}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 76,
    .species = SPECIES_SPIRITOMB,
    .heldItem = ITEM_LEFTOVERS,
    .ability = ABILITY_INFILTRATOR,
    .moves = {MOVE_CALM_MIND, MOVE_WILL_O_WISP, MOVE_SHADOW_BALL, MOVE_PSYCHIC}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 77,
    .species = SPECIES_ABSOL,
    .heldItem = ITEM_ABSOLITE,
    .moves = {MOVE_PLAY_ROUGH, MOVE_SWORDS_DANCE, MOVE_CLOSE_COMBAT, MOVE_SUCKER_PUNCH}
    }
};

static const struct TrainerMon sParty_Phoebe[] = {
    {
    .iv = TRAINER_PARTY_IVS(30, 30, 30, 30, 30, 30),
    .lvl = 77,
    .species = SPECIES_GRIMMSNARL,
    .heldItem = ITEM_LIGHT_CLAY,
    .ability = ABILITY_PRANKSTER,
    .moves = {MOVE_TAUNT, MOVE_REFLECT, MOVE_LIGHT_SCREEN, MOVE_SPIRIT_BREAK}
    },
    {
    .iv = TRAINER_PARTY_IVS(30, 30, 30, 30, 30, 30),
    .lvl = 77,
    .species = SPECIES_ROSERADE,
    .heldItem = ITEM_WHITE_HERB,
    .ability = ABILITY_NATURAL_CURE,
    .moves = {MOVE_LEAF_STORM, MOVE_SLUDGE_BOMB, MOVE_DAZZLING_GLEAM, MOVE_TOXIC_SPIKES}
    },
    {
    .iv = TRAINER_PARTY_IVS(30, 30, 30, 30, 30, 30),
    .lvl = 77,
    .species = SPECIES_GHOLDENGO,
    .heldItem = ITEM_AIR_BALLOON,
    .ability = ABILITY_GOOD_AS_GOLD,
    .moves = {MOVE_MAKE_IT_RAIN, MOVE_SHADOW_BALL, MOVE_RECOVER, MOVE_THUNDER_WAVE}
    },
    {
    .iv = TRAINER_PARTY_IVS(30, 30, 30, 30, 30, 30),
    .lvl = 77,
    .species = SPECIES_ESPATHRA,
    .heldItem = ITEM_LIFE_ORB,
    .ability = ABILITY_SPEED_BOOST,
    .moves = {MOVE_PROTECT, MOVE_CALM_MIND, MOVE_STORED_POWER, MOVE_DAZZLING_GLEAM}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 78,
    .species = SPECIES_CERULEDGE,
    .heldItem = ITEM_FOCUS_SASH,
    .ability = ABILITY_WEAK_ARMOR,
    .moves = {MOVE_SWORDS_DANCE, MOVE_BITTER_BLADE, MOVE_POLTERGEIST, MOVE_SHADOW_SNEAK}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 78,
    .species = SPECIES_BANETTE,
    .heldItem = ITEM_BANETTITE,
    .moves = {MOVE_WILL_O_WISP, MOVE_POLTERGEIST, MOVE_SUCKER_PUNCH, MOVE_DESTINY_BOND}
    }
};

static const struct TrainerMon sParty_Glacia[] = {
    {
    .iv = TRAINER_PARTY_IVS(30, 30, 30, 30, 30, 30),
    .lvl = 77,
    .species = SPECIES_NINETALES_ALOLAN,
    .heldItem = ITEM_LIGHT_CLAY,
    .ability = ABILITY_SNOW_WARNING,
    .moves = {MOVE_AURORA_VEIL, MOVE_FREEZE_DRY, MOVE_MOONBLAST, MOVE_NASTY_PLOT}
    },
    {
    .iv = TRAINER_PARTY_IVS(30, 30, 30, 30, 30, 30),
    .lvl = 77,
    .species = SPECIES_DARMANITAN_GALARIAN,
    .heldItem = ITEM_EXPERT_BELT,
    .ability = ABILITY_GORILLA_TACTICS,
    .moves = {MOVE_FLARE_BLITZ, MOVE_ICICLE_CRASH, MOVE_EARTHQUAKE, MOVE_U_TURN}
    },
    {
    .iv = TRAINER_PARTY_IVS(30, 30, 30, 30, 30, 30),
    .lvl = 78,
    .species = SPECIES_WALKING_WAKE,
    .heldItem = ITEM_LIFE_ORB,
    .ability = ABILITY_PROTOSYNTHESIS,
    .moves = {MOVE_HYDRO_STEAM, MOVE_DRACO_METEOR, MOVE_FLAMETHROWER, MOVE_HURRICANE}
    },
    {
    .iv = TRAINER_PARTY_IVS(30, 30, 30, 30, 30, 30),
    .lvl = 78,
    .species = SPECIES_CLEFABLE,
    .heldItem = ITEM_LIFE_ORB,
    .ability = ABILITY_MAGIC_GUARD,
    .moves = {MOVE_BLIZZARD, MOVE_MOONBLAST, MOVE_FLAMETHROWER, MOVE_SOFT_BOILED}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 78,
    .species = SPECIES_SLOWKING_GALARIAN,
    .heldItem = ITEM_BLACK_SLUDGE,
    .ability = ABILITY_REGENERATOR,
    .moves = {MOVE_WEATHER_BALL, MOVE_CHILLY_RECEPTION, MOVE_PSYCHIC, MOVE_SLUDGE_WAVE}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 79,
    .species = SPECIES_GLALIE,
    .heldItem = ITEM_GLALITITE,
    .moves = {MOVE_DOUBLE_EDGE, MOVE_EARTHQUAKE, MOVE_ICE_SHARD, MOVE_TRAILBLAZE}
    }
};

static const struct TrainerMon sParty_Drake[] = {
    {
    .iv = TRAINER_PARTY_IVS(30, 30, 30, 30, 30, 30),
    .lvl = 78,
    .species = SPECIES_DRAGALGE,
    .heldItem = ITEM_BLACK_SLUDGE,
    .ability = ABILITY_ADAPTABILITY,
    .moves = {MOVE_SLUDGE_WAVE, MOVE_DRACO_METEOR, MOVE_FLIP_TURN, MOVE_TOXIC_SPIKES}
    },
    {
    .iv = TRAINER_PARTY_IVS(30, 30, 30, 30, 30, 30),
    .lvl = 78,
    .species = SPECIES_GYARADOS,
    .heldItem = ITEM_WACAN_BERRY,
    .ability = ABILITY_INTIMIDATE,
    .moves = {MOVE_WATERFALL, MOVE_EARTHQUAKE, MOVE_POWER_WHIP, MOVE_DRAGON_DANCE}
    },
    {
    .iv = TRAINER_PARTY_IVS(30, 30, 30, 30, 30, 30),
    .lvl = 77,
    .species = SPECIES_EMPOLEON,
    .heldItem = ITEM_SHUCA_BERRY,
    .ability = ABILITY_COMPETITIVE,
    .moves = {MOVE_SURF, MOVE_ICE_BEAM, MOVE_GRASS_KNOT, MOVE_AGILITY}
    },
    {
    .iv = TRAINER_PARTY_IVS(30, 30, 30, 30, 30, 30),
    .lvl = 78,
    .species = SPECIES_FLYGON,
    .heldItem = ITEM_LIFE_ORB,
    .ability = ABILITY_LEVITATE,
    .moves = {MOVE_FIRST_IMPRESSION, MOVE_U_TURN, MOVE_EARTHQUAKE, MOVE_DRACO_METEOR}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 78,
    .species = SPECIES_GOODRA,
    .heldItem = ITEM_ASSAULT_VEST,
    .ability = ABILITY_GOOEY,
    .moves = {MOVE_THUNDERBOLT, MOVE_ICE_BEAM, MOVE_FLAMETHROWER, MOVE_SCALD}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 79,
    .species = SPECIES_SALAMENCE,
    .heldItem = ITEM_SALAMENCITE,
    .moves = {MOVE_DOUBLE_EDGE, MOVE_DRAGON_DANCE, MOVE_DRAGON_RUSH, MOVE_FIRE_FANG}
    }
};

static const struct TrainerMon sParty_Roxanne1[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 13,
    .species = SPECIES_GLIMMET,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_TACKLE, MOVE_SAND_TOMB, MOVE_ACID_SPRAY, MOVE_ROCK_TOMB}
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 13,
    .species = SPECIES_GEODUDE_ALOLAN,
    .heldItem = ITEM_NONE,
    .ability = ABILITY_GALVANIZE,
    .moves = {MOVE_TACKLE, MOVE_DEFENSE_CURL, MOVE_BULLDOZE, MOVE_ROCK_TOMB}
    },
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 15,
    .species = SPECIES_NOSEPASS,
    .heldItem = ITEM_ORAN_BERRY,
    .moves = {MOVE_BLOCK, MOVE_HARDEN, MOVE_TACKLE, MOVE_ROCK_TOMB}
    },
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 15,
    .species = SPECIES_ARCHEN,
    .heldItem = ITEM_ORAN_BERRY,
    .moves = {MOVE_PLUCK, MOVE_ROCK_TOMB, MOVE_TACKLE, MOVE_BITE}
    }
};

static const struct TrainerMon sParty_Brawly1[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 18,
    .species = SPECIES_STUFFUL,
    .heldItem = ITEM_NONE,
    .ability = ABILITY_FLUFFY,
    .moves = {MOVE_FORCE_PALM, MOVE_STOMP, MOVE_BRUTAL_SWING, MOVE_BULK_UP}
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 19,
    .species = SPECIES_MIENFOO,
    .heldItem = ITEM_NONE,
    .ability = ABILITY_REGENERATOR,
    .moves = {MOVE_FAKE_OUT, MOVE_U_TURN, MOVE_LOW_SWEEP, MOVE_BOUNCE}
    },
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 20,
    .species = SPECIES_MEDITITE,
    .heldItem = ITEM_NORMAL_GEM,
    .moves = {MOVE_FAKE_OUT, MOVE_MEGA_PUNCH, MOVE_REVERSAL, MOVE_ZEN_HEADBUTT}
    },
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 20,
    .species = SPECIES_HITMONTOP,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_TRIPLE_KICK, MOVE_TRIPLE_AXEL, MOVE_BULK_UP, MOVE_RAPID_SPIN}
    },
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 20,
    .species = SPECIES_LUCARIO,
    .heldItem = ITEM_STEEL_GEM,
    .moves = {MOVE_AGILITY, MOVE_VACUUM_WAVE, MOVE_FLASH_CANNON, MOVE_CALM_MIND}
    },
};

static const struct TrainerMon sParty_Wattson1[] = {
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 24,
    .species = SPECIES_LUXIO,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_ICE_FANG, MOVE_BITE, MOVE_THUNDER_FANG, MOVE_HOWL}
    },
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 25,
    .species = SPECIES_PIKACHU,
    .heldItem = ITEM_LIGHT_BALL,
    .moves = {MOVE_VOLT_TACKLE, MOVE_IRON_TAIL, MOVE_EXTREMESPEED, MOVE_VOLT_SWITCH}
    },
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 25,
    .species = SPECIES_ROTOM,
    .heldItem = ITEM_MAGNET,
    .moves = {MOVE_THUNDERBOLT, MOVE_WILL_O_WISP, MOVE_SHADOW_BALL, MOVE_VOLT_SWITCH}
    },
    {
    .iv = TRAINER_PARTY_IVS(26, 26, 26, 26, 26, 26),
    .lvl = 26,
    .species = SPECIES_MAGNETON,
    .heldItem = ITEM_AIR_BALLOON,
    .moves = {MOVE_MIRROR_SHOT, MOVE_VOLT_SWITCH, MOVE_THUNDER_WAVE, MOVE_SONIC_BOOM}
    },
    {
    .iv = TRAINER_PARTY_IVS(30, 30, 30, 30, 30, 30),
    .lvl = 26,
    .species = SPECIES_MANECTRIC,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_QUICK_ATTACK, MOVE_THUNDER_WAVE, MOVE_FLAMETHROWER, MOVE_VOLT_SWITCH}
    }
};

static const struct TrainerMon sParty_Flannery1[] = {
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 34,
    .species = SPECIES_TORKOAL,
    .ability = ABILITY_DROUGHT,
    .heldItem = ITEM_HEAT_ROCK,
    .moves = {MOVE_OVERHEAT, MOVE_ATTRACT, MOVE_MAGNITUDE, MOVE_STEALTH_ROCK}
    },
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 34,
    .species = SPECIES_COMBUSKEN,
    .ability = ABILITY_SPEED_BOOST,
    .heldItem = ITEM_EVIOLITE,
    .moves = {MOVE_PROTECT, MOVE_FIRE_PUNCH, MOVE_ROLLING_KICK, MOVE_THUNDER_PUNCH}
    },
    {
    .iv = TRAINER_PARTY_IVS(30, 30, 30, 30, 30, 30),
    .lvl = 34,
    .species = SPECIES_MAROWAK_ALOLAN,
    .heldItem = ITEM_THICK_CLUB,
    .moves = {MOVE_SHADOW_BONE, MOVE_FLAME_CHARGE, MOVE_BONEMERANG, MOVE_SWORDS_DANCE}
    },
    {
    .iv = TRAINER_PARTY_IVS(30, 30, 30, 30, 30, 30),
    .lvl = 35,
    .species = SPECIES_HOUNDOOM,
    .heldItem = ITEM_HOUNDOOMINITE,
    .moves = {MOVE_OVERHEAT, MOVE_CRUNCH, MOVE_SOLAR_BEAM, MOVE_NASTY_PLOT}
    },
    {
    .iv = TRAINER_PARTY_IVS(30, 30, 30, 30, 30, 30),
    .lvl = 35,
    .species = SPECIES_SCOVILLAIN,
    .heldItem = ITEM_WHITE_HERB,
    .ability = ABILITY_CHLOROPHYLL,
    .moves = {MOVE_OVERHEAT, MOVE_LEAF_STORM, MOVE_EARTH_POWER, MOVE_GROWTH}
    }
};

static const struct TrainerMon sParty_Norman1[] = {
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 38,
    .species = SPECIES_SPINDA,
    .heldItem = ITEM_CHOPLE_BERRY,
    .ability = ABILITY_CONTRARY,
    .moves = {MOVE_SUPERPOWER, MOVE_FACADE, MOVE_TEETER_DANCE, MOVE_ENCORE}
    },
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 38,
    .species = SPECIES_LOUDRED,
    .heldItem = ITEM_THROAT_SPRAY,
    .ability = ABILITY_PUNK_ROCK,
    .moves = {MOVE_BOOMBURST, MOVE_FLAMETHROWER, MOVE_SURF, MOVE_SHADOW_BALL}
    },
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 39,
    .species = SPECIES_LINOONE,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_EXTREMESPEED, MOVE_BELLY_DRUM, MOVE_SHADOW_CLAW, MOVE_FACADE}
    },
    {
    .iv = TRAINER_PARTY_IVS(30, 30, 30, 30, 30, 30),
    .lvl = 39,
    .species = SPECIES_SNORLAX,
    .heldItem = ITEM_LEFTOVERS,
    .ability = ABILITY_THICK_FAT,
    .moves = {MOVE_CURSE, MOVE_REST, MOVE_FACADE, MOVE_EARTHQUAKE}
    },
    {
    .iv = TRAINER_PARTY_IVS(30, 30, 30, 30, 30, 30),
    .lvl = 39,
    .species = SPECIES_SLAKING,
    .heldItem = ITEM_LEFTOVERS,
    .ability = ABILITY_STALL,
    .moves = {MOVE_BULK_UP, MOVE_SLACK_OFF, MOVE_FACADE, MOVE_NIGHT_SLASH}
    },
    {
    .iv = TRAINER_PARTY_IVS(30, 30, 30, 30, 30, 30),
    .lvl = 39,
    .species = SPECIES_AUDINO,
    .heldItem = ITEM_AUDINITE,
    .ability = ABILITY_REGENERATOR,
    .moves = {MOVE_CALM_MIND, MOVE_MOONBLAST, MOVE_FLAMETHROWER, MOVE_WISH}
    }
};

static const struct TrainerMon sParty_Winona1[] = {
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 47,
    .species = SPECIES_STARAPTOR,
    .heldItem = ITEM_LIFE_ORB,
    .moves = {MOVE_CLOSE_COMBAT, MOVE_U_TURN, MOVE_BRAVE_BIRD, MOVE_ROOST}
    },
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 47,
    .species = SPECIES_TROPIUS,
    .heldItem = ITEM_YACHE_BERRY,
    .moves = {MOVE_SUNNY_DAY, MOVE_WEATHER_BALL, MOVE_SOLAR_BEAM, MOVE_SYNTHESIS}
    },
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 47,
    .species = SPECIES_TOGEKISS,
    .heldItem = ITEM_CHOICE_SCARF,
    .ability = ABILITY_SERENE_GRACE,
    .moves = {MOVE_AIR_SLASH, MOVE_HEAT_WAVE, MOVE_MOONBLAST, MOVE_AURA_SPHERE}
    },
    {
    .iv = TRAINER_PARTY_IVS(26, 26, 26, 26, 26, 26),
    .lvl = 48,
    .species = SPECIES_SKARMORY,
    .heldItem = ITEM_WACAN_BERRY,
    .moves = {MOVE_SPIKES, MOVE_ROOST, MOVE_STEEL_WING, MOVE_AERIAL_ACE}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 48,
    .species = SPECIES_HAWLUCHA,
    .heldItem = ITEM_FLYING_GEM,
    .moves = {MOVE_ACROBATICS, MOVE_CLOSE_COMBAT, MOVE_SWORDS_DANCE, MOVE_KNOCK_OFF}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 49,
    .species = SPECIES_ALTARIA,
    .heldItem = ITEM_ALTARIANITE,
    .moves = {MOVE_EARTHQUAKE, MOVE_DRAGON_CLAW, MOVE_DRAGON_DANCE, MOVE_TAKE_DOWN}
    }
};

static const struct TrainerMon sParty_TateAndLiza1[] = {
    {
    .iv = TRAINER_PARTY_IVS(30, 30, 30, 30, 30, 30),
    .lvl = 59,
    .species = SPECIES_SWOOBAT,
    .heldItem = ITEM_PSYCHIC_SEED,
    .ability = ABILITY_SIMPLE,
    .moves = {MOVE_TAILWIND, MOVE_STORED_POWER, MOVE_AIR_SLASH, MOVE_CALM_MIND}
    },
    {
    .iv = TRAINER_PARTY_IVS(30, 30, 30, 30, 30, 30),
    .lvl = 59,
    .species = SPECIES_INDEEDEE_MALE,
    .heldItem = ITEM_TERRAIN_EXTENDER,
    .ability = ABILITY_PSYCHIC_SURGE,
    .moves = {MOVE_PROTECT, MOVE_SHADOW_BALL, MOVE_EXPANDING_FORCE, MOVE_HELPING_HAND}
    },
    {
    .iv = TRAINER_PARTY_IVS(30, 30, 30, 30, 30, 30),
    .lvl = 60,
    .species = SPECIES_DELPHOX,
    .heldItem = ITEM_LIFE_ORB,
    .ability = ABILITY_MAGIC_GUARD,
    .moves = {MOVE_EXPANDING_FORCE, MOVE_HEAT_WAVE, MOVE_GRASS_KNOT, MOVE_CALM_MIND}
    },
    {
    .iv = TRAINER_PARTY_IVS(30, 30, 30, 30, 30, 30),
    .lvl = 60,
    .species = SPECIES_ORANGURU,
    .heldItem = ITEM_COLBUR_BERRY,
    .ability = ABILITY_SYMBIOSIS,
    .moves = {MOVE_INSTRUCT, MOVE_ALLY_SWITCH, MOVE_PSYCHIC, MOVE_PROTECT}
    },
    {
    .iv = TRAINER_PARTY_IVS(30, 30, 30, 30, 30, 30),
    .lvl = 60,
    .species = SPECIES_RAPIDASH_GALARIAN,
    .heldItem = ITEM_EXPERT_BELT,
    .moves = {MOVE_PLAY_ROUGH, MOVE_HIGH_HORSEPOWER, MOVE_FLARE_BLITZ, MOVE_PROTECT}
    },
    {
    .iv = TRAINER_PARTY_IVS(30, 30, 30, 30, 30, 30),
    .lvl = 60,
    .species = SPECIES_MEDICHAM,
    .heldItem = ITEM_MEDICHAMITE,
    .moves = {MOVE_HIGH_JUMP_KICK, MOVE_ICE_PUNCH, MOVE_FIRE_PUNCH, MOVE_THUNDER_PUNCH}
    }
};

static const struct TrainerMon sParty_Juan1[] = {
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 67,
    .species = SPECIES_GRENINJA,
    .heldItem = ITEM_EXPERT_BELT,
    .ability = ABILITY_PROTEAN,
    .moves = {MOVE_WATER_SHURIKEN, MOVE_ICE_BEAM, MOVE_DARK_PULSE, MOVE_RAIN_DANCE}
    },
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 66,
    .species = SPECIES_ROTOM_WASH,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_THUNDERBOLT, MOVE_HYDRO_PUMP, MOVE_NASTY_PLOT, MOVE_WILL_O_WISP}
    },
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 67,
    .species = SPECIES_KINGDRA,
    .heldItem = ITEM_LIFE_ORB,
    .ability = ABILITY_SWIFT_SWIM,
    .moves = {MOVE_SURF, MOVE_DRACO_METEOR, MOVE_ICE_BEAM, MOVE_RAIN_DANCE}
    },
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 67,
    .species = SPECIES_CRAWDAUNT,
    .heldItem = ITEM_WATER_GEM,
    .ability = ABILITY_ADAPTABILITY,
    .moves = {MOVE_KNOCK_OFF, MOVE_CRABHAMMER, MOVE_TAUNT, MOVE_AQUA_JET}
    },
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 68,
    .species = SPECIES_FROSMOTH,
    .heldItem = ITEM_LEFTOVERS,
    .ability = ABILITY_ICE_SCALES,
    .moves = {MOVE_GIGA_DRAIN, MOVE_QUIVER_DANCE, MOVE_HURRICANE, MOVE_ICE_BEAM}
    },
    {
    .iv = TRAINER_PARTY_IVS(30, 30, 30, 30, 30, 30),
    .lvl = 69,
    .species = SPECIES_GYARADOS,
    .heldItem = ITEM_GYARADOSITE,
    .moves = {MOVE_CRUNCH, MOVE_EARTHQUAKE, MOVE_LIQUIDATION, MOVE_DRAGON_DANCE}
    }
};

static const struct TrainerMon sParty_Jerry1[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 12,
    .species = SPECIES_YAMPER,
    }
};

static const struct TrainerMon sParty_Ted[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 18,
    .species = SPECIES_LEDIAN,
    }
};

static const struct TrainerMon sParty_Paul[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 15,
    .species = SPECIES_NUMEL,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 15,
    .species = SPECIES_ODDISH,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 15,
    .species = SPECIES_WINGULL,
    }
};

static const struct TrainerMon sParty_Jerry2[] = {
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 28,
    .species = SPECIES_BOLTUND,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 28,
    .species = SPECIES_TEDDIURSA,
    }
};

static const struct TrainerMon sParty_Jerry3[] = {
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 32,
    .species = SPECIES_BOLTUND,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 32,
    .species = SPECIES_URSARING,
    }
};

static const struct TrainerMon sParty_Jerry4[] = {
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 36,
    .species = SPECIES_BOLTUND,
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 37,
    .species = SPECIES_URSARING,
    }
};

static const struct TrainerMon sParty_Jerry5[] = {
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 45,
    .species = SPECIES_BOLTUND,
    },
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 45,
    .species = SPECIES_BOUFFALANT,
    },
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 45,
    .species = SPECIES_URSALUNA,
    }
};

static const struct TrainerMon sParty_Karen1[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 11,
    .species = SPECIES_TOGEPI,
    .moves = {MOVE_CHARM, MOVE_REST, MOVE_METRONOME, MOVE_BATON_PASS}
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 11,
    .species = SPECIES_CLEFAIRY,
    .moves = {MOVE_CHARM, MOVE_WISH, MOVE_METRONOME, MOVE_BATON_PASS}
    }
};

static const struct TrainerMon sParty_Georgia[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 16,
    .species = SPECIES_SHROOMISH,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 16,
    .species = SPECIES_BEAUTIFLY,
    }
};

static const struct TrainerMon sParty_Karen2[] = {
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 26,
    .species = SPECIES_TOGETIC,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 26,
    .species = SPECIES_SUDOWOODO,
    }
};

static const struct TrainerMon sParty_Karen3[] = {
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 34,
    .species = SPECIES_TOGETIC,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 34,
    .species = SPECIES_SUDOWOODO,
    }
};

static const struct TrainerMon sParty_Karen4[] = {
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 36,
    .species = SPECIES_TOGEKISS,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 36,
    .species = SPECIES_SUDOWOODO,
    }
};

static const struct TrainerMon sParty_Karen5[] = {
     {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 43,
    .species = SPECIES_TOGEKISS,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 43,
    .species = SPECIES_SUDOWOODO,
    }
};

static const struct TrainerMon sParty_KateAndJoy[] = {
    {
    .lvl = 45,
    .species = SPECIES_MALAMAR,
    .ability = ABILITY_CONTRARY,
    .moves = {MOVE_SKILL_SWAP, MOVE_PSYBEAM, MOVE_NIGHT_SLASH, MOVE_PROTECT}
    },
    {
    .lvl = 46,
    .species = SPECIES_SLAKING,
    .moves = {MOVE_SUPERPOWER, MOVE_YAWN, MOVE_SLACK_OFF, MOVE_SHADOW_CLAW}
    }
};

static const struct TrainerMon sParty_AnnaAndMeg1[] = {
    {
    .lvl = 17,
    .species = SPECIES_ZIGZAGOON,
    .moves = {MOVE_GROWL, MOVE_TAIL_WHIP, MOVE_HEADBUTT, MOVE_DIG}
    },
    {
    .lvl = 19,
    .species = SPECIES_MAKUHITA,
    .moves = {MOVE_FAKE_OUT, MOVE_FOCUS_ENERGY, MOVE_ARM_THRUST, MOVE_SHADOW_PUNCH}
    },

};

static const struct TrainerMon sParty_AnnaAndMeg2[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 31,
    .species = SPECIES_LINOONE,
    .moves = {MOVE_EXTREMESPEED, MOVE_DIG, MOVE_HEADBUTT, MOVE_SHADOW_CLAW}
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 33,
    .species = SPECIES_HARIYAMA,
    .moves = {MOVE_FAKE_OUT, MOVE_FOCUS_ENERGY, MOVE_ARM_THRUST, MOVE_ROCK_SLIDE}
    }
};

static const struct TrainerMon sParty_AnnaAndMeg3[] = {
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 36,
    .species = SPECIES_LINOONE,
    .moves = {MOVE_EXTREMESPEED, MOVE_DIG, MOVE_HEADBUTT, MOVE_SHADOW_CLAW}
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 36,
    .species = SPECIES_HARIYAMA,
    .moves = {MOVE_FAKE_OUT, MOVE_FOCUS_ENERGY, MOVE_ARM_THRUST, MOVE_ROCK_SLIDE}
    }
};

static const struct TrainerMon sParty_AnnaAndMeg4[] = {
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 40,
    .species = SPECIES_LINOONE,
    .moves = {MOVE_EXTREMESPEED, MOVE_DIG, MOVE_HEADBUTT, MOVE_SHADOW_CLAW}
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 40,
    .species = SPECIES_HARIYAMA,
    .moves = {MOVE_FAKE_OUT, MOVE_FOCUS_ENERGY, MOVE_ARM_THRUST, MOVE_ROCK_SLIDE}
    }
};

static const struct TrainerMon sParty_AnnaAndMeg5[] = {
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 46,
    .species = SPECIES_LINOONE,
    .moves = {MOVE_EXTREMESPEED, MOVE_DIG, MOVE_HEADBUTT, MOVE_SHADOW_CLAW}
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 46,
    .species = SPECIES_HARIYAMA,
    .moves = {MOVE_FAKE_OUT, MOVE_FOCUS_ENERGY, MOVE_ARM_THRUST, MOVE_ROCK_SLIDE}
    }
};

static const struct TrainerMon sParty_Victor[] = {
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 21,
    .species = SPECIES_PIDGEOTTO,
    .heldItem = ITEM_ORAN_BERRY,
    .moves = {MOVE_QUICK_ATTACK, MOVE_STEEL_WING, MOVE_AIR_SLASH, MOVE_FEATHER_DANCE}
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 21,
    .species = SPECIES_SWELLOW,
    .heldItem = ITEM_FLAME_ORB,
    .ability = ABILITY_GUTS,
    .moves = {MOVE_QUICK_ATTACK, MOVE_STEEL_WING, MOVE_FACADE, MOVE_WING_ATTACK}
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 21,
    .species = SPECIES_NOCTOWL,
    .heldItem = ITEM_TWISTED_SPOON,
    .ability = ABILITY_TINTED_LENS,
    .moves = {MOVE_PSYBEAM, MOVE_HYPNOSIS, MOVE_AIR_SLASH, MOVE_ROOST}
    },
};

static const struct TrainerMon sParty_Miguel1[] = {
    {
    .lvl = 15,
    .species = SPECIES_GOREBYSS,
    .heldItem = ITEM_ORAN_BERRY
    }
};

static const struct TrainerMon sParty_Colton[] = {
    {
    .lvl = 72,
    .species = SPECIES_BLISSEY,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_SOFT_BOILED, MOVE_ICE_BEAM, MOVE_THUNDERBOLT, MOVE_TOXIC}
    },
    {
    .lvl = 72,
    .species = SPECIES_CLEFABLE,
    .heldItem = ITEM_LIFE_ORB,
    .moves = {MOVE_MOONBLAST, MOVE_FLAMETHROWER, MOVE_SOFT_BOILED, MOVE_CALM_MIND}
    },
    {
    .lvl = 72,
    .species = SPECIES_ALOMOMOLA,
    .heldItem = ITEM_ROCKY_HELMET,
    .ability = ABILITY_REGENERATOR,
    .moves = {MOVE_FLIP_TURN, MOVE_TOXIC, MOVE_SCALD, MOVE_WISH}
    },
    {
    .lvl = 71,
    .species = SPECIES_GRANBULL,
    .heldItem = ITEM_CHOICE_BAND,
    .moves = {MOVE_PLAY_ROUGH, MOVE_CLOSE_COMBAT, MOVE_CRUNCH, MOVE_FIRE_PUNCH}
    },
    {
    .lvl = 70,
    .species = SPECIES_SLOWBRO,
    .heldItem = ITEM_ROCKY_HELMET,
    .ability = ABILITY_REGENERATOR,
    .moves = {MOVE_IRON_DEFENSE, MOVE_BODY_PRESS, MOVE_SLACK_OFF, MOVE_SCALD}
    },
    {
    .lvl = 73,
    .species = SPECIES_AUDINO,
    .heldItem = ITEM_AUDINITE,
    .ability = ABILITY_REGENERATOR,
    .moves = {MOVE_WISH, MOVE_MOONBLAST, MOVE_ICE_BEAM, MOVE_TELEPORT}
    }
};

static const struct TrainerMon sParty_Miguel2[] = {
    {
    .lvl = 32,
    .species = SPECIES_DELCATTY,
    .heldItem = ITEM_ORAN_BERRY
    }
};

static const struct TrainerMon sParty_Miguel3[] = {
    {
    .lvl = 35,
    .species = SPECIES_DELCATTY,
    .heldItem = ITEM_ORAN_BERRY
    }
};

static const struct TrainerMon sParty_Miguel4[] = {
    {
    .lvl = 39,
    .species = SPECIES_DELCATTY,
    .heldItem = ITEM_ORAN_BERRY
    }
};

static const struct TrainerMon sParty_Miguel5[] = {
    {
    .lvl = 43,
    .species = SPECIES_DELCATTY,
    .heldItem = ITEM_SITRUS_BERRY
    }
};

static const struct TrainerMon sParty_Victoria[] = {
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 21, 
    .species = SPECIES_IVYSAUR,
    .heldItem = ITEM_MIRACLE_SEED,
    .moves = {MOVE_SLEEP_POWDER, MOVE_SLUDGE, MOVE_RAZOR_LEAF, MOVE_GROWTH}
    },
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 21, 
    .species = SPECIES_CHARMELEON,
    .heldItem = ITEM_CHARCOAL,
    .moves = {MOVE_FIRE_SPIN, MOVE_METAL_CLAW, MOVE_SLASH, MOVE_DRAGON_RAGE}
    },
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 21, 
    .species = SPECIES_WARTORTLE,
    .heldItem = ITEM_MYSTIC_WATER,
    .moves = {MOVE_RAPID_SPIN, MOVE_BITE, MOVE_WHIRLPOOL, MOVE_ICE_FANG}
    }
    
};

static const struct TrainerMon sParty_Vanessa[] = {
    {
    .lvl = 47,
    .species = SPECIES_RAICHU_ALOLAN,
    .heldItem = ITEM_MAGNET
    }
};

static const struct TrainerMon sParty_Bethany[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 100,
    .species = SPECIES_AZURILL,
    .ability = ABILITY_HUGE_POWER,
    .heldItem = ITEM_ORAN_BERRY,
    .moves = {MOVE_BODY_SLAM, MOVE_AQUA_JET, MOVE_CHARM, MOVE_NONE}
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 100,
    .species = SPECIES_DELIBIRD,
    .ability = ABILITY_INSOMNIA,
    .moves = {MOVE_PRESENT, MOVE_DESTINY_BOND, MOVE_NONE, MOVE_NONE}
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 100,
    .species = SPECIES_UNOWN,
    .ability = ABILITY_LEVITATE,
    .heldItem = ITEM_CHOICE_SPECS,
    .moves = {MOVE_HIDDEN_POWER, MOVE_COSMIC_POWER, MOVE_NONE, MOVE_NONE}
    }
};

static const struct TrainerMon sParty_Isabel1[] = {
    {
    .lvl = 18,
    .species = SPECIES_PLUSLE,
    .heldItem = ITEM_ORAN_BERRY
    },
    {
    .lvl = 18,
    .species = SPECIES_MINUN,
    .heldItem = ITEM_ORAN_BERRY
    }
};

static const struct TrainerMon sParty_Isabel2[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 26,
    .species = SPECIES_PLUSLE,
    .heldItem = ITEM_ORAN_BERRY
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 26,
    .species = SPECIES_MINUN,
    .heldItem = ITEM_ORAN_BERRY
    }
};

static const struct TrainerMon sParty_Isabel3[] = {
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 29,
    .species = SPECIES_PLUSLE,
    .heldItem = ITEM_ORAN_BERRY
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 29,
    .species = SPECIES_MINUN,
    .heldItem = ITEM_ORAN_BERRY
    }
};

static const struct TrainerMon sParty_Isabel4[] = {
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 32,
    .species = SPECIES_PLUSLE,
    .heldItem = ITEM_ORAN_BERRY
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 32,
    .species = SPECIES_MINUN,
    .heldItem = ITEM_ORAN_BERRY
    }
};

static const struct TrainerMon sParty_Isabel5[] = {
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 35,
    .species = SPECIES_PLUSLE,
    .heldItem = ITEM_SITRUS_BERRY
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 35,
    .species = SPECIES_MINUN,
    .heldItem = ITEM_SITRUS_BERRY
    }
};

static const struct TrainerMon sParty_Timothy1[] = {
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 27,
    .species = SPECIES_HARIYAMA,
    }
};

static const struct TrainerMon sParty_Timothy2[] = {
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 35,
    .species = SPECIES_HARIYAMA,
    .moves = {MOVE_ARM_THRUST, MOVE_KNOCK_OFF, MOVE_SAND_ATTACK, MOVE_DIG}
    }
};

static const struct TrainerMon sParty_Timothy3[] = {
    {
    .iv = TRAINER_PARTY_IVS(26, 26, 26, 26, 26, 26),
    .lvl = 39,
    .species = SPECIES_HARIYAMA,
    .moves = {MOVE_ARM_THRUST, MOVE_KNOCK_OFF, MOVE_SAND_ATTACK, MOVE_DIG}
    }
};

static const struct TrainerMon sParty_Timothy4[] = {
    {
    .iv = TRAINER_PARTY_IVS(27, 27, 27, 27, 27, 27),
    .lvl = 42,
    .species = SPECIES_HARIYAMA,
    .moves = {MOVE_ARM_THRUST, MOVE_BELLY_DRUM, MOVE_SAND_ATTACK, MOVE_DIG}
    }
};

static const struct TrainerMon sParty_Timothy5[] = {
    {
    .iv = TRAINER_PARTY_IVS(29, 29, 29, 29, 29, 29),
    .lvl = 49,
    .species = SPECIES_HARIYAMA,
    .moves = {MOVE_ARM_THRUST, MOVE_BELLY_DRUM, MOVE_KNOCK_OFF, MOVE_DIG}
    }
};

static const struct TrainerMon sParty_Vicky[] = {
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 22,
    .species = SPECIES_HITMONCHAN,
    .moves = {MOVE_ICE_PUNCH, MOVE_FIRE_PUNCH, MOVE_MACH_PUNCH, MOVE_SHADOW_PUNCH}
    },
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 22,
    .species = SPECIES_HITMONLEE,
    .moves = {MOVE_MEGA_KICK, MOVE_BLAZE_KICK, MOVE_ROLLING_KICK, MOVE_DETECT}
    },
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 22,
    .species = SPECIES_HITMONTOP,
    .moves = {MOVE_TRIPLE_AXEL, MOVE_TRIPLE_KICK, MOVE_NONE, MOVE_NONE}
    }
};

static const struct TrainerMon sParty_Shelby1[] = {
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 21,
    .species = SPECIES_MEDITITE,
    },
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 21,
    .species = SPECIES_MAKUHITA,
    }
};

static const struct TrainerMon sParty_Shelby2[] = {
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 30,
    .species = SPECIES_MEDITITE,
    },
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 30,
    .species = SPECIES_MAKUHITA,
    }
};

static const struct TrainerMon sParty_Shelby3[] = {
    {
    .iv = TRAINER_PARTY_IVS(26, 26, 26, 26, 26, 26),
    .lvl = 33,
    .species = SPECIES_MEDICHAM,
    },
    {
    .iv = TRAINER_PARTY_IVS(26, 26, 26, 26, 26, 26),
    .lvl = 33,
    .species = SPECIES_HARIYAMA,
    }
};

static const struct TrainerMon sParty_Shelby4[] = {
    {
    .iv = TRAINER_PARTY_IVS(27, 27, 27, 27, 27, 27),
    .lvl = 36,
    .species = SPECIES_MEDICHAM,
    },
    {
    .iv = TRAINER_PARTY_IVS(27, 27, 27, 27, 27, 27),
    .lvl = 36,
    .species = SPECIES_HARIYAMA,
    }
};

static const struct TrainerMon sParty_Shelby5[] = {
    {
    .iv = TRAINER_PARTY_IVS(29, 29, 29, 29, 29, 29),
    .lvl = 39,
    .species = SPECIES_MEDICHAM,
    },
    {
    .iv = TRAINER_PARTY_IVS(29, 29, 29, 29, 29, 29),
    .lvl = 39,
    .species = SPECIES_HARIYAMA,
    }
};

static const struct TrainerMon sParty_Calvin1[] = {
    {
    .lvl = 6,
    .species = SPECIES_LILLIPUP,
    },
    {
    .lvl = 6,
    .species = SPECIES_POOCHYENA,
    .moves = {MOVE_BITE, MOVE_HOWL, MOVE_PURSUIT, MOVE_NONE}
    },
    {
    .lvl = 6,
    .species = SPECIES_SPOINK,
    .moves = {MOVE_CONFUSION, MOVE_BOUNCE, MOVE_NONE, MOVE_NONE}
    },
};

static const struct TrainerMon sParty_Billy[] = {
    {
    .lvl = 6,
    .species = SPECIES_SKWOVET,
    },
    {
    .lvl = 7,
    .species = SPECIES_MILCERY,
    }
};

static const struct TrainerMon sParty_Josh[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 10,
    .species = SPECIES_NACLI,
    .moves = {MOVE_TACKLE, MOVE_ROCK_THROW, MOVE_NONE, MOVE_NONE}
    }
};

static const struct TrainerMon sParty_Tommy[] = {
    {
    .iv = TRAINER_PARTY_IVS(13, 13, 13, 13, 13, 13),
    .lvl = 12,
    .species = SPECIES_ROGGENROLA,
    },
    {
    .iv = TRAINER_PARTY_IVS(14, 14, 14, 14, 14, 14),
    .lvl = 13,
    .species = SPECIES_LARVITAR,
    }
};

static const struct TrainerMon sParty_Joey[] = {
    {
    .lvl = 10,
    .species = SPECIES_MACHOP,
    }
};

static const struct TrainerMon sParty_Ben[] = {
    {
    .iv = TRAINER_PARTY_IVS(18, 18, 18, 18, 18, 18),
    .lvl = 21,
    .species = SPECIES_ZIGZAGOON,
    .moves = {MOVE_HEADBUTT, MOVE_DIG, MOVE_GROWL, MOVE_THUNDERBOLT}
    },
    {
    .iv = TRAINER_PARTY_IVS(18, 18, 18, 18, 18, 18),
    .lvl = 22,
    .species = SPECIES_KOFFING,
    .moves = {MOVE_EMBER, MOVE_SLUDGE, MOVE_THUNDERBOLT, MOVE_SELF_DESTRUCT}
    },
    {
    .iv = TRAINER_PARTY_IVS(18, 18, 18, 18, 18, 18),
    .lvl = 22,
    .species = SPECIES_PIKACHU_ALOLA_CAP,
    .moves = {MOVE_THUNDER_SHOCK, MOVE_IRON_TAIL, MOVE_EXTREMESPEED, MOVE_VOLT_SWITCH}
    }

};

static const struct TrainerMon sParty_Quincy[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 69,
    .species = SPECIES_SLAKING,
    .moves = {MOVE_ATTRACT, MOVE_ICE_BEAM, MOVE_THUNDERBOLT, MOVE_FLAMETHROWER}
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 69,
    .species = SPECIES_DUSCLOPS,
    .heldItem = ITEM_EVIOLITE,
    .moves = {MOVE_SKILL_SWAP, MOVE_PROTECT, MOVE_WILL_O_WISP, MOVE_TOXIC}
    }
};

static const struct TrainerMon sParty_Katelynn[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 69,
    .species = SPECIES_GARDEVOIR,
    .heldItem = ITEM_GARDEVOIRITE,
    .moves = {MOVE_SKILL_SWAP, MOVE_PSYCHIC, MOVE_THUNDERBOLT, MOVE_CALM_MIND}
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 69,
    .species = SPECIES_REGIGIGAS,
    .moves = {MOVE_EARTHQUAKE, MOVE_BODY_SLAM, MOVE_AERIAL_ACE, MOVE_BRICK_BREAK}
    }
};

static const struct TrainerMon sParty_Jaylen[] = {
    {
    .lvl = 26,
    .species = SPECIES_TRAPINCH,
    }
};

static const struct TrainerMon sParty_Dillon[] = {
    {
    .lvl = 26,
    .species = SPECIES_SPIRITOMB,
    },
    {
    .lvl = 26,
    .species = SPECIES_SHUPPET,
    }
};

static const struct TrainerMon sParty_Calvin2[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 27,
    .species = SPECIES_HERDIER,
    }
};

static const struct TrainerMon sParty_Calvin3[] = {
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 32,
    .species = SPECIES_SWELLOW,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 32,
    .species = SPECIES_MIGHTYENA,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 32,
    .species = SPECIES_HERDIER,
    }
};

static const struct TrainerMon sParty_Calvin4[] = {
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 35,
    .species = SPECIES_SWELLOW,
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 35,
    .species = SPECIES_LINOONE,
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 35,
    .species = SPECIES_MIGHTYENA,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 36,
    .species = SPECIES_STOUTLAND,
    }
};

static const struct TrainerMon sParty_Calvin5[] = {
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 41,
    .species = SPECIES_SWELLOW,
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 41,
    .species = SPECIES_LINOONE,
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 42,
    .species = SPECIES_MIGHTYENA,
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 44,
    .species = SPECIES_STOUTLAND,
    }
};

static const struct TrainerMon sParty_Eddie[] = {
    {
    .lvl = 14,
    .species = SPECIES_ZIGZAGOON,
    },
    {
    .lvl = 16,
    .species = SPECIES_ZIGZAGOON,
    }
};

static const struct TrainerMon sParty_Allen[] = {
    {
    .lvl = 6,
    .species = SPECIES_WATTREL,
    },
    {
    .lvl = 6,
    .species = SPECIES_TAILLOW,
    }
};

static const struct TrainerMon sParty_Timmy[] = {
    {
    .lvl = 15,
    .species = SPECIES_ARON,
    },
    {
    .lvl = 13,
    .species = SPECIES_ELECTRIKE,
    }
};

static const struct TrainerMon sParty_Wallace[] = {
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 79,
    .species = SPECIES_WAILORD,
    .heldItem = ITEM_ASSAULT_VEST,
    .ability = ABILITY_DRIZZLE,
    .moves = {MOVE_HYPER_VOICE, MOVE_WATER_SPOUT, MOVE_HEAVY_SLAM, MOVE_BLIZZARD}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 79,
    .species = SPECIES_GOODRA_HISUIAN,
    .heldItem = ITEM_ASSAULT_VEST,
    .ability = ABILITY_SAP_SIPPER,
    .moves = {MOVE_WEATHER_BALL, MOVE_THUNDER, MOVE_SLUDGE_BOMB, MOVE_DRAGON_TAIL}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_MILOTIC,
    .heldItem = ITEM_LEFTOVERS,
    .ability = ABILITY_COMPETITIVE,
    .moves = {MOVE_ALLURING_VOICE, MOVE_SCALD, MOVE_ICE_BEAM, MOVE_RECOVER}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_DRACOVISH,
    .heldItem = ITEM_CHOICE_SCARF,
    .ability = ABILITY_STRONG_JAW,
    .moves = {MOVE_FISHIOUS_REND, MOVE_PSYCHIC_FANGS, MOVE_ICE_FANG, MOVE_OUTRAGE}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 81,
    .species = SPECIES_HELIOLISK,
    .heldItem = ITEM_LIFE_ORB,
    .ability = ABILITY_DRY_SKIN,
    .moves = {MOVE_WEATHER_BALL, MOVE_THUNDER, MOVE_GRASS_KNOT, MOVE_U_TURN}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 81,
    .species = SPECIES_BLASTOISE,
    .heldItem = ITEM_BLASTOISINITE,
    .moves = {MOVE_WATER_PULSE, MOVE_AURA_SPHERE, MOVE_DRAGON_PULSE, MOVE_SHELL_SMASH}
    }
};

static const struct TrainerMon sParty_Andrew[] = {
    {
    .lvl = 40,
    .species = SPECIES_MAGIKARP,
    },
    {
    .lvl = 17,
    .species = SPECIES_TENTACOOL,
    },
    {
    .lvl = 40,
    .species = SPECIES_MAGIKARP,
    }
};

static const struct TrainerMon sParty_Ivan[] = {
    {
    .lvl = 11,
    .species = SPECIES_SHELLDER,
    .moves = {MOVE_SUPERSONIC, MOVE_ICY_WIND, MOVE_CLAMP, MOVE_SHELL_SMASH}
    },
    {
    .lvl = 10,
    .species = SPECIES_SHELLOS,
    .moves = {MOVE_WATER_PULSE, MOVE_RECOVER, MOVE_MUD_SLAP, MOVE_ICY_WIND}
    },
    {
    .lvl = 11,
    .species = SPECIES_TENTACOOL,
    .moves = {MOVE_ACID, MOVE_CONSTRICT, MOVE_WATER_GUN, MOVE_NONE}
    }
};

static const struct TrainerMon sParty_Claude[] = {
    {
    .lvl = 27,
    .species = SPECIES_GYARADOS,
    },
    {
    .lvl = 27,
    .species = SPECIES_GOLDEEN,
    },
    {
    .lvl = 29,
    .species = SPECIES_DREDNAW,
    }
};

static const struct TrainerMon sParty_Elliot1[] = {
    {
    .lvl = 15,
    .species = SPECIES_GOLDEEN,
    },
    {
    .lvl = 16,
    .species = SPECIES_TENTACOOL,
    },
    {
    .lvl = 16,
    .species = SPECIES_SHELLDER,
    }
};

static const struct TrainerMon sParty_Ned[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 16,
    .species = SPECIES_TENTACOOL,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 16,
    .species = SPECIES_CORPHISH,
    }
};

static const struct TrainerMon sParty_Dale[] = {
    {
    .lvl = 19,
    .species = SPECIES_TENTACOOL,
    },
    {
    .lvl = 18,
    .species = SPECIES_WAILMER,
    },
    {
    .lvl = 19,
    .species = SPECIES_OMANYTE,
    },
    {
    .lvl = 18,
    .species = SPECIES_KABUTO,
    }
};

static const struct TrainerMon sParty_Nolan[] = {
    {
    .lvl = 28,
    .species = SPECIES_PALPITOAD,
    }
};

static const struct TrainerMon sParty_Barny[] = {
    {
    .lvl = 33,
    .species = SPECIES_TENTACRUEL,
    },
    {
    .lvl = 36,
    .species = SPECIES_KINGLER,
    }
};

static const struct TrainerMon sParty_Wade[] = {
    {
    .lvl = 16,
    .species = SPECIES_TENTACOOL,
    }
};

static const struct TrainerMon sParty_Carter[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 35,
    .species = SPECIES_WAILMER,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 35,
    .species = SPECIES_TENTACRUEL,
    }
};

static const struct TrainerMon sParty_Elliot2[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 27,
    .species = SPECIES_GOLDEEN,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 28,
    .species = SPECIES_CLOYSTER,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 28,
    .species = SPECIES_GYARADOS,
    }
};

static const struct TrainerMon sParty_Elliot3[] = {
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 34,
    .species = SPECIES_GYARADOS,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 34,
    .species = SPECIES_CLOYSTER,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 36,
    .species = SPECIES_SEAKING,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 36,
    .species = SPECIES_GASTRODON,
    }
};

static const struct TrainerMon sParty_Elliot4[] = {
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 41,
    .species = SPECIES_GYARADOS,
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 41,
    .species = SPECIES_SEAKING,
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 42,
    .species = SPECIES_TENTACRUEL,
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 44,
    .species = SPECIES_GASTRODON,
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 44,
    .species = SPECIES_CLOYSTER,
    }
};

static const struct TrainerMon sParty_Elliot5[] = {
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 47,
    .species = SPECIES_GYARADOS,
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 46,
    .species = SPECIES_SEAKING,
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 46,
    .species = SPECIES_TENTACRUEL,
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 46,
    .species = SPECIES_GASTRODON,
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 48,
    .species = SPECIES_CLOYSTER,
    }
};

static const struct TrainerMon sParty_Ronald[] = {
    {
    .lvl = 100,
    .species = SPECIES_MAGIKARP,
    },
    {
    .lvl = 39,
    .species = SPECIES_JELLICENT,
    },
    {
    .lvl = 40,
    .species = SPECIES_BASCULEGION,
    },
    {
    .lvl = 40,
    .species = SPECIES_FLOATZEL,
    },
    {
    .lvl = 42,
    .species = SPECIES_GYARADOS,
    },
    {
    .lvl = 42,
    .species = SPECIES_GYARADOS,
    }
};

static const struct TrainerMon sParty_Jacob[] = {
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 18,
    .species = SPECIES_VOLBEAT,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 18,
    .species = SPECIES_LUXIO,
    },
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 18,
    .species = SPECIES_MAGNEMITE,
    }
};

static const struct TrainerMon sParty_Anthony[] = {
    {
    .lvl = 18,
    .species = SPECIES_MAGNEMITE,
    },
    {
    .lvl = 18,
    .species = SPECIES_MAGNEMITE,
    }
};

static const struct TrainerMon sParty_Benjamin1[] = {
    {
    .lvl = 19,
    .species = SPECIES_MAGNEMITE,
    }
};

static const struct TrainerMon sParty_Benjamin2[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 30,
    .species = SPECIES_MAGNEMITE,
    }
};

static const struct TrainerMon sParty_Benjamin3[] = {
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 35,
    .species = SPECIES_MAGNETON,
    }
};

static const struct TrainerMon sParty_Benjamin4[] = {
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 38,
    .species = SPECIES_MAGNETON,
    }
};

static const struct TrainerMon sParty_Benjamin5[] = {
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 43,
    .species = SPECIES_MAGNEZONE,
    }
};

static const struct TrainerMon sParty_Abigail1[] = {
    {
    .lvl = 16,
    .species = SPECIES_MAGNEMITE,
    }
};

static const struct TrainerMon sParty_Jasmine[] = {
    {
    .iv = TRAINER_PARTY_IVS(9, 9, 9, 9, 9, 9),
    .lvl = 19,
    .species = SPECIES_GRIMER_ALOLAN,
    },
    {
    .iv = TRAINER_PARTY_IVS(9, 9, 9, 9, 9, 9),
    .lvl = 17,
    .species = SPECIES_MAGNEMITE,
    },
    {
    .lvl = 17,
    .species = SPECIES_VOLTORB,
    }
};

static const struct TrainerMon sParty_Abigail2[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 28,
    .species = SPECIES_MAGNEMITE,
    }
};

static const struct TrainerMon sParty_Abigail3[] = {
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 33,
    .species = SPECIES_MAGNETON,
    }
};

static const struct TrainerMon sParty_Abigail4[] = {
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 37,
    .species = SPECIES_MAGNETON,
    }
};

static const struct TrainerMon sParty_Abigail5[] = {
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 43,
    .species = SPECIES_MAGNEZONE,
    }
};

static const struct TrainerMon sParty_Dylan1[] = {
    {
    .lvl = 17,
    .species = SPECIES_DODUO,
    }
};

static const struct TrainerMon sParty_Dylan2[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 28,
    .species = SPECIES_DODUO,
    }
};

static const struct TrainerMon sParty_Dylan3[] = {
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 33,
    .species = SPECIES_DODUO,
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 33,
    .species = SPECIES_SQUAWKABILLY,
    }
};

static const struct TrainerMon sParty_Dylan4[] = {
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 37,
    .species = SPECIES_DODRIO,
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 37,
    .species = SPECIES_SQUAWKABILLY,
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 37,
    .species = SPECIES_AUDINO,
    }
};

static const struct TrainerMon sParty_Dylan5[] = {
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 44,
    .species = SPECIES_DODRIO,
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 45,
    .species = SPECIES_SQUAWKABILLY,
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 46,
    .species = SPECIES_AUDINO,
    }
};

static const struct TrainerMon sParty_Maria1[] = {
    {
    .lvl = 17,
    .species = SPECIES_DODUO,
    }
};

static const struct TrainerMon sParty_Maria2[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 28,
    .species = SPECIES_DODUO,
    }
};

static const struct TrainerMon sParty_Maria3[] = {
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 33,
    .species = SPECIES_DODUO,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 33,
    .species = SPECIES_PIDGEOTTO,
    }
};

static const struct TrainerMon sParty_Maria4[] = {
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 37,
    .species = SPECIES_DODRIO,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 37,
    .species = SPECIES_PIDGEOT,
    }
};

static const struct TrainerMon sParty_Maria5[] = {
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 44,
    .species = SPECIES_DODRIO,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 44,
    .species = SPECIES_PIDGEOT,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 44,
    .species = SPECIES_BLISSEY,
    }
};

static const struct TrainerMon sParty_Camden[] = {
    {
    .lvl = 33,
    .species = SPECIES_STARYU,
    },
    {
    .lvl = 33,
    .species = SPECIES_STARYU,
    }
};

static const struct TrainerMon sParty_Demetrius[] = {
    {
    .lvl = 34,
    .species = SPECIES_LINOONE,
    },
    {
    .lvl = 34,
    .species = SPECIES_MANECTRIC,
    .heldItem = ITEM_MANECTITE,
    }
};

static const struct TrainerMon sParty_Isaiah1[] = {
    {
    .lvl = 55,
    .species = SPECIES_STARMIE,
    },
    {
    .lvl = 55,
    .species = SPECIES_SWAMPERT,
    .moves = {MOVE_HYDRO_PUMP, MOVE_EARTHQUAKE, MOVE_ICE_PUNCH, MOVE_PROTECT}
    }
};

static const struct TrainerMon sParty_Pablo1[] = {
    {
    .lvl = 33,
    .species = SPECIES_STARYU,
    },
    {
    .lvl = 33,
    .species = SPECIES_STARYU,
    }
};

static const struct TrainerMon sParty_Chase[] = {
    {
    .lvl = 51,
    .species = SPECIES_PELIPPER,
    },
    {
    .lvl = 52,
    .species = SPECIES_CRAMORANT,
    .moves = {MOVE_SURF, MOVE_DRILL_PECK, MOVE_ICE_BEAM, MOVE_PROTECT}
    }
};

static const struct TrainerMon sParty_Isaiah2[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 41,
    .species = SPECIES_STARMIE,
    }
};

static const struct TrainerMon sParty_Isaiah3[] = {
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 45,
    .species = SPECIES_STARMIE,
    }
};

static const struct TrainerMon sParty_Isaiah4[] = {
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 49,
    .species = SPECIES_STARMIE,
    }
};

static const struct TrainerMon sParty_Isaiah5[] = {
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 55,
    .species = SPECIES_STARMIE,
    }
};

static const struct TrainerMon sParty_Isobel[] = {
    {
    .lvl = 54,
    .species = SPECIES_STARMIE,
    },
    {
    .lvl = 54,
    .species = SPECIES_ALCREMIE,
    },
};

static const struct TrainerMon sParty_Donny[] = {
    {
    .lvl = 26,
    .species = SPECIES_WINGULL,
    },
    {
    .iv = TRAINER_PARTY_IVS(19, 19, 19, 19, 19, 19),
    .lvl = 34,
    .species = SPECIES_STARYU,
    }
};

static const struct TrainerMon sParty_Talia[] = {
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 50,
    .species = SPECIES_MANTINE,
    .ability = ABILITY_WATER_ABSORB,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_SCALD, MOVE_AIR_SLASH, MOVE_ROOST, MOVE_HAZE}
    }
};

static const struct TrainerMon sParty_Katelyn1[] = {
    {
    .lvl = 45,
    .species = SPECIES_STARMIE,
    }
};

static const struct TrainerMon sParty_Allison[] = {
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 55,
    .species = SPECIES_BARRASKEWDA,
    .ability = ABILITY_SWIFT_SWIM,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_LIQUIDATION, MOVE_CRUNCH, MOVE_CLOSE_COMBAT, MOVE_POISON_JAB}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 55,
    .species = SPECIES_LAPRAS,
    .ability = ABILITY_WATER_ABSORB,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_SURF, MOVE_ICE_BEAM, MOVE_THUNDERBOLT, MOVE_CONFUSE_RAY}
    }
};

static const struct TrainerMon sParty_Katelyn2[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 39,
    .species = SPECIES_STARYU,
    }
};

static const struct TrainerMon sParty_Katelyn3[] = {
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 42,
    .species = SPECIES_STARMIE,
    }
};

static const struct TrainerMon sParty_Katelyn4[] = {
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 47,
    .species = SPECIES_STARMIE,
    }
};

static const struct TrainerMon sParty_Katelyn5[] = {
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 52,
    .species = SPECIES_STARMIE,
    }
};

static const struct TrainerMon sParty_Nicolas1[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 69,
    .species = SPECIES_ALTARIA,
    .moves = {MOVE_COTTON_GUARD, MOVE_ROOST, MOVE_DRAGON_PULSE, MOVE_MOONBLAST}
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 69,
    .species = SPECIES_SALAMENCE,
    .heldItem = ITEM_EXPERT_BELT,
    .moves = {MOVE_BRICK_BREAK, MOVE_DRAGON_DANCE, MOVE_DUAL_WINGBEAT, MOVE_FIRE_BLAST}
    }
};

static const struct TrainerMon sParty_Nicolas2[] = {
    {
    .iv = TRAINER_PARTY_IVS(13, 13, 13, 13, 13, 13),
    .lvl = 62,
    .species = SPECIES_ALTARIA,
    },
    {
    .iv = TRAINER_PARTY_IVS(13, 13, 13, 13, 13, 13),
    .lvl = 62,
    .species = SPECIES_SALAMENCE,
    }
};

static const struct TrainerMon sParty_Nicolas3[] = {
    {
    .iv = TRAINER_PARTY_IVS(14, 14, 14, 14, 14, 14),
    .lvl = 67,
    .species = SPECIES_ALTARIA,
    },
    {
    .iv = TRAINER_PARTY_IVS(14, 14, 14, 14, 14, 14),
    .lvl = 67,
    .species = SPECIES_SALAMENCE,
    }
};

static const struct TrainerMon sParty_Nicolas4[] = {
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 70,
    .species = SPECIES_SALAMENCE,
    },
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 70,
    .species = SPECIES_ALTARIA,
    .heldItem = ITEM_ALTARIANITE,
    },
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 70,
    .species = SPECIES_DRAGONITE,
    }
};

static const struct TrainerMon sParty_Nicolas5[] = {
    {
    .iv = TRAINER_PARTY_IVS(17, 17, 17, 17, 17, 17),
    .lvl = 76,
    .species = SPECIES_ALTARIA,
    .heldItem = ITEM_ALTARIANITE,
    },
    {
    .iv = TRAINER_PARTY_IVS(17, 17, 17, 17, 17, 17),
    .lvl = 76,
    .species = SPECIES_SALAMENCE,
    .heldItem = ITEM_NONE,
    },
    {
    .iv = TRAINER_PARTY_IVS(17, 17, 17, 17, 17, 17),
    .lvl = 76,
    .species = SPECIES_DRAGONITE,
    .heldItem = ITEM_DRAGON_FANG
    },
    {
    .iv = TRAINER_PARTY_IVS(17, 17, 17, 17, 17, 17),
    .lvl = 76,
    .species = SPECIES_DRAGAPULT,
    .heldItem = ITEM_YACHE_BERRY
    }
};

static const struct TrainerMon sParty_Aaron[] = {
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 52,
    .species = SPECIES_KINGDRA,
    .heldItem = ITEM_SCOPE_LENS,
    .ability = ABILITY_SNIPER,
    .moves = {MOVE_DRACO_METEOR, MOVE_SURF, MOVE_FOCUS_ENERGY, MOVE_HURRICANE}
    }
};

static const struct TrainerMon sParty_Perry[] = {
    {
    .lvl = 37,
    .species = SPECIES_CORVIKNIGHT,
    }
};

static const struct TrainerMon sParty_Hugh[] = {
    {
    .lvl = 35,
    .species = SPECIES_TOUCANNON,
    },
    {
    .lvl = 35,
    .species = SPECIES_ALTARIA,
    }
};

static const struct TrainerMon sParty_Phil[] = {
    {
    .lvl = 36,
    .species = SPECIES_SWELLOW,
    }
};

static const struct TrainerMon sParty_Jared[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 44,
    .species = SPECIES_TALONFLAME,
    .moves = {MOVE_ACROBATICS, MOVE_FLARE_BLITZ, MOVE_U_TURN, MOVE_SWORDS_DANCE}
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 45,
    .species = SPECIES_SKARMORY,
    .moves = {MOVE_BODY_PRESS, MOVE_STEEL_WING, MOVE_ROOST, MOVE_IRON_DEFENSE}
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 46,
    .species = SPECIES_TROPIUS,
    .moves = {MOVE_DRAGON_HAMMER, MOVE_WOOD_HAMMER, MOVE_DRAGON_DANCE, MOVE_SYNTHESIS}
    }
};

static const struct TrainerMon sParty_Humberto[] = {
    {
    .iv = TRAINER_PARTY_IVS(30, 30, 30, 30, 30, 30),
    .lvl = 46,
    .species = SPECIES_SKARMORY,
    .heldItem = ITEM_WACAN_BERRY,
    }
};

static const struct TrainerMon sParty_Presley[] = {
    {
    .lvl = 37,
    .species = SPECIES_VIKAVOLT,
    },
    {
    .lvl = 38,
    .species = SPECIES_CLAYDOL,
    }
};

static const struct TrainerMon sParty_Edwardo[] = {
    {
    .iv = TRAINER_PARTY_IVS(18, 18, 18, 18, 18, 18),
    .lvl = 43,
    .species = SPECIES_SIRFETCHD,
    },
    {
    .iv = TRAINER_PARTY_IVS(18, 18, 18, 18, 18, 18),
    .lvl = 45,
    .species = SPECIES_ROTOM_FAN,
    }
};

static const struct TrainerMon sParty_Colin[] = {
    {
    .lvl = 44,
    .species = SPECIES_EMOLGA,
    },
    {
    .lvl = 44,
    .species = SPECIES_FLAMIGO,
    }
};

static const struct TrainerMon sParty_Robert1[] = {
    {
    .lvl = 41,
    .species = SPECIES_NOIVERN,
    }
};

static const struct TrainerMon sParty_Benny[] = {
    {
    .lvl = 41,
    .species = SPECIES_ORICORIO,
    },
    {
    .lvl = 41,
    .species = SPECIES_ORICORIO_POM_POM,
    },
    {
    .lvl = 41,
    .species = SPECIES_ORICORIO_SENSU,
    },
    {
    .lvl = 41,
    .species = SPECIES_ORICORIO_PAU,
    }
};

static const struct TrainerMon sParty_Chester[] = {
    {
    .lvl = 39,
    .species = SPECIES_UNFEZANT,
    },
    {
    .lvl = 39,
    .species = SPECIES_MANTINE,
    }
};

static const struct TrainerMon sParty_Robert2[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 44,
    .species = SPECIES_NOIVERN,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 44,
    .species = SPECIES_MANDIBUZZ,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 44,
    .species = SPECIES_BRAVIARY,
    }
};

static const struct TrainerMon sParty_Robert3[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 47,
    .species = SPECIES_NOIVERN,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 47,
    .species = SPECIES_MANDIBUZZ,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 47,
    .species = SPECIES_BRAVIARY,
    }
};

static const struct TrainerMon sParty_Robert4[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 52,
    .species = SPECIES_NOIVERN,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 52,
    .species = SPECIES_MANDIBUZZ,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 52,
    .species = SPECIES_BRAVIARY,
    }
};

static const struct TrainerMon sParty_Robert5[] = {
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 56,
    .species = SPECIES_NOIVERN,
    },
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 56,
    .species = SPECIES_MANDIBUZZ,
    },
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 56,
    .species = SPECIES_BRAVIARY,
    }
};

static const struct TrainerMon sParty_Alex[] = {
    {
    .iv = TRAINER_PARTY_IVS(18, 18, 18, 18, 18, 18),
    .lvl = 33,
    .species = SPECIES_DRIFBLIM,
    },
    {
    .iv = TRAINER_PARTY_IVS(18, 18, 18, 18, 18, 18),
    .lvl = 36,
    .species = SPECIES_CHARIZARD,
    }
};

static const struct TrainerMon sParty_Beck[] = {
    {
    .lvl = 37,
    .species = SPECIES_TROPIUS,
    }
};

static const struct TrainerMon sParty_Yasu[] = {
    {
    .lvl = 38,
    .species = SPECIES_RUNERIGUS,
    },
    {
    .lvl = 39,
    .species = SPECIES_DUSKNOIR,
    }

};

static const struct TrainerMon sParty_Takashi[] = {
    {
    .lvl = 35,
    .species = SPECIES_NINJASK,
    },
    {
    .lvl = 35,
    .species = SPECIES_WEEZING,
    }
};

static const struct TrainerMon sParty_Dianne[] = {
    {
    .lvl = 66,
    .species = SPECIES_CLAYDOL,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_SKILL_SWAP, MOVE_EARTHQUAKE, MOVE_RECOVER, MOVE_EXPLOSION}
    },
    {
    .lvl = 69,
    .species = SPECIES_LANTURN,
    .heldItem = ITEM_AIR_BALLOON,
    .moves = {MOVE_THUNDERBOLT, MOVE_EARTHQUAKE, MOVE_ICE_BEAM, MOVE_HYDRO_PUMP}
    }
};

static const struct TrainerMon sParty_Jani[] = {
    {
    .lvl = 36,
    .species = SPECIES_REMORAID
    },
    {
    .lvl = 36,
    .species = SPECIES_OCTILLERY
    },
    {
    .lvl = 36,
    .species = SPECIES_ELECTABUZZ
    }
};

static const struct TrainerMon sParty_Lao1[] = {
    {
    .lvl = 23,
    .species = SPECIES_KOFFING,
    .moves = {MOVE_POISON_GAS, MOVE_TACKLE, MOVE_SMOG, MOVE_EXPLOSION}
    },
    {
    .lvl = 24,
    .species = SPECIES_KOFFING,
    .moves = {MOVE_POISON_GAS, MOVE_TACKLE, MOVE_SLUDGE, MOVE_EXPLOSION}
    },
    {
    .lvl = 25,
    .species = SPECIES_KOFFING,
    .moves = {MOVE_POISON_GAS, MOVE_TACKLE, MOVE_SLUDGE, MOVE_EXPLOSION}
    }
};

static const struct TrainerMon sParty_Lung[] = {
    {
    .lvl = 26,
    .species = SPECIES_KOFFING,
    },
    {
    .lvl = 27,
    .species = SPECIES_NINJASK,
    }
};

static const struct TrainerMon sParty_Lao2[] = {
    {
    .lvl = 26,
    .species = SPECIES_KOFFING,
    .moves = {MOVE_POISON_GAS, MOVE_TACKLE, MOVE_SLUDGE, MOVE_SELF_DESTRUCT}
    },
    {
    .lvl = 26,
    .species = SPECIES_TRUBBISH,
    .moves = {MOVE_POISON_GAS, MOVE_TACKLE, MOVE_SLUDGE, MOVE_SELF_DESTRUCT}
    },
    {
    .lvl = 26,
    .species = SPECIES_GRIMER,
    .moves = {MOVE_POISON_GAS, MOVE_TACKLE, MOVE_SLUDGE, MOVE_SELF_DESTRUCT}
    },
    {
    .lvl = 27,
    .species = SPECIES_QWILFISH,
    .moves = {MOVE_TACKLE, MOVE_SLUDGE, MOVE_SELF_DESTRUCT, MOVE_NONE}
    }
};

static const struct TrainerMon sParty_Lao3[] = {
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 34,
    .species = SPECIES_KOFFING,
    .moves = {MOVE_POISON_GAS, MOVE_TACKLE, MOVE_SLUDGE, MOVE_SELF_DESTRUCT}
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 34,
    .species = SPECIES_TRUBBISH,
    .moves = {MOVE_POISON_GAS, MOVE_TACKLE, MOVE_SLUDGE, MOVE_SELF_DESTRUCT}
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 34,
    .species = SPECIES_GRIMER,
    .moves = {MOVE_POISON_GAS, MOVE_TACKLE, MOVE_SLUDGE, MOVE_SELF_DESTRUCT}
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 34,
    .species = SPECIES_QWILFISH,
    .moves = {MOVE_TACKLE, MOVE_SLUDGE, MOVE_SELF_DESTRUCT, MOVE_NONE}
    }
};

static const struct TrainerMon sParty_Lao4[] = {
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 39,
    .species = SPECIES_WEEZING,
    .moves = {MOVE_POISON_GAS, MOVE_TACKLE, MOVE_SLUDGE_BOMB, MOVE_EXPLOSION}
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 40,
    .species = SPECIES_GARBODOR,
    .moves = {MOVE_POISON_GAS, MOVE_BODY_SLAM, MOVE_SLUDGE, MOVE_EXPLOSION}
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 40,
    .species = SPECIES_GRIMER,
    .moves = {MOVE_POISON_GAS, MOVE_DRAIN_PUNCH, MOVE_POISON_JAB, MOVE_EXPLOSION}
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 39,
    .species = SPECIES_QWILFISH,
    .moves = {MOVE_LIQUIDATION, MOVE_POISON_JAB, MOVE_EXPLOSION, MOVE_NONE}
    }
};

static const struct TrainerMon sParty_Lao5[] = {
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 45,
    .species = SPECIES_WEEZING,
    .moves = {MOVE_POISON_GAS, MOVE_TACKLE, MOVE_SLUDGE_BOMB, MOVE_EXPLOSION}
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 45,
    .species = SPECIES_GARBODOR,
    .moves = {MOVE_POISON_GAS, MOVE_BODY_SLAM, MOVE_SLUDGE, MOVE_EXPLOSION}
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 46,
    .species = SPECIES_MUK,
    .moves = {MOVE_POISON_GAS, MOVE_DRAIN_PUNCH, MOVE_POISON_JAB, MOVE_EXPLOSION}
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 46,
    .species = SPECIES_QWILFISH,
    .moves = {MOVE_LIQUIDATION, MOVE_POISON_JAB, MOVE_EXPLOSION, MOVE_NONE}
    }
};

static const struct TrainerMon sParty_Jocelyn[] = {
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 15,
    .species = SPECIES_MEDITITE,
    },
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 15,
    .species = SPECIES_HERACROSS,
    }

};

static const struct TrainerMon sParty_Laura[] = {
    {
    .iv = TRAINER_PARTY_IVS(18, 18, 18, 18, 18, 18),
    .lvl = 13,
    .species = SPECIES_MEDITITE,
    }
};

static const struct TrainerMon sParty_Cyndy1[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 27,
    .species = SPECIES_MEDITITE,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 27,
    .species = SPECIES_PRIMEAPE,
    }
};

static const struct TrainerMon sParty_Cora[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 27,
    .species = SPECIES_MEDITITE,
    }
};

static const struct TrainerMon sParty_Paula[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 27,
    .species = SPECIES_BRELOOM,
    }
};

static const struct TrainerMon sParty_Cyndy2[] = {
    {
    .iv = TRAINER_PARTY_IVS(13, 13, 13, 13, 13, 13),
    .lvl = 31,
    .species = SPECIES_MEDITITE,
    },
    {
    .iv = TRAINER_PARTY_IVS(13, 13, 13, 13, 13, 13),
    .lvl = 31,
    .species = SPECIES_MAKUHITA,
    }
};

static const struct TrainerMon sParty_Cyndy3[] = {
    {
    .iv = TRAINER_PARTY_IVS(14, 14, 14, 14, 14, 14),
    .lvl = 34,
    .species = SPECIES_MEDITITE,
    },
    {
    .iv = TRAINER_PARTY_IVS(14, 14, 14, 14, 14, 14),
    .lvl = 35,
    .species = SPECIES_HARIYAMA,
    }
};

static const struct TrainerMon sParty_Cyndy4[] = {
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 39,
    .species = SPECIES_MEDICHAM,
    },
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 39,
    .species = SPECIES_HARIYAMA,
    }
};

static const struct TrainerMon sParty_Cyndy5[] = {
    {
    .iv = TRAINER_PARTY_IVS(17, 17, 17, 17, 17, 17),
    .lvl = 45,
    .species = SPECIES_MEDICHAM,
    },
    {
    .iv = TRAINER_PARTY_IVS(17, 17, 17, 17, 17, 17),
    .lvl = 45,
    .species = SPECIES_HARIYAMA,
    }
};

static const struct TrainerMon sParty_Madeline1[] = {
    {
    .lvl = 25,
    .species = SPECIES_NUMEL,
    .moves = {MOVE_EMBER, MOVE_TACKLE, MOVE_MAGNITUDE, MOVE_SUNNY_DAY}
    }
};

static const struct TrainerMon sParty_Clarissa[] = {
    {
    .lvl = 42,
    .species = SPECIES_ROSERADE,
    },
    {
    .lvl = 43,
    .species = SPECIES_ALCREMIE,
    }
};

static const struct TrainerMon sParty_Angelica[] = {
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 43,
    .species = SPECIES_CASTFORM_NORMAL,
    .moves = {MOVE_RAIN_DANCE, MOVE_WEATHER_BALL, MOVE_THUNDER, MOVE_WATER_PULSE}
    }
};

static const struct TrainerMon sParty_Madeline2[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 34,
    .species = SPECIES_NUMEL,
    .moves = {MOVE_EMBER, MOVE_ROCK_BLAST, MOVE_MAGNITUDE, MOVE_SUNNY_DAY}
    }
};

static const struct TrainerMon sParty_Madeline3[] = {
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 36,
    .species = SPECIES_CAMERUPT,
    .moves = {MOVE_EMBER, MOVE_TAKE_DOWN, MOVE_MAGNITUDE, MOVE_SUNNY_DAY}
    }
};

static const struct TrainerMon sParty_Madeline4[] = {
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 38,
    .species = SPECIES_JUMPLUFF,
    .moves = {MOVE_LEECH_SEED, MOVE_MEGA_DRAIN, MOVE_SLEEP_POWDER, MOVE_SUNNY_DAY}
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 38,
    .species = SPECIES_CAMERUPT,
    .moves = {MOVE_FLAMETHROWER, MOVE_ROCK_SLIDE, MOVE_EARTH_POWER, MOVE_SUNNY_DAY}
    }
};

static const struct TrainerMon sParty_Madeline5[] = {
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 43,
    .species = SPECIES_JUMPLUFF,
    .moves = {MOVE_LEECH_SEED, MOVE_MEGA_DRAIN, MOVE_SLEEP_POWDER, MOVE_SUNNY_DAY}
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 44,
    .species = SPECIES_CAMERUPT,
    .moves = {MOVE_FLAMETHROWER, MOVE_ROCK_SLIDE, MOVE_EARTH_POWER, MOVE_SUNNY_DAY}
    }
};

static const struct TrainerMon sParty_Beverly[] = {
    {
    .lvl = 35,
    .species = SPECIES_PERSIAN,
    },
    {
    .lvl = 36,
    .species = SPECIES_SAMUROTT,
    }
};

static const struct TrainerMon sParty_Imani[] = {
    {
    .lvl = 35,
    .species = SPECIES_AZUMARILL,
    },
    {
    .lvl = 34,
    .species = SPECIES_LICKILICKY,
    }
};

static const struct TrainerMon sParty_Kyla[] = {
    {
    .lvl = 34,
    .species = SPECIES_WAILMER,
    }
};

static const struct TrainerMon sParty_Denise[] = {
    {
    .lvl = 36,
    .species = SPECIES_PIDGEOT,
    },
    {
    .lvl = 35,
    .species = SPECIES_POLIWRATH,
    }
};

static const struct TrainerMon sParty_Beth[] = {
    {
    .lvl = 31,
    .species = SPECIES_GOREBYSS,
    }
};

static const struct TrainerMon sParty_Tara[] = {
    {
    .lvl = 34,
    .species = SPECIES_KINGDRA,
    },
    {
    .lvl = 35,
    .species = SPECIES_AZUMARILL,
    .ability = ABILITY_SAP_SIPPER,
    }
};

static const struct TrainerMon sParty_Missy[] = {
    {
    .lvl = 31,
    .species = SPECIES_SCIZOR,
    },
    {
    .lvl = 31,
    .species = SPECIES_KINGDRA,
    },
};

static const struct TrainerMon sParty_Alice[] = {
    {
    .lvl = 34,
    .species = SPECIES_SHELGON,
    },
    {
    .lvl = 35,
    .species = SPECIES_CRAMORANT,
    },
    {
    .lvl = 36,
    .species = SPECIES_SEISMITOAD,
    }
};

static const struct TrainerMon sParty_Jenny1[] = {
    {
    .lvl = 34,
    .species = SPECIES_WAILMER,
    }
};

static const struct TrainerMon sParty_Grace[] = {
    {
    .lvl = 44,
    .species = SPECIES_AZUMARILL,
    }
};

static const struct TrainerMon sParty_Tanya[] = {
    {
    .lvl = 34,
    .species = SPECIES_LUVDISC,
    }
};

static const struct TrainerMon sParty_Sharon[] = {
    {
    .lvl = 34,
    .species = SPECIES_SEAKING,
    }
};

static const struct TrainerMon sParty_Nikki[] = {
    {
    .lvl = 33,
    .species = SPECIES_MARILL,
    },
    {
    .lvl = 33,
    .species = SPECIES_SPHEAL,
    }
};

static const struct TrainerMon sParty_Brenda[] = {
    {
    .lvl = 34,
    .species = SPECIES_GOLDEEN,
    }
};

static const struct TrainerMon sParty_Katie[] = {
    {
    .lvl = 50,
    .species = SPECIES_SCRAFTY,
    .ability = ABILITY_MOXIE,
    .moves = {MOVE_KNOCK_OFF, MOVE_DRAIN_PUNCH, MOVE_IRON_HEAD, MOVE_DRAGON_DANCE}
    },
    {
    .lvl = 50,
    .species = SPECIES_DECIDUEYE,
    .ability = ABILITY_LONG_REACH,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_SPIRIT_SHACKLE, MOVE_LEAF_BLADE, MOVE_BRAVE_BIRD, MOVE_SWORDS_DANCE}
    }
};

static const struct TrainerMon sParty_Susie[] = {
    {
    .lvl = 54,
    .species = SPECIES_LUVDISC,
    .ability = ABILITY_SOUL_HEART,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_HYDRO_PUMP, MOVE_ATTRACT, MOVE_AGILITY, MOVE_MOONBLAST}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 52,
    .species = SPECIES_GOREBYSS,
    .ability = ABILITY_SWIFT_SWIM,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_SURF, MOVE_ICE_BEAM, MOVE_PSYCHIC, MOVE_AMNESIA}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 52,
    .species = SPECIES_HUNTAIL,
    .ability = ABILITY_SWIFT_SWIM,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_WATERFALL, MOVE_CRUNCH, MOVE_ICE_FANG, MOVE_AGILITY}
    }
};

static const struct TrainerMon sParty_Kara[] = {
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 55,
    .species = SPECIES_SEAKING,
    .ability = ABILITY_SWIFT_SWIM,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_WATERFALL, MOVE_MEGAHORN, MOVE_ICE_BEAM, MOVE_AQUA_RING}
    }
};

static const struct TrainerMon sParty_Dana[] = {
    {
    .lvl = 34,
    .species = SPECIES_AZUMARILL,
    }
};

static const struct TrainerMon sParty_Sienna[] = {
    {
    .lvl = 33,
    .species = SPECIES_LUVDISC,
    },
    {
    .lvl = 33,
    .species = SPECIES_LUVDISC,
    }
};

static const struct TrainerMon sParty_Debra[] = {
    {
    .lvl = 34,
    .species = SPECIES_SEAKING,
    }
};

static const struct TrainerMon sParty_Linda[] = {
    {
    .lvl = 33,
    .species = SPECIES_HORSEA,
    },
    {
    .lvl = 33,
    .species = SPECIES_SEADRA,
    }
};

static const struct TrainerMon sParty_Kaylee[] = {
    {
    .lvl = 34,
    .species = SPECIES_LANTURN,
    },
    {
    .lvl = 34,
    .species = SPECIES_PELIPPER,
    }
};

static const struct TrainerMon sParty_Laurel[] = {
    {
    .lvl = 33,
    .species = SPECIES_LUVDISC,
    },
    {
    .lvl = 33,
    .species = SPECIES_LUVDISC,
    }
};

static const struct TrainerMon sParty_Carlee[] = {
    {
    .lvl = 35,
    .species = SPECIES_SEAKING,
    }
};

static const struct TrainerMon sParty_Jenny2[] = {
    {
    .lvl = 38,
    .species = SPECIES_WAILMER,
    }
};

static const struct TrainerMon sParty_Jenny3[] = {
    {
    .lvl = 41,
    .species = SPECIES_WAILMER,
    }
};

static const struct TrainerMon sParty_Jenny4[] = {
    {
    .lvl = 43,
    .species = SPECIES_STARYU,
    },
    {
    .lvl = 43,
    .species = SPECIES_WAILMER,
    }
};

static const struct TrainerMon sParty_Jenny5[] = {
    {
    .lvl = 45,
    .species = SPECIES_LUVDISC,
    },
    {
    .lvl = 45,
    .species = SPECIES_WAILORD,
    },
    {
    .lvl = 45,
    .species = SPECIES_STARMIE,
    }
};

static const struct TrainerMon sParty_Heidi[] = {
    {
    .lvl = 25,
    .species = SPECIES_SANDSLASH,
    .moves = {MOVE_DIG, MOVE_SAND_ATTACK, MOVE_POISON_STING, MOVE_SLASH}
    },
    {
    .lvl = 27,
    .species = SPECIES_BALTOY,
    .moves = {MOVE_RAPID_SPIN, MOVE_MUD_SLAP, MOVE_PSYBEAM, MOVE_ROCK_TOMB}
    }
};

static const struct TrainerMon sParty_Becky[] = {
    {
    .lvl = 27,
    .species = SPECIES_CLODSIRE,
    .moves = {MOVE_SAND_ATTACK, MOVE_POISON_STING, MOVE_SLASH, MOVE_DIG}
    },
    {
    .lvl = 27,
    .species = SPECIES_DREDNAW,
    .moves = {MOVE_ROLLOUT, MOVE_BUBBLE_BEAM, MOVE_TAIL_WHIP, MOVE_DEFENSE_CURL}
    }
};

static const struct TrainerMon sParty_Carol[] = {
    {
    .lvl = 27,
    .species = SPECIES_NOCTOWL,
    },
    {
    .lvl = 28,
    .species = SPECIES_KINGLER,
    }
};

static const struct TrainerMon sParty_Nancy[] = {
    {
    .lvl = 28,
    .species = SPECIES_MARILL,
    },
    {
    .lvl = 28,
    .species = SPECIES_LOMBRE,
    }
};

static const struct TrainerMon sParty_Martha[] = {
    {
    .lvl = 23,
    .species = SPECIES_DELCATTY,
    },
    {
    .lvl = 23,
    .species = SPECIES_SWABLU,
    }
};

static const struct TrainerMon sParty_Diana1[] = {
    {
    .lvl = 25,
    .species = SPECIES_BRELOOM,
    },
    {
    .lvl = 25,
    .species = SPECIES_PERRSERKER,
    },
    {
    .lvl = 27,
    .species = SPECIES_MORPEKO,
    }
};

static const struct TrainerMon sParty_Cedric[] = {
    {
    .lvl = 100,
    .species = SPECIES_WOBBUFFET,
    .moves = {MOVE_DESTINY_BOND, MOVE_SAFEGUARD, MOVE_COUNTER, MOVE_MIRROR_COAT}
    }
};

static const struct TrainerMon sParty_Irene[] = {
    {
    .lvl = 22,
    .species = SPECIES_BRELOOM,
    },
    {
    .lvl = 23,
    .species = SPECIES_AZUMARILL,
    }
};

static const struct TrainerMon sParty_Diana2[] = {
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 33,
    .species = SPECIES_BRELOOM,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 33,
    .species = SPECIES_PERRSERKER,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 33,
    .species = SPECIES_MORPEKO,
    }
};

static const struct TrainerMon sParty_Diana3[] = {
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 38,
    .species = SPECIES_BRELOOM,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 38,
    .species = SPECIES_PERRSERKER,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 39,
    .species = SPECIES_MORPEKO,
    }
};

static const struct TrainerMon sParty_Diana4[] = {
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 44,
    .species = SPECIES_BRELOOM,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 44,
    .species = SPECIES_PERRSERKER,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 44,
    .species = SPECIES_MORPEKO,
    }
};

static const struct TrainerMon sParty_Diana5[] = {
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 49,
    .species = SPECIES_BRELOOM,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 49,
    .species = SPECIES_PERRSERKER,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 49,
    .species = SPECIES_MORPEKO,
    }
};

static const struct TrainerMon sParty_AmyAndLiv1[] = {
    {
    .lvl = 15,
    .species = SPECIES_PLUSLE,
    },
    {
    .lvl = 15,
    .species = SPECIES_MINUN,
    },
    {
    .lvl = 15,
    .species = SPECIES_PIKACHU,
    },
    {
    .lvl = 15,
    .species = SPECIES_DEDENNE,
    },
    {
    .lvl = 15,
    .species = SPECIES_MORPEKO,
    },
    {
    .lvl = 15,
    .species = SPECIES_PACHIRISU,
    },
};

static const struct TrainerMon sParty_AmyAndLiv2[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 27,
    .species = SPECIES_PLUSLE,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 27,
    .species = SPECIES_MINUN,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 27,
    .species = SPECIES_PIKACHU,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 27,
    .species = SPECIES_DEDENNE,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 27,
    .species = SPECIES_MORPEKO,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 27,
    .species = SPECIES_PACHIRISU,
    },
};

static const struct TrainerMon sParty_GinaAndMia1[] = {
    {
    .lvl = 10,
    .species = SPECIES_GOSSIFLEUR,
    },
    {
    .lvl = 10,
    .species = SPECIES_LOTAD,
    },
    {
    .lvl = 9,
    .species = SPECIES_SEEDOT,
    },
    {
    .lvl = 9,
    .species = SPECIES_CHERUBI,
    }
};

static const struct TrainerMon sParty_MiuAndYuki[] = {
    {
    .lvl = 56,
    .species = SPECIES_MAGMORTAR,
    },
    {
    .lvl = 56,
    .species = SPECIES_ELECTIVIRE,
    },
    {
    .lvl = 56,
    .species = SPECIES_DUSKNOIR,
    },
    {
    .lvl = 56,
    .species = SPECIES_HONCHKROW,
    },
};

static const struct TrainerMon sParty_AmyAndLiv3[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 31,
    .species = SPECIES_PLUSLE,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 31,
    .species = SPECIES_MINUN,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 31,
    .species = SPECIES_PIKACHU,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 31,
    .species = SPECIES_DEDENNE,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 31,
    .species = SPECIES_MORPEKO,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 31,
    .species = SPECIES_PACHIRISU,
    },
};

static const struct TrainerMon sParty_GinaAndMia2[] = {
    {
    .lvl = 15,
    .species = SPECIES_DUSKULL,
    .moves = {MOVE_NIGHT_SHADE, MOVE_DISABLE, MOVE_NONE, MOVE_NONE}
    },
    {
    .lvl = 15,
    .species = SPECIES_SHROOMISH,
    .moves = {MOVE_ABSORB, MOVE_LEECH_SEED, MOVE_HEADBUTT, MOVE_NONE}
    },
    {
    .lvl = 15,
    .species = SPECIES_GOSSIFLEUR,
    .moves = {MOVE_MEGA_DRAIN, MOVE_LEECH_SEED, MOVE_SUNNY_DAY, MOVE_NONE}
    },
    {
    .lvl = 14,
    .species = SPECIES_LOTAD,
    .moves = {MOVE_BUBBLE_BEAM, MOVE_ABSORB, MOVE_GROWL, MOVE_NONE}
    }
};

static const struct TrainerMon sParty_AmyAndLiv4[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 35,
    .species = SPECIES_PLUSLE,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 35,
    .species = SPECIES_MINUN,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 35,
    .species = SPECIES_PIKACHU,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 35,
    .species = SPECIES_DEDENNE,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 35,
    .species = SPECIES_MORPEKO,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 35,
    .species = SPECIES_PACHIRISU,
    }
};

static const struct TrainerMon sParty_AmyAndLiv5[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 41,
    .species = SPECIES_PLUSLE,
    .ability = ABILITY_TRANSISTOR,
    .moves = {MOVE_QUICK_ATTACK, MOVE_THUNDER, MOVE_ENCORE, MOVE_FAKE_TEARS}
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 41,
    .species = SPECIES_MINUN,
    .ability = ABILITY_GALVANIZE,
    .moves = {MOVE_EXTREMESPEED, MOVE_HELPING_HAND, MOVE_ENCORE, MOVE_FAKE_OUT}
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 41,
    .species = SPECIES_PIKACHU,
    .ability = ABILITY_LIGHTNING_ROD,
    .moves = {MOVE_EXTREMESPEED, MOVE_IRON_TAIL, MOVE_THUNDERBOLT, MOVE_FAKE_OUT}
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 41,
    .species = SPECIES_DEDENNE,
    .ability = ABILITY_ELECTRIC_SURGE,
    .moves = {MOVE_DAZZLING_GLEAM, MOVE_PROTECT, MOVE_DISCHARGE, MOVE_HELPING_HAND}
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 41,
    .species = SPECIES_MORPEKO,
    .moves = {MOVE_AURA_WHEEL, MOVE_PROTECT, MOVE_VOLT_SWITCH, MOVE_HELPING_HAND}
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 41,
    .species = SPECIES_PACHIRISU,
    .moves = {MOVE_FOLLOW_ME, MOVE_SUPER_FANG, MOVE_VOLT_SWITCH, MOVE_HELPING_HAND}
    }
};

static const struct TrainerMon sParty_AmyAndLiv6[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 45,
    .species = SPECIES_PLUSLE,
    .ability = ABILITY_TRANSISTOR,
    .moves = {MOVE_VOLT_SWITCH, MOVE_THUNDER, MOVE_ENCORE, MOVE_FAKE_TEARS}
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 45,
    .species = SPECIES_MINUN,
    .ability = ABILITY_GALVANIZE,
    .moves = {MOVE_EXTREMESPEED, MOVE_HELPING_HAND, MOVE_ENCORE, MOVE_FAKE_OUT}
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 45,
    .species = SPECIES_PIKACHU,
    .ability = ABILITY_LIGHTNING_ROD,
    .heldItem = ITEM_LIGHT_BALL,
    .moves = {MOVE_EXTREMESPEED, MOVE_IRON_TAIL, MOVE_THUNDERBOLT, MOVE_FAKE_OUT}
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 45,
    .species = SPECIES_DEDENNE,
    .ability = ABILITY_ELECTRIC_SURGE,
    .moves = {MOVE_DAZZLING_GLEAM, MOVE_PROTECT, MOVE_DISCHARGE, MOVE_HELPING_HAND}
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 46,
    .species = SPECIES_MORPEKO,
    .moves = {MOVE_AURA_WHEEL, MOVE_PROTECT, MOVE_CRUNCH, MOVE_HELPING_HAND}
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 45,
    .species = SPECIES_PACHIRISU,
    .moves = {MOVE_FOLLOW_ME, MOVE_SUPER_FANG, MOVE_VOLT_SWITCH, MOVE_HELPING_HAND}
    }
};

static const struct TrainerMon sParty_Huey[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 12,
    .species = SPECIES_WINGULL,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 12,
    .species = SPECIES_MACHOP,
    }
};

static const struct TrainerMon sParty_Edmond[] = {
    {
    .lvl = 17,
    .species = SPECIES_WINGULL,
    }
};

static const struct TrainerMon sParty_Ernest1[] = {
    {
    .lvl = 33,
    .species = SPECIES_WINGULL,
    },
    {
    .lvl = 33,
    .species = SPECIES_MACHOKE,
    }
};

static const struct TrainerMon sParty_Dwayne[] = {
    {
    .lvl = 11,
    .species = SPECIES_WINGULL,
    },
    {
    .lvl = 11,
    .species = SPECIES_MACHOP,
    },
    {
    .lvl = 11,
    .species = SPECIES_TENTACOOL,
    }
};

static const struct TrainerMon sParty_Phillip[] = {
    {
    .lvl = 74,
    .species = SPECIES_TENTACRUEL,
    },
    {
    .lvl = 74,
    .species = SPECIES_MACHAMP,
    }
};

static const struct TrainerMon sParty_Leonard[] = {
    {
    .lvl = 72,
    .species = SPECIES_SNORLAX,
    .ability = ABILITY_THICK_FAT,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_EXPLOSION, MOVE_BELLY_DRUM, MOVE_EARTHQUAKE, MOVE_CRUNCH}
    },
    {
    .lvl = 72,
    .species = SPECIES_GOURGEIST_SUPER,
    .ability = ABILITY_INSOMNIA,
    .heldItem = ITEM_CUSTAP_BERRY,
    .moves = {MOVE_EXPLOSION, MOVE_POWER_WHIP, MOVE_POLTERGEIST, MOVE_LEECH_SEED}
    },
    {
    .lvl = 72,
    .species = SPECIES_BRONZONG,
    .ability = ABILITY_LEVITATE,
    .heldItem = ITEM_LIFE_ORB,
    .moves = {MOVE_EXPLOSION, MOVE_HEAVY_SLAM, MOVE_ZEN_HEADBUTT, MOVE_EARTHQUAKE}
    },
    {
    .lvl = 72,
    .species = SPECIES_SHIFTRY,
    .ability = ABILITY_WIND_RIDER,
    .heldItem = ITEM_FOCUS_BAND,
    .moves = {MOVE_EXPLOSION, MOVE_LEAF_STORM, MOVE_KNOCK_OFF, MOVE_TAILWIND}
    },
    {
    .lvl = 72,
    .species = SPECIES_GOLEM_ALOLAN,
    .ability = ABILITY_GALVANIZE,
    .heldItem = ITEM_CUSTAP_BERRY,
    .moves = {MOVE_EXPLOSION, MOVE_DOUBLE_EDGE, MOVE_EARTHQUAKE, MOVE_STONE_EDGE}
    },
    {
    .lvl = 74,
    .species = SPECIES_GLALIE,
    .ability = ABILITY_MOODY,
    .heldItem = ITEM_GLALITITE,
    .moves = {MOVE_EXPLOSION, MOVE_DOUBLE_EDGE, MOVE_EARTHQUAKE, MOVE_CRUNCH}
    }

};

static const struct TrainerMon sParty_Duncan[] = {
    {
    .lvl = 35,
    .species = SPECIES_SEALEO,
    },
    {
    .lvl = 35,
    .species = SPECIES_MACHAMP,
    },
    {
    .lvl = 35,
    .species = SPECIES_CONKELDURR,
    },

};

static const struct TrainerMon sParty_Ernest2[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 36,
    .species = SPECIES_WINGULL,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 36,
    .species = SPECIES_TENTACOOL,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 36,
    .species = SPECIES_MACHOKE,
    }
};

static const struct TrainerMon sParty_Ernest3[] = {
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 39,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 39,
    .species = SPECIES_TENTACOOL,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 39,
    .species = SPECIES_MACHOKE,
    }
};

static const struct TrainerMon sParty_Ernest4[] = {
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 42,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 42,
    .species = SPECIES_TENTACOOL,
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 42,
    .species = SPECIES_MACHOKE,
    }
};

static const struct TrainerMon sParty_Ernest5[] = {
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 45,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 45,
    .species = SPECIES_MACHOKE,
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 45,
    .species = SPECIES_TENTACRUEL,
    }
};

static const struct TrainerMon sParty_Eli[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 25,
    .species = SPECIES_NUMEL,
    }
};

static const struct TrainerMon sParty_Annika[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 65,
    .species = SPECIES_MILOTIC,
    .heldItem = ITEM_ORAN_BERRY,
    .moves = {MOVE_RECOVER, MOVE_WATER_PULSE, MOVE_PSYCHIC, MOVE_ATTRACT}
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 65,
    .species = SPECIES_GYARADOS,
    .heldItem = ITEM_ORAN_BERRY,
    .moves = {MOVE_WATERFALL, MOVE_EARTHQUAKE, MOVE_RAIN_DANCE, MOVE_DRAGON_DANCE}
    }
};

static const struct TrainerMon sParty_Jazmyn[] = {
    {
    .lvl = 72,
    .species = SPECIES_SMEARGLE,
    .ability = ABILITY_OWN_TEMPO,
    .heldItem = ITEM_FOCUS_SASH,
    .moves = {MOVE_DARK_VOID, MOVE_SPORE, MOVE_MIND_READER, MOVE_HELPING_HAND}
    },
    {
    .lvl = 72,
    .species = SPECIES_BISHARP,
    .ability = ABILITY_DEFIANT,
    .heldItem = ITEM_LIFE_ORB,
    .moves = {MOVE_KNOCK_OFF, MOVE_IRON_HEAD, MOVE_SUCKER_PUNCH, MOVE_SWORDS_DANCE}
    },
    {
    .lvl = 72,
    .species = SPECIES_ABSOL,
    .heldItem = ITEM_ABSOLITE,
    .moves = {MOVE_PLAY_ROUGH, MOVE_PSYCHO_CUT, MOVE_RAIN_DANCE, MOVE_NIGHT_SLASH}
    },

};

static const struct TrainerMon sParty_Jonas[] = {
    {
    .lvl = 47,
    .species = SPECIES_WEEZING,
    .moves = {MOVE_TOXIC, MOVE_THUNDER, MOVE_EXPLOSION, MOVE_SLUDGE_BOMB}
    },
    {
    .lvl = 48,
    .species = SPECIES_GOLEM,
    .moves = {MOVE_EARTHQUAKE, MOVE_STONE_EDGE, MOVE_EXPLOSION, MOVE_ROCK_POLISH}
    },
    {
    .lvl = 48,
    .species = SPECIES_SMEARGLE,
    .moves = {MOVE_BELLY_DRUM, MOVE_EXTREMESPEED, MOVE_EXPLOSION, MOVE_EARTHQUAKE}
    }
};

static const struct TrainerMon sParty_Kayley[] = {
    {
    .lvl = 48,
    .species = SPECIES_CASTFORM_NORMAL,
    .moves = {MOVE_SUNNY_DAY, MOVE_WEATHER_BALL, MOVE_FLAMETHROWER, MOVE_SOLAR_BEAM}
    }
};

static const struct TrainerMon sParty_Auron[] = {
    {
    .lvl = 38,
    .species = SPECIES_MANECTRIC,
    },
    {
    .lvl = 38,
    .species = SPECIES_MACHAMP,
    }
};

static const struct TrainerMon sParty_Kelvin[] = {
    {
    .iv = TRAINER_PARTY_IVS(18, 18, 18, 18, 18, 18),
    .lvl = 35,
    .species = SPECIES_MACHOKE,
    },
    {
    .iv = TRAINER_PARTY_IVS(18, 18, 18, 18, 18, 18),
    .lvl = 36,
    .species = SPECIES_WEAVILE,
    }
};

static const struct TrainerMon sParty_Marley[] = {
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 36,
    .species = SPECIES_MANECTRIC,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_BITE, MOVE_ROAR, MOVE_THUNDER_WAVE, MOVE_THUNDERBOLT}
    }
};

static const struct TrainerMon sParty_Reyna[] = {
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 36,
    .species = SPECIES_MEDITITE,
    },
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 36,
    .species = SPECIES_HARIYAMA,
    }
};

static const struct TrainerMon sParty_Hudson[] = {
    {
    .lvl = 34,
    .species = SPECIES_WAILMER,
    }
};

static const struct TrainerMon sParty_Conor[] = {
    {
    .lvl = 33,
    .species = SPECIES_LANTURN,
    },
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 33,
    .species = SPECIES_HARIYAMA,
    }
};

static const struct TrainerMon sParty_Edwin1[] = {
    {
    .lvl = 19,
    .species = SPECIES_LOMBRE,
    },
    {
    .lvl = 18,
    .species = SPECIES_NUZLEAF,
    }
};

static const struct TrainerMon sParty_Hector[] = {
    {
    .lvl = 28,
    .species = SPECIES_ZANGOOSE,
    },
    {
    .lvl = 28,
    .species = SPECIES_SEVIPER,
    }
};

static const struct TrainerMon sParty_TabithaMossdeep[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 60,
    .species = SPECIES_CAMERUPT,
    .heldItem = ITEM_PASSHO_BERRY,
    .ability = ABILITY_SOLID_ROCK,
    .moves = {MOVE_EARTH_POWER, MOVE_HEAT_WAVE, MOVE_PROTECT, MOVE_SOLAR_BEAM}
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 61,
    .species = SPECIES_MAWILE,
    .heldItem = ITEM_MAWILITE,
    .moves = {MOVE_FAKE_OUT, MOVE_PLAY_ROUGH, MOVE_SUCKER_PUNCH, MOVE_IRON_HEAD}
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 60,
    .species = SPECIES_BELLOSSOM,
    .ability = ABILITY_CHLOROPHYLL,
    .heldItem = ITEM_GRASS_GEM,
    .moves = {MOVE_FIERY_DANCE, MOVE_MOONBLAST, MOVE_QUIVER_DANCE, MOVE_GIGA_DRAIN}
    }
};

static const struct TrainerMon sParty_Edwin2[] = {
    {
    .lvl = 26,
    .species = SPECIES_LOMBRE,
    },
    {
    .lvl = 26,
    .species = SPECIES_NUZLEAF,
    }
};

static const struct TrainerMon sParty_Edwin3[] = {
    {
    .lvl = 29,
    .species = SPECIES_LOMBRE,
    },
    {
    .lvl = 29,
    .species = SPECIES_NUZLEAF,
    }
};

static const struct TrainerMon sParty_Edwin4[] = {
    {
    .lvl = 32,
    .species = SPECIES_LOMBRE,
    },
    {
    .lvl = 32,
    .species = SPECIES_NUZLEAF,
    }
};

static const struct TrainerMon sParty_Edwin5[] = {
    {
    .lvl = 35,
    .species = SPECIES_LUDICOLO,
    },
    {
    .lvl = 35,
    .species = SPECIES_SHIFTRY,
    }
};

static const struct TrainerMon sParty_WallyVR1[] = {
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 75,
    .species = SPECIES_ROSERADE,
    .heldItem = ITEM_FOCUS_SASH,
    .ability = ABILITY_POISON_POINT,
    .moves = {MOVE_TOXIC_SPIKES, MOVE_SPIKES, MOVE_LEAF_STORM, MOVE_SLUDGE_BOMB}
    },
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 75,
    .species = SPECIES_TALONFLAME,
    .heldItem = ITEM_CHOICE_BAND,
    .ability = ABILITY_GALE_WINGS,
    .moves = {MOVE_BRAVE_BIRD, MOVE_FLARE_BLITZ, MOVE_U_TURN, MOVE_NONE}
    },
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 75,
    .species = SPECIES_AZUMARILL,
    .heldItem = ITEM_SITRUS_BERRY,
    .ability = ABILITY_HUGE_POWER,
    .moves = {MOVE_BELLY_DRUM, MOVE_AQUA_JET, MOVE_PLAY_ROUGH, MOVE_IRON_TAIL}
    },
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 76,
    .species = SPECIES_MAGNEZONE,
    .heldItem = ITEM_AIR_BALLOON,
    .ability = ABILITY_MAGNET_PULL,
    .moves = {MOVE_ZAP_CANNON, MOVE_LOCK_ON, MOVE_FLASH_CANNON, MOVE_BODY_PRESS}
    },
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 76,
    .species = SPECIES_GARCHOMP,
    .heldItem = ITEM_GROUND_GEM,
    .ability = ABILITY_SAND_VEIL,
    .moves = {MOVE_EARTHQUAKE, MOVE_DRAGON_RUSH, MOVE_SWORDS_DANCE, MOVE_FIRE_FANG}
    },
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 77,
    .species = SPECIES_GALLADE,
    .heldItem = ITEM_GALLADITE,
    .ability = ABILITY_SHARPNESS,
    .moves = {MOVE_SWORDS_DANCE, MOVE_AGILITY, MOVE_PSYCHO_CUT, MOVE_SACRED_SWORD}
    }
};

static const struct TrainerMon sParty_BrendanRoute103Mudkip[] = {
    {
    .lvl = 5,
    .species = SPECIES_TREECKO,
    .moves = {MOVE_POUND, MOVE_LEER, MOVE_NONE, MOVE_NONE}
    }
};

static const struct TrainerMon sParty_BrendanRoute110Mudkip[] = {
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 19,
    .species = SPECIES_TAILLOW,
    },
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 19,
    .species = SPECIES_LITWICK,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 21,
    .species = SPECIES_GROVYLE,
    },
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 19,
    .species = SPECIES_POLIWAG,
    },
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 20,
    .species = SPECIES_ARON,
    }
};

static const struct TrainerMon sParty_BrendanRoute119Mudkip[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 40,
    .species = SPECIES_SWELLOW,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 40,
    .species = SPECIES_LAMPENT,
    },
    {
    .iv = TRAINER_PARTY_IVS(18, 18, 18, 18, 18, 18),
    .lvl = 40,
    .species = SPECIES_SCEPTILE,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 40,
    .species = SPECIES_POLIWRATH,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 40,
    .species = SPECIES_LAIRON,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 40,
    .species = SPECIES_FRAXURE,
    }
};

static const struct TrainerMon sParty_BrendanRoute103Treecko[] = {
    {
    .lvl = 5,
    .species = SPECIES_TORCHIC,
    .moves = {MOVE_SCRATCH, MOVE_GROWL, MOVE_NONE, MOVE_NONE}
    }
};

static const struct TrainerMon sParty_BrendanRoute110Treecko[] = {
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 19,
    .species = SPECIES_TAILLOW,
    },
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 19,
    .species = SPECIES_TANGELA,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 21,
    .species = SPECIES_COMBUSKEN,
    },
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 19,
    .species = SPECIES_POLIWAG,
    },
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 20,
    .species = SPECIES_ARON,
    }
};

static const struct TrainerMon sParty_BrendanRoute119Treecko[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 40,
    .species = SPECIES_SWELLOW,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 40,
    .species = SPECIES_TANGROWTH,
    },
    {
    .iv = TRAINER_PARTY_IVS(18, 18, 18, 18, 18, 18),
    .lvl = 40,
    .species = SPECIES_BLAZIKEN,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 40,
    .species = SPECIES_POLIWRATH,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 40,
    .species = SPECIES_LAIRON,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 40,
    .species = SPECIES_FRAXURE,
    }
};

static const struct TrainerMon sParty_BrendanRoute103Torchic[] = {
    {
    .lvl = 5,
    .species = SPECIES_MUDKIP,
    .moves = {MOVE_TACKLE, MOVE_GROWL, MOVE_NONE, MOVE_NONE}
    }
};

static const struct TrainerMon sParty_BrendanRoute110Torchic[] = {
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 19,
    .species = SPECIES_TAILLOW,
    },
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 19,
    .species = SPECIES_TANGELA,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 21,
    .species = SPECIES_MARSHTOMP,
    },
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 19,
    .species = SPECIES_LITWICK,
    },
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 20,
    .species = SPECIES_ARON,
    }
};

static const struct TrainerMon sParty_BrendanRoute119Torchic[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 40,
    .species = SPECIES_SWELLOW,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 40,
    .species = SPECIES_TANGROWTH,
    },
    {
    .iv = TRAINER_PARTY_IVS(18, 18, 18, 18, 18, 18),
    .lvl = 40,
    .species = SPECIES_SWAMPERT,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 40,
    .species = SPECIES_LAMPENT,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 40,
    .species = SPECIES_LAIRON,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 40,
    .species = SPECIES_FRAXURE,
    }
};

static const struct TrainerMon sParty_MayRoute103Mudkip[] = {
    {
    .lvl = 5,
    .species = SPECIES_TREECKO,
    .moves = {MOVE_POUND, MOVE_LEER, MOVE_NONE, MOVE_NONE}
    }
};

static const struct TrainerMon sParty_MayRoute110Mudkip[] = {
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 19,
    .species = SPECIES_TAILLOW,
    },
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 19,
    .species = SPECIES_LITWICK,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 21,
    .species = SPECIES_GROVYLE,
    },
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 19,
    .species = SPECIES_POLIWAG,
    },
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 20,
    .species = SPECIES_ARON,
    }
};

static const struct TrainerMon sParty_MayRoute119Mudkip[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 40,
    .species = SPECIES_SWELLOW,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 42,
    .species = SPECIES_LAMPENT,
    },
    {
    .iv = TRAINER_PARTY_IVS(18, 18, 18, 18, 18, 18),
    .lvl = 42,
    .species = SPECIES_SCEPTILE,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 42,
    .species = SPECIES_POLIWRATH,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 40,
    .species = SPECIES_LAIRON,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 40,
    .species = SPECIES_FRAXURE,
    }
};

static const struct TrainerMon sParty_MayRoute103Treecko[] = {
    {
    .lvl = 5,
    .species = SPECIES_TORCHIC,
    .moves = {MOVE_SCRATCH, MOVE_GROWL, MOVE_NONE, MOVE_NONE}
    }
};

static const struct TrainerMon sParty_MayRoute110Treecko[] = {
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 19,
    .species = SPECIES_TAILLOW,
    },
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 19,
    .species = SPECIES_TANGELA,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 21,
    .species = SPECIES_COMBUSKEN,
    },
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 19,
    .species = SPECIES_POLIWAG,
    },
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 19,
    .species = SPECIES_ARON,
    }
};

static const struct TrainerMon sParty_MayRoute119Treecko[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 40,
    .species = SPECIES_SWELLOW,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 42,
    .species = SPECIES_TANGROWTH,
    },
    {
    .iv = TRAINER_PARTY_IVS(18, 18, 18, 18, 18, 18),
    .lvl = 42,
    .species = SPECIES_BLAZIKEN,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 42,
    .species = SPECIES_POLIWRATH,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 40,
    .species = SPECIES_LAIRON,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 40,
    .species = SPECIES_FRAXURE,
    }
};

static const struct TrainerMon sParty_MayRoute103Torchic[] = {
    {
    .lvl = 5,
    .species = SPECIES_MUDKIP,
    .moves = {MOVE_TACKLE, MOVE_GROWL, MOVE_NONE, MOVE_NONE}
    }
};

static const struct TrainerMon sParty_MayRoute110Torchic[] = {
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 19,
    .species = SPECIES_TAILLOW,
    },
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 19,
    .species = SPECIES_TANGELA,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 21,
    .species = SPECIES_MARSHTOMP,
    },
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 19,
    .species = SPECIES_LITWICK,
    },
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 20,
    .species = SPECIES_ARON,
    }
};

static const struct TrainerMon sParty_MayRoute119Torchic[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 40,
    .species = SPECIES_SWELLOW,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 42,
    .species = SPECIES_TANGROWTH,
    },
    {
    .iv = TRAINER_PARTY_IVS(18, 18, 18, 18, 18, 18),
    .lvl = 42,
    .species = SPECIES_SWAMPERT,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 42,
    .species = SPECIES_LAMPENT,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 40,
    .species = SPECIES_LAIRON,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 40,
    .species = SPECIES_FRAXURE,
    }
};

static const struct TrainerMon sParty_Isaac1[] = {
    {
    .lvl = 18,
    .species = SPECIES_LOUDRED,
    },
    {
    .lvl = 18,
    .species = SPECIES_AUDINO,
    },
    {
    .lvl = 18,
    .species = SPECIES_ARON,
    },
    {
    .lvl = 18,
    .species = SPECIES_MIGHTYENA,
    },
    {
    .lvl = 18,
    .species = SPECIES_TAILLOW,
    },
    {
    .lvl = 18,
    .species = SPECIES_MAKUHITA,
    }
};

static const struct TrainerMon sParty_Davis[] = {
    {
    .lvl = 71,
    .species = SPECIES_ESCAVALIER,
    .ability = ABILITY_SHELL_ARMOR,
    .moves = {MOVE_IRON_HEAD, MOVE_MEGAHORN, MOVE_KNOCK_OFF, MOVE_POLLEN_PUFF}
    },
    {
    .lvl = 72,
    .species = SPECIES_VOLCARONA,
    .ability = ABILITY_FLAME_BODY,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_QUIVER_DANCE, MOVE_FIERY_DANCE, MOVE_GIGA_DRAIN, MOVE_ROOST}
    },
    {
    .lvl = 72,
    .species = SPECIES_GALVANTULA,
    .ability = ABILITY_COMPOUND_EYES,
    .moves = {MOVE_BUG_BUZZ, MOVE_THUNDER, MOVE_VOLT_SWITCH, MOVE_ENERGY_BALL}
    }

};

static const struct TrainerMon sParty_Mitchell[] = {
    {
    .lvl = 71,
    .species = SPECIES_LUNATONE,
    .moves = {MOVE_EXPLOSION, MOVE_REFLECT, MOVE_LIGHT_SCREEN, MOVE_PSYCHIC}
    },
    {
    .lvl = 72,
    .species = SPECIES_SOLROCK,
    .moves = {MOVE_EXPLOSION, MOVE_REFLECT, MOVE_LIGHT_SCREEN, MOVE_SHADOW_BALL}
    }
};

static const struct TrainerMon sParty_Isaac2[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 31,
    .species = SPECIES_LOUDRED,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 31,
    .species = SPECIES_AUDINO,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 32,
    .species = SPECIES_LAIRON,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 32,
    .species = SPECIES_MIGHTYENA,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 32,
    .species = SPECIES_SWELLOW,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 32,
    .species = SPECIES_HARIYAMA,
    }
};

static const struct TrainerMon sParty_Isaac3[] = {
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 36,
    .species = SPECIES_LOUDRED,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 36,
    .species = SPECIES_AUDINO,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 36,
    .species = SPECIES_LAIRON,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 36,
    .species = SPECIES_MIGHTYENA,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 36,
    .species = SPECIES_SWELLOW,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 36,
    .species = SPECIES_HARIYAMA,
    }
};

static const struct TrainerMon sParty_Isaac4[] = {
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 41,
    .species = SPECIES_EXPLOUD,
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 42,
    .species = SPECIES_AUDINO,
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 42,
    .species = SPECIES_LAIRON,
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 42,
    .species = SPECIES_MIGHTYENA,
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 42,
    .species = SPECIES_SWELLOW,
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 42,
    .species = SPECIES_HARIYAMA,
    }
};

static const struct TrainerMon sParty_Isaac5[] = {
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 48,
    .species = SPECIES_LOUDRED,
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 51,
    .species = SPECIES_AUDINO,
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 49,
    .species = SPECIES_AGGRON,
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 49,
    .species = SPECIES_MIGHTYENA,
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 49,
    .species = SPECIES_SWELLOW,
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 48,
    .species = SPECIES_HARIYAMA,
    }
};

static const struct TrainerMon sParty_Lydia1[] = {
    {
    .lvl = 17,
    .species = SPECIES_WINGULL,
    },
    {
    .lvl = 17,
    .species = SPECIES_SHROOMISH,
    },
    {
    .lvl = 18,
    .species = SPECIES_MARILL,
    },
    {
    .lvl = 18,
    .species = SPECIES_ROSELIA,
    },
    {
    .lvl = 18,
    .species = SPECIES_CHANSEY,
    },
    {
    .lvl = 17,
    .species = SPECIES_GOLDEEN,
    }
};

static const struct TrainerMon sParty_Halle[] = {
    {
    .lvl = 73,
    .species = SPECIES_SABLEYE,
    .heldItem = ITEM_SABLENITE
    },
    {
    .lvl = 72,
    .species = SPECIES_ABSOL,
    }
};

static const struct TrainerMon sParty_Garrison[] = {
    {
    .lvl = 36,
    .species = SPECIES_SANDSLASH,
    },
    {
    .lvl = 36,
    .species = SPECIES_GIGALITH,
    }
};

static const struct TrainerMon sParty_Lydia2[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 31,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 31,
    .species = SPECIES_BRELOOM,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 31,
    .species = SPECIES_AZUMARILL,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 31,
    .species = SPECIES_ROSELIA,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 32,
    .species = SPECIES_CHANSEY,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 31,
    .species = SPECIES_GOLDEEN,
    }
};

static const struct TrainerMon sParty_Lydia3[] = {
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 36,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 36,
    .species = SPECIES_BRELOOM,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 36,
    .species = SPECIES_AZUMARILL,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 36,
    .species = SPECIES_ROSERADE,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 36,
    .species = SPECIES_BLISSEY,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 38,
    .species = SPECIES_SEAKING,
    }
};

static const struct TrainerMon sParty_Lydia4[] = {
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 41,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 41,
    .species = SPECIES_BRELOOM,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 42,
    .species = SPECIES_AZUMARILL,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 42,
    .species = SPECIES_ROSERADE,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 42,
    .species = SPECIES_BLISSEY,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 41,
    .species = SPECIES_SEAKING,
    }
};

static const struct TrainerMon sParty_Lydia5[] = {
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 48,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 48,
    .species = SPECIES_BRELOOM,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 48,
    .species = SPECIES_AZUMARILL,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 48,
    .species = SPECIES_ROSERADE,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 50,
    .species = SPECIES_BLISSEY,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 48,
    .species = SPECIES_SEAKING,
    }
};

static const struct TrainerMon sParty_Jackson1[] = {
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 41,
    .species = SPECIES_BRELOOM,
    }
};

static const struct TrainerMon sParty_Lorenzo[] = {
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 44,
    .species = SPECIES_ARIADOS,
    },
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 45,
    .species = SPECIES_SHIINOTIC,
    },
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 44,
    .species = SPECIES_AERODACTYL,
    }
};

static const struct TrainerMon sParty_Sebastian[] = {
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 39,
    .species = SPECIES_CACTURNE,
    }
};

static const struct TrainerMon sParty_Jackson2[] = {
    {
    .iv = TRAINER_PARTY_IVS(7, 7, 7, 7, 7, 7),
    .lvl = 44,
    .species = SPECIES_BRELOOM,
    }
};

static const struct TrainerMon sParty_Jackson3[] = {
    {
    .iv = TRAINER_PARTY_IVS(8, 8, 8, 8, 8, 8),
    .lvl = 48,
    .species = SPECIES_BRELOOM,
    }
};

static const struct TrainerMon sParty_Jackson4[] = {
    {
    .iv = TRAINER_PARTY_IVS(9, 9, 9, 9, 9, 9),
    .lvl = 51,
    .species = SPECIES_BRELOOM,
    }
};

static const struct TrainerMon sParty_Jackson5[] = {
    {
    .iv = TRAINER_PARTY_IVS(10, 10, 10, 10, 10, 10),
    .lvl = 54,
    .species = SPECIES_KECLEON,
    },
    {
    .iv = TRAINER_PARTY_IVS(10, 10, 10, 10, 10, 10),
    .lvl = 54,
    .species = SPECIES_BRELOOM,
    }
};

static const struct TrainerMon sParty_Catherine1[] = {
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 35,
    .species = SPECIES_VILEPLUME,
    },
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 35,
    .species = SPECIES_VICTREEBEL,
    }
};

static const struct TrainerMon sParty_Jenna[] = {
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 44,
    .species = SPECIES_GOURGEIST,
    },
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 43,
    .species = SPECIES_LUDICOLO,
    },
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 43,
    .species = SPECIES_SHIFTRY,
    }
};

static const struct TrainerMon sParty_Sophia[] = {
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 38,
    .species = SPECIES_ALTARIA,
    },
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 38,
    .species = SPECIES_ROSELIA,
    }
};

static const struct TrainerMon sParty_Catherine2[] = {
    {
    .iv = TRAINER_PARTY_IVS(7, 7, 7, 7, 7, 7),
    .lvl = 36,
    .species = SPECIES_VILEPLUME,
    },
    {
    .iv = TRAINER_PARTY_IVS(7, 7, 7, 7, 7, 7),
    .lvl = 36,
    .species = SPECIES_VICTREEBEL,
    }
};

static const struct TrainerMon sParty_Catherine3[] = {
    {
    .iv = TRAINER_PARTY_IVS(8, 8, 8, 8, 8, 8),
    .lvl = 41,
    .species = SPECIES_VILEPLUME,
    },
    {
    .iv = TRAINER_PARTY_IVS(8, 8, 8, 8, 8, 8),
    .lvl = 41,
    .species = SPECIES_VICTREEBEL,
    }
};

static const struct TrainerMon sParty_Catherine4[] = {
    {
    .iv = TRAINER_PARTY_IVS(9, 9, 9, 9, 9, 9),
    .lvl = 45,
    .species = SPECIES_VILEPLUME,
    },
    {
    .iv = TRAINER_PARTY_IVS(9, 9, 9, 9, 9, 9),
    .lvl = 46,
    .species = SPECIES_VICTREEBEL,
    }
};

static const struct TrainerMon sParty_Catherine5[] = {
    {
    .iv = TRAINER_PARTY_IVS(10, 10, 10, 10, 10, 10),
    .lvl = 55,
    .species = SPECIES_VILEPLUME,
    },
    {
    .iv = TRAINER_PARTY_IVS(10, 10, 10, 10, 10, 10),
    .lvl = 55,
    .species = SPECIES_VICTREEBEL,
    }
};

static const struct TrainerMon sParty_Julio[] = {
    {
    .lvl = 27,
    .species = SPECIES_EELEKTRIK,
    },
    {
    .lvl = 27,
    .species = SPECIES_ROTOM,
    }
};

static const struct TrainerMon sParty_GruntSeafloorCavern5[] = {
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 60,
    .species = SPECIES_MIGHTYENA,
    .ability = ABILITY_INTIMIDATE,
    .heldItem = ITEM_BLACK_GLASSES,
    .moves = {MOVE_CRUNCH, MOVE_PLAY_ROUGH, MOVE_SUCKER_PUNCH, MOVE_IRON_TAIL}
    },
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 60,
    .species = SPECIES_DRAPION,
    .ability = ABILITY_BATTLE_ARMOR,
    .heldItem = ITEM_BLACK_SLUDGE,
    .moves = {MOVE_POISON_JAB, MOVE_KNOCK_OFF, MOVE_EARTHQUAKE, MOVE_TOXIC_SPIKES}
    }
};

static const struct TrainerMon sParty_GruntUnused[] = {
    {
    .lvl = 31,
    .species = SPECIES_WAILMER,
    },
    {
    .lvl = 31,
    .species = SPECIES_ZUBAT,
    }
};

static const struct TrainerMon sParty_GruntMtPyre4[] = {
    {
    .lvl = 51,
    .species = SPECIES_WAILORD,
    .moves = {MOVE_CURSE, MOVE_WATERFALL, MOVE_HEAVY_SLAM, MOVE_BODY_SLAM}
    },
    {
    .lvl = 52,
    .species = SPECIES_HUNTAIL,
    .moves = {MOVE_SHELL_SMASH, MOVE_WATERFALL, MOVE_ICE_FANG, MOVE_CRUNCH}
    },
    {
    .lvl = 53,
    .species = SPECIES_POLIWRATH,
    .moves = {MOVE_BELLY_DRUM, MOVE_WATERFALL, MOVE_ICE_PUNCH, MOVE_STORM_THROW}
    }
};

static const struct TrainerMon sParty_GruntJaggedPass[] = {
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 28,
    .species = SPECIES_DRILBUR,
    },
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 28,
    .species = SPECIES_NUMEL,
    }
};

static const struct TrainerMon sParty_Marc[] = {
    {
    .iv = TRAINER_PARTY_IVS(14, 14, 14, 14, 14, 14),
    .lvl = 12,
    .species = SPECIES_GEODUDE,
    },
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 11,
    .species = SPECIES_ARON,
    }
};

static const struct TrainerMon sParty_Brenden[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 13,
    .species = SPECIES_MACHOP,
    }
};

static const struct TrainerMon sParty_Lilith[] = {
    {
    .iv = TRAINER_PARTY_IVS(18, 18, 18, 18, 18, 18),
    .lvl = 16,
    .species = SPECIES_MEDITITE,
    },
    {
    .iv = TRAINER_PARTY_IVS(18, 18, 18, 18, 18, 18),
    .lvl = 16,
    .species = SPECIES_CROAGUNK,
    },
    {
    .iv = TRAINER_PARTY_IVS(18, 18, 18, 18, 18, 18),
    .lvl = 17,
    .species = SPECIES_SAWK,
    }
};

static const struct TrainerMon sParty_Cristian[] = {
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 15,
    .species = SPECIES_MAKUHITA,
    },
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 16,
    .species = SPECIES_TIMBURR,
    }
};

static const struct TrainerMon sParty_Sylvia[] = {
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 57,
    .species = SPECIES_AZUMARILL,
    .ability = ABILITY_HUGE_POWER,
    .moves = {MOVE_AQUA_JET, MOVE_PLAY_ROUGH, MOVE_KNOCK_OFF, MOVE_PROTECT}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 56,
    .species = SPECIES_MAROWAK_ALOLAN,
    .ability = ABILITY_LIGHTNING_ROD,
    .heldItem = ITEM_THICK_CLUB,
    .moves = {MOVE_SHADOW_BONE, MOVE_FLARE_BLITZ, MOVE_BONEMERANG, MOVE_PROTECT}
    }
};

static const struct TrainerMon sParty_Leonardo[] = {
    {
    .lvl = 51,
    .species = SPECIES_SHARPEDO,
    }
};

static const struct TrainerMon sParty_Athena[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 42,
    .species = SPECIES_MANECTRIC,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_THUNDER, MOVE_THUNDER_WAVE, MOVE_QUICK_ATTACK, MOVE_NONE}
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 42,
    .species = SPECIES_LINOONE,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_SURF, MOVE_THIEF, MOVE_NONE, MOVE_NONE}
    }
};

static const struct TrainerMon sParty_Harrison[] = {
    {
    .lvl = 49,
    .species = SPECIES_TENTACRUEL,
    }
};

static const struct TrainerMon sParty_GruntMtChimney2[] = {
    {
    .lvl = 27,
    .species = SPECIES_TURTONATOR,
    }
};

static const struct TrainerMon sParty_Clarence[] = {
    {
    .lvl = 51,
    .species = SPECIES_SHARPEDO,
    },
    {
    .lvl = 51,
    .species = SPECIES_GASTRODON,
    .ability = ABILITY_STORM_DRAIN,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_SCALD, MOVE_EARTH_POWER, MOVE_RECOVER, MOVE_CLEAR_SMOG}
    }
};

static const struct TrainerMon sParty_Terry[] = {
    {
    .lvl = 37,
    .species = SPECIES_GIRAFARIG,
    }
};

static const struct TrainerMon sParty_Nate[] = {
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 56,
    .species = SPECIES_GARDEVOIR,
    .ability = ABILITY_TRACE,
    .moves = {MOVE_MOONBLAST, MOVE_PSYCHIC, MOVE_SHADOW_BALL, MOVE_PROTECT}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 57,
    .species = SPECIES_WORMADAM_PLANT_CLOAK,
    .ability = ABILITY_ANTICIPATION,
    .heldItem = ITEM_BIG_ROOT,
    .moves = {MOVE_LEAF_STORM, MOVE_BUG_BUZZ, MOVE_PROTECT, MOVE_SYNTHESIS}
    }
};

static const struct TrainerMon sParty_Kathleen[] = {
    {
    .iv = TRAINER_PARTY_IVS(31, 0, 31, 31, 31, 0),
    .lvl = 55,
    .species = SPECIES_REUNICLUS,
    .ability = ABILITY_MAGIC_GUARD,
    .moves = {MOVE_PSYCHIC, MOVE_FOCUS_BLAST, MOVE_TRICK_ROOM, MOVE_RECOVER}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 0, 31, 31, 31, 0),
    .lvl = 56,
    .species = SPECIES_ESPEON,
    .ability = ABILITY_MAGIC_BOUNCE,
    .moves = {MOVE_PSYCHIC, MOVE_DAZZLING_GLEAM, MOVE_SHADOW_BALL, MOVE_PROTECT}
    }
};

static const struct TrainerMon sParty_Clifford[] = {
    {
    .lvl = 53,
    .species = SPECIES_ELECTRODE,
    .moves = {MOVE_EXPLOSION, MOVE_THUNDERBOLT, MOVE_NONE, MOVE_NONE}
    },
    {
    .lvl = 53,
    .species = SPECIES_GENGAR,
    .moves = {MOVE_EXPLOSION, MOVE_SHADOW_BALL, MOVE_SLUDGE_BOMB, MOVE_NONE}
    },
    {
    .lvl = 44,
    .species = SPECIES_GOLEM_ALOLAN,
    .ability = ABILITY_GALVANIZE,
    .moves = {MOVE_EXPLOSION, MOVE_ROCK_SLIDE, MOVE_SUPERPOWER, MOVE_NONE}
    }
};

static const struct TrainerMon sParty_Nicholas[] = {
    {
    .lvl = 52,
    .species = SPECIES_WOBBUFFET,
    .ability = ABILITY_SHADOW_TAG,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_COUNTER, MOVE_MIRROR_COAT, MOVE_DESTINY_BOND, MOVE_ENCORE}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 0, 31, 31, 31, 0),
    .lvl = 51,
    .species = SPECIES_GOTHITELLE,
    .ability = ABILITY_SHADOW_TAG,
    .moves = {MOVE_PSYCHIC, MOVE_SHADOW_BALL, MOVE_TRICK_ROOM, MOVE_PROTECT}
    }
};

static const struct TrainerMon sParty_GruntSpaceCenter3[] = {
    {
    .lvl = 60,
    .species = SPECIES_MACHAMP,
    .ability = ABILITY_NO_GUARD,
    .heldItem = ITEM_CHOICE_BAND,
    .moves = {MOVE_DYNAMIC_PUNCH, MOVE_KNOCK_OFF, MOVE_STONE_EDGE, MOVE_BULLET_PUNCH}
    },
    {
    .lvl = 60,
    .species = SPECIES_AMOONGUSS,
    .ability = ABILITY_REGENERATOR,
    .moves = {MOVE_SPORE, MOVE_GIGA_DRAIN, MOVE_SLUDGE_BOMB, MOVE_SYNTHESIS}
    }
};

static const struct TrainerMon sParty_GruntSpaceCenter4[] = {
    {
    .lvl = 58,
    .species = SPECIES_KROOKODILE,
    .ability = ABILITY_MOLD_BREAKER,
    .heldItem = ITEM_CHOICE_SCARF,
    .moves = {MOVE_EARTHQUAKE, MOVE_CRUNCH, MOVE_IRON_HEAD, MOVE_FIRE_FANG}
    },
    {
    .lvl = 58,
    .species = SPECIES_SPIRITOMB,
    .ability = ABILITY_INFILTRATOR,
    .moves = {MOVE_SHADOW_SNEAK, MOVE_WILL_O_WISP, MOVE_SUCKER_PUNCH, MOVE_PAIN_SPLIT}
    }
};

static const struct TrainerMon sParty_GruntSpaceCenter5[] = {
    {
    .lvl = 60,
    .species = SPECIES_ALAKAZAM,
    .ability = ABILITY_MAGIC_GUARD,
    .moves = {MOVE_PSYCHIC, MOVE_SHADOW_BALL, MOVE_FOCUS_BLAST, MOVE_CALM_MIND}
    },
    {
    .lvl = 58,
    .species = SPECIES_TOXICROAK,
    .ability = ABILITY_POISON_TOUCH,
    .heldItem = ITEM_BLACK_SLUDGE,
    .moves = {MOVE_POISON_JAB, MOVE_DRAIN_PUNCH, MOVE_SUCKER_PUNCH, MOVE_FAKE_OUT}
    },
    {
    .lvl = 60,
    .species = SPECIES_MAGMAR,
    .ability = ABILITY_FLAME_BODY,
    .moves = {MOVE_FLAMETHROWER, MOVE_THUNDER_PUNCH, MOVE_FOCUS_BLAST, MOVE_WILL_O_WISP}
    }
};

static const struct TrainerMon sParty_GruntSpaceCenter6[] = {
    {
    .lvl = 60,
    .species = SPECIES_GOLEM,
    .ability = ABILITY_STURDY,
    .moves = {MOVE_STONE_EDGE, MOVE_EARTHQUAKE, MOVE_EXPLOSION, MOVE_PROTECT}
    },
    {
    .lvl = 58,
    .species = SPECIES_MUK_ALOLAN,
    .moves = {MOVE_SHADOW_SNEAK, MOVE_FIRE_PUNCH, MOVE_EXPLOSION, MOVE_PROTECT}
    },
    {
    .lvl = 60,
    .species = SPECIES_TORKOAL,
    .ability = ABILITY_DROUGHT,
    .moves = {MOVE_ERUPTION, MOVE_SOLAR_BEAM, MOVE_YAWN, MOVE_SHELL_SMASH}
    }
};

static const struct TrainerMon sParty_GruntSpaceCenter7[] = {
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 60,
    .species = SPECIES_CLEFABLE,
    .ability = ABILITY_MAGIC_GUARD,
    .moves = {MOVE_MOONBLAST, MOVE_SOFT_BOILED, MOVE_CALM_MIND, MOVE_FLAMETHROWER}
    },
    {
    .lvl = 60,
    .species = SPECIES_CLAYDOL,
    .ability = ABILITY_LEVITATE,
    .moves = {MOVE_EARTH_POWER, MOVE_PSYCHIC, MOVE_RAPID_SPIN, MOVE_CALM_MIND}
    },
    {
    .lvl = 60,
    .species = SPECIES_STARMIE,
    .ability = ABILITY_ILLUMINATE,
    .moves = {MOVE_SURF, MOVE_PSYCHIC, MOVE_RAPID_SPIN, MOVE_RECOVER}
    }
};

static const struct TrainerMon sParty_Macey[] = {
    {
    .lvl = 36,
    .species = SPECIES_NATU,
    }
};

static const struct TrainerMon sParty_BrendanRustboroTreecko[] = {
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 15,
    .species = SPECIES_TAILLOW,
    },
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 16,
    .species = SPECIES_COMBUSKEN,
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 15,
    .species = SPECIES_POLIWAG,
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 15,
    .species = SPECIES_TANGELA,
    }
};

static const struct TrainerMon sParty_BrendanRustboroMudkip[] = {
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 15,
    .species = SPECIES_TAILLOW,
    },
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 16,
    .species = SPECIES_GROVYLE,
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 15,
    .species = SPECIES_POLIWAG,
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 15,
    .species = SPECIES_LITWICK,
    }
};

static const struct TrainerMon sParty_Paxton[] = {
    {
    .lvl = 33,
    .species = SPECIES_SWELLOW,
    },
    {
    .lvl = 33,
    .species = SPECIES_BRELOOM,
    }
};

static const struct TrainerMon sParty_Isabella[] = {
    {
    .lvl = 45,
    .species = SPECIES_STARMIE,
    }
};

static const struct TrainerMon sParty_GruntWeatherInst5[] = {
    {
    .lvl = 36,
    .species = SPECIES_GOLISOPOD,
    },
    {
    .lvl = 36,
    .species = SPECIES_SHEDINJA,
    }
};

static const struct TrainerMon sParty_TabithaMtChimney[] = {
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 31,
    .species = SPECIES_ARBOK,
    },
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 31,
    .species = SPECIES_PIGNITE,
    },
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 31,
    .species = SPECIES_COMBUSKEN,
    },
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 32,
    .species = SPECIES_CROBAT,
    }
};

static const struct TrainerMon sParty_Jonathan[] = {
    {
    .lvl = 33,
    .species = SPECIES_KECLEON,
    },
    {
    .lvl = 33,
    .species = SPECIES_LOUDRED,
    }
};

static const struct TrainerMon sParty_BrendanRustboroTorchic[] = {
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 15,
    .species = SPECIES_TAILLOW,
    },
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 16,
    .species = SPECIES_MARSHTOMP,
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 15,
    .species = SPECIES_TANGELA,
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 15,
    .species = SPECIES_LITWICK,
    }
};

static const struct TrainerMon sParty_MayRustboroMudkip[] = {
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 15,
    .species = SPECIES_TAILLOW,
    },
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 16,
    .species = SPECIES_GROVYLE,
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 15,
    .species = SPECIES_POLIWAG,
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 15,
    .species = SPECIES_LITWICK,
    }
};

static const struct TrainerMon sParty_MaxieMagmaHideout[] = {
    {
    .iv = TRAINER_PARTY_IVS(18, 18, 18, 18, 18, 18),
    .lvl = 54,
    .species = SPECIES_EXEGGUTOR,
    .ability = ABILITY_CHLOROPHYLL,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_MEGA_DRAIN, MOVE_PSYBEAM, MOVE_SLEEP_POWDER, MOVE_MOONLIGHT}
    },
    {
    .iv = TRAINER_PARTY_IVS(18, 18, 18, 18, 18, 18),
    .lvl = 54,
    .species = SPECIES_CROBAT,
    .ability = ABILITY_INFILTRATOR,
    .heldItem = ITEM_EXPERT_BELT,
    .moves = {MOVE_BRAVE_BIRD, MOVE_HEAT_WAVE, MOVE_GIGA_DRAIN, MOVE_U_TURN}
    },
    {
    .iv = TRAINER_PARTY_IVS(18, 18, 18, 18, 18, 18),
    .lvl = 54,
    .species = SPECIES_RHYPERIOR,
    .ability = ABILITY_SOLID_ROCK,
    .heldItem = ITEM_WEAKNESS_POLICY,
    .moves = {MOVE_ROCK_POLISH, MOVE_EARTHQUAKE, MOVE_SUPERCELL_SLAM, MOVE_ROCK_SLIDE}
    },
    {
    .iv = TRAINER_PARTY_IVS(18, 18, 18, 18, 18, 18),
    .lvl = 54,
    .species = SPECIES_TYRANTRUM,
    .ability = ABILITY_ROCK_HEAD,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_HEAD_SMASH, MOVE_CLOSE_COMBAT, MOVE_DRAGON_CLAW, MOVE_DRAGON_DANCE}
    },
    {
    .iv = TRAINER_PARTY_IVS(18, 18, 18, 18, 18, 18),
    .lvl = 56,
    .species = SPECIES_CAMERUPT,
    .ability = ABILITY_SOLID_ROCK,
    .heldItem = ITEM_CAMERUPTITE,
    .moves = {MOVE_EARTH_POWER, MOVE_ERUPTION, MOVE_SOLAR_BEAM, MOVE_EXPLOSION}
    },
    {
    .iv = TRAINER_PARTY_IVS(18, 18, 18, 18, 18, 18),
    .lvl = 56,
    .species = SPECIES_VOLCANION,
    .ability = ABILITY_WATER_ABSORB,
    .heldItem = ITEM_AIR_BALLOON,
    .moves = {MOVE_HEAT_WAVE, MOVE_STEAM_ERUPTION, MOVE_EXPLOSION, MOVE_NONE}
    }
};

static const struct TrainerMon sParty_MaxieMtChimney[] = {
    {
    .iv = TRAINER_PARTY_IVS(18, 18, 18, 18, 18, 18),
    .lvl = 32,
    .species = SPECIES_GOLEM,
    .ability = ABILITY_STURDY,
    .heldItem = ITEM_WEAKNESS_POLICY,
    .moves = {MOVE_SMACK_DOWN, MOVE_EARTHQUAKE, MOVE_EXPLOSION, MOVE_DOUBLE_EDGE}
    },
    {
    .iv = TRAINER_PARTY_IVS(18, 18, 18, 18, 18, 18),
    .lvl = 32,
    .species = SPECIES_ARBOK,
    .ability = ABILITY_INTIMIDATE,
    .heldItem = ITEM_CHESTO_BERRY,
    .moves = {MOVE_POISON_FANG, MOVE_COIL, MOVE_CRUNCH, MOVE_REST}
    },
    {
    .iv = TRAINER_PARTY_IVS(18, 18, 18, 18, 18, 18),
    .lvl = 32,
    .species = SPECIES_TORKOAL,
    .ability = ABILITY_DROUGHT,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_IRON_DEFENSE, MOVE_LAVA_PLUME, MOVE_BODY_PRESS, MOVE_REST}
    },
    {
    .iv = TRAINER_PARTY_IVS(18, 18, 18, 18, 18, 18),
    .lvl = 33,
    .species = SPECIES_CAMERUPT,
    .moves = {MOVE_EARTH_POWER, MOVE_POWER_GEM, MOVE_SOLAR_BEAM, MOVE_LAVA_PLUME}
    },
    {
    .iv = TRAINER_PARTY_IVS(18, 18, 18, 18, 18, 18),
    .lvl = 33,
    .species = SPECIES_GOURGEIST,
    .ability = ABILITY_FLARE_BOOST,
    .heldItem = ITEM_FLAME_ORB,
    .moves = {MOVE_GIGA_DRAIN, MOVE_SHADOW_BALL, MOVE_LEECH_SEED, MOVE_SHADOW_SNEAK}
    }
};

static const struct TrainerMon sParty_Tiana[] = {
    {
    .lvl = 6,
    .species = SPECIES_RATTATA,
    },
    {
    .lvl = 6,
    .species = SPECIES_SENTRET,
    }
};

static const struct TrainerMon sParty_Haley1[] = {
    {
    .lvl = 6,
    .species = SPECIES_BELLSPROUT,
    },
    {
    .lvl = 6,
    .species = SPECIES_PUMPKABOO,
    }
};

static const struct TrainerMon sParty_Janice[] = {
    {
    .lvl = 12,
    .species = SPECIES_MARILL,
    .ability = ABILITY_HUGE_POWER,
    .moves = {MOVE_SECRET_POWER, MOVE_AQUA_JET, MOVE_DEFENSE_CURL, MOVE_ROLLOUT}
    },
    {
    .lvl = 12,
    .species = SPECIES_CHIMCHAR,
    .ability = ABILITY_IRON_FIST,
    .moves = {MOVE_MACH_PUNCH, MOVE_EMBER, MOVE_GROWL, MOVE_ROLLOUT}
    }
};

static const struct TrainerMon sParty_Vivi[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 21,
    .species = SPECIES_DEDENNE,
    .moves = {MOVE_CHARGE, MOVE_CHARGE_BEAM, MOVE_QUICK_ATTACK, MOVE_DRAINING_KISS}
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 21,
    .species = SPECIES_TOGEDEMARU,
    .moves = {MOVE_FAKE_OUT, MOVE_NUZZLE, MOVE_IRON_HEAD, MOVE_SPIKY_SHIELD}
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 21,
    .species = SPECIES_MORPEKO,
    .moves = {MOVE_AURA_WHEEL, MOVE_NUZZLE, MOVE_QUICK_ATTACK, MOVE_FLATTER}
    }
};

static const struct TrainerMon sParty_Haley2[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 26,
    .species = SPECIES_WEEPINBELL,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 26,
    .species = SPECIES_PUMPKABOO,
    }
};

static const struct TrainerMon sParty_Haley3[] = {
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 31,
    .species = SPECIES_VICTREEBEL,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 31,
    .species = SPECIES_GOURGEIST,
    }
};

static const struct TrainerMon sParty_Haley4[] = {
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 34,
    .species = SPECIES_VICTREEBEL,
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 35,
    .species = SPECIES_GOURGEIST,
    }
};

static const struct TrainerMon sParty_Haley5[] = {
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 39,
    .species = SPECIES_SWELLOW,
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 39,
    .species = SPECIES_VICTREEBEL,
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 41,
    .species = SPECIES_GOURGEIST,
    }
};

static const struct TrainerMon sParty_Sally[] = {
    {
    .lvl = 16,
    .species = SPECIES_ODDISH,
    }
};

static const struct TrainerMon sParty_Robin[] = {
    {
    .lvl = 14,
    .species = SPECIES_SKITTY,
    },
    {
    .lvl = 14,
    .species = SPECIES_SHROOMISH,
    },
    {
    .lvl = 14,
    .species = SPECIES_MARILL,
    }
};

static const struct TrainerMon sParty_Andrea[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 64,
    .species = SPECIES_LUDICOLO,
    .ability = ABILITY_SWIFT_SWIM,
    .moves = {MOVE_HYDRO_PUMP, MOVE_GIGA_DRAIN, MOVE_ICE_BEAM, MOVE_FOCUS_BLAST}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 63,
    .species = SPECIES_TOXICROAK,
    .ability = ABILITY_DRY_SKIN,
    .moves = {MOVE_DRAIN_PUNCH, MOVE_POISON_JAB, MOVE_SUCKER_PUNCH, MOVE_KNOCK_OFF}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 63,
    .species = SPECIES_LUVDISC,
    .ability = ABILITY_SOUL_HEART,
    .heldItem = ITEM_CHOICE_SCARF,
    .moves = {MOVE_HYDRO_PUMP, MOVE_ICE_BEAM, MOVE_PSYCHIC, MOVE_DRAINING_KISS}
    }

};

static const struct TrainerMon sParty_Crissy[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 65,
    .species = SPECIES_INTELEON,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 65,
    .species = SPECIES_WAILORD,
    }
};

static const struct TrainerMon sParty_Rick[] = {
    {
    .lvl = 7,
    .species = SPECIES_WEEDLE,
    .moves = {MOVE_BUG_BITE, MOVE_STRING_SHOT, MOVE_POISON_STING, MOVE_NONE}
    },
    {
    .lvl = 7,
    .species = SPECIES_SCATTERBUG,
    .moves = {MOVE_STRUGGLE_BUG, MOVE_STRING_SHOT, MOVE_STUN_SPORE, MOVE_NONE}
    }
};

static const struct TrainerMon sParty_Lyle[] = {
    {
    .lvl = 7,
    .species = SPECIES_SIZZLIPEDE,
    },
    {
    .lvl = 7,
    .species = SPECIES_CATERPIE,
    },
    {
    .lvl = 8,
    .species = SPECIES_WEEDLE,
    },
    {
    .lvl = 8,
    .species = SPECIES_SURSKIT,
    }
};

static const struct TrainerMon sParty_Jose[] = {
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 11,
    .species = SPECIES_DUSTOX,
    .moves = {MOVE_STRUGGLE_BUG, MOVE_GUST, MOVE_HARDEN, MOVE_POISON_STING}
    },
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 12,
    .species = SPECIES_BUTTERFREE,
    .moves = {MOVE_STRUGGLE_BUG, MOVE_GUST, MOVE_CONFUSION, MOVE_SLEEP_POWDER}
    }
};

static const struct TrainerMon sParty_Doug[] = {
    {
    .lvl = 36,
    .species = SPECIES_VIKAVOLT,
    },
    {
    .lvl = 37,
    .species = SPECIES_YANMEGA,
    }
};

static const struct TrainerMon sParty_Greg[] = {
    {
    .lvl = 39,
    .species = SPECIES_VOLBEAT,
    },
    {
    .lvl = 39,
    .species = SPECIES_ILLUMISE,
    }
};

static const struct TrainerMon sParty_Kent[] = {
    {
    .lvl = 37,
    .species = SPECIES_NINJASK,
    }
};

static const struct TrainerMon sParty_James1[] = {
    {
    .lvl = 6,
    .species = SPECIES_NINCADA,
    },
    {
    .lvl = 6,
    .species = SPECIES_NINCADA,
    }
};

static const struct TrainerMon sParty_James2[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 27,
    .species = SPECIES_NINJASK,
    }
};

static const struct TrainerMon sParty_James3[] = {
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 29,
    .species = SPECIES_DUSTOX,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 29,
    .species = SPECIES_NINJASK,
    }
};

static const struct TrainerMon sParty_James4[] = {
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 31,
    .species = SPECIES_SURSKIT,
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 31,
    .species = SPECIES_DUSTOX,
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 31,
    .species = SPECIES_NINJASK,
    }
};

static const struct TrainerMon sParty_James5[] = {
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 33,
    .species = SPECIES_MASQUERAIN,
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 33,
    .species = SPECIES_NINJASK,
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 33,
    .species = SPECIES_DUSTOX,
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 33,
    .species = SPECIES_NINJASK,
    }
};

static const struct TrainerMon sParty_Brice[] = {
    {
    .lvl = 27,
    .species = SPECIES_NUMEL,
    },
    {
    .lvl = 28,
    .species = SPECIES_MACHOKE,
    }
};

static const struct TrainerMon sParty_Trent1[] = {
    {
    .lvl = 26,
    .species = SPECIES_GEODUDE,
    },
    {
    .lvl = 26,
    .species = SPECIES_RHYHORN,
    },
    {
    .lvl = 26,
    .species = SPECIES_CARBINK,
    }
};

static const struct TrainerMon sParty_Lenny[] = {
    {
    .lvl = 29,
    .species = SPECIES_GEODUDE,
    },
    {
    .lvl = 28,
    .species = SPECIES_LUCARIO,
    }
};

static const struct TrainerMon sParty_Lucas1[] = {
    {
    .lvl = 26,
    .species = SPECIES_LILEEP,
    },
    {
    .lvl = 26,
    .species = SPECIES_ANORITH,
    }
};

static const struct TrainerMon sParty_Alan[] = {
    {
    .lvl = 22,
    .species = SPECIES_GEODUDE,
    },
    {
    .lvl = 22,
    .species = SPECIES_NOSEPASS,
    },
    {
    .lvl = 22,
    .species = SPECIES_GRAVELER,
    }
};

static const struct TrainerMon sParty_Clark[] = {
    {
    .lvl = 11,
    .species = SPECIES_AMAURA,
    },
    {
    .lvl = 11,
    .species = SPECIES_CRANIDOS,
    }
};

static const struct TrainerMon sParty_Eric[] = {
    {
    .lvl = 30,
    .species = SPECIES_BOLDORE,
    },
    {
    .lvl = 31,
    .species = SPECIES_PROBOPASS,
    }
};

static const struct TrainerMon sParty_Lucas2[] = {
    {
    .lvl = 9,
    .species = SPECIES_WAILMER,
    .moves = {MOVE_SPLASH, MOVE_WATER_GUN, MOVE_NONE, MOVE_NONE}
    }
};

static const struct TrainerMon sParty_Mike1[] = {
    {
    .lvl = 10,
    .species = SPECIES_PELIPPER,
    .moves = {MOVE_GUST, MOVE_GROWL, MOVE_NONE, MOVE_NONE}
    },
    {
    .lvl = 10,
    .species = SPECIES_POOCHYENA,
    .moves = {MOVE_BITE, MOVE_SCARY_FACE, MOVE_NONE, MOVE_NONE}
    }
};

static const struct TrainerMon sParty_Mike2[] = {
    {
    .lvl = 16,
    .species = SPECIES_GEODUDE,
    },
    {
    .lvl = 16,
    .species = SPECIES_GEODUDE,
    },
    {
    .lvl = 16,
    .species = SPECIES_MACHOP,
    }
};

static const struct TrainerMon sParty_Trent2[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 34,
    .species = SPECIES_GRAVELER,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 34,
    .species = SPECIES_RHYHORN,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 34,
    .species = SPECIES_MINIOR,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 34,
    .species = SPECIES_CARBINK,
    }
};

static const struct TrainerMon sParty_Trent3[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 38,
    .species = SPECIES_GOLEM,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 38,
    .species = SPECIES_RHYHORN,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 38,
    .species = SPECIES_MINIOR,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 38,
    .species = SPECIES_CARBINK,
    }
};

static const struct TrainerMon sParty_Trent4[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 46,
    .species = SPECIES_GOLEM,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 46,
    .species = SPECIES_RHYPERIOR,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 46,
    .species = SPECIES_MINIOR,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 46,
    .species = SPECIES_CARBINK,
    }
};

static const struct TrainerMon sParty_Trent5[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 56,
    .species = SPECIES_GOLEM,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 56,
    .species = SPECIES_RHYPERIOR,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 56,
    .species = SPECIES_MINIOR,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 56,
    .species = SPECIES_CARBINK,
    }
};

static const struct TrainerMon sParty_DezAndLuke[] = {
    {
    .lvl = 46,
    .species = SPECIES_DELCATTY,
    .moves = {MOVE_FAKE_OUT, MOVE_HELPING_HAND, MOVE_BODY_SLAM, MOVE_ASSIST}
    },
    {
    .lvl = 46,
    .species = SPECIES_MANECTRIC,
    .moves = {MOVE_DISCHARGE, MOVE_CRUNCH, MOVE_QUICK_ATTACK, MOVE_OVERHEAT}
    },
    {
    .lvl = 47,
    .species = SPECIES_SEAKING,
    .ability = ABILITY_LIGHTNING_ROD,
    .moves = {MOVE_WATER_PULSE, MOVE_ICE_BEAM, MOVE_HORN_ATTACK, MOVE_MEGAHORN}
    },
    {
    .lvl = 47,
    .species = SPECIES_RHYDON,
    .ability = ABILITY_LIGHTNING_ROD,
    .moves = {MOVE_EARTH_POWER, MOVE_ROCK_SLIDE, MOVE_FLAMETHROWER, MOVE_ICE_BEAM}
    },


};

static const struct TrainerMon sParty_LeaAndJed[] = {
    {
    .lvl = 95,
    .species = SPECIES_LUVDISC,
    },
    {
    .lvl = 95,
    .species = SPECIES_LUVDISC,
    }
};

static const struct TrainerMon sParty_KiraAndDan1[] = {
    {
    .lvl = 36,
    .species = SPECIES_VOLBEAT,
    },
    {
    .lvl = 36,
    .species = SPECIES_ILLUMISE,
    },
    {
    .lvl = 36,
    .species = SPECIES_KLEAVOR,
    },
    {
    .lvl = 36,
    .species = SPECIES_SCIZOR,
    }
};

static const struct TrainerMon sParty_KiraAndDan2[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 44,
    .species = SPECIES_VOLBEAT,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 44,
    .species = SPECIES_ILLUMISE,
    },
    {
    .lvl = 44,
    .species = SPECIES_KLEAVOR,
    },
    {
    .lvl = 44,
    .species = SPECIES_SCIZOR,
    }
};

static const struct TrainerMon sParty_KiraAndDan3[] = {
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 48,
    .species = SPECIES_VOLBEAT,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 48,
    .species = SPECIES_ILLUMISE,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 48,
    .species = SPECIES_KLEAVOR,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 48,
    .species = SPECIES_SCIZOR,
    }
};

static const struct TrainerMon sParty_KiraAndDan4[] = {
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 54,
    .species = SPECIES_VOLBEAT,
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 54,
    .species = SPECIES_ILLUMISE,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 54,
    .species = SPECIES_KLEAVOR,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 54,
    .species = SPECIES_SCIZOR,
    }
};

static const struct TrainerMon sParty_KiraAndDan5[] = {
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 58,
    .species = SPECIES_VOLBEAT,
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 58,
    .species = SPECIES_ILLUMISE,
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 58,
    .species = SPECIES_KLEAVOR,
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 58,
    .species = SPECIES_SCIZOR,
    }
};

static const struct TrainerMon sParty_Johanna[] = {
    {
    .lvl = 13,
    .species = SPECIES_GOLDEEN,
    }
};

static const struct TrainerMon sParty_Gerald[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 32,
    .species = SPECIES_KECLEON,
    .moves = {MOVE_FLAMETHROWER, MOVE_FURY_SWIPES, MOVE_FEINT_ATTACK, MOVE_BIND}
    }
};

static const struct TrainerMon sParty_Vivian[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 17,
    .species = SPECIES_MEDITITE,
    .moves = {MOVE_BIDE, MOVE_DETECT, MOVE_CONFUSION, MOVE_THUNDER_PUNCH}
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 17,
    .species = SPECIES_MEDITITE,
    .moves = {MOVE_THUNDER_PUNCH, MOVE_DETECT, MOVE_CONFUSION, MOVE_MEDITATE}
    }
};

static const struct TrainerMon sParty_Danielle[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 32,
    .species = SPECIES_MEDICHAM,
    .moves = {MOVE_BIDE, MOVE_DETECT, MOVE_CONFUSION, MOVE_FIRE_PUNCH}
    }
};

static const struct TrainerMon sParty_Hideo[] = {
    {
    .lvl = 36,
    .species = SPECIES_KOFFING,
    .moves = {MOVE_TACKLE, MOVE_EXPLOSION, MOVE_SLUDGE, MOVE_SMOKESCREEN}
    },
    {
    .lvl = 37,
    .species = SPECIES_WEEZING,
    .moves = {MOVE_TACKLE, MOVE_EXPLOSION, MOVE_SLUDGE, MOVE_SMOKESCREEN}
    }
};

static const struct TrainerMon sParty_Keigo[] = {
    {
    .lvl = 48,
    .species = SPECIES_MUK,
    .moves = {MOVE_POISON_JAB, MOVE_EXPLOSION, MOVE_SHADOW_SNEAK, MOVE_MINIMIZE}
    },
    {
    .lvl = 48,
    .species = SPECIES_MUK_ALOLAN,
    .moves = {MOVE_CRUNCH, MOVE_GUNK_SHOT, MOVE_FIRE_PUNCH, MOVE_PURSUIT}
    }
};

static const struct TrainerMon sParty_Riley[] = {
    {
    .lvl = 42,
    .species = SPECIES_NINJASK,
    .moves = {MOVE_LEECH_LIFE, MOVE_DUAL_WINGBEAT, MOVE_SWORDS_DANCE, MOVE_DIG}
    },
    {
    .lvl = 41,
    .species = SPECIES_WEEZING,
    .moves = {MOVE_SLUDGE_BOMB, MOVE_SELF_DESTRUCT, MOVE_THUNDERBOLT, MOVE_SMOKESCREEN}
    }
};

static const struct TrainerMon sParty_Flint[] = {
    {
    .iv = TRAINER_PARTY_IVS(18, 18, 18, 18, 18, 18),
    .lvl = 43,
    .species = SPECIES_SWELLOW,
    },
    {
    .iv = TRAINER_PARTY_IVS(18, 18, 18, 18, 18, 18),
    .lvl = 43,
    .species = SPECIES_XATU,
    }
};

static const struct TrainerMon sParty_Ashley[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 45,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 45,
    .species = SPECIES_CYCLIZAR,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 45,
    .species = SPECIES_GLISCOR,
    }
};

static const struct TrainerMon sParty_WallyMauville[] = {
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 23,
    .species = SPECIES_ROSELIA,
    .heldItem = ITEM_BLACK_SLUDGE,
    .moves = {MOVE_LEECH_SEED, MOVE_GIGA_DRAIN, MOVE_TOXIC_SPIKES, MOVE_SYNTHESIS}
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 23,
    .species = SPECIES_FLETCHINDER,
    .moves = {MOVE_PLUCK, MOVE_FLAME_CHARGE, MOVE_ROOST, MOVE_U_TURN}
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 25,
    .species = SPECIES_KIRLIA,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_PSYBEAM, MOVE_SHOCK_WAVE, MOVE_TELEPORT, MOVE_FUTURE_SIGHT}
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 24,
    .species = SPECIES_AZUMARILL,
    .heldItem = ITEM_MUSCLE_BAND,
    .ability = ABILITY_HUGE_POWER,
    .moves = {MOVE_AQUA_JET, MOVE_PLAY_ROUGH, MOVE_ROLLOUT, MOVE_DEFENSE_CURL}
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 24,
    .species = SPECIES_GABITE,
    .heldItem = ITEM_YACHE_BERRY,
    .moves = {MOVE_EARTHQUAKE, MOVE_ROCK_TOMB, MOVE_DUAL_CHOP, MOVE_SWORDS_DANCE}
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 24,
    .species = SPECIES_MAGNEMITE,
    .heldItem = ITEM_EVIOLITE,
    .moves = {MOVE_VOLT_SWITCH, MOVE_SONIC_BOOM, MOVE_MAGNET_BOMB, MOVE_THUNDER_WAVE}
    }
};

static const struct TrainerMon sParty_WallyVR2[] = {
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 77,
    .species = SPECIES_ROSERADE,
    .heldItem = ITEM_FOCUS_SASH,
    .ability = ABILITY_POISON_POINT,
    .moves = {MOVE_TOXIC_SPIKES, MOVE_SPIKES, MOVE_LEAF_STORM, MOVE_SLUDGE_BOMB}
    },
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 77,
    .species = SPECIES_TALONFLAME,
    .heldItem = ITEM_CHOICE_BAND,
    .ability = ABILITY_GALE_WINGS,
    .moves = {MOVE_BRAVE_BIRD, MOVE_FLARE_BLITZ, MOVE_U_TURN, MOVE_STEEL_WING}
    },
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 77,
    .species = SPECIES_AZUMARILL,
    .heldItem = ITEM_SITRUS_BERRY,
    .ability = ABILITY_HUGE_POWER,
    .moves = {MOVE_BELLY_DRUM, MOVE_AQUA_JET, MOVE_PLAY_ROUGH, MOVE_IRON_TAIL}
    },
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 77,
    .species = SPECIES_MAGNEZONE,
    .heldItem = ITEM_AIR_BALLOON,
    .ability = ABILITY_MAGNET_PULL,
    .moves = {MOVE_ZAP_CANNON, MOVE_LOCK_ON, MOVE_FLASH_CANNON, MOVE_BODY_PRESS}
    },
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 78,
    .species = SPECIES_GARCHOMP,
    .heldItem = ITEM_GROUND_GEM,
    .ability = ABILITY_SAND_VEIL,
    .moves = {MOVE_EARTHQUAKE, MOVE_DRAGON_RUSH, MOVE_SWORDS_DANCE, MOVE_FIRE_FANG}
    },
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 79,
    .species = SPECIES_GALLADE,
    .heldItem = ITEM_GALLADITE,
    .ability = ABILITY_SHARPNESS,
    .moves = {MOVE_SWORDS_DANCE, MOVE_AGILITY, MOVE_PSYCHO_CUT, MOVE_SACRED_SWORD}
    }
};

static const struct TrainerMon sParty_WallyVR3[] = {
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 80,
    .species = SPECIES_ROSERADE,
    .heldItem = ITEM_FOCUS_SASH,
    .ability = ABILITY_POISON_POINT,
    .moves = {MOVE_TOXIC_SPIKES, MOVE_SPIKES, MOVE_LEAF_STORM, MOVE_SLUDGE_BOMB}
    },
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 80,
    .species = SPECIES_TALONFLAME,
    .heldItem = ITEM_CHOICE_BAND,
    .ability = ABILITY_GALE_WINGS,
    .moves = {MOVE_BRAVE_BIRD, MOVE_FLARE_BLITZ, MOVE_U_TURN, MOVE_NONE}
    },
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 80,
    .species = SPECIES_AZUMARILL,
    .heldItem = ITEM_SITRUS_BERRY,
    .ability = ABILITY_HUGE_POWER,
    .moves = {MOVE_BELLY_DRUM, MOVE_AQUA_JET, MOVE_PLAY_ROUGH, MOVE_IRON_TAIL}
    },
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 80,
    .species = SPECIES_MAGNEZONE,
    .heldItem = ITEM_AIR_BALLOON,
    .ability = ABILITY_MAGNET_PULL,
    .moves = {MOVE_ZAP_CANNON, MOVE_LOCK_ON, MOVE_FLASH_CANNON, MOVE_BODY_PRESS}
    },
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 81,
    .species = SPECIES_GARCHOMP,
    .heldItem = ITEM_GROUND_GEM,
    .ability = ABILITY_SAND_VEIL,
    .moves = {MOVE_EARTHQUAKE, MOVE_DRAGON_RUSH, MOVE_SWORDS_DANCE, MOVE_FIRE_FANG}
    },
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 82,
    .species = SPECIES_GALLADE,
    .heldItem = ITEM_GALLADITE,
    .ability = ABILITY_SHARPNESS,
    .moves = {MOVE_SWORDS_DANCE, MOVE_AGILITY, MOVE_PSYCHO_CUT, MOVE_SACRED_SWORD}
    }
};

static const struct TrainerMon sParty_WallyVR4[] = {
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 84,
    .species = SPECIES_ROSERADE,
    .heldItem = ITEM_FOCUS_SASH,
    .ability = ABILITY_POISON_POINT,
    .moves = {MOVE_TOXIC_SPIKES, MOVE_SPIKES, MOVE_LEAF_STORM, MOVE_SLUDGE_BOMB}
    },
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 84,
    .species = SPECIES_TALONFLAME,
    .heldItem = ITEM_CHOICE_BAND,
    .ability = ABILITY_GALE_WINGS,
    .moves = {MOVE_BRAVE_BIRD, MOVE_FLARE_BLITZ, MOVE_U_TURN, MOVE_NONE}
    },
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 84,
    .species = SPECIES_AZUMARILL,
    .heldItem = ITEM_SITRUS_BERRY,
    .ability = ABILITY_HUGE_POWER,
    .moves = {MOVE_BELLY_DRUM, MOVE_AQUA_JET, MOVE_PLAY_ROUGH, MOVE_IRON_TAIL}
    },
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 84,
    .species = SPECIES_MAGNEZONE,
    .heldItem = ITEM_AIR_BALLOON,
    .ability = ABILITY_MAGNET_PULL,
    .moves = {MOVE_ZAP_CANNON, MOVE_LOCK_ON, MOVE_FLASH_CANNON, MOVE_BODY_PRESS}
    },
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 85,
    .species = SPECIES_GARCHOMP,
    .heldItem = ITEM_GROUND_GEM,
    .ability = ABILITY_SAND_VEIL,
    .moves = {MOVE_EARTHQUAKE, MOVE_DRAGON_RUSH, MOVE_SWORDS_DANCE, MOVE_FIRE_FANG}
    },
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 88,
    .species = SPECIES_GALLADE,
    .heldItem = ITEM_GALLADITE,
    .ability = ABILITY_SHARPNESS,
    .moves = {MOVE_SWORDS_DANCE, MOVE_AGILITY, MOVE_PSYCHO_CUT, MOVE_SACRED_SWORD}
    }
};

static const struct TrainerMon sParty_WallyVR5[] = {
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 99,
    .species = SPECIES_ROSERADE,
    .heldItem = ITEM_FOCUS_SASH,
    .ability = ABILITY_POISON_POINT,
    .moves = {MOVE_TOXIC_SPIKES, MOVE_SPIKES, MOVE_LEAF_STORM, MOVE_SLUDGE_BOMB}
    },
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 99,
    .species = SPECIES_TALONFLAME,
    .heldItem = ITEM_CHOICE_BAND,
    .ability = ABILITY_GALE_WINGS,
    .moves = {MOVE_BRAVE_BIRD, MOVE_FLARE_BLITZ, MOVE_U_TURN, MOVE_STEEL_WING}
    },
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 99,
    .species = SPECIES_AZUMARILL,
    .heldItem = ITEM_SITRUS_BERRY,
    .ability = ABILITY_HUGE_POWER,
    .moves = {MOVE_BELLY_DRUM, MOVE_AQUA_JET, MOVE_PLAY_ROUGH, MOVE_SUPERPOWER}
    },
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 99,
    .species = SPECIES_MAGNEZONE,
    .heldItem = ITEM_AIR_BALLOON,
    .ability = ABILITY_MAGNET_PULL,
    .moves = {MOVE_THUNDERBOLT, MOVE_THUNDER_WAVE, MOVE_FLASH_CANNON, MOVE_BODY_PRESS}
    },
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 100,
    .species = SPECIES_GARCHOMP,
    .heldItem = ITEM_GROUND_GEM,
    .ability = ABILITY_SAND_VEIL,
    .moves = {MOVE_EARTHQUAKE, MOVE_SCALE_SHOT, MOVE_SWORDS_DANCE, MOVE_FIRE_FANG}
    },
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 100,
    .species = SPECIES_GALLADE,
    .heldItem = ITEM_GALLADITE,
    .ability = ABILITY_SHARPNESS,
    .moves = {MOVE_SWORDS_DANCE, MOVE_AQUA_CUTTER, MOVE_PSYCHO_CUT, MOVE_SACRED_SWORD}
    }
};

static const struct TrainerMon sParty_BrendanLilycoveMudkip[] = {
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 51,
    .species = SPECIES_SWELLOW,
    .ability = ABILITY_SCRAPPY,
    .heldItem = ITEM_THROAT_SPRAY,
    .moves = {MOVE_BOOMBURST, MOVE_HURRICANE, MOVE_HEAT_WAVE, MOVE_QUICK_ATTACK}
    },
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 53,
    .species = SPECIES_SCEPTILE,
    .heldItem = ITEM_SCEPTILITE,
    .moves = {MOVE_DRAGON_DANCE, MOVE_SEED_BOMB, MOVE_DRAGON_CLAW, MOVE_EARTHQUAKE}
    },
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 52,
    .species = SPECIES_POLIWRATH,
    .ability = ABILITY_IRON_FIST,
    .heldItem = ITEM_PUNCHING_GLOVE,
    .moves = {MOVE_SURGING_STRIKES, MOVE_ICE_PUNCH, MOVE_CLOSE_COMBAT, MOVE_POWER_UP_PUNCH}
    },
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 52,
    .species = SPECIES_CHANDELURE,
    .ability = ABILITY_SHADOW_TAG,
    .heldItem = ITEM_LIFE_ORB,
    .moves = {MOVE_SHADOW_BALL, MOVE_FLAMETHROWER, MOVE_CALM_MIND, MOVE_ENERGY_BALL}
    },
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 52,
    .species = SPECIES_AGGRON,
    .moves = {MOVE_CURSE, MOVE_HEAVY_SLAM, MOVE_REST, MOVE_BODY_PRESS}
    },
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 53,
    .species = SPECIES_HAXORUS,
    .ability = ABILITY_MOLD_BREAKER,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_SWORDS_DANCE, MOVE_IRON_TAIL, MOVE_DRAGON_CLAW, MOVE_CRUNCH}
    }
};

static const struct TrainerMon sParty_BrendanLilycoveTreecko[] = {
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 51,
    .species = SPECIES_SWELLOW,
    .ability = ABILITY_SCRAPPY,
    .heldItem = ITEM_THROAT_SPRAY,
    .moves = {MOVE_BOOMBURST, MOVE_HURRICANE, MOVE_HEAT_WAVE, MOVE_QUICK_ATTACK}
    },
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 52,
    .species = SPECIES_TANGROWTH,
    .heldItem = ITEM_ASSAULT_VEST,
    .ability = ABILITY_REGENERATOR,
    .moves = {MOVE_POWER_WHIP, MOVE_EARTHQUAKE, MOVE_ROCK_SLIDE, MOVE_KNOCK_OFF}
    },
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 52,
    .species = SPECIES_POLIWRATH,
    .ability = ABILITY_IRON_FIST,
    .heldItem = ITEM_PUNCHING_GLOVE,
    .moves = {MOVE_SURGING_STRIKES, MOVE_ICE_PUNCH, MOVE_CLOSE_COMBAT, MOVE_POWER_UP_PUNCH}
    },
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 53,
    .species = SPECIES_BLAZIKEN,
    .ability = ABILITY_SPEED_BOOST,
    .heldItem = ITEM_BLAZIKENITE,
    .moves = {MOVE_HIGH_JUMP_KICK, MOVE_FLARE_BLITZ, MOVE_SWORDS_DANCE, MOVE_PROTECT}
    },
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 52,
    .species = SPECIES_AGGRON,
    .moves = {MOVE_CURSE, MOVE_HEAVY_SLAM, MOVE_REST, MOVE_BODY_PRESS}
    },
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 53,
    .species = SPECIES_HAXORUS,
    .ability = ABILITY_MOLD_BREAKER,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_SWORDS_DANCE, MOVE_IRON_TAIL, MOVE_DRAGON_CLAW, MOVE_CRUNCH}
    }
};

static const struct TrainerMon sParty_BrendanLilycoveTorchic[] = {
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 51,
    .species = SPECIES_SWELLOW,
    .ability = ABILITY_SCRAPPY,
    .heldItem = ITEM_THROAT_SPRAY,
    .moves = {MOVE_BOOMBURST, MOVE_HURRICANE, MOVE_HEAT_WAVE, MOVE_QUICK_ATTACK}
    },
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 52,
    .species = SPECIES_TANGROWTH,
    .heldItem = ITEM_ASSAULT_VEST,
    .ability = ABILITY_REGENERATOR,
    .moves = {MOVE_POWER_WHIP, MOVE_EARTHQUAKE, MOVE_ROCK_SLIDE, MOVE_KNOCK_OFF}
    },
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 53,
    .species = SPECIES_SWAMPERT,
    .ability = ABILITY_DAMP,
    .heldItem = ITEM_SWAMPERTITE,
    .moves = {MOVE_RAIN_DANCE, MOVE_ICE_PUNCH, MOVE_EARTHQUAKE, MOVE_LIQUIDATION}
    },
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 52,
    .species = SPECIES_CHANDELURE,
    .ability = ABILITY_SHADOW_TAG,
    .heldItem = ITEM_LIFE_ORB,
    .moves = {MOVE_SHADOW_BALL, MOVE_FLAMETHROWER, MOVE_CALM_MIND, MOVE_ENERGY_BALL}
    },
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 52,
    .species = SPECIES_AGGRON,
    .moves = {MOVE_CURSE, MOVE_HEAVY_SLAM, MOVE_REST, MOVE_BODY_PRESS}
    },
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 53,
    .species = SPECIES_HAXORUS,
    .ability = ABILITY_MOLD_BREAKER,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_SWORDS_DANCE, MOVE_IRON_TAIL, MOVE_DRAGON_CLAW, MOVE_CRUNCH}
    }
};

static const struct TrainerMon sParty_MayLilycoveMudkip[] = {
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 51,
    .species = SPECIES_SWELLOW,
    .ability = ABILITY_SCRAPPY,
    .heldItem = ITEM_THROAT_SPRAY,
    .moves = {MOVE_BOOMBURST, MOVE_HURRICANE, MOVE_HEAT_WAVE, MOVE_QUICK_ATTACK}
    },
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 53,
    .species = SPECIES_SCEPTILE,
    .heldItem = ITEM_SCEPTILITE,
    .moves = {MOVE_DRAGON_DANCE, MOVE_SEED_BOMB, MOVE_DRAGON_CLAW, MOVE_EARTHQUAKE}
    },
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 52,
    .species = SPECIES_POLIWRATH,
    .ability = ABILITY_IRON_FIST,
    .heldItem = ITEM_PUNCHING_GLOVE,
    .moves = {MOVE_SURGING_STRIKES, MOVE_ICE_PUNCH, MOVE_CLOSE_COMBAT, MOVE_POWER_UP_PUNCH}
    },
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 52,
    .species = SPECIES_CHANDELURE,
    .ability = ABILITY_SHADOW_TAG,
    .heldItem = ITEM_LIFE_ORB,
    .moves = {MOVE_SHADOW_BALL, MOVE_FLAMETHROWER, MOVE_CALM_MIND, MOVE_ENERGY_BALL}
    },
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 52,
    .species = SPECIES_AGGRON,
    .moves = {MOVE_CURSE, MOVE_HEAVY_SLAM, MOVE_REST, MOVE_BODY_PRESS}
    },
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 53,
    .species = SPECIES_HAXORUS,
    .ability = ABILITY_MOLD_BREAKER,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_SWORDS_DANCE, MOVE_IRON_TAIL, MOVE_DRAGON_CLAW, MOVE_CRUNCH}
    }
};

static const struct TrainerMon sParty_MayLilycoveTreecko[] = {
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 51,
    .species = SPECIES_SWELLOW,
    .ability = ABILITY_SCRAPPY,
    .heldItem = ITEM_THROAT_SPRAY,
    .moves = {MOVE_BOOMBURST, MOVE_HURRICANE, MOVE_HEAT_WAVE, MOVE_QUICK_ATTACK}
    },
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 52,
    .species = SPECIES_TANGROWTH,
    .heldItem = ITEM_ASSAULT_VEST,
    .ability = ABILITY_REGENERATOR,
    .moves = {MOVE_POWER_WHIP, MOVE_EARTHQUAKE, MOVE_ROCK_SLIDE, MOVE_KNOCK_OFF}
    },
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 52,
    .species = SPECIES_POLIWRATH,
    .ability = ABILITY_IRON_FIST,
    .heldItem = ITEM_PUNCHING_GLOVE,
    .moves = {MOVE_SURGING_STRIKES, MOVE_ICE_PUNCH, MOVE_CLOSE_COMBAT, MOVE_POWER_UP_PUNCH}
    },
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 53,
    .species = SPECIES_BLAZIKEN,
    .ability = ABILITY_SPEED_BOOST,
    .heldItem = ITEM_BLAZIKENITE,
    .moves = {MOVE_HIGH_JUMP_KICK, MOVE_FLARE_BLITZ, MOVE_SWORDS_DANCE, MOVE_PROTECT}
    },
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 52,
    .species = SPECIES_AGGRON,
    .moves = {MOVE_CURSE, MOVE_HEAVY_SLAM, MOVE_REST, MOVE_BODY_PRESS}
    },
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 53,
    .species = SPECIES_HAXORUS,
    .ability = ABILITY_MOLD_BREAKER,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_SWORDS_DANCE, MOVE_IRON_TAIL, MOVE_DRAGON_CLAW, MOVE_CRUNCH}
    }
};

static const struct TrainerMon sParty_MayLilycoveTorchic[] = {
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 51,
    .species = SPECIES_SWELLOW,
    .ability = ABILITY_SCRAPPY,
    .heldItem = ITEM_THROAT_SPRAY,
    .moves = {MOVE_BOOMBURST, MOVE_HURRICANE, MOVE_HEAT_WAVE, MOVE_QUICK_ATTACK}
    },
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 52,
    .species = SPECIES_TANGROWTH,
    .heldItem = ITEM_ASSAULT_VEST,
    .ability = ABILITY_REGENERATOR,
    .moves = {MOVE_POWER_WHIP, MOVE_EARTHQUAKE, MOVE_ROCK_SLIDE, MOVE_KNOCK_OFF}
    },
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 53,
    .species = SPECIES_SWAMPERT,
    .ability = ABILITY_DAMP,
    .heldItem = ITEM_SWAMPERTITE,
    .moves = {MOVE_RAIN_DANCE, MOVE_ICE_PUNCH, MOVE_EARTHQUAKE, MOVE_LIQUIDATION}
    },
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 52,
    .species = SPECIES_CHANDELURE,
    .ability = ABILITY_SHADOW_TAG,
    .heldItem = ITEM_LIFE_ORB,
    .moves = {MOVE_SHADOW_BALL, MOVE_FLAMETHROWER, MOVE_CALM_MIND, MOVE_ENERGY_BALL}
    },
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 52,
    .species = SPECIES_AGGRON,
    .moves = {MOVE_CURSE, MOVE_HEAVY_SLAM, MOVE_REST, MOVE_BODY_PRESS}
    },
    {
    .iv = TRAINER_PARTY_IVS(25, 25, 25, 25, 25, 25),
    .lvl = 53,
    .species = SPECIES_HAXORUS,
    .ability = ABILITY_MOLD_BREAKER,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_SWORDS_DANCE, MOVE_IRON_TAIL, MOVE_DRAGON_CLAW, MOVE_CRUNCH}
    }
};

static const struct TrainerMon sParty_Jonah[] = {
    {
    .lvl = 30,
    .species = SPECIES_WAILMER,
    },
    {
    .lvl = 31,
    .species = SPECIES_TENTACOOL,
    },
    {
    .lvl = 32,
    .species = SPECIES_SHARPEDO,
    }
};

static const struct TrainerMon sParty_Henry[] = {
    {
    .lvl = 31,
    .species = SPECIES_CARVANHA,
    },
    {
    .lvl = 34,
    .species = SPECIES_TENTACRUEL,
    }
};

static const struct TrainerMon sParty_Roger[] = {
    {
    .lvl = 95,
    .species = SPECIES_MAGIKARP,
    },
    {
    .lvl = 95,
    .species = SPECIES_MAGIKARP,
    },
    {
    .lvl = 45,
    .species = SPECIES_GYARADOS,
    }
};

static const struct TrainerMon sParty_Alexa[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 34,
    .species = SPECIES_GLOOM,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 34,
    .species = SPECIES_AZUMARILL,
    }
};

static const struct TrainerMon sParty_Ruben[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 34,
    .species = SPECIES_SHIFTRY,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 34,
    .species = SPECIES_NOSEPASS,
    }
};

static const struct TrainerMon sParty_Koji1[] = {
    {
    .lvl = 44,
    .species = SPECIES_MACHOKE,
    },
    {
    .lvl = 44,
    .species = SPECIES_HITMONTOP,
    }
};

static const struct TrainerMon sParty_Wayne[] = {
    {
    .lvl = 31,
    .species = SPECIES_TENTACOOL,
    },
    {
    .lvl = 31,
    .species = SPECIES_TENTACOOL,
    },
    {
    .lvl = 36,
    .species = SPECIES_WAILMER,
    }
};

static const struct TrainerMon sParty_Aidan[] = {
    {
    .lvl = 42,
    .species = SPECIES_SWELLOW,
    },
    {
    .lvl = 42,
    .species = SPECIES_SKARMORY,
    }
};

static const struct TrainerMon sParty_Reed[] = {
    {
    .lvl = 52,
    .species = SPECIES_PRIMARINA,
    .moves = {MOVE_HYDRO_PUMP, MOVE_MOONBLAST, MOVE_ICE_BEAM, MOVE_AQUA_RING}
    },
    {
    .lvl = 54,
    .species = SPECIES_BLASTOISE,
    .moves = {MOVE_HYDRO_PUMP, MOVE_ICE_BEAM, MOVE_BRICK_BREAK, MOVE_RAIN_DANCE}
    }
};

static const struct TrainerMon sParty_Tisha[] = {
    {
    .lvl = 54,
    .species = SPECIES_LANTURN,
    .ability = ABILITY_VOLT_ABSORB,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_SCALD, MOVE_THUNDERBOLT, MOVE_ICE_BEAM, MOVE_VOLT_SWITCH}
    }
};

static const struct TrainerMon sParty_ToriAndTia[] = {
    {
    .lvl = 25,
    .species = SPECIES_BRONZOR,
    .moves = {MOVE_TRICK_ROOM, MOVE_GYRO_BALL, MOVE_PSYBEAM, MOVE_WIDE_GUARD}
    },
    {
    .lvl = 26,
    .species = SPECIES_SUDOWOODO,
    .ability = ABILITY_ROCK_HEAD,
    .moves = {MOVE_WOOD_HAMMER, MOVE_HEAD_SMASH, MOVE_DOUBLE_EDGE, MOVE_EARTHQUAKE}
    },
    {
    .lvl = 26,
    .species = SPECIES_GASTRODON,
    .ability = ABILITY_STORM_DRAIN,
    .moves = {MOVE_WATERFALL, MOVE_ICE_BEAM, MOVE_ROCK_SLIDE, MOVE_RECOVER}
    },
};

static const struct TrainerMon sParty_KimAndIris[] = {
    {
    .lvl = 52,
    .species = SPECIES_ALTARIA,
    .heldItem = ITEM_ALTARIANITE,
    .moves = {MOVE_SING, MOVE_HYPER_VOICE, MOVE_ROOST, MOVE_DRAGON_PULSE}
    },
    {
    .lvl = 55,
    .species = SPECIES_REUNICLUS,
    .ability = ABILITY_MAGIC_GUARD,
    .moves = {MOVE_TRICK_ROOM, MOVE_PSYCHIC, MOVE_FOCUS_BLAST, MOVE_SHADOW_BALL}
    },
    {
    .lvl = 55,
    .species = SPECIES_CAMERUPT,
    .ability = ABILITY_SOLID_ROCK,
    .moves = {MOVE_FLAMETHROWER, MOVE_ROCK_SLIDE, MOVE_REST, MOVE_EARTH_POWER}
    },

};

static const struct TrainerMon sParty_TyraAndIvy[] = {
    {
    .lvl = 27,
    .species = SPECIES_ROSELIA,
    .moves = {MOVE_GROWTH, MOVE_STUN_SPORE, MOVE_MEGA_DRAIN, MOVE_LEECH_SEED}
    },
    {
    .lvl = 28,
    .species = SPECIES_GOLEM,
    .moves = {MOVE_DEFENSE_CURL, MOVE_ROLLOUT, MOVE_MUD_SPORT, MOVE_ROCK_THROW}
    }
};

static const struct TrainerMon sParty_MelAndPaul[] = {
    {
    .lvl = 37,
    .species = SPECIES_DUSTOX,
    .moves = {MOVE_BUG_BUZZ, MOVE_SLUDGE_BOMB, MOVE_TOXIC, MOVE_PROTECT}
    },
    {
    .lvl = 37,
    .species = SPECIES_BEAUTIFLY,
    .moves = {MOVE_HURRICANE, MOVE_GIGA_DRAIN, MOVE_QUIVER_DANCE, MOVE_SLEEP_POWDER}
    }
};

static const struct TrainerMon sParty_JohnAndJay1[] = {
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 69,
    .species = SPECIES_MEDICHAM,
    .moves = {MOVE_PSYCHO_CUT, MOVE_FIRE_PUNCH, MOVE_PSYCH_UP, MOVE_FAKE_OUT}
    },
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 69,
    .species = SPECIES_HARIYAMA,
    .moves = {MOVE_CLOSE_COMBAT, MOVE_ROCK_SLIDE, MOVE_REST, MOVE_BELLY_DRUM}
    },
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 69,
    .species = SPECIES_LUCARIO,
    .moves = {MOVE_CLOSE_COMBAT, MOVE_METEOR_MASH, MOVE_EXTREME_SPEED, MOVE_HELPING_HAND}
    },
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 69,
    .species = SPECIES_FERROTHORN,
    .moves = {MOVE_GYRO_BALL, MOVE_ROCK_SLIDE, MOVE_POWER_WHIP, MOVE_BODY_PRESS}
    }
};

static const struct TrainerMon sParty_JohnAndJay2[] = {
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 72,
    .species = SPECIES_MEDICHAM,
    .moves = {MOVE_PSYCHO_CUT, MOVE_FIRE_PUNCH, MOVE_PSYCH_UP, MOVE_FAKE_OUT}
    },
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 72,
    .species = SPECIES_HARIYAMA,
    .moves = {MOVE_CLOSE_COMBAT, MOVE_ROCK_SLIDE, MOVE_REST, MOVE_BELLY_DRUM}
    },
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 72,
    .species = SPECIES_LUCARIO,
    .moves = {MOVE_CLOSE_COMBAT, MOVE_METEOR_MASH, MOVE_EXTREME_SPEED, MOVE_HELPING_HAND}
    },
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 72,
    .species = SPECIES_FERROTHORN,
    .moves = {MOVE_GYRO_BALL, MOVE_ROCK_SLIDE, MOVE_POWER_WHIP, MOVE_BODY_PRESS}
    }
};

static const struct TrainerMon sParty_JohnAndJay3[] = {
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 75,
    .species = SPECIES_MEDICHAM,
    .moves = {MOVE_PSYCHO_CUT, MOVE_FIRE_PUNCH, MOVE_PSYCH_UP, MOVE_FAKE_OUT}
    },
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 75,
    .species = SPECIES_HARIYAMA,
    .moves = {MOVE_CLOSE_COMBAT, MOVE_ROCK_SLIDE, MOVE_REST, MOVE_BELLY_DRUM}
    },
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 75,
    .species = SPECIES_LUCARIO,
    .moves = {MOVE_CLOSE_COMBAT, MOVE_METEOR_MASH, MOVE_EXTREME_SPEED, MOVE_HELPING_HAND}
    },
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 74,
    .species = SPECIES_FERROTHORN,
    .moves = {MOVE_GYRO_BALL, MOVE_ROCK_SLIDE, MOVE_POWER_WHIP, MOVE_BODY_PRESS}
    }
};

static const struct TrainerMon sParty_JohnAndJay4[] = {
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 77,
    .species = SPECIES_MEDICHAM,
    .moves = {MOVE_PSYCHO_CUT, MOVE_FIRE_PUNCH, MOVE_PSYCH_UP, MOVE_FAKE_OUT}
    },
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 77,
    .species = SPECIES_HARIYAMA,
    .moves = {MOVE_CLOSE_COMBAT, MOVE_ROCK_SLIDE, MOVE_REST, MOVE_BELLY_DRUM}
    },
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 77,
    .species = SPECIES_LUCARIO,
    .moves = {MOVE_CLOSE_COMBAT, MOVE_METEOR_MASH, MOVE_EXTREME_SPEED, MOVE_HELPING_HAND}
    },
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 78,
    .species = SPECIES_FERROTHORN,
    .moves = {MOVE_GYRO_BALL, MOVE_ROCK_SLIDE, MOVE_POWER_WHIP, MOVE_BODY_PRESS}
    }
};

static const struct TrainerMon sParty_JohnAndJay5[] = {
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 80,
    .species = SPECIES_MEDICHAM,
    .moves = {MOVE_PSYCHO_CUT, MOVE_FIRE_PUNCH, MOVE_PSYCH_UP, MOVE_FAKE_OUT}
    },
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 80,
    .species = SPECIES_HARIYAMA,
    .moves = {MOVE_CLOSE_COMBAT, MOVE_ROCK_SLIDE, MOVE_REST, MOVE_BELLY_DRUM}
    },
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 80,
    .species = SPECIES_LUCARIO,
    .moves = {MOVE_CLOSE_COMBAT, MOVE_METEOR_MASH, MOVE_EXTREME_SPEED, MOVE_HELPING_HAND}
    },
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 80,
    .species = SPECIES_FERROTHORN,
    .moves = {MOVE_GYRO_BALL, MOVE_ROCK_SLIDE, MOVE_POWER_WHIP, MOVE_BODY_PRESS}
    }
};

static const struct TrainerMon sParty_ReliAndIan[] = {
    {
    .lvl = 55,
    .species = SPECIES_AZUMARILL,
    .ability = ABILITY_SAP_SIPPER,
    },
    {
    .lvl = 53,
    .species = SPECIES_LANTURN,
    .ability = ABILITY_VOLT_ABSORB,
    }
};

static const struct TrainerMon sParty_LilaAndRoy1[] = {
    {
    .lvl = 44,
    .species = SPECIES_LANTURN,
    },
    {
    .lvl = 43,
    .species = SPECIES_SHARPEDO,
    }
};

static const struct TrainerMon sParty_LilaAndRoy2[] = {
    {
    .lvl = 42,
    .species = SPECIES_LANTURN,
    },
    {
    .lvl = 40,
    .species = SPECIES_SHARPEDO,
    }
};

static const struct TrainerMon sParty_LilaAndRoy3[] = {
    {
    .lvl = 45,
    .species = SPECIES_LANTURN,
    },
    {
    .lvl = 43,
    .species = SPECIES_SHARPEDO,
    }
};

static const struct TrainerMon sParty_LilaAndRoy4[] = {
    {
    .lvl = 48,
    .species = SPECIES_LANTURN,
    },
    {
    .lvl = 46,
    .species = SPECIES_SHARPEDO,
    }
};

static const struct TrainerMon sParty_LilaAndRoy5[] = {
    {
    .lvl = 51,
    .species = SPECIES_LANTURN,
    },
    {
    .lvl = 49,
    .species = SPECIES_SHARPEDO,
    }
};

static const struct TrainerMon sParty_LisaAndRay[] = {
    {
    .lvl = 32,
    .species = SPECIES_GRAPPLOCT,
    },
    {
    .lvl = 35,
    .species = SPECIES_GRANBULL,
    }
};

static const struct TrainerMon sParty_Chris[] = {
    {
    .lvl = 55,
    .species = SPECIES_MAGIKARP,
    },
    {
    .lvl = 55,
    .species = SPECIES_TENTACOOL,
    },
    {
    .lvl = 55,
    .species = SPECIES_FEEBAS,
    },
    {
    .lvl = 50,
    .species = SPECIES_CARVANHA,
    }
};

static const struct TrainerMon sParty_Dawson[] = {
    {
    .lvl = 10,
    .species = SPECIES_ZIGZAGOON,
    .heldItem = ITEM_NUGGET
    },
    {
    .lvl = 10,
    .species = SPECIES_POOCHYENA,
    .heldItem = ITEM_BIG_NUGGET
    }
};

static const struct TrainerMon sParty_Sarah[] = {
    {
    .lvl = 10,
    .species = SPECIES_LOTAD,
    .heldItem = ITEM_ORAN_BERRY
    },
    {
    .lvl = 11,
    .species = SPECIES_ZIGZAGOON,
    .heldItem = ITEM_ORAN_BERRY
    }
};

static const struct TrainerMon sParty_Darian[] = {
    {
    .lvl = 9,
    .species = SPECIES_TOTODILE,
    }
};

static const struct TrainerMon sParty_Hailey[] = {
    {
    .lvl = 19,
    .species = SPECIES_POLITOED,
    },
    {
    .lvl = 17,
    .species = SPECIES_SNUBBULL,
    },
    {
    .lvl = 18,
    .species = SPECIES_CURSOLA,
    }
};

static const struct TrainerMon sParty_Chandler[] = {
    {
    .lvl = 16,
    .species = SPECIES_TENTACOOL,
    },
    {
    .lvl = 18,
    .species = SPECIES_SLOWPOKE,
    }
};

static const struct TrainerMon sParty_Kaleb[] = {
    {
    .lvl = 17,
    .species = SPECIES_MINUN,
    .heldItem = ITEM_ORAN_BERRY
    },
    {
    .lvl = 17,
    .species = SPECIES_PLUSLE,
    .heldItem = ITEM_ORAN_BERRY
    }
};

static const struct TrainerMon sParty_Joseph[] = {
    {
    .lvl = 18,
    .species = SPECIES_ELECTRIKE,
    },
    {
    .lvl = 19,
    .species = SPECIES_VOLTORB,
    }
};

static const struct TrainerMon sParty_Alyssa[] = {
    {
    .lvl = 19,
    .species = SPECIES_PAWMO
    }
};

static const struct TrainerMon sParty_Marcos[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 15,
    .species = SPECIES_VOLTORB,
    .moves = {MOVE_DISCHARGE, MOVE_ROLLOUT, MOVE_TACKLE, MOVE_NONE}
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 15,
    .species = SPECIES_MAGBY,
    .moves = {MOVE_LAVA_PLUME, MOVE_SMOG, MOVE_NONE, MOVE_NONE}
    }

};

static const struct TrainerMon sParty_Rhett[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 15,
    .species = SPECIES_MAKUHITA,
    .ability = ABILITY_GUTS,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 15,
    .species = SPECIES_TEDDIURSA,
    .ability = ABILITY_GUTS,
    },
};

static const struct TrainerMon sParty_Tyron[] = {
    {
    .lvl = 19,
    .species = SPECIES_SANDSHREW,
    }
};

static const struct TrainerMon sParty_Celina[] = {
    {
    .lvl = 18,
    .species = SPECIES_ROSELIA,
    }
};

static const struct TrainerMon sParty_Bianca[] = {
    {
    .lvl = 18,
    .species = SPECIES_SHROOMISH,
    }
};

static const struct TrainerMon sParty_Hayden[] = {
    {
    .lvl = 18,
    .species = SPECIES_NUMEL,
    }
};

static const struct TrainerMon sParty_Sophie[] = {
    {
    .lvl = 27,
    .species = SPECIES_AZUMARILL,
    },
    {
    .lvl = 27,
    .species = SPECIES_LUDICOLO,
    }
};

static const struct TrainerMon sParty_Coby[] = {
    {
    .lvl = 26,
    .species = SPECIES_SKARMORY,
    },
    {
    .lvl = 27,
    .species = SPECIES_SWELLOW,
    }
};

static const struct TrainerMon sParty_Lawrence[] = {
    {
    .lvl = 24,
    .species = SPECIES_BALTOY,
    },
    {
    .lvl = 26,
    .species = SPECIES_STEELIX,
    }
};

static const struct TrainerMon sParty_Wyatt[] = {
    {
    .lvl = 28,
    .species = SPECIES_ARON,
    },
    {
    .lvl = 28,
    .species = SPECIES_STUNFISK_GALARIAN,
    }
};

static const struct TrainerMon sParty_Angelina[] = {
    {
    .lvl = 27,
    .species = SPECIES_LOMBRE,
    },
    {
    .lvl = 26,
    .species = SPECIES_AIPOM,
    }
};

static const struct TrainerMon sParty_Kai[] = {
    {
    .lvl = 25,
    .species = SPECIES_BARBOACH,
    }
};

static const struct TrainerMon sParty_Charlotte[] = {
    {
    .lvl = 25,
    .species = SPECIES_NUZLEAF,
    }
};

static const struct TrainerMon sParty_Deandre[] = {
    {
    .lvl = 19,
    .species = SPECIES_ZIGZAGOON,
    },
    {
    .lvl = 18,
    .species = SPECIES_ARON,
    },
    {
    .lvl = 18,
    .species = SPECIES_ELECTRIKE,
    }
};

static const struct TrainerMon sParty_GruntMagmaHideout1[] = {
    {
    .lvl = 50,
    .species = SPECIES_DELPHOX,
    },
    {
    .lvl = 50,
    .species = SPECIES_CHARIZARD,
    },
    {
    .lvl = 50,
    .species = SPECIES_VENUSAUR,
    }
};

static const struct TrainerMon sParty_GruntMagmaHideout2[] = {
    {
    .lvl = 49,
    .species = SPECIES_KLAWF,
    },
    {
    .lvl = 49,
    .species = SPECIES_RHYDON,
    },
    {
    .lvl = 49,
    .species = SPECIES_CHANDELURE,
    }
};

static const struct TrainerMon sParty_GruntMagmaHideout3[] = {
    {
    .lvl = 48,
    .species = SPECIES_KANGASKHAN,
    },
    {
    .lvl = 48,
    .species = SPECIES_TANGELA,
    .heldItem = ITEM_EVIOLITE,
    }
};

static const struct TrainerMon sParty_GruntMagmaHideout4[] = {
    {
    .lvl = 50,
    .species = SPECIES_CLAYDOL,
    },
    {
    .lvl = 49,
    .species = SPECIES_MUK,
    }
};

static const struct TrainerMon sParty_GruntMagmaHideout5[] = {
    {
    .lvl = 49,
    .species = SPECIES_GOLEM_ALOLAN,
    },
    {
    .lvl = 51,
    .species = SPECIES_DUGTRIO_ALOLAN,
    }
};

static const struct TrainerMon sParty_GruntMagmaHideout6[] = {
    {
    .lvl = 49,
    .species = SPECIES_LICKILICKY,
    },
    {
    .lvl = 49,
    .species = SPECIES_OBSTAGOON,
    },
    {
    .lvl = 51,
    .species = SPECIES_EXEGGUTOR_ALOLAN,
    }
};

static const struct TrainerMon sParty_GruntMagmaHideout7[] = {
    {
    .lvl = 53,
    .species = SPECIES_TOEDSCRUEL,
    }
};

static const struct TrainerMon sParty_GruntMagmaHideout8[] = {
    {
    .lvl = 49,
    .species = SPECIES_PERSIAN,
    },
    {
    .lvl = 51,
    .species = SPECIES_HOUNDOOM
    }
};

static const struct TrainerMon sParty_GruntMagmaHideout9[] = {
    {
    .lvl = 49,
    .species = SPECIES_AERODACTYL,
    },
    {
    .lvl = 50,
    .species = SPECIES_GOLURK,
    }
};

static const struct TrainerMon sParty_GruntMagmaHideout10[] = {
    {
    .lvl = 52,
    .species = SPECIES_GENGAR,
    },
    {
    .lvl = 52,
    .species = SPECIES_MISMAGIUS,
    },
    {
    .lvl = 52,
    .species = SPECIES_MIENSHAO,
    }
};

static const struct TrainerMon sParty_GruntMagmaHideout11[] = {
    {
    .lvl = 51,
    .species = SPECIES_JUMPLUFF,
    .ability = ABILITY_CHLOROPHYLL,
    },
    {
    .lvl = 51,
    .species = SPECIES_EXEGGUTOR,
    .ability = ABILITY_CHLOROPHYLL,
    }
};

static const struct TrainerMon sParty_GruntMagmaHideout12[] = {
    {
    .lvl = 52,
    .species = SPECIES_CAMERUPT,
    },
    {
    .lvl = 51,
    .species = SPECIES_DURANT,
    },
    {
    .lvl = 51,
    .species = SPECIES_QWILFISH,
    .moves = {MOVE_EXPLOSION, MOVE_SLUDGE_BOMB, MOVE_WATERFALL, MOVE_TOXIC}
    }
};

static const struct TrainerMon sParty_GruntMagmaHideout13[] = {
    {
    .lvl = 49,
    .species = SPECIES_TANGROWTH,
    },
    {
    .lvl = 49,
    .species = SPECIES_SHIFTRY,
    }
};

static const struct TrainerMon sParty_GruntMagmaHideout14[] = {
    {
    .lvl = 49,
    .species = SPECIES_TURTONATOR,
    .moves = {MOVE_DRAGON_PULSE, MOVE_FLAMETHROWER, MOVE_SHELL_TRAP, MOVE_RAPID_SPIN}
    }
};

static const struct TrainerMon sParty_GruntMagmaHideout15[] = {
    {
    .lvl = 52,
    .species = SPECIES_KROOKODILE,
    }
};

static const struct TrainerMon sParty_GruntMagmaHideout16[] = {
    {
    .lvl = 51,
    .species = SPECIES_SCOVILLAIN,
    },
    {
    .lvl = 52,
    .species = SPECIES_EXCADRILL,
    },
    {
    .lvl = 52,
    .species = SPECIES_TOXTRICITY,
    }
};

static const struct TrainerMon sParty_TabithaMagmaHideout[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 53,
    .species = SPECIES_CAMERUPT,
    .heldItem = ITEM_PASSHO_BERRY,
    .ability = ABILITY_SOLID_ROCK,
    .moves = {MOVE_EARTH_POWER, MOVE_HEAT_WAVE, MOVE_PROTECT, MOVE_SOLAR_BEAM}
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 54,
    .species = SPECIES_MAWILE,
    .heldItem = ITEM_MAWILITE,
    .moves = {MOVE_FAKE_OUT, MOVE_PLAY_ROUGH, MOVE_SUCKER_PUNCH, MOVE_IRON_HEAD}
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 53,
    .species = SPECIES_BELLOSSOM,
    .ability = ABILITY_CHLOROPHYLL,
    .heldItem = ITEM_GRASS_GEM,
    .moves = {MOVE_FIERY_DANCE, MOVE_MOONBLAST, MOVE_QUIVER_DANCE, MOVE_GIGA_DRAIN}
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 53,
    .species = SPECIES_CHARIZARD,
    .ability = ABILITY_SOLAR_POWER,
    .heldItem = ITEM_HEAVY_DUTY_BOOTS,
    .moves = {MOVE_FIRE_BLAST, MOVE_AIR_SLASH, MOVE_ROOST, MOVE_DRAGON_PULSE}
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 53,
    .species = SPECIES_CHERRIM,
    .heldItem = ITEM_LIFE_ORB,
    .moves = {MOVE_SOLAR_BEAM, MOVE_GROWTH, MOVE_SYNTHESIS, MOVE_WEATHER_BALL}
    }
};

static const struct TrainerMon sParty_Darcy[] = {
    {
    .lvl = 37,
    .species = SPECIES_PELIPPER,
    },
    {
    .lvl = 39,
    .species = SPECIES_CAMERUPT,
    }
};

static const struct TrainerMon sParty_MaxieMossdeep[] = {
    {
    .iv = TRAINER_PARTY_IVS(18, 18, 18, 18, 18, 18),
    .lvl = 62,
    .species = SPECIES_NINETALES,
    .ability = ABILITY_DROUGHT,
    .heldItem = ITEM_HEAT_ROCK,
    .moves = {MOVE_SOLAR_BEAM, MOVE_HEAT_WAVE, MOVE_HELPING_HAND, MOVE_WILL_O_WISP}
    },
    {
    .iv = TRAINER_PARTY_IVS(18, 18, 18, 18, 18, 18),
    .lvl = 62,
    .species = SPECIES_VOLCANION,
    .moves = {MOVE_SLUDGE_BOMB, MOVE_FLAMETHROWER, MOVE_STEAM_ERUPTION, MOVE_WILL_O_WISP}
    },
    {
    .iv = TRAINER_PARTY_IVS(18, 18, 18, 18, 18, 18),
    .lvl = 62,
    .species = SPECIES_CAMERUPT,
    .heldItem = ITEM_CAMERUPTITE,
    .moves = {MOVE_EARTH_POWER, MOVE_POWER_GEM, MOVE_HEAT_WAVE, MOVE_PROTECT}
    }
};

static const struct TrainerMon sParty_Pete[] = {
    {
    .lvl = 34,
    .species = SPECIES_TENTACRUEL,
    .moves = {MOVE_HYDRO_PUMP, MOVE_SLUDGE_WAVE, MOVE_BARRIER, MOVE_PROTECT}
    },

};

static const struct TrainerMon sParty_Isabelle[] = {
    {
    .lvl = 36,
    .species = SPECIES_EMPOLEON,
    .moves = {MOVE_WATER_PULSE, MOVE_FLASH_CANNON, MOVE_GRASS_KNOT, MOVE_PROTECT}
    }
};

static const struct TrainerMon sParty_Andres1[] = {
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 35,
    .species = SPECIES_SANDSHREW,
    },
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 34,
    .species = SPECIES_NIDOQUEEN,
    }
};

static const struct TrainerMon sParty_Josue[] = {
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 37,
    .species = SPECIES_SWANNA,
    },
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 36,
    .species = SPECIES_PELIPPER,
    }
};

static const struct TrainerMon sParty_Camron[] = {
    {
    .lvl = 32,
    .species = SPECIES_STARMIE,
    }
};

static const struct TrainerMon sParty_Cory1[] = {
    {
    .lvl = 32,
    .species = SPECIES_PELIPPER,
    },
    {
    .lvl = 32,
    .species = SPECIES_MACHOKE,
    },
    {
    .lvl = 34,
    .species = SPECIES_LUMINEON,
    }
};

static const struct TrainerMon sParty_Carolina[] = {
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 35,
    .species = SPECIES_MANECTRIC,
    },
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 33,
    .species = SPECIES_SWELLOW,
    },
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 35,
    .species = SPECIES_MANECTRIC,
    }
};

static const struct TrainerMon sParty_Elijah[] = {
    {
    .lvl = 34,
    .species = SPECIES_SKARMORY,
    },
    {
    .lvl = 36,
    .species = SPECIES_CORVIKNIGHT,
    }
};

static const struct TrainerMon sParty_Celia[] = {
    {
    .lvl = 26,
    .species = SPECIES_SIMIPOUR,
    },
    {
    .lvl = 27,
    .species = SPECIES_LOMBRE,
    }
};

static const struct TrainerMon sParty_Bryan[] = {
    {
    .lvl = 23,
    .species = SPECIES_SANDSHREW,
    },
    {
    .lvl = 26,
    .species = SPECIES_SANDSLASH,
    }
};

static const struct TrainerMon sParty_Branden[] = {
    {
    .lvl = 23,
    .species = SPECIES_CORVISQUIRE,
    },
    {
    .lvl = 23,
    .species = SPECIES_NUZLEAF,
    }
};

static const struct TrainerMon sParty_Bryant[] = {
    {
    .lvl = 26,
    .species = SPECIES_TORKOAL,
    .ability = ABILITY_DROUGHT
    },
    {
    .lvl = 25,
    .species = SPECIES_SOLROCK,
    }
};

static const struct TrainerMon sParty_Shayla[] = {
    {
    .lvl = 26,
    .species = SPECIES_LEAVANNY,
    },
    {
    .lvl = 25,
    .species = SPECIES_ROSELIA,
    }
};

static const struct TrainerMon sParty_Kyra[] = {
    {
    .lvl = 26,
    .species = SPECIES_DODUO,
    },
    {
    .lvl = 28,
    .species = SPECIES_DODRIO,
    }
};

static const struct TrainerMon sParty_Jaiden[] = {
    {
    .lvl = 26,
    .species = SPECIES_NINJASK,
    },
    {
    .lvl = 26,
    .species = SPECIES_GULPIN,
    }
};

static const struct TrainerMon sParty_Alix[] = {
    {
    .lvl = 26,
    .species = SPECIES_KADABRA,
    },
    {
    .lvl = 26,
    .species = SPECIES_KIRLIA,
    }
};

static const struct TrainerMon sParty_Helene[] = {
    {
    .lvl = 26,
    .species = SPECIES_MEDITITE,
    },
    {
    .lvl = 26,
    .species = SPECIES_MAKUHITA,
    }
};

static const struct TrainerMon sParty_Marlene[] = {
    {
    .lvl = 28,
    .species = SPECIES_MEDITITE,
    },
    {
    .lvl = 28,
    .species = SPECIES_GRUMPIG,
    }
};

static const struct TrainerMon sParty_Devan[] = {
    {
    .lvl = 10,
    .species = SPECIES_GEODUDE,
    },
    {
    .lvl = 12,
    .species = SPECIES_ROGGENROLA,
    }
};

static const struct TrainerMon sParty_Johnson[] = {
    {
    .lvl = 10,
    .species = SPECIES_SHROOMISH,
    },
    {
    .lvl = 11,
    .species = SPECIES_LOTAD,
    }
};

static const struct TrainerMon sParty_Melina[] = {
    {
    .lvl = 17,
    .species = SPECIES_DODUO,
    }
};

static const struct TrainerMon sParty_Brandi[] = {
    {
    .lvl = 17,
    .species = SPECIES_KIRLIA,
    }
};

static const struct TrainerMon sParty_Aisha[] = {
    {
    .lvl = 17,
    .species = SPECIES_MEDITITE,
    }
};

static const struct TrainerMon sParty_Makayla[] = {
    {
    .lvl = 33,
    .species = SPECIES_ROSELIA,
    },
    {
    .lvl = 33,
    .species = SPECIES_MEDICHAM,
    }
};

static const struct TrainerMon sParty_Fabian[] = {
    {
    .lvl = 36,
    .species = SPECIES_MANECTRIC,
    }
};

static const struct TrainerMon sParty_Dayton[] = {
    {
    .lvl = 35,
    .species = SPECIES_MAGCARGO,
    },
    {
    .lvl = 35,
    .species = SPECIES_COMBUSKEN,
    }
};

static const struct TrainerMon sParty_Rachel[] = {
    {
    .lvl = 38,
    .species = SPECIES_SEAKING,
    }
};

static const struct TrainerMon sParty_Leonel[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 42,
    .species = SPECIES_MANECTRIC,
    .moves = {MOVE_THUNDER, MOVE_QUICK_ATTACK, MOVE_THUNDER_WAVE, MOVE_FLAMETHROWER}
    }
};

static const struct TrainerMon sParty_Callie[] = {
    {
    .lvl = 41,
    .species = SPECIES_MEDICHAM,
    },
    {
    .lvl = 42,
    .species = SPECIES_GARBODOR,
    }
};

static const struct TrainerMon sParty_Cale[] = {
    {
    .lvl = 43,
    .species = SPECIES_DUSTOX,
    },
    {
    .lvl = 43,
    .species = SPECIES_BEAUTIFLY,
    .ability = ABILITY_WIND_RIDER,
    }
};

static const struct TrainerMon sParty_Myles[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 45,
    .species = SPECIES_HARIYAMA,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 46,
    .species = SPECIES_CRAMORANT,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 46,
    .species = SPECIES_TROPIUS,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 46,
    .species = SPECIES_DUBWOOL,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 46,
    .species = SPECIES_BOLTUND,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 47,
    .species = SPECIES_CAMERUPT,
    }
};

static const struct TrainerMon sParty_Pat[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 45,
    .species = SPECIES_MIGHTYENA,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 45,
    .species = SPECIES_BRELOOM,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 45,
    .species = SPECIES_MANECTRIC,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 46,
    .species = SPECIES_AZUMARILL,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 46,
    .species = SPECIES_SANDSLASH,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 45,
    .species = SPECIES_GRUMPIG,
    }
};

static const struct TrainerMon sParty_Cristin1[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 46,
    .species = SPECIES_EXPLOUD,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 46,
    .species = SPECIES_SLAKING,
    }
};

static const struct TrainerMon sParty_MayRustboroTreecko[] = {
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 15,
    .species = SPECIES_TAILLOW,
    },
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 16,
    .species = SPECIES_COMBUSKEN,
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 15,
    .species = SPECIES_POLIWAG,
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 15,
    .species = SPECIES_TANGELA,
    }
};

static const struct TrainerMon sParty_MayRustboroTorchic[] = {
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 15,
    .species = SPECIES_TAILLOW,
    },
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 16,
    .species = SPECIES_MARSHTOMP,
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 15,
    .species = SPECIES_TANGELA,
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 15,
    .species = SPECIES_LITWICK,
    }
};

static const struct TrainerMon sParty_Roxanne2[] = {
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_GLIMMORA,
    .heldItem = ITEM_FOCUS_SASH,
    .moves = {MOVE_MORTAL_SPIN, MOVE_STEALTH_ROCK, MOVE_EARTH_POWER, MOVE_SLUDGE_BOMB}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_GOLEM,
    .heldItem = ITEM_WEAKNESS_POLICY,
    .ability = ABILITY_STURDY,
    .moves = {MOVE_ROCK_POLISH, MOVE_ROCK_BLAST, MOVE_EARTHQUAKE, MOVE_EXPLOSION}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_KABUTOPS,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_SWORDS_DANCE, MOVE_X_SCISSOR, MOVE_LIQUIDATION, MOVE_ROCK_SLIDE}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 82,
    .species = SPECIES_STEELIX,
    .heldItem = ITEM_STEELIXITE,
    .moves = {MOVE_GYRO_BALL, MOVE_IRON_DEFENSE, MOVE_BODY_PRESS, MOVE_REST}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_TYRANITAR,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_DRAGON_DANCE, MOVE_EARTHQUAKE, MOVE_CRUNCH, MOVE_ROCK_SLIDE}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_TERRAKION,
    .heldItem = ITEM_LIFE_ORB,
    .moves = {MOVE_SWORDS_DANCE, MOVE_EARTHQUAKE, MOVE_CLOSE_COMBAT, MOVE_ROCK_SLIDE}
    },
};

static const struct TrainerMon sParty_Roxanne3[] = {
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_GLIMMORA,
    .heldItem = ITEM_FOCUS_SASH,
    .moves = {MOVE_MORTAL_SPIN, MOVE_STEALTH_ROCK, MOVE_EARTH_POWER, MOVE_SLUDGE_BOMB}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_GLISCOR,
    .heldItem = ITEM_TOXIC_ORB,
    .ability = ABILITY_POISON_HEAL,
    .moves = {MOVE_PROTECT, MOVE_KNOCK_OFF, MOVE_SWORDS_DANCE, MOVE_FACADE}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_STAKATAKA,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_TRICK_ROOM, MOVE_GYRO_BALL, MOVE_HEAT_CRASH, MOVE_STONE_EDGE}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 82,
    .species = SPECIES_STEELIX,
    .heldItem = ITEM_STEELIXITE,
    .moves = {MOVE_GYRO_BALL, MOVE_IRON_DEFENSE, MOVE_BODY_PRESS, MOVE_REST}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_TYRANITAR,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_DRAGON_DANCE, MOVE_EARTHQUAKE, MOVE_CRUNCH, MOVE_ROCK_SLIDE}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_TERRAKION,
    .heldItem = ITEM_LIFE_ORB,
    .moves = {MOVE_SWORDS_DANCE, MOVE_EARTHQUAKE, MOVE_CLOSE_COMBAT, MOVE_ROCK_SLIDE}
    },
};

static const struct TrainerMon sParty_Roxanne4[] = {
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_GLIMMORA,
    .heldItem = ITEM_FOCUS_SASH,
    .moves = {MOVE_MORTAL_SPIN, MOVE_STEALTH_ROCK, MOVE_EARTH_POWER, MOVE_SLUDGE_BOMB}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_GLISCOR,
    .heldItem = ITEM_TOXIC_ORB,
    .ability = ABILITY_POISON_HEAL,
    .moves = {MOVE_PROTECT, MOVE_KNOCK_OFF, MOVE_SWORDS_DANCE, MOVE_FACADE}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_STAKATAKA,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_TRICK_ROOM, MOVE_GYRO_BALL, MOVE_HEAT_CRASH, MOVE_STONE_EDGE}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 82,
    .species = SPECIES_STEELIX,
    .heldItem = ITEM_STEELIXITE,
    .moves = {MOVE_GYRO_BALL, MOVE_IRON_DEFENSE, MOVE_BODY_PRESS, MOVE_REST}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_TYRANITAR,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_DRAGON_DANCE, MOVE_EARTHQUAKE, MOVE_CRUNCH, MOVE_ROCK_SLIDE}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_TERRAKION,
    .heldItem = ITEM_LIFE_ORB,
    .moves = {MOVE_SWORDS_DANCE, MOVE_EARTHQUAKE, MOVE_CLOSE_COMBAT, MOVE_ROCK_SLIDE}
    },
};

static const struct TrainerMon sParty_Roxanne5[] = {
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_GLIMMORA,
    .heldItem = ITEM_FOCUS_SASH,
    .moves = {MOVE_MORTAL_SPIN, MOVE_STEALTH_ROCK, MOVE_EARTH_POWER, MOVE_SLUDGE_BOMB}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_GLISCOR,
    .heldItem = ITEM_TOXIC_ORB,
    .ability = ABILITY_POISON_HEAL,
    .moves = {MOVE_PROTECT, MOVE_KNOCK_OFF, MOVE_SWORDS_DANCE, MOVE_FACADE}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_STAKATAKA,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_TRICK_ROOM, MOVE_GYRO_BALL, MOVE_HEAT_CRASH, MOVE_STONE_EDGE}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 82,
    .species = SPECIES_STEELIX,
    .heldItem = ITEM_STEELIXITE,
    .isShiny = TRUE,
    .moves = {MOVE_GYRO_BALL, MOVE_IRON_DEFENSE, MOVE_BODY_PRESS, MOVE_REST}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_TYRANITAR,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_DRAGON_DANCE, MOVE_EARTHQUAKE, MOVE_CRUNCH, MOVE_ROCK_SLIDE}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_TERRAKION,
    .heldItem = ITEM_LIFE_ORB,
    .moves = {MOVE_SWORDS_DANCE, MOVE_EARTHQUAKE, MOVE_CLOSE_COMBAT, MOVE_ROCK_SLIDE}
    },
};

static const struct TrainerMon sParty_Brawly2[] = {
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_MACHAMP,
    .heldItem = ITEM_SITRUS_BERRY,
    .ability = ABILITY_NO_GUARD,
    .moves = {MOVE_DYNAMIC_PUNCH, MOVE_STONE_EDGE, MOVE_KNOCK_OFF, MOVE_BULK_UP}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_HITMONCHAN,
    .heldItem = ITEM_PUNCHING_GLOVE,
    .ability = ABILITY_IRON_FIST,
    .moves = {MOVE_CLOSE_COMBAT, MOVE_FIRE_PUNCH, MOVE_ICE_PUNCH, MOVE_THUNDER_PUNCH}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_HITMONTOP,
    .heldItem = ITEM_NONE,
    .ability = ABILITY_INTIMIDATE,
    .moves = {MOVE_FAKE_OUT, MOVE_HELPING_HAND, MOVE_ROCK_SLIDE, MOVE_TRIPLE_KICK}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_HARIYAMA,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_FAKE_OUT, MOVE_BULLET_PUNCH, MOVE_BELLY_DRUM, MOVE_DRAIN_PUNCH}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_BLAZIKEN,
    .heldItem = ITEM_LIFE_ORB,
    .ability = ABILITY_SPEED_BOOST,
    .moves = {MOVE_PROTECT, MOVE_CLOSE_COMBAT, MOVE_FLARE_BLITZ, MOVE_THUNDER_PUNCH}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 82,
    .species = SPECIES_MEDICHAM,
    .heldItem = ITEM_MEDICHAMITE,
    .isShiny = TRUE,
    .moves = {MOVE_FAKE_OUT, MOVE_HI_JUMP_KICK, MOVE_PSYCHO_CUT, MOVE_BULLET_PUNCH}
    }
};

static const struct TrainerMon sParty_Brawly3[] = {
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_MACHAMP,
    .heldItem = ITEM_SITRUS_BERRY,
    .ability = ABILITY_NO_GUARD,
    .moves = {MOVE_DYNAMIC_PUNCH, MOVE_STONE_EDGE, MOVE_KNOCK_OFF, MOVE_BULK_UP}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_KELDEO,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_CALM_MIND, MOVE_HYDRO_PUMP, MOVE_FOCUS_BLAST, MOVE_BATON_PASS}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_SNEASLER,
    .heldItem = ITEM_NORMAL_GEM,
    .ability = ABILITY_UNBURDEN,
    .moves = {MOVE_FAKE_OUT, MOVE_DIRE_CLAW, MOVE_CLOSE_COMBAT, MOVE_SWORDS_DANCE}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_IRON_HANDS,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_SWORDS_DANCE, MOVE_THUNDER_PUNCH, MOVE_DRAIN_PUNCH, MOVE_ICE_PUNCH}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_BLAZIKEN,
    .heldItem = ITEM_LIFE_ORB,
    .ability = ABILITY_SPEED_BOOST,
    .moves = {MOVE_PROTECT, MOVE_CLOSE_COMBAT, MOVE_FLARE_BLITZ, MOVE_THUNDER_PUNCH}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 82,
    .species = SPECIES_MEDICHAM,
    .heldItem = ITEM_MEDICHAMITE,
    .isShiny = TRUE,
    .moves = {MOVE_FAKE_OUT, MOVE_HI_JUMP_KICK, MOVE_PSYCHO_CUT, MOVE_BULLET_PUNCH}
    }
};

static const struct TrainerMon sParty_Brawly4[] = {
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_MACHAMP,
    .heldItem = ITEM_SITRUS_BERRY,
    .ability = ABILITY_NO_GUARD,
    .moves = {MOVE_DYNAMIC_PUNCH, MOVE_STONE_EDGE, MOVE_KNOCK_OFF, MOVE_BULK_UP}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_KELDEO,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_CALM_MIND, MOVE_HYDRO_PUMP, MOVE_FOCUS_BLAST, MOVE_BATON_PASS}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_SNEASLER,
    .heldItem = ITEM_NORMAL_GEM,
    .ability = ABILITY_UNBURDEN,
    .moves = {MOVE_FAKE_OUT, MOVE_DIRE_CLAW, MOVE_CLOSE_COMBAT, MOVE_SWORDS_DANCE}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_IRON_HANDS,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_SWORDS_DANCE, MOVE_THUNDER_PUNCH, MOVE_DRAIN_PUNCH, MOVE_ICE_PUNCH}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_BLAZIKEN,
    .heldItem = ITEM_LIFE_ORB,
    .ability = ABILITY_SPEED_BOOST,
    .moves = {MOVE_PROTECT, MOVE_CLOSE_COMBAT, MOVE_FLARE_BLITZ, MOVE_THUNDER_PUNCH}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 82,
    .species = SPECIES_MEDICHAM,
    .heldItem = ITEM_MEDICHAMITE,
    .isShiny = TRUE,
    .moves = {MOVE_FAKE_OUT, MOVE_HI_JUMP_KICK, MOVE_PSYCHO_CUT, MOVE_BULLET_PUNCH}
    }
};

static const struct TrainerMon sParty_Brawly5[] = {
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_MACHAMP,
    .heldItem = ITEM_SITRUS_BERRY,
    .ability = ABILITY_NO_GUARD,
    .moves = {MOVE_DYNAMIC_PUNCH, MOVE_STONE_EDGE, MOVE_KNOCK_OFF, MOVE_BULK_UP}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_KELDEO,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_CALM_MIND, MOVE_HYDRO_PUMP, MOVE_FOCUS_BLAST, MOVE_BATON_PASS}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_SNEASLER,
    .heldItem = ITEM_NORMAL_GEM,
    .ability = ABILITY_UNBURDEN,
    .moves = {MOVE_FAKE_OUT, MOVE_DIRE_CLAW, MOVE_CLOSE_COMBAT, MOVE_SWORDS_DANCE}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_IRON_HANDS,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_SWORDS_DANCE, MOVE_THUNDER_PUNCH, MOVE_DRAIN_PUNCH, MOVE_ICE_PUNCH}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_BLAZIKEN,
    .heldItem = ITEM_LIFE_ORB,
    .ability = ABILITY_SPEED_BOOST,
    .moves = {MOVE_PROTECT, MOVE_CLOSE_COMBAT, MOVE_FLARE_BLITZ, MOVE_THUNDER_PUNCH}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 82,
    .species = SPECIES_MEDICHAM,
    .heldItem = ITEM_MEDICHAMITE,
    .isShiny = TRUE,
    .moves = {MOVE_FAKE_OUT, MOVE_HI_JUMP_KICK, MOVE_PSYCHO_CUT, MOVE_BULLET_PUNCH}
    }
};

static const struct TrainerMon sParty_Wattson2[] = {
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_PINCURCHIN,
    .heldItem = ITEM_FOCUS_SASH,
    .ability = ABILITY_ELECTRIC_SURGE,
    .moves = {MOVE_TOXIC_SPIKES, MOVE_SPIKES, MOVE_RISING_VOLTAGE, MOVE_SCALD}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_RAICHU_ALOLAN,
    .heldItem = ITEM_LIFE_ORB,
    .ability = ABILITY_SURGE_SURFER,
    .moves = {MOVE_NASTY_PLOT, MOVE_SURF, MOVE_PSYCHIC, MOVE_RISING_VOLTAGE}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_ROTOM_WASH,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_HYDRO_PUMP, MOVE_VOLT_SWITCH, MOVE_WILL_O_WISP, MOVE_HEX}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_TOXTRICITY,
    .heldItem = ITEM_THROAT_SPRAY,
    .ability = ABILITY_PUNK_ROCK,
    .moves = {MOVE_HYPER_VOICE, MOVE_OVERDRIVE, MOVE_SHIFT_GEAR, MOVE_TORCH_SONG}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_DRIFBLIM,
    .heldItem = ITEM_ELECTRIC_SEED,
    .ability = ABILITY_UNBURDEN,
    .moves = {MOVE_THUNDERBOLT, MOVE_SHADOW_BALL, MOVE_CALM_MIND, MOVE_STRENGTH_SAP}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 82,
    .species = SPECIES_MANECTRIC,
    .heldItem = ITEM_MANECTITE,
    .isShiny = TRUE,
    .moves = {MOVE_ELECTRO_DRIFT, MOVE_FLAMETHROWER, MOVE_VOLT_SWITCH, MOVE_THUNDER_WAVE}
    }
};

static const struct TrainerMon sParty_Wattson3[] = {
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_TAPU_KOKO,
    .heldItem = ITEM_LIFE_ORB,
    .ability = ABILITY_ELECTRIC_SURGE,
    .moves = {MOVE_BRAVE_BIRD, MOVE_WILD_CHARGE, MOVE_DAZZLING_GLEAM, MOVE_U_TURN}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_ROTOM_WASH,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_HYDRO_PUMP, MOVE_VOLT_SWITCH, MOVE_WILL_O_WISP, MOVE_HEX}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_RAIKOU,
    .heldItem = ITEM_LIFE_ORB,
    .moves = {MOVE_CALM_MIND, MOVE_SCALD, MOVE_AURA_SPHERE, MOVE_THUNDER_CAGE}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_TOXTRICITY,
    .heldItem = ITEM_THROAT_SPRAY,
    .ability = ABILITY_PUNK_ROCK,
    .moves = {MOVE_HYPER_VOICE, MOVE_OVERDRIVE, MOVE_SHIFT_GEAR, MOVE_TORCH_SONG}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_DRIFBLIM,
    .heldItem = ITEM_ELECTRIC_SEED,
    .ability = ABILITY_UNBURDEN,
    .moves = {MOVE_THUNDERBOLT, MOVE_SHADOW_BALL, MOVE_CALM_MIND, MOVE_STRENGTH_SAP}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 82,
    .species = SPECIES_MANECTRIC,
    .heldItem = ITEM_MANECTITE,
    .isShiny = TRUE,
    .moves = {MOVE_ELECTRO_DRIFT, MOVE_FLAMETHROWER, MOVE_VOLT_SWITCH, MOVE_THUNDER_WAVE}
    }
};

static const struct TrainerMon sParty_Wattson4[] = {
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_TAPU_KOKO,
    .heldItem = ITEM_LIFE_ORB,
    .ability = ABILITY_ELECTRIC_SURGE,
    .moves = {MOVE_BRAVE_BIRD, MOVE_WILD_CHARGE, MOVE_DAZZLING_GLEAM, MOVE_U_TURN}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_ROTOM_WASH,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_HYDRO_PUMP, MOVE_VOLT_SWITCH, MOVE_WILL_O_WISP, MOVE_HEX}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_RAIKOU,
    .heldItem = ITEM_LIFE_ORB,
    .moves = {MOVE_CALM_MIND, MOVE_SCALD, MOVE_AURA_SPHERE, MOVE_THUNDER_CAGE}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_TOXTRICITY,
    .heldItem = ITEM_THROAT_SPRAY,
    .ability = ABILITY_PUNK_ROCK,
    .moves = {MOVE_HYPER_VOICE, MOVE_OVERDRIVE, MOVE_SHIFT_GEAR, MOVE_TORCH_SONG}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_DRIFBLIM,
    .heldItem = ITEM_ELECTRIC_SEED,
    .ability = ABILITY_UNBURDEN,
    .moves = {MOVE_THUNDERBOLT, MOVE_SHADOW_BALL, MOVE_CALM_MIND, MOVE_STRENGTH_SAP}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 82,
    .species = SPECIES_MANECTRIC,
    .heldItem = ITEM_MANECTITE,
    .isShiny = TRUE,
    .moves = {MOVE_ELECTRO_DRIFT, MOVE_FLAMETHROWER, MOVE_VOLT_SWITCH, MOVE_THUNDER_WAVE}
    }
};

static const struct TrainerMon sParty_Wattson5[] = {
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_TAPU_KOKO,
    .heldItem = ITEM_LIFE_ORB,
    .ability = ABILITY_ELECTRIC_SURGE,
    .moves = {MOVE_BRAVE_BIRD, MOVE_WILD_CHARGE, MOVE_DAZZLING_GLEAM, MOVE_U_TURN}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_ROTOM_WASH,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_HYDRO_PUMP, MOVE_VOLT_SWITCH, MOVE_WILL_O_WISP, MOVE_HEX}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_RAIKOU,
    .heldItem = ITEM_LIFE_ORB,
    .moves = {MOVE_CALM_MIND, MOVE_SCALD, MOVE_AURA_SPHERE, MOVE_THUNDER_CAGE}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_TOXTRICITY,
    .heldItem = ITEM_THROAT_SPRAY,
    .ability = ABILITY_PUNK_ROCK,
    .moves = {MOVE_BOOMBURST, MOVE_OVERDRIVE, MOVE_SHIFT_GEAR, MOVE_TORCH_SONG}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_DRIFBLIM,
    .heldItem = ITEM_ELECTRIC_SEED,
    .ability = ABILITY_UNBURDEN,
    .moves = {MOVE_THUNDERBOLT, MOVE_SHADOW_BALL, MOVE_CALM_MIND, MOVE_STRENGTH_SAP}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 82,
    .species = SPECIES_MANECTRIC,
    .heldItem = ITEM_MANECTITE,
    .isShiny = TRUE,
    .moves = {MOVE_ELECTRO_DRIFT, MOVE_FLAMETHROWER, MOVE_VOLT_SWITCH, MOVE_THUNDER_WAVE}
    }
};

static const struct TrainerMon sParty_Flannery2[] = {
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_TORKOAL,
    .heldItem = ITEM_HEAT_ROCK,
    .ability = ABILITY_DROUGHT,
    .moves = {MOVE_STEALTH_ROCK, MOVE_HEAT_WAVE, MOVE_RAPID_SPIN, MOVE_YAWN}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_VENUSAUR,
    .heldItem = ITEM_LIFE_ORB,
    .ability = ABILITY_CHLOROPHYLL,
    .moves = {MOVE_GROWTH, MOVE_WEATHER_BALL, MOVE_SLUDGE_BOMB, MOVE_EARTH_POWER}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_SCOVILLAIN,
    .heldItem = ITEM_WHITE_HERB,
    .ability = ABILITY_CHLOROPHYLL,
    .moves = {MOVE_GROWTH, MOVE_LEAF_STORM, MOVE_OVERHEAT, MOVE_STOMPING_TANTRUM}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_LILLIGANT_HISUIAN,
    .heldItem = ITEM_FOCUS_SASH,
    .ability = ABILITY_CHLOROPHYLL,
    .moves = {MOVE_VICTORY_DANCE, MOVE_CLOSE_COMBAT, MOVE_TRIPLE_AXEL, MOVE_SOLAR_BLADE}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_HEATRAN,
    .heldItem = ITEM_AIR_BALLOON,
    .ability = ABILITY_FLASH_FIRE,
    .moves = {MOVE_MAGMA_STORM, MOVE_EARTH_POWER, MOVE_FLASH_CANNON, MOVE_SOLAR_BEAM}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 82,
    .species = SPECIES_CHARIZARD,
    .heldItem = ITEM_CHARIZARDITE_Y,
    .isShiny = TRUE,
    .ability = ABILITY_DROUGHT,
    .moves = {MOVE_WEATHER_BALL, MOVE_SOLAR_BEAM, MOVE_FOCUS_BLAST, MOVE_SCORCHING_SANDS}
    }
};

static const struct TrainerMon sParty_Flannery3[] = {
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_TORKOAL,
    .heldItem = ITEM_HEAT_ROCK,
    .ability = ABILITY_DROUGHT,
    .moves = {MOVE_STEALTH_ROCK, MOVE_HEAT_WAVE, MOVE_RAPID_SPIN, MOVE_YAWN}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_VENUSAUR,
    .heldItem = ITEM_LIFE_ORB,
    .ability = ABILITY_CHLOROPHYLL,
    .moves = {MOVE_GROWTH, MOVE_WEATHER_BALL, MOVE_SLUDGE_BOMB, MOVE_EARTH_POWER}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_WALKING_WAKE,
    .heldItem = ITEM_CHOICE_SPECS,
    .ability = ABILITY_PROTOSYNTHESIS,
    .moves = {MOVE_HYDRO_STEAM, MOVE_DRACO_METEOR, MOVE_FLAMETHROWER, MOVE_DRAGON_PULSE}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_LILLIGANT_HISUIAN,
    .heldItem = ITEM_FOCUS_SASH,
    .ability = ABILITY_CHLOROPHYLL,
    .moves = {MOVE_VICTORY_DANCE, MOVE_CLOSE_COMBAT, MOVE_TRIPLE_AXEL, MOVE_SOLAR_BLADE}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_HEATRAN,
    .heldItem = ITEM_AIR_BALLOON,
    .ability = ABILITY_FLASH_FIRE,
    .moves = {MOVE_MAGMA_STORM, MOVE_EARTH_POWER, MOVE_FLASH_CANNON, MOVE_SOLAR_BEAM}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 82,
    .species = SPECIES_CHARIZARD,
    .heldItem = ITEM_CHARIZARDITE_Y,
    .isShiny = TRUE,
    .ability = ABILITY_DROUGHT,
    .moves = {MOVE_WEATHER_BALL, MOVE_SOLAR_BEAM, MOVE_FOCUS_BLAST, MOVE_SCORCHING_SANDS}
    }
};

static const struct TrainerMon sParty_Flannery4[] = {
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_TORKOAL,
    .heldItem = ITEM_HEAT_ROCK,
    .ability = ABILITY_DROUGHT,
    .moves = {MOVE_STEALTH_ROCK, MOVE_HEAT_WAVE, MOVE_RAPID_SPIN, MOVE_YAWN}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_VENUSAUR,
    .heldItem = ITEM_LIFE_ORB,
    .ability = ABILITY_CHLOROPHYLL,
    .moves = {MOVE_GROWTH, MOVE_WEATHER_BALL, MOVE_SLUDGE_BOMB, MOVE_EARTH_POWER}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_WALKING_WAKE,
    .heldItem = ITEM_CHOICE_SPECS,
    .ability = ABILITY_PROTOSYNTHESIS,
    .moves = {MOVE_HYDRO_STEAM, MOVE_DRACO_METEOR, MOVE_FLAMETHROWER, MOVE_DRAGON_PULSE}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_LILLIGANT_HISUIAN,
    .heldItem = ITEM_FOCUS_SASH,
    .ability = ABILITY_CHLOROPHYLL,
    .moves = {MOVE_VICTORY_DANCE, MOVE_CLOSE_COMBAT, MOVE_TRIPLE_AXEL, MOVE_SOLAR_BLADE}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_HEATRAN,
    .heldItem = ITEM_AIR_BALLOON,
    .ability = ABILITY_FLASH_FIRE,
    .moves = {MOVE_MAGMA_STORM, MOVE_EARTH_POWER, MOVE_FLASH_CANNON, MOVE_SOLAR_BEAM}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 82,
    .species = SPECIES_CHARIZARD,
    .heldItem = ITEM_CHARIZARDITE_Y,
    .isShiny = TRUE,
    .ability = ABILITY_DROUGHT,
    .moves = {MOVE_WEATHER_BALL, MOVE_SOLAR_BEAM, MOVE_FOCUS_BLAST, MOVE_SCORCHING_SANDS}
    }
};

static const struct TrainerMon sParty_Flannery5[] = {
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_TORKOAL,
    .heldItem = ITEM_HEAT_ROCK,
    .ability = ABILITY_DROUGHT,
    .moves = {MOVE_STEALTH_ROCK, MOVE_HEAT_WAVE, MOVE_RAPID_SPIN, MOVE_YAWN}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_VENUSAUR,
    .heldItem = ITEM_LIFE_ORB,
    .ability = ABILITY_CHLOROPHYLL,
    .moves = {MOVE_GROWTH, MOVE_WEATHER_BALL, MOVE_SLUDGE_BOMB, MOVE_EARTH_POWER}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_WALKING_WAKE,
    .heldItem = ITEM_CHOICE_SPECS,
    .ability = ABILITY_PROTOSYNTHESIS,
    .moves = {MOVE_HYDRO_STEAM, MOVE_DRACO_METEOR, MOVE_FLAMETHROWER, MOVE_DRAGON_PULSE}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_LILLIGANT_HISUIAN,
    .heldItem = ITEM_FOCUS_SASH,
    .ability = ABILITY_CHLOROPHYLL,
    .moves = {MOVE_VICTORY_DANCE, MOVE_CLOSE_COMBAT, MOVE_TRIPLE_AXEL, MOVE_SOLAR_BLADE}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_HEATRAN,
    .heldItem = ITEM_AIR_BALLOON,
    .ability = ABILITY_FLASH_FIRE,
    .moves = {MOVE_MAGMA_STORM, MOVE_EARTH_POWER, MOVE_FLASH_CANNON, MOVE_SOLAR_BEAM}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 82,
    .species = SPECIES_CHARIZARD,
    .heldItem = ITEM_CHARIZARDITE_Y,
    .isShiny = TRUE,
    .ability = ABILITY_DROUGHT,
    .moves = {MOVE_WEATHER_BALL, MOVE_SOLAR_BEAM, MOVE_FOCUS_BLAST, MOVE_SCORCHING_SANDS}
    }
};

static const struct TrainerMon sParty_Norman2[] = {
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_SLAKING,
    .heldItem = ITEM_LIFE_ORB,
    .ability = ABILITY_SLOW_START,
    .moves = {MOVE_DOUBLE_EDGE, MOVE_ROCK_SLIDE, MOVE_SUCKER_PUNCH, MOVE_PROTECT}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_WEEZING,
    .heldItem = ITEM_ROCKY_HELMET,
    .ability = ABILITY_NEUTRALIZING_GAS,
    .moves = {MOVE_WILL_O_WISP, MOVE_PROTECT,MOVE_SLUDGE_BOMB, MOVE_HEAT_WAVE}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_ZOROARK_HISUIAN,
    .heldItem = ITEM_SILK_SCARF,
    .ability = ABILITY_ILLUSION,
    .moves = {MOVE_TAUNT, MOVE_BITTER_MALICE, MOVE_HYPER_VOICE, MOVE_NASTY_PLOT}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_URSALUNA,
    .heldItem = ITEM_CHOPLE_BERRY,
    .ability = ABILITY_MINDS_EYE,
    .moves = {MOVE_BLOOD_MOON, MOVE_EARTH_POWER, MOVE_CALM_MIND, MOVE_PROTECT}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_DITTO,
    .heldItem = ITEM_CHOICE_SCARF,
    .ability = ABILITY_IMPOSTER,
    .moves = {MOVE_TRANSFORM}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 82,
    .species = SPECIES_PIDGEOT,
    .heldItem = ITEM_PIDGEOTITE,
    .isShiny = TRUE,
    .ability = ABILITY_NO_GUARD,
    .moves = {MOVE_BLEAKWIND_STORM, MOVE_FOCUS_BLAST, MOVE_TAILWIND, MOVE_HEAT_WAVE}
    }
};

static const struct TrainerMon sParty_Norman3[] = {
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_SLAKING,
    .heldItem = ITEM_LIFE_ORB,
    .ability = ABILITY_SLOW_START,
    .moves = {MOVE_DOUBLE_EDGE, MOVE_ROCK_SLIDE, MOVE_SUCKER_PUNCH, MOVE_PROTECT}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_WEEZING,
    .heldItem = ITEM_ROCKY_HELMET,
    .ability = ABILITY_NEUTRALIZING_GAS,
    .moves = {MOVE_WILL_O_WISP, MOVE_PROTECT,MOVE_SLUDGE_BOMB, MOVE_HEAT_WAVE}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_ZOROARK_HISUIAN,
    .heldItem = ITEM_SILK_SCARF,
    .ability = ABILITY_ILLUSION,
    .moves = {MOVE_TAUNT, MOVE_BITTER_MALICE, MOVE_HYPER_VOICE, MOVE_NASTY_PLOT}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_URSALUNA,
    .heldItem = ITEM_CHOPLE_BERRY,
    .ability = ABILITY_MINDS_EYE,
    .moves = {MOVE_BLOOD_MOON, MOVE_EARTH_POWER, MOVE_CALM_MIND, MOVE_PROTECT}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_DITTO,
    .heldItem = ITEM_CHOICE_SCARF,
    .ability = ABILITY_IMPOSTER,
    .moves = {MOVE_TRANSFORM}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 82,
    .species = SPECIES_PIDGEOT,
    .heldItem = ITEM_PIDGEOTITE,
    .isShiny = TRUE,
    .ability = ABILITY_NO_GUARD,
    .moves = {MOVE_BLEAKWIND_STORM, MOVE_FOCUS_BLAST, MOVE_TAILWIND, MOVE_HEAT_WAVE}
    }
};

static const struct TrainerMon sParty_Norman4[] = {
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_SLAKING,
    .heldItem = ITEM_LIFE_ORB,
    .ability = ABILITY_SLOW_START,
    .moves = {MOVE_DOUBLE_EDGE, MOVE_ROCK_SLIDE, MOVE_SUCKER_PUNCH, MOVE_PROTECT}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_WEEZING,
    .heldItem = ITEM_ROCKY_HELMET,
    .ability = ABILITY_NEUTRALIZING_GAS,
    .moves = {MOVE_WILL_O_WISP, MOVE_PROTECT,MOVE_SLUDGE_BOMB, MOVE_HEAT_WAVE}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_ZOROARK_HISUIAN,
    .heldItem = ITEM_SILK_SCARF,
    .ability = ABILITY_ILLUSION,
    .moves = {MOVE_TAUNT, MOVE_BITTER_MALICE, MOVE_HYPER_VOICE, MOVE_NASTY_PLOT}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_URSALUNA,
    .heldItem = ITEM_CHOPLE_BERRY,
    .ability = ABILITY_MINDS_EYE,
    .moves = {MOVE_BLOOD_MOON, MOVE_EARTH_POWER, MOVE_CALM_MIND, MOVE_PROTECT}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_MARSHADOW,
    .heldItem = ITEM_FOCUS_SASH,
    .moves = {MOVE_SHADOW_SNEAK, MOVE_CLOSE_COMBAT, MOVE_SPECTRAL_THIEF, MOVE_DETECT}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 82,
    .species = SPECIES_KANGASKHAN,
    .heldItem = ITEM_KANGASKHANITE,
    .isShiny = TRUE,
    .ability = ABILITY_SCRAPPY,
    .moves = {MOVE_FAKE_OUT, MOVE_POWER_UP_PUNCH, MOVE_SUCKER_PUNCH, MOVE_DOUBLE_EDGE}
    }
};

static const struct TrainerMon sParty_Norman5[] = {
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_SLAKING,
    .heldItem = ITEM_LIFE_ORB,
    .ability = ABILITY_SLOW_START,
    .moves = {MOVE_DOUBLE_EDGE, MOVE_ROCK_SLIDE, MOVE_SUCKER_PUNCH, MOVE_PROTECT}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_WEEZING,
    .heldItem = ITEM_ROCKY_HELMET,
    .ability = ABILITY_NEUTRALIZING_GAS,
    .moves = {MOVE_WILL_O_WISP, MOVE_PROTECT,MOVE_SLUDGE_BOMB, MOVE_HEAT_WAVE}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_ZOROARK_HISUIAN,
    .heldItem = ITEM_SILK_SCARF,
    .ability = ABILITY_ILLUSION,
    .moves = {MOVE_TAUNT, MOVE_BITTER_MALICE, MOVE_HYPER_VOICE, MOVE_NASTY_PLOT}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_URSALUNA,
    .heldItem = ITEM_CHOPLE_BERRY,
    .ability = ABILITY_MINDS_EYE,
    .moves = {MOVE_BLOOD_MOON, MOVE_EARTH_POWER, MOVE_CALM_MIND, MOVE_PROTECT}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_MARSHADOW,
    .heldItem = ITEM_FOCUS_SASH,
    .moves = {MOVE_SHADOW_SNEAK, MOVE_CLOSE_COMBAT, MOVE_SPECTRAL_THIEF, MOVE_DETECT}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 82,
    .species = SPECIES_KANGASKHAN,
    .heldItem = ITEM_KANGASKHANITE,
    .isShiny = TRUE,
    .ability = ABILITY_SCRAPPY,
    .moves = {MOVE_FAKE_OUT, MOVE_POWER_UP_PUNCH, MOVE_SUCKER_PUNCH, MOVE_DOUBLE_EDGE}
    }
};

static const struct TrainerMon sParty_Winona2[] = {
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_STARAPTOR,
    .heldItem = ITEM_LIFE_ORB,
    .moves = {MOVE_CLOSE_COMBAT, MOVE_U_TURN, MOVE_BRAVE_BIRD, MOVE_TAILWIND}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_LANDORUS_THERIAN,
    .heldItem = ITEM_YACHE_BERRY,
    .moves = {MOVE_SWORDS_DANCE, MOVE_ROCK_SLIDE, MOVE_EARTHQUAKE, MOVE_PROTECT}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_TOGEKISS,
    .heldItem = ITEM_CHOICE_SCARF,
    .ability = ABILITY_SERENE_GRACE,
    .moves = {MOVE_AIR_SLASH, MOVE_HEAT_WAVE, MOVE_MOONBLAST, MOVE_AURA_SPHERE}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_CELESTEELA,
    .heldItem = ITEM_POWER_HERB,
    .moves = {MOVE_METEOR_BEAM, MOVE_FLAMETHROWER, MOVE_PROTECT, MOVE_FLASH_CANNON}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_HAWLUCHA,
    .heldItem = ITEM_FLYING_GEM,
    .ability = ABILITY_UNBURDEN,
    .moves = {MOVE_ACROBATICS, MOVE_CLOSE_COMBAT, MOVE_SWORDS_DANCE, MOVE_KNOCK_OFF}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 82,
    .species = SPECIES_ALTARIA,
    .heldItem = ITEM_ALTARIANITE,
    .isShiny = TRUE,
    .moves = {MOVE_EARTHQUAKE, MOVE_DRAGON_CLAW, MOVE_DRAGON_DANCE, MOVE_FIRE_BLAST}
    }
};

static const struct TrainerMon sParty_Winona3[] = {
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_STARAPTOR,
    .heldItem = ITEM_LIFE_ORB,
    .moves = {MOVE_CLOSE_COMBAT, MOVE_U_TURN, MOVE_BRAVE_BIRD, MOVE_TAILWIND}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_LANDORUS_THERIAN,
    .heldItem = ITEM_YACHE_BERRY,
    .moves = {MOVE_SWORDS_DANCE, MOVE_ROCK_SLIDE, MOVE_EARTHQUAKE, MOVE_PROTECT}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_TOGEKISS,
    .heldItem = ITEM_CHOICE_SCARF,
    .ability = ABILITY_SERENE_GRACE,
    .moves = {MOVE_AIR_SLASH, MOVE_HEAT_WAVE, MOVE_MOONBLAST, MOVE_AURA_SPHERE}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_CELESTEELA,
    .heldItem = ITEM_POWER_HERB,
    .moves = {MOVE_METEOR_BEAM, MOVE_FLAMETHROWER, MOVE_PROTECT, MOVE_FLASH_CANNON}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_HAWLUCHA,
    .heldItem = ITEM_FLYING_GEM,
    .ability = ABILITY_UNBURDEN,
    .moves = {MOVE_ACROBATICS, MOVE_CLOSE_COMBAT, MOVE_SWORDS_DANCE, MOVE_KNOCK_OFF}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 82,
    .species = SPECIES_ALTARIA,
    .heldItem = ITEM_ALTARIANITE,
    .isShiny = TRUE,
    .moves = {MOVE_EARTHQUAKE, MOVE_DRAGON_CLAW, MOVE_DRAGON_DANCE, MOVE_FIRE_BLAST}
    }
};

static const struct TrainerMon sParty_Winona4[] = {
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_STARAPTOR,
    .heldItem = ITEM_LIFE_ORB,
    .moves = {MOVE_CLOSE_COMBAT, MOVE_U_TURN, MOVE_BRAVE_BIRD, MOVE_TAILWIND}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_LANDORUS_THERIAN,
    .heldItem = ITEM_YACHE_BERRY,
    .moves = {MOVE_SWORDS_DANCE, MOVE_ROCK_SLIDE, MOVE_EARTHQUAKE, MOVE_PROTECT}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_TOGEKISS,
    .heldItem = ITEM_CHOICE_SCARF,
    .ability = ABILITY_SERENE_GRACE,
    .moves = {MOVE_AIR_SLASH, MOVE_HEAT_WAVE, MOVE_MOONBLAST, MOVE_AURA_SPHERE}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_CELESTEELA,
    .heldItem = ITEM_POWER_HERB,
    .moves = {MOVE_METEOR_BEAM, MOVE_FLAMETHROWER, MOVE_PROTECT, MOVE_FLASH_CANNON}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_HAWLUCHA,
    .heldItem = ITEM_FLYING_GEM,
    .ability = ABILITY_UNBURDEN,
    .moves = {MOVE_ACROBATICS, MOVE_CLOSE_COMBAT, MOVE_SWORDS_DANCE, MOVE_KNOCK_OFF}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 82,
    .species = SPECIES_ALTARIA,
    .heldItem = ITEM_ALTARIANITE,
    .isShiny = TRUE,
    .moves = {MOVE_EARTHQUAKE, MOVE_DRAGON_CLAW, MOVE_DRAGON_DANCE, MOVE_FIRE_BLAST}
    }
};

static const struct TrainerMon sParty_Winona5[] = {
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_STARAPTOR,
    .heldItem = ITEM_LIFE_ORB,
    .moves = {MOVE_CLOSE_COMBAT, MOVE_U_TURN, MOVE_BRAVE_BIRD, MOVE_TAILWIND}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_LANDORUS_THERIAN,
    .heldItem = ITEM_YACHE_BERRY,
    .moves = {MOVE_SWORDS_DANCE, MOVE_ROCK_SLIDE, MOVE_EARTHQUAKE, MOVE_PROTECT}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_TOGEKISS,
    .heldItem = ITEM_CHOICE_SCARF,
    .ability = ABILITY_SERENE_GRACE,
    .moves = {MOVE_AIR_SLASH, MOVE_HEAT_WAVE, MOVE_MOONBLAST, MOVE_AURA_SPHERE}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_CELESTEELA,
    .heldItem = ITEM_POWER_HERB,
    .moves = {MOVE_METEOR_BEAM, MOVE_FLAMETHROWER, MOVE_PROTECT, MOVE_FLASH_CANNON}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_HAWLUCHA,
    .heldItem = ITEM_FLYING_GEM,
    .ability = ABILITY_UNBURDEN,
    .moves = {MOVE_ACROBATICS, MOVE_CLOSE_COMBAT, MOVE_SWORDS_DANCE, MOVE_KNOCK_OFF}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 82,
    .species = SPECIES_ALTARIA,
    .heldItem = ITEM_ALTARIANITE,
    .isShiny = TRUE,
    .moves = {MOVE_EARTHQUAKE, MOVE_DRAGON_CLAW, MOVE_DRAGON_DANCE, MOVE_FIRE_BLAST}
    }
};

static const struct TrainerMon sParty_TateAndLiza2[] = {
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_SLOWBRO,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_YAWN, MOVE_PSYCHIC, MOVE_CALM_MIND, MOVE_SCALD}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_INDEEDEE_FEMALE,
    .heldItem = ITEM_PSYCHIC_SEED,
    .moves = {MOVE_EXPANDING_FORCE, MOVE_DAZZLING_GLEAM, MOVE_MYSTICAL_FIRE, MOVE_SHADOW_BALL}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_ARMAROUGE,
    .heldItem = ITEM_CUSTAP_BERRY,
    .moves = {MOVE_ARMOR_CANNON, MOVE_DESTINY_BOND, MOVE_ALLY_SWITCH, MOVE_WILL_O_WISP}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_CLAYDOL,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_BULLDOZE, MOVE_PSYCHIC, MOVE_ALLY_SWITCH, MOVE_EERIE_IMPULSE}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_METAGROSS,
    .heldItem = ITEM_WEAKNESS_POLICY,
    .moves = {MOVE_HEAVY_SLAM, MOVE_PSYSHIELD_BASH, MOVE_ROCK_SLIDE, MOVE_PROTECT}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 82,
    .species = SPECIES_ALAKAZAM,
    .heldItem = ITEM_ALAKAZITE,
    .moves = {MOVE_EXPANDING_FORCE, MOVE_DAZZLING_GLEAM, MOVE_NASTY_PLOT, MOVE_PROTECT}
    }
};

static const struct TrainerMon sParty_TateAndLiza3[] = {
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_SLOWBRO,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_YAWN, MOVE_PSYCHIC, MOVE_CALM_MIND, MOVE_SCALD}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_INDEEDEE_FEMALE,
    .heldItem = ITEM_PSYCHIC_SEED,
    .moves = {MOVE_EXPANDING_FORCE, MOVE_DAZZLING_GLEAM, MOVE_MYSTICAL_FIRE, MOVE_SHADOW_BALL}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_ARMAROUGE,
    .heldItem = ITEM_CUSTAP_BERRY,
    .moves = {MOVE_ARMOR_CANNON, MOVE_DESTINY_BOND, MOVE_ALLY_SWITCH, MOVE_WILL_O_WISP}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_CLAYDOL,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_BULLDOZE, MOVE_PSYCHIC, MOVE_ALLY_SWITCH, MOVE_EERIE_IMPULSE}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_METAGROSS,
    .heldItem = ITEM_WEAKNESS_POLICY,
    .moves = {MOVE_HEAVY_SLAM, MOVE_PSYSHIELD_BASH, MOVE_ROCK_SLIDE, MOVE_PROTECT}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 82,
    .species = SPECIES_ALAKAZAM,
    .heldItem = ITEM_ALAKAZITE,
    .moves = {MOVE_EXPANDING_FORCE, MOVE_DAZZLING_GLEAM, MOVE_NASTY_PLOT, MOVE_PROTECT}
    }
};

static const struct TrainerMon sParty_TateAndLiza4[] = {
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_SLOWBRO,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_YAWN, MOVE_PSYCHIC, MOVE_CALM_MIND, MOVE_SCALD}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_INDEEDEE_FEMALE,
    .heldItem = ITEM_PSYCHIC_SEED,
    .moves = {MOVE_EXPANDING_FORCE, MOVE_DAZZLING_GLEAM, MOVE_MYSTICAL_FIRE, MOVE_SHADOW_BALL}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_ARMAROUGE,
    .heldItem = ITEM_CUSTAP_BERRY,
    .moves = {MOVE_ARMOR_CANNON, MOVE_DESTINY_BOND, MOVE_ALLY_SWITCH, MOVE_WILL_O_WISP}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_CLAYDOL,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_BULLDOZE, MOVE_PSYCHIC, MOVE_ALLY_SWITCH, MOVE_EERIE_IMPULSE}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_METAGROSS,
    .heldItem = ITEM_WEAKNESS_POLICY,
    .moves = {MOVE_HEAVY_SLAM, MOVE_PSYSHIELD_BASH, MOVE_ROCK_SLIDE, MOVE_PROTECT}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 82,
    .species = SPECIES_ALAKAZAM,
    .heldItem = ITEM_ALAKAZITE,
    .moves = {MOVE_EXPANDING_FORCE, MOVE_DAZZLING_GLEAM, MOVE_NASTY_PLOT, MOVE_PROTECT}
    }
};

static const struct TrainerMon sParty_TateAndLiza5[] = {
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_HOOPA_UNBOUND,
    .heldItem = ITEM_PSYCHIC_SEED,
    .moves = {MOVE_EXPANDING_FORCE, MOVE_SIGNAL_BEAM, MOVE_HYPERSPACE_FURY, MOVE_PROTECT}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_INDEEDEE_FEMALE,
    .heldItem = ITEM_TERRAIN_EXTENDER,
    .moves = {MOVE_EXPANDING_FORCE, MOVE_DAZZLING_GLEAM, MOVE_MYSTICAL_FIRE, MOVE_SHADOW_BALL}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_ARMAROUGE,
    .heldItem = ITEM_CUSTAP_BERRY,
    .ability = ABILITY_FLASH_FIRE,
    .moves = {MOVE_ARMOR_CANNON, MOVE_DESTINY_BOND, MOVE_ALLY_SWITCH, MOVE_WILL_O_WISP}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_VICTINI,
    .heldItem = ITEM_CHOICE_BAND,
    .moves = {MOVE_V_CREATE,  MOVE_BOLT_STRIKE, MOVE_ZEN_HEADBUTT, MOVE_U_TURN}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_DEOXYS_ATTACK,
    .heldItem = ITEM_FOCUS_SASH,
    .moves = {MOVE_PSYCHO_BOOST, MOVE_SUPERPOWER, MOVE_EXTREME_SPEED, MOVE_ICE_BEAM}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 82,
    .species = SPECIES_ALAKAZAM,
    .heldItem = ITEM_ALAKAZITE,
    .moves = {MOVE_EXPANDING_FORCE, MOVE_DAZZLING_GLEAM, MOVE_NASTY_PLOT, MOVE_PROTECT}
    }
};

static const struct TrainerMon sParty_Juan2[] = {
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_MANECTRIC,
    .heldItem = ITEM_LIFE_ORB,
    .ability = ABILITY_LIGHTNING_ROD,
    .moves = {MOVE_THUNDER, MOVE_RAIN_DANCE, MOVE_VOLT_SWITCH, MOVE_PROTECT}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_KINGDRA,
    .heldItem = ITEM_CHOICE_SPECS,
    .moves = {MOVE_MUDDY_WATER, MOVE_HURRICANE, MOVE_DRAGON_PULSE, MOVE_ICE_BEAM}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_TOXICROAK,
    .heldItem = ITEM_BLACK_SLUDGE,
    .ability = ABILITY_DRY_SKIN,
    .moves = {MOVE_FAKE_OUT, MOVE_NASTY_PLOT, MOVE_VACUUM_WAVE, MOVE_SLUDGE_BOMB}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_LUDICOLO,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_FAKE_OUT, MOVE_SURF, MOVE_GIGA_DRAIN, MOVE_ICY_WIND}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_GASTRODON,
    .heldItem = ITEM_RINDO_BERRY,
    .ability = ABILITY_STORM_DRAIN,
    .moves = {MOVE_EARTH_POWER, MOVE_MUDDY_WATER, MOVE_SLUDGE_BOMB, MOVE_PROTECT}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 82,
    .species = SPECIES_SWAMPERT,
    .heldItem = ITEM_SWAMPERTITE,
    .moves = {MOVE_LIQUIDATION, MOVE_POWER_UP_PUNCH, MOVE_HIGH_HORSEPOWER, MOVE_KNOCK_OFF}
    }
};

static const struct TrainerMon sParty_Juan3[] = {
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_MANAPHY,
    .heldItem = ITEM_LEFTOVERS,
    .ability = ABILITY_HYDRATION,
    .moves = {MOVE_REST, MOVE_TAIL_GLOW, MOVE_ICE_BEAM, MOVE_ENERGY_BALL}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_KINGDRA,
    .heldItem = ITEM_CHOICE_SPECS,
    .moves = {MOVE_MUDDY_WATER, MOVE_HURRICANE, MOVE_DRAGON_PULSE, MOVE_ICE_BEAM}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_TOXICROAK,
    .heldItem = ITEM_BLACK_SLUDGE,
    .ability = ABILITY_DRY_SKIN,
    .moves = {MOVE_FAKE_OUT, MOVE_NASTY_PLOT, MOVE_VACUUM_WAVE, MOVE_SLUDGE_BOMB}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_PALKIA,
    .heldItem = ITEM_ROSELI_BERRY,
    .moves = {MOVE_THUNDER, MOVE_HYDRO_CANNON, MOVE_SPACIAL_REND, MOVE_SUBSTITUTE}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_GASTRODON,
    .heldItem = ITEM_RINDO_BERRY,
    .ability = ABILITY_STORM_DRAIN,
    .moves = {MOVE_EARTH_POWER, MOVE_MUDDY_WATER, MOVE_SLUDGE_BOMB, MOVE_PROTECT}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 82,
    .species = SPECIES_SWAMPERT,
    .heldItem = ITEM_SWAMPERTITE,
    .moves = {MOVE_LIQUIDATION, MOVE_POWER_UP_PUNCH, MOVE_HIGH_HORSEPOWER, MOVE_KNOCK_OFF}
    }
};

static const struct TrainerMon sParty_Juan4[] = {
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_MANAPHY,
    .heldItem = ITEM_LEFTOVERS,
    .ability = ABILITY_HYDRATION,
    .moves = {MOVE_REST, MOVE_TAIL_GLOW, MOVE_ICE_BEAM, MOVE_ENERGY_BALL}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_KINGDRA,
    .heldItem = ITEM_CHOICE_SPECS,
    .moves = {MOVE_MUDDY_WATER, MOVE_HURRICANE, MOVE_DRAGON_PULSE, MOVE_ICE_BEAM}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_TOXICROAK,
    .heldItem = ITEM_BLACK_SLUDGE,
    .ability = ABILITY_DRY_SKIN,
    .moves = {MOVE_FAKE_OUT, MOVE_NASTY_PLOT, MOVE_VACUUM_WAVE, MOVE_SLUDGE_BOMB}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_PALKIA,
    .heldItem = ITEM_ROSELI_BERRY,
    .moves = {MOVE_THUNDER, MOVE_HYDRO_CANNON, MOVE_SPACIAL_REND, MOVE_SUBSTITUTE}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_GASTRODON,
    .heldItem = ITEM_RINDO_BERRY,
    .ability = ABILITY_STORM_DRAIN,
    .moves = {MOVE_EARTH_POWER, MOVE_MUDDY_WATER, MOVE_SLUDGE_BOMB, MOVE_PROTECT}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 82,
    .species = SPECIES_SWAMPERT,
    .heldItem = ITEM_SWAMPERTITE,
    .moves = {MOVE_LIQUIDATION, MOVE_POWER_UP_PUNCH, MOVE_HIGH_HORSEPOWER, MOVE_KNOCK_OFF}
    }
};

static const struct TrainerMon sParty_Juan5[] = {
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_MANAPHY,
    .heldItem = ITEM_LEFTOVERS,
    .ability = ABILITY_HYDRATION,
    .moves = {MOVE_REST, MOVE_TAIL_GLOW, MOVE_ICE_BEAM, MOVE_ENERGY_BALL}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_KINGDRA,
    .heldItem = ITEM_CHOICE_SPECS,
    .moves = {MOVE_MUDDY_WATER, MOVE_HURRICANE, MOVE_DRAGON_PULSE, MOVE_ICE_BEAM}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_TOXICROAK,
    .heldItem = ITEM_BLACK_SLUDGE,
    .ability = ABILITY_DRY_SKIN,
    .moves = {MOVE_FAKE_OUT, MOVE_NASTY_PLOT, MOVE_VACUUM_WAVE, MOVE_SLUDGE_BOMB}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_PALKIA,
    .heldItem = ITEM_ROSELI_BERRY,
    .moves = {MOVE_THUNDER, MOVE_HYDRO_CANNON, MOVE_SPACIAL_REND, MOVE_SUBSTITUTE}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 80,
    .species = SPECIES_GASTRODON,
    .heldItem = ITEM_RINDO_BERRY,
    .ability = ABILITY_STORM_DRAIN,
    .moves = {MOVE_EARTH_POWER, MOVE_MUDDY_WATER, MOVE_SLUDGE_BOMB, MOVE_PROTECT}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 82,
    .species = SPECIES_SWAMPERT,
    .heldItem = ITEM_SWAMPERTITE,
    .moves = {MOVE_LIQUIDATION, MOVE_POWER_UP_PUNCH, MOVE_HIGH_HORSEPOWER, MOVE_KNOCK_OFF}
    }
};

static const struct TrainerMon sParty_Angelo[] = {
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 17,
    .species = SPECIES_ILLUMISE,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_SHOCK_WAVE, MOVE_QUICK_ATTACK, MOVE_CHARM, MOVE_NONE}
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 17,
    .species = SPECIES_VOLBEAT,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_SHOCK_WAVE, MOVE_QUICK_ATTACK, MOVE_CONFUSE_RAY, MOVE_NONE}
    }
};

static const struct TrainerMon sParty_Darius[] = {
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 45,
    .species = SPECIES_TROPIUS,
    },
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 46,
    .species = SPECIES_CHARIZARD,
    },
    {
    .iv = TRAINER_PARTY_IVS(24, 24, 24, 24, 24, 24),
    .lvl = 46,
    .species = SPECIES_SCYTHER,
    }
};

static const struct TrainerMon sParty_Steven[] = {
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 87,
    .species = SPECIES_LANDORUS_THERIAN,
    .heldItem = ITEM_FOCUS_SASH,
    .moves = {MOVE_STEALTH_ROCK, MOVE_EARTHQUAKE, MOVE_STONE_EDGE, MOVE_U_TURN}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 85,
    .species = SPECIES_DRACOVISH,
    .heldItem = ITEM_CHOICE_SCARF,
    .ability = ABILITY_STRONG_JAW,
    .moves = {MOVE_FISHIOUS_REND, MOVE_OUTRAGE, MOVE_PSYCHIC_FANGS, MOVE_CRUNCH}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 86,
    .species = SPECIES_MELMETAL,
    .heldItem = ITEM_ASSAULT_VEST,
    .ability = ABILITY_IRON_FIST,
    .moves = {MOVE_DOUBLE_IRON_BASH, MOVE_EARTHQUAKE, MOVE_THUNDER_PUNCH, MOVE_SUPERPOWER}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 87,
    .species = SPECIES_DEOXYS_ATTACK,
    .heldItem = ITEM_LIFE_ORB,
    .moves = {MOVE_PSYSTRIKE, MOVE_EXTREMESPEED, MOVE_ICE_BEAM, MOVE_THUNDERBOLT}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 87,
    .species = SPECIES_JIRACHI,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_IRON_HEAD, MOVE_BODY_SLAM, MOVE_FIRE_PUNCH, MOVE_WISH}
    },
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 87,
    .species = SPECIES_METAGROSS,
    .heldItem = ITEM_METAGROSSITE,
    .moves = {MOVE_EARTHQUAKE, MOVE_ZEN_HEADBUTT, MOVE_METEOR_MASH, MOVE_BULLET_PUNCH}
    }
};

static const struct TrainerMon sParty_Anabel[] = {
    {
    .lvl = 5,
    .species = SPECIES_BELDUM,
    }
};

static const struct TrainerMon sParty_Tucker[] = {
    {
    .lvl = 5,
    .species = SPECIES_BELDUM,
    }
};

static const struct TrainerMon sParty_Spenser[] = {
    {
    .lvl = 5,
    .species = SPECIES_BELDUM,
    }
};

static const struct TrainerMon sParty_Greta[] = {
    {
    .lvl = 5,
    .species = SPECIES_BELDUM,
    }
};

static const struct TrainerMon sParty_Noland[] = {
    {
    .lvl = 5,
    .species = SPECIES_BELDUM,
    }
};

static const struct TrainerMon sParty_Lucy[] = {
    {
    .lvl = 5,
    .species = SPECIES_BELDUM,
    }
};

static const struct TrainerMon sParty_Brandon[] = {
    {
    .lvl = 5,
    .species = SPECIES_BELDUM,
    }
};

static const struct TrainerMon sParty_Andres2[] = {
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 37,
    .species = SPECIES_SANDSLASH,
    },
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 38,
    .species = SPECIES_NIDOQUEEN,
    .ability = ABILITY_SHEER_FORCE,
    }
};

static const struct TrainerMon sParty_Andres3[] = {
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 44,
    .species = SPECIES_SANDSLASH,
    },
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 44,
    .species = SPECIES_NIDOQUEEN,
    .ability = ABILITY_SHEER_FORCE,
    .heldItem = ITEM_LIFE_ORB,
    .moves = {MOVE_THUNDERBOLT, MOVE_EARTH_POWER, MOVE_SLUDGE_BOMB, MOVE_ICE_BEAM}
    },
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 43,
    .species = SPECIES_PROBOPASS,
    .ability = ABILITY_MAGNET_PULL,
    .moves = {MOVE_THUNDERBOLT, MOVE_EARTH_POWER, MOVE_POWER_GEM, MOVE_VOLT_SWITCH}
    }
};

static const struct TrainerMon sParty_Andres4[] = {
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 47,
    .species = SPECIES_SANDSLASH,
    .heldItem = ITEM_LIFE_ORB,
    .moves = {MOVE_SANDSTORM, MOVE_SWORDS_DANCE, MOVE_EARTHQUAKE, MOVE_ROCK_BLAST}
    },
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 47,
    .species = SPECIES_NIDOQUEEN,
    .ability = ABILITY_SHEER_FORCE,
    .heldItem = ITEM_LIFE_ORB,
    .moves = {MOVE_THUNDERBOLT, MOVE_EARTH_POWER, MOVE_SLUDGE_BOMB, MOVE_ICE_BEAM}
    },
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 47,
    .species = SPECIES_PROBOPASS,
    .ability = ABILITY_MAGNET_PULL,
    .moves = {MOVE_THUNDERBOLT, MOVE_EARTH_POWER, MOVE_POWER_GEM, MOVE_VOLT_SWITCH}
    }
};

static const struct TrainerMon sParty_Andres5[] = {
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 53,
    .species = SPECIES_SANDSLASH,
    .heldItem = ITEM_LIFE_ORB,
    .moves = {MOVE_SANDSTORM, MOVE_SWORDS_DANCE, MOVE_EARTHQUAKE, MOVE_ROCK_BLAST}
    },
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 53,
    .species = SPECIES_NIDOQUEEN,
    .ability = ABILITY_SHEER_FORCE,
    .heldItem = ITEM_LIFE_ORB,
    .moves = {MOVE_THUNDERBOLT, MOVE_EARTH_POWER, MOVE_SLUDGE_BOMB, MOVE_ICE_BEAM}
    },
    {
    .iv = TRAINER_PARTY_IVS(6, 6, 6, 6, 6, 6),
    .lvl = 53,
    .species = SPECIES_PROBOPASS,
    .ability = ABILITY_MAGNET_PULL,
    .moves = {MOVE_THUNDERBOLT, MOVE_EARTH_POWER, MOVE_POWER_GEM, MOVE_VOLT_SWITCH}
    }
};

static const struct TrainerMon sParty_Cory2[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 39,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 39,
    .species = SPECIES_MACHAMP,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 39,
    .species = SPECIES_TENTACRUEL,
    }
};

static const struct TrainerMon sParty_Cory3[] = {
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 43,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 43,
    .species = SPECIES_MACHAMP,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 43,
    .species = SPECIES_TENTACRUEL,
    }
};

static const struct TrainerMon sParty_Cory4[] = {
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 48,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 48,
    .species = SPECIES_MACHAMP,
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 48,
    .species = SPECIES_TENTACRUEL,
    }
};

static const struct TrainerMon sParty_Cory5[] = {
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 56,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 56,
    .species = SPECIES_MACHAMP,
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 56,
    .species = SPECIES_TENTACRUEL,
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 56,
    .species = SPECIES_KINGDRA,
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 56,
    .species = SPECIES_KINGAMBIT,
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 56,
    .species = SPECIES_FLAMIGO,
    }
};

static const struct TrainerMon sParty_Pablo2[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 37,
    .species = SPECIES_STARYU,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 37,
    .species = SPECIES_STARYU,
    }
};

static const struct TrainerMon sParty_Pablo3[] = {
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 39,
    .species = SPECIES_WINGULL,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 39,
    .species = SPECIES_STARYU,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 39,
    .species = SPECIES_STARYU,
    }
};

static const struct TrainerMon sParty_Pablo4[] = {
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 41,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 41,
    .species = SPECIES_STARYU,
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 41,
    .species = SPECIES_STARYU,
    }
};

static const struct TrainerMon sParty_Pablo5[] = {
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 43,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 43,
    .species = SPECIES_STARMIE,
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 43,
    .species = SPECIES_STARMIE,
    }
};

static const struct TrainerMon sParty_Koji2[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 47,
    .species = SPECIES_MACHAMP,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 47,
    .species = SPECIES_CONKELDURR,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 47,
    .species = SPECIES_WHIMSICOTT,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 47,
    .species = SPECIES_HITMONTOP,
    }
};

static const struct TrainerMon sParty_Koji3[] = {
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 51,
    .species = SPECIES_MACHAMP,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 51,
    .species = SPECIES_CONKELDURR,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 51,
    .species = SPECIES_WHIMSICOTT,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 51,
    .species = SPECIES_HITMONTOP,
    }
};

static const struct TrainerMon sParty_Koji4[] = {
    {
    .iv = TRAINER_PARTY_IVS(5, 5, 5, 5, 5, 5),
    .lvl = 57,
    .species = SPECIES_MACHAMP,
    },
    {
    .iv = TRAINER_PARTY_IVS(5, 5, 5, 5, 5, 5),
    .lvl = 57,
    .species = SPECIES_CONKELDURR,
    },
    {
    .iv = TRAINER_PARTY_IVS(5, 5, 5, 5, 5, 5),
    .lvl = 57,
    .species = SPECIES_WHIMSICOTT,
    },
    {
    .iv = TRAINER_PARTY_IVS(5, 5, 5, 5, 5, 5),
    .lvl = 57,
    .species = SPECIES_HITMONTOP,
    }
};

static const struct TrainerMon sParty_Koji5[] = {
    {
    .iv = TRAINER_PARTY_IVS(14, 14, 15, 15, 15, 15),
    .lvl = 60,
    .species = SPECIES_MACHAMP,
    },
    {
    .iv = TRAINER_PARTY_IVS(14, 14, 15, 15, 15, 15),
    .lvl = 60,
    .species = SPECIES_CONKELDURR,
    },
    {
    .iv = TRAINER_PARTY_IVS(14, 14, 15, 15, 15, 15),
    .lvl = 60,
    .species = SPECIES_WHIMSICOTT,
    },
    {
    .iv = TRAINER_PARTY_IVS(14, 14, 15, 15, 15, 15),
    .lvl = 60,
    .species = SPECIES_HITMONTOP,
    }
};

static const struct TrainerMon sParty_Cristin2[] = {
    {
    .iv = TRAINER_PARTY_IVS(13, 13, 13, 13, 13, 13),
    .lvl = 52,
    .species = SPECIES_EXPLOUD,
    },
    {
    .iv = TRAINER_PARTY_IVS(13, 13, 13, 13, 13, 13),
    .lvl = 53,
    .species = SPECIES_SLAKING,
    }
};

static const struct TrainerMon sParty_Cristin3[] = {
    {
    .iv = TRAINER_PARTY_IVS(14, 14, 14, 14, 14, 14),
    .lvl = 56,
    .species = SPECIES_SPINDA,
    },
    {
    .iv = TRAINER_PARTY_IVS(14, 14, 14, 14, 14, 14),
    .lvl = 57,
    .species = SPECIES_EXPLOUD,
    },
    {
    .iv = TRAINER_PARTY_IVS(14, 14, 14, 14, 14, 14),
    .lvl = 57,
    .species = SPECIES_SLAKING,
    }
};

static const struct TrainerMon sParty_Cristin4[] = {
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 61,
    .species = SPECIES_SPINDA,
    },
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 61,
    .species = SPECIES_EXPLOUD,
    },
    {
    .iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
    .lvl = 61,
    .species = SPECIES_SLAKING,
    }
};

static const struct TrainerMon sParty_Cristin5[] = {
    {
    .iv = TRAINER_PARTY_IVS(17, 17, 17, 17, 17, 17),
    .lvl = 69,
    .species = SPECIES_SPINDA,
    },
    {
    .iv = TRAINER_PARTY_IVS(17, 17, 17, 17, 17, 17),
    .lvl = 69,
    .species = SPECIES_EXPLOUD,
    },
    {
    .iv = TRAINER_PARTY_IVS(17, 17, 17, 17, 17, 17),
    .lvl = 69,
    .species = SPECIES_SLAKING,
    }
};

static const struct TrainerMon sParty_Fernando2[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 35,
    .species = SPECIES_ELECTRIKE,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 35,
    .species = SPECIES_ELECTRIKE,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 35,
    .species = SPECIES_LOUDRED,
    }
};

static const struct TrainerMon sParty_Fernando3[] = {
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 37,
    .species = SPECIES_ELECTRIKE,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 37,
    .species = SPECIES_MANECTRIC,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 37,
    .species = SPECIES_LOUDRED,
    }
};

static const struct TrainerMon sParty_Fernando4[] = {
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 39,
    .species = SPECIES_MANECTRIC,
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 39,
    .species = SPECIES_MANECTRIC,
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 39,
    .species = SPECIES_LOUDRED,
    }
};

static const struct TrainerMon sParty_Fernando5[] = {
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 41,
    .species = SPECIES_MANECTRIC,
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 41,
    .species = SPECIES_MANECTRIC,
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 41,
    .species = SPECIES_EXPLOUD,
    }
};

static const struct TrainerMon sParty_Sawyer2[] = {
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 26,
    .species = SPECIES_GEODUDE,
    },
    {
    .iv = TRAINER_PARTY_IVS(1, 1, 1, 1, 1, 1),
    .lvl = 26,
    .species = SPECIES_NUMEL,
    }
};

static const struct TrainerMon sParty_Sawyer3[] = {
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 28,
    .species = SPECIES_MACHOP,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 28,
    .species = SPECIES_NUMEL,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 28,
    .species = SPECIES_GRAVELER,
    }
};

static const struct TrainerMon sParty_Sawyer4[] = {
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 30,
    .species = SPECIES_MACHOP,
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 30,
    .species = SPECIES_NUMEL,
    },
    {
    .iv = TRAINER_PARTY_IVS(3, 3, 3, 3, 3, 3),
    .lvl = 30,
    .species = SPECIES_GRAVELER,
    }
};

static const struct TrainerMon sParty_Sawyer5[] = {
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 33,
    .species = SPECIES_MACHOKE,
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 33,
    .species = SPECIES_CAMERUPT,
    },
    {
    .iv = TRAINER_PARTY_IVS(4, 4, 4, 4, 4, 4),
    .lvl = 33,
    .species = SPECIES_GOLEM,
    }
};

static const struct TrainerMon sParty_Gabrielle2[] = {
    {
    .lvl = 34,
    .species = SPECIES_DELCATTY,
    },
    {
    .lvl = 34,
    .species = SPECIES_MIGHTYENA,
    },
    {
    .lvl = 34,
    .species = SPECIES_LINOONE,
    },
    {
    .lvl = 34,
    .species = SPECIES_LOMBRE,
    },
    {
    .lvl = 34,
    .species = SPECIES_NUZLEAF,
    },
    {
    .lvl = 34,
    .species = SPECIES_SWELLOW,
    .heldItem = ITEM_FLAME_ORB
    }
};

static const struct TrainerMon sParty_Gabrielle3[] = {
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 37,
    .species = SPECIES_DELCATTY
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 37,
    .species = SPECIES_MIGHTYENA,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 37,
    .species = SPECIES_LINOONE,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 37,
    .species = SPECIES_LUDICOLO,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 37,
    .species = SPECIES_SHIFTRY,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 37,
    .species = SPECIES_SWELLOW,
    .heldItem = ITEM_FLAME_ORB
    }
};

static const struct TrainerMon sParty_Gabrielle4[] = {
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 39,
    .species = SPECIES_DELCATTY
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 39,
    .species = SPECIES_MIGHTYENA,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 39,
    .species = SPECIES_LINOONE,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 39,
    .species = SPECIES_LUDICOLO,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 39,
    .species = SPECIES_SHIFTRY,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 39,
    .species = SPECIES_SWELLOW,
    .heldItem = ITEM_FLAME_ORB
    }
};

static const struct TrainerMon sParty_Gabrielle5[] = {
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 43,
    .species = SPECIES_DELCATTY
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 43,
    .species = SPECIES_MIGHTYENA,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 43,
    .species = SPECIES_LINOONE,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 43,
    .species = SPECIES_LUDICOLO,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 43,
    .species = SPECIES_SHIFTRY,
    },
    {
    .iv = TRAINER_PARTY_IVS(2, 2, 2, 2, 2, 2),
    .lvl = 44,
    .species = SPECIES_SWELLOW,
    .heldItem = ITEM_FLAME_ORB
    }
};

static const struct TrainerMon sParty_Thalia2[] = {
    {
    .lvl = 40,
    .species = SPECIES_STANTLER,
    },
    {
    .lvl = 40,
    .species = SPECIES_LANTURN,
    }
};

static const struct TrainerMon sParty_Thalia3[] = {
    {
    .lvl = 45,
    .species = SPECIES_SNEASLER,
    },
    {
    .lvl = 45,
    .species = SPECIES_LANTURN,
    },
    {
    .lvl = 45,
    .species = SPECIES_WYRDEER,
    },
    {
    .lvl = 45,
    .species = SPECIES_ZANGOOSE,
    }
};

static const struct TrainerMon sParty_Thalia4[] = {
    {
    .lvl = 51,
    .species = SPECIES_SNEASLER,
    },
    {
    .lvl = 51,
    .species = SPECIES_LANTURN,
    },
    {
    .lvl = 51,
    .species = SPECIES_WYRDEER,
    },
    {
    .lvl = 51,
    .species = SPECIES_ZANGOOSE,
    }
};

static const struct TrainerMon sParty_Thalia5[] = {
    {
    .lvl = 57,
    .species = SPECIES_SNEASLER,
    },
    {
    .lvl = 57,
    .species = SPECIES_LANTURN,
    },
    {
    .lvl = 57,
    .species = SPECIES_WYRDEER,
    },
    {
    .lvl = 57,
    .species = SPECIES_ZANGOOSE,
    },
    {
    .lvl = 57,
    .species = SPECIES_KINGDRA,
    },
    {
    .lvl = 57,
    .species = SPECIES_WAILORD,
    },
};

static const struct TrainerMon sParty_Mariela[] = {
    {
    .lvl = 41,
    .species = SPECIES_CHIMECHO,
    }
};

static const struct TrainerMon sParty_Alvaro[] = {
    {
    .lvl = 41,
    .species = SPECIES_BANETTE,
    },
    {
    .lvl = 41,
    .species = SPECIES_KADABRA,
    }
};

static const struct TrainerMon sParty_Everett[] = {
    {
    .lvl = 41,
    .species = SPECIES_WOBBUFFET,
    }
};

static const struct TrainerMon sParty_Red[] = {
    {
    .iv = TRAINER_PARTY_IVS(30, 30, 30, 30, 30, 30),
    .lvl = 73,
    .species = SPECIES_PIKACHU_HOENN_CAP,
    .heldItem = ITEM_LIGHT_BALL,
    .ability = ABILITY_LIGHTNING_ROD,
    .isShiny = 1,
    .moves = {MOVE_SURF, MOVE_VOLT_TACKLE, MOVE_VOLT_SWITCH, MOVE_EXTREMESPEED}
    },
    {
    .iv = TRAINER_PARTY_IVS(30, 30, 30, 30, 30, 30),
    .lvl = 73,
    .species = SPECIES_CHARIZARD,
    .heldItem = ITEM_CHARIZARDITE_X,
    .moves = {MOVE_DRAGON_CLAW, MOVE_FLARE_BLITZ, MOVE_DRAGON_DANCE, MOVE_SHADOW_CLAW}
    },
    {
    .iv = TRAINER_PARTY_IVS(30, 30, 30, 30, 30, 30),
    .lvl = 73,
    .species = SPECIES_LAPRAS,
    .heldItem = ITEM_THROAT_SPRAY,
    .ability = ABILITY_LIQUID_VOICE,
    .moves = {MOVE_HYPER_VOICE, MOVE_FREEZE_DRY, MOVE_REST, MOVE_SLEEP_TALK}
    },
    {
    .iv = TRAINER_PARTY_IVS(30, 30, 30, 30, 30, 30),
    .lvl = 73,
    .species = SPECIES_KARTANA,
    .heldItem = ITEM_LIFE_ORB,
    .ability = ABILITY_BEAST_BOOST,
    .moves = {MOVE_SWORDS_DANCE, MOVE_SACRED_SWORD, MOVE_SMART_STRIKE, MOVE_LEAF_BLADE}
    },
    {
    .iv = TRAINER_PARTY_IVS(30, 30, 30, 30, 30, 30),
    .lvl = 73,
    .species = SPECIES_AEGISLASH,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_SWORDS_DANCE, MOVE_KINGS_SHIELD, MOVE_SHADOW_SNEAK, MOVE_CLOSE_COMBAT}
    },
    {
    .iv = TRAINER_PARTY_IVS(30, 30, 30, 30, 30, 30),
    .lvl = 73,
    .species = SPECIES_DRAGAPULT,
    .heldItem = ITEM_CHOICE_SPECS,
    .ability = ABILITY_INFILTRATOR,
    .moves = {MOVE_DRACO_METEOR, MOVE_SHADOW_BALL, MOVE_U_TURN, MOVE_SURF}
    }


};

static const struct TrainerMon sParty_Leaf[] = {
    {
    .lvl = 23,
    .species = SPECIES_RATICATE,
    .ability = ABILITY_HUSTLE,
    .moves = {MOVE_BITE, MOVE_HYPER_FANG, MOVE_QUICK_ATTACK, MOVE_FLAME_WHEEL},
    },
    {
    .lvl = 23,
    .species = SPECIES_IVYSAUR,
    .ability = ABILITY_OVERGROW,
    .moves = {MOVE_RAZOR_LEAF, MOVE_LEECH_SEED, MOVE_SLUDGE, MOVE_SLEEP_POWDER},
    },
    {
    .lvl = 23,
    .species = SPECIES_GROWLITHE,
    .ability = ABILITY_INTIMIDATE,
    .moves = {MOVE_BITE, MOVE_EMBER, MOVE_THUNDER_FANG, MOVE_WILL_O_WISP},
    },
    {
    .lvl = 23,
    .species = SPECIES_GYARADOS,
    .ability = ABILITY_INTIMIDATE,
    .moves = {MOVE_BITE, MOVE_THRASH, MOVE_LEER, MOVE_WATER_PULSE},
    },
    {
    .lvl = 22,
    .species = SPECIES_KADABRA,
    .ability = ABILITY_MAGIC_GUARD,
    .moves = {MOVE_CONFUSION, MOVE_MIRACLE_EYE, MOVE_HIDDEN_POWER, MOVE_SHOCK_WAVE},
    }

};

static const struct TrainerMon sParty_Leaf_2[] = {
    {
    .lvl = 39,
    .species = SPECIES_MR_RIME,
    .moves = {MOVE_FREEZE_DRY, MOVE_PSYBEAM, MOVE_CHARGE_BEAM, MOVE_HYPNOSIS},
    },
    {
    .lvl = 40,
    .species = SPECIES_VENUSAUR,
    .ability = ABILITY_OVERGROW,
    .heldItem = ITEM_VENUSAURITE,
    .moves = {MOVE_GIGA_DRAIN, MOVE_LEECH_SEED, MOVE_SLUDGE_BOMB, MOVE_SLEEP_POWDER},
    },
    {
    .lvl = 42,
    .species = SPECIES_ARCANINE_HISUIAN,
    .ability = ABILITY_ROCK_HEAD,
    .moves = {MOVE_HEAD_SMASH, MOVE_FLARE_BLITZ, MOVE_EXTREMESPEED, MOVE_WILL_O_WISP},
    },
    {
    .lvl = 41,
    .species = SPECIES_GYARADOS,
    .ability = ABILITY_INTIMIDATE,
    .moves = {MOVE_CRUNCH, MOVE_DRAGON_DANCE, MOVE_ICE_FANG, MOVE_THRASH},
    },
    {
    .lvl = 42,
    .species = SPECIES_KLAWF,
    .moves = {MOVE_CRABHAMMER, MOVE_ROCK_TOMB, MOVE_KNOCK_OFF, MOVE_DIG},
    }

};

static const struct TrainerMon sParty_MOM[] = {
    {
    .iv = TRAINER_PARTY_IVS(30, 30, 30, 30, 30, 30),
    .lvl = 38,
    .species = SPECIES_PIKACHU,
    .heldItem = ITEM_LIGHT_BALL,
    .ability = ABILITY_LIGHTNING_ROD,
    .isShiny = 1,
    .moves = {MOVE_SURF, MOVE_IRON_TAIL, MOVE_VOLT_SWITCH, MOVE_EXTREMESPEED}
    },
    {
    .iv = TRAINER_PARTY_IVS(30, 30, 30, 30, 30, 30),
    .lvl = 39,
    .species = SPECIES_KANGASKHAN,
    .heldItem = ITEM_LUM_BERRY,
    .ability = ABILITY_SCRAPPY,
    .moves = {MOVE_FAKE_OUT, MOVE_POWER_UP_PUNCH, MOVE_DIZZY_PUNCH, MOVE_CRUNCH}
    },
    {
    .iv = TRAINER_PARTY_IVS(30, 30, 30, 30, 30, 30),
    .lvl = 39,
    .species = SPECIES_BLAZIKEN,
    .ability = ABILITY_SPEED_BOOST,
    .moves = {MOVE_FLARE_BLITZ, MOVE_HI_JUMP_KICK, MOVE_EARTHQUAKE, MOVE_SWORDS_DANCE}
    },
    {
    .iv = TRAINER_PARTY_IVS(30, 30, 30, 30, 30, 30),
    .lvl = 39,
    .species = SPECIES_SWAMPERT,
    .moves = {MOVE_SURF, MOVE_MUD_BOMB, MOVE_TOXIC, MOVE_ICE_PUNCH}
    },
    {
    .iv = TRAINER_PARTY_IVS(30, 30, 30, 30, 30, 30),
    .lvl = 40,
    .species = SPECIES_SCEPTILE,
    .heldItem = ITEM_FLYING_GEM,
    .ability = ABILITY_UNBURDEN,
    .moves = {MOVE_SWORDS_DANCE, MOVE_ACROBATICS, MOVE_LEAF_BLADE, MOVE_DUAL_CHOP}
    },
    {
    .iv = TRAINER_PARTY_IVS(30, 30, 30, 30, 30, 30),
    .lvl = 41,
    .species = SPECIES_FLORGES,
    .moves = {MOVE_CALM_MIND, MOVE_GIGA_DRAIN, MOVE_DRAINING_KISS, MOVE_SYNTHESIS}
    }


};

static const struct TrainerMon sParty_BrendanLinkPlaceholder[] = {
    {
    .lvl = 5,
    .species = SPECIES_GROUDON,
    }
};

static const struct TrainerMon sParty_MayLinkPlaceholder[] = {
    {
    .lvl = 5,
    .species = SPECIES_KYOGRE,
    }
};

static const struct TrainerMon sParty_StevenGraniteCave[] = {
    {
    .iv = TRAINER_PARTY_IVS(11, 11, 11, 11, 11, 11),
    .lvl = 16,
    .species = SPECIES_ARON,
    .heldItem = ITEM_FOCUS_SASH,
    .moves = {MOVE_STEALTH_ROCK, MOVE_DIG, MOVE_ROCK_THROW, MOVE_METAL_CLAW}
    },
    {
    .iv = TRAINER_PARTY_IVS(11, 11, 11, 11, 11, 11),
    .lvl = 16,
    .species = SPECIES_ANORITH,
    .moves = {MOVE_FURY_CUTTER, MOVE_ROCK_THROW, MOVE_METAL_CLAW, MOVE_AQUA_JET}
    },
    {
    .iv = TRAINER_PARTY_IVS(11, 11, 11, 11, 11, 11),
    .lvl = 16,
    .species = SPECIES_LILEEP,
    .moves = {MOVE_MEGA_DRAIN, MOVE_ACID, MOVE_INGRAIN, MOVE_LEECH_SEED}
    },
    {
    .iv = TRAINER_PARTY_IVS(11, 11, 11, 11, 11, 11),
    .lvl = 18,
    .species = SPECIES_METANG,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_IRON_HEAD, MOVE_CONFUSION, MOVE_TAKE_DOWN, MOVE_IRON_DEFENSE}
    },
    {
    .iv = TRAINER_PARTY_IVS(11, 11, 11, 11, 11, 11),
    .lvl = 16,
    .species = SPECIES_GIBLE,
    .heldItem = ITEM_BERRY_JUICE,
    .moves = {MOVE_BULLDOZE, MOVE_BITE, MOVE_ROCK_SLIDE, MOVE_DRAGON_DANCE}
    }
};

static const struct TrainerMon sParty_RedAlteringCave[] = {
    {
    .iv = TRAINER_PARTY_IVS(21, 21, 21, 21, 21, 21),
    .lvl = 22,
    .species = SPECIES_PIKACHU_HOENN_CAP,
    .heldItem = ITEM_LIGHT_BALL,
    .moves = {MOVE_VOLT_TACKLE, MOVE_IRON_TAIL, MOVE_EXTREME_SPEED, MOVE_VOLT_SWITCH}
    },
    {
    .iv = TRAINER_PARTY_IVS(21, 21, 21, 21, 21, 21),
    .lvl = 21,
    .species = SPECIES_SNORLAX,
    .moves = {MOVE_BODY_SLAM, MOVE_CRUNCH, MOVE_REST, MOVE_DEFENSE_CURL}
    },
    {
    .iv = TRAINER_PARTY_IVS(21, 21, 21, 21, 21, 21),
    .lvl = 21,
    .species = SPECIES_CHARMELEON,
    .heldItem = ITEM_CHARCOAL,
    .moves = {MOVE_FLAME_CHARGE, MOVE_DRAGON_BREATH, MOVE_METAL_CLAW, MOVE_INCINERATE}
    },
    {
    .iv = TRAINER_PARTY_IVS(21, 21, 21, 21, 21, 21),
    .lvl = 21,
    .species = SPECIES_FROGADIER,
    .heldItem = ITEM_MYSTIC_WATER,
    .moves = {MOVE_WATER_PULSE, MOVE_U_TURN, MOVE_MUD_SHOT, MOVE_GRASS_KNOT}
    },
    {
    .iv = TRAINER_PARTY_IVS(21, 21, 21, 21, 21, 21),
    .lvl = 21,
    .species = SPECIES_GROTLE,
    .heldItem = ITEM_EVIOLITE,
    .moves = {MOVE_SUBSTITUTE, MOVE_SHELL_SMASH, MOVE_EARTH_POWER, MOVE_MEGA_DRAIN}
    },

};
static const struct TrainerMon sParty_Mt_Pyre_Archie[] = {
    {
    .iv = TRAINER_PARTY_IVS(31, 31, 31, 31, 31, 31),
    .lvl = 52,
    .species = SPECIES_CROBAT,
    .ability = ABILITY_INFILTRATOR,
    .heldItem = ITEM_BLACK_SLUDGE,
    .moves = {MOVE_BRAVE_BIRD, MOVE_CROSS_POISON, MOVE_U_TURN, MOVE_TAILWIND}
    },
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 52,
    .species = SPECIES_QWILFISH,
    .ability = ABILITY_INTIMIDATE,
    .heldItem = ITEM_BRIGHT_POWDER,
    .moves = {MOVE_BARB_BARRAGE, MOVE_AQUA_JET, MOVE_TOXIC_SPIKES, MOVE_DIVE}
    },
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 52,
    .species = SPECIES_INTELEON,
    .ability = ABILITY_SNIPER,
    .heldItem = ITEM_SCOPE_LENS,
    .moves = {MOVE_SNIPE_SHOT, MOVE_FOCUS_ENERGY, MOVE_VACUUM_WAVE, MOVE_RAIN_DANCE}
    },
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 53,
    .species = SPECIES_THIEVUL,
    .ability = ABILITY_UNBURDEN,
    .heldItem = ITEM_MISTY_SEED,
    .moves = {MOVE_NASTY_PLOT, MOVE_DARK_PULSE, MOVE_BURNING_JEALOUSY, MOVE_SHADOW_BALL}
    },
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 53,
    .species = SPECIES_SHARPEDO,
    .ability = ABILITY_SPEED_BOOST,
    .heldItem = ITEM_SHARPEDONITE,
    .moves = {MOVE_PROTECT, MOVE_WATERFALL, MOVE_CRUNCH, MOVE_PSYCHIC_FANGS}
    },
    {
    .iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
    .lvl = 54,
    .species = SPECIES_METAGROSS,
    .heldItem = ITEM_MISTY_SEED,
    .moves = {MOVE_IRON_DEFENSE, MOVE_BODY_PRESS, MOVE_HEAVY_SLAM, MOVE_REST}
    }

};