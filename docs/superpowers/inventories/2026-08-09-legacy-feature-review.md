...
| Custom trainer content | [`custom-trainer-content`](#feature-manifest) — `src/data/trainer_parties.h` and `src/data/trainers.h` | Current trainer party/trainer source tables | Modern adaptation | 2 | Preserve the Mom challenge team (`c73e48076b`), V2.2.2 mandatory trainers (`0080ad90f7`), and stronger gym rebattle teams (`c73e48076b`). Verify each battle family against the current tables. |
| Mom Mega-Stone challenge and reward | [`mom-mega-stone-challenge`](#feature-manifest) — Littleroot house scripts, Mom flags/opponent, and Mom trainer tables | Current Poryscript, player-house scripts, flags, opponent IDs, and trainer tables | Modern adaptation | 2 | `c73e48076b` adds an optional Mom battle after the Amulet Coin and rewards Sceptilite, Blazikenite, and Swampertite. Port the dialogue, accept/decline/repeat flags, trainer, and reward sequence; verify all branches and bag-full handling. |
| Current move animations and Psychic Noise effect | [`c73e-move-animations`](#feature-manifest) — legacy battle animation scripts and move data | Current 1.16.3 battle-animation and move-data sources | Use upstream equivalent | 4 | Use 1.16.3's animations for Lumina Crash, Population Bomb, Ruination, Collision Course, Electro Drift, Pounce, Rage Fist, Aqua Step, Shed Tail, Tidy Up, Trailblaze, and Chilling Water. Use 1.16.3's Psychic Noise healing-prevention behavior. |
| Burning Bulwark V2 behavior | [`burning-bulwark-v2`](#feature-manifest) — `data/battle_scripts_1.s` | Current `BattleScript_BurningBulwarkEffect` | Use upstream equivalent | 4 | `4f838e8e13` removes `HITMARKER_STATUS_ABILITY_EFFECT`; current master already uses that required marker set. Verify contact into Burning Bulwark applies the passive effect without an ability-status side effect. |
...
<!-- feature-manifest
{
  "features": [
    ...
    },
    {
      ...
      "decision": "Modern adaptation", "phase": 2, "status": "planned",
      "behavior": "Preserve Mom challenge, V2.2.2 mandatory-trainer, and stronger gym-rebattle teams.",
      "dependencies": ["current trainer source tables", "map/event progression"],
      "validation": ["Mom challenge team", "mandatory trainer encounters", "stronger gym rebattle teams", "supported build"],
      "evidence": ["git show c73e48076b -- src/data/trainer_parties.h src/data/trainers.h", "git show 0080ad90f7 -- src/data/trainer_parties.h src/data/trainers.h"]
    },
    {"id":"mom-mega-stone-challenge","legacy_paths":[{"category":"maps-and-events","prefixes":["data/maps/LittlerootTown_BrendansHouse_1F/scripts.inc","data/maps/LittlerootTown_BrendansHouse_1F/scripts.pory"]},{"category":"tooling-and-generated-output","prefixes":["data/scripts/players_house.inc"]},{"category":"gameplay-and-battle","prefixes":["include/constants/flags.h","include/constants/opponents.h"]}],"target_paths":["data/maps/LittlerootTown_BrendansHouse_1F/**","data/scripts/players_house.inc","current flags/opponents"],"decision":"Modern adaptation","phase":2,"status":"planned","behavior":"Optional Mom battle awards three starter Mega Stones.","dependencies":["current Poryscript and trainer content"],"validation":["accept/decline/repeat battle", "three reward items", "bag-full handling"],"evidence":["git show c73e48076b -- data/maps/LittlerootTown_BrendansHouse_1F/scripts.pory data/scripts/players_house.inc include/constants/flags.h include/constants/opponents.h"]},
    {"id":"c73e-move-animations","legacy_paths":[{"category":"gameplay-and-battle","prefixes":["data/battle_anim_scripts.s","src/data/battle_moves.h"]}],"target_paths":["data/battle_anim_scripts.s","src/data/moves_info.h","current battle animation sources"],"decision":"Use upstream equivalent","phase":4,"status":"planned","behavior":"Use the current 1.16.3 animations for Lumina Crash, Population Bomb, Ruination, Collision Course, Electro Drift, Pounce, Rage Fist, Aqua Step, Shed Tail, Tidy Up, Trailblaze, and Chilling Water. Use the current 1.16.3 Psychic Noise healing-prevention behavior rather than the legacy move-table implementation.","dependencies":["current battle animation engine","current move data"],"validation":["confirm each listed move has a current animation without importing legacy scripts","confirm current src/data/moves_info.h gives Psychic Noise MOVE_EFFECT_PSYCHIC_NOISE and a healing-prevention description","supported build"],"evidence":["user decision: prefer 1.16.3 animations and Psychic Noise mechanics","src/data/moves_info.h defines MOVE_PSYCHIC_NOISE with MOVE_EFFECT_PSYCHIC_NOISE and a two-turn healing-prevention description","git show c73e48076b -- data/battle_anim_scripts.s src/data/battle_moves.h"]},
    ...
    {"id":"burning-bulwark-v2","legacy_paths":[{"category":"gameplay-and-battle","prefixes":["data/battle_scripts_1.s"]}],"target_paths":["data/battle_scripts_1.s","BattleScript_BurningBulwarkEffect"],"decision":"Use upstream equivalent","phase":4,"status":"planned","behavior":"Burning Bulwark uses passive damage without HITMARKER_STATUS_ABILITY_EFFECT.","dependencies":["current battle script engine"],"validation":["contact into Burning Bulwark", "confirm passive effect and no ability-status side effect"],"evidence":["git show 4f838e8e13 -- data/battle_scripts_1.s", "git show master:data/battle_scripts_1.s matches the legacy V2 marker set"]},
    ...
    {"id":"level-cap-progression","legacy_paths":[{"category":"gameplay-and-battle","prefixes":["src/battle_script_commands.c","include/event_data.h"]}],"target_paths":["src/battle_script_commands.c","include/event_data.h","include/config/level_caps.h"],"decision":"Modern adaptation","phase":4,"status":"planned","behavior":"Preserve pre-cap XP reduction and post-Lilycove cap progression.","dependencies":["current level-cap configuration and event flags"],"validation":["XP below/at cap", "post-Lilycove cap flag", "supported build"],"evidence":["git show 0080ad90f7 -- src/battle_script_commands.c include/event_data.h","0080ad90f7 commit message"]},
    {
      ...
      "legacy_paths": [{"category": "gameplay-and-battle", "prefixes": [""], "exclude_prefixes": ["src/data/battle_frontier/battle_frontier_mons.h", "src/data/battle_frontier/battle_frontier_trainer_mons.h", "src/battle_tower.c", "include/constants/battle_frontier.h", "src/data/trainer_parties.h", "src/data/trainers.h", "include/config/battle.h", "include/constants/flags.h", "include/constants/opponents.h", "src/battle_main.c", "src/data/wild_encounters.json", "src/berry.c", "src/battle_script_commands.c", "include/event_data.h", "data/battle_anim_scripts.s", "data/battle_scripts_1.s", "src/data/battle_moves.h", "src/ui_startmenu_full.c", "src/pokedex_plus_hgss.c", "include/start_menu.h", "include/ui_startmenu_full.h"]}],
